...
enum Announces
{
    ANNOUNCE_KINGSTREET                        = -1595048,
    ANNOUNCE_FESTIVALSTREET_PLACE              = -1595049,
    ANNOUNCE_COUNCIL                           = -1595050,
    ANNOUNCE_KINGSTREET_PLACE                  = -1595051,
};
...
float AnnouncerPos[4] = {2104.8505f, 1312.0559f, 144.8915f, 0.0f};
...
#define GOSSIP_ITEM_ARTHAS_0 "Wir sind bereit das Ausmerzen von Stratholme zu beginnen."
#define GOSSIP_ITEM_ARTHAS_1 "Ja mein Prinz, wir sind bereit."
#define GOSSIP_ITEM_ARTHAS_2 "Wir tuen nur das was am besten für Lordaeron ist."
#define GOSSIP_ITEM_ARTHAS_3 "Wir sind bereit."
#define GOSSIP_ITEM_ARTHAS_4 "Für Lordaeron!"
#define GOSSIP_ITEM_ARTHAS_5 "Wir sind bereit für den Kampf mit Mal`Ganis, Sir."
...
public:
        ...
        if (ai && ai->bStepping == false)
        {
            switch (ai->uiGossipStep)
            {
                case 0: //This one is a workaround since the very beggining of the script is wrong.
                {
                    ...
                    ai->instance->DoUpdateWorldState(WORLDSTATE_SHOW_CRATES, 0);
                    break;
                }
                case 1:
                ...
                case 2:
                    ...
                    ai->instance->DoUpdateWorldState(WORLDSTATE_WAVE_COUNT, 0);
                    break;
                case 3:
    ...
    struct npc_arthasAI : public npc_escortAI
    {
        ...
        void SpawnWaveGroup(uint32 waveID, uint64* guidVector)
        {
            ...
            npc_arthasAI* ai = CAST_AI(npc_arthas::npc_arthasAI, me->AI());
            ai->instance->SetData(DATA_WAVE_COUNT, ai->instance->GetData(DATA_WAVE_COUNT) + 1);
            announceWave(waveID);
        }

        void announceWave(uint32 waveID)
        {
            uint32 txt;
            switch (waveID)
            {
                case 0:
                    startTimeEvent();
                    txt = ANNOUNCE_KINGSTREET;
                    break;
                case 1:
                    txt = ANNOUNCE_FESTIVALSTREET_PLACE;
                    break;
                case 2:
                    txt = ANNOUNCE_COUNCIL;
                    break;
                case 3:
                    txt = ANNOUNCE_KINGSTREET_PLACE;
                    break;
                case 4:
                    txt = ANNOUNCE_FESTIVALSTREET_PLACE;
                    break;
            ...
            if (Unit* announcer = me->SummonCreature(NPC_ANNOUNCER, AnnouncerPos[0], AnnouncerPos[1], AnnouncerPos[2], AnnouncerPos[3], TEMPSUMMON_TIMED_DESPAWN, 100))
            {
                DoScriptText(txt, announcer);
            }
        }

        void startTimeEvent()
        {
            if (instance->GetData(DATA_INFINITE_EVENT) == NOT_STARTED)
            {
                Creature* infinite = me->SummonCreature(NPC_INFINITE, 2328.38f, 1273.05f, 132.7235f, 2.1935f, TEMPSUMMON_DEAD_DESPAWN);
                infinite->AI()->DoAction(ACTION_INFINITE_ADD_PORTAL);
            }
        }
