...
class instance_culling_of_stratholme : public InstanceMapScript
{
    public:
        ...
        struct instance_culling_of_stratholme_InstanceMapScript : public InstanceScript
        {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            case EVENT_TIME:
                                timeEventTimeLeft--;
                                DoUpdateWorldState(WORLDSTATE_TIME_GUARDIAN, timeEventTimeLeft);
                                if (timeEventTimeLeft == 0)
                                {
                                    if (Creature* infinite = instance->GetCreature(_infiniteGUID))
                                        infinite->AI()->DoAction(ACTION_INFINITE_EVADE);
                                    DoUpdateWorldState(WORLDSTATE_TIME_GUARDIAN_SHOW, 0);
                                }
                                else
                                    events.RescheduleEvent(EVENT_TIME, 60000);
                                break;
                        }
                    }
            }
            ...
            void startTimeRun()
            {
                if (GetData(DATA_INFINITE_EVENT) != DONE)
                {
                    events.ScheduleEvent(EVENT_TIME, 60000);
            ...
            std::string GetSaveData()
            {
                ...
                saveStream << "C S " << _encounterState[0] << ' ' << _encounterState[1] << ' '
                    << _encounterState[2] << ' ' << _encounterState[3] << ' ' << _encounterState[4] << ' ' << _encounterState[5];
