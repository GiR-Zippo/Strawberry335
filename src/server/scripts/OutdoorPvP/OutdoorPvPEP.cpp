...
// ep
OutdoorPvPEP::OutdoorPvPEP() : OutdoorPvP(sMapMgr->FindBaseNonInstanceMap(0))
{
...
{
    for (std::set<uint64>::iterator itr = m_players[0].begin(); itr != m_players[0].end(); ++itr)
    {
    ...
    for (std::set<uint64>::iterator itr = m_players[1].begin(); itr != m_players[1].end(); ++itr)
    {
        if (Player* player = m_map->GetPlayer(*itr))
        {
            for (int i = 0; i < 4; ++i)
