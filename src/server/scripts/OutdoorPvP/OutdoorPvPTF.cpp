...
OutdoorPvPTF::OutdoorPvPTF() : OutdoorPvP(sMapMgr->FindBaseNonInstanceMap(530))
{
...
void OPvPCapturePointTF::ChangeState()
{
    ...
    switch (m_State)
    {
        case OBJECTIVESTATE_ALLIANCE:
        {
            ...
            for (std::set<uint64>::iterator itr = m_activePlayers[0].begin(); itr != m_activePlayers[0].end(); ++itr)
        ...
        case OBJECTIVESTATE_HORDE:
        {
            ...
            for (std::set<uint64>::iterator itr = m_activePlayers[1].begin(); itr != m_activePlayers[1].end(); ++itr)
                if (Player* player = m_PvP->GetMap()->GetPlayer(*itr))
