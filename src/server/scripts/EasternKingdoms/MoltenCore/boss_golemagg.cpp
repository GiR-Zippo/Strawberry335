...
enum Texts
{
    // ToDo: Emote nach dem Tod der "Core Ragor"
    EMOTE_LOWHP             = -1409002,
};
    ...
    public:
        ...
        struct boss_golemaggAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                memset(&pCoreRagor, NULL, sizeof(pCoreRagor));
            };
            ...
            void EnterCombat(Unit* victim)
            {
                ...
                if (instance)
                {
                    coreRagor[0]= instance->GetData64(DATA_GOLEMAGG_CORE_RAGE_FIRST);
                    coreRagor[1]= instance->GetData64(DATA_GOLEMAGG_CORE_RAGE_SECOND);

                    Unit* target = SelectTarget(SELECT_TARGET_FARTHEST, 0, 100, true);
                    for (uint8 i=0; i < 2; ++i)
                    {
                        if (pCoreRagor[i]= me->GetCreature((*me), coreRagor[i]))
                        {
                            pCoreRagor[i]->AddThreat(target, 0.0f);
                            pCoreRagor[i]->AI()->AttackStart(target);
                        }
                    }
                }
            };
            ...
            {
                if (HealthAbovePct(10) || me->HasAura(SPELL_ENRAGE))
                    return;
            ...
            void JustDied(Unit* /*killer*/)
            {
                if (instance)
                {
                    instance->SetBossState(BOSS_GOLEMAGG_THE_INCINERATOR, DONE);

                    for (uint8 i=0; i < 2; ++i)
                    {
                        if (pCoreRagor[i])
                            pCoreRagor[i]->Kill(pCoreRagor[i]);
                    }
                }
            };
