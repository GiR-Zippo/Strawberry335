    ...
    struct boss_headless_horsemanAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            for (auto &ref : players)
                sLFGMgr->RewardDungeonDoneFor(285, ref.getSource());
        }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* soil)
    {
        if (InstanceScript* pInstance = player->GetInstanceScript()) {
            if (pInstance->GetData(DATA_HORSEMAN_EVENT) == DONE)
                return true;
        ...
        if (player->GetQuestStatus(11405) != QUEST_STATUS_NONE && player->getLevel() > 64)
        {
