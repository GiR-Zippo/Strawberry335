...
enum WarlockSpells
{
    ...
    WARLOCK_TALENT_IMPROVED_HEALTH_FUNNEL   = 18703,
    WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_1  = 60955,
    WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_2  = 60956,

    WARLOCK_GLYPH_SUCCUBUS                  = 56250,
    WARLOCK_SPELL_SECUDE                    = 6358,

    WARLOCK_SPELL_CONFLAGRATE               = 17962,
    WARLOCK_SPELL_CONFLAGRATE_CD            = 1500 // in ms
};
...
class spell_warl_health_funnel : public SpellScriptLoader
{
    ...
    class spell_warl_health_funnel_AuraScript : public AuraScript
    {
        ...
        {
            if (!sSpellMgr->GetSpellInfo(WARLOCK_TALENT_IMPROVED_HEALTH_FUNNEL)
                || !sSpellMgr->GetSpellInfo(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_1)
                || !sSpellMgr->GetSpellInfo(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_2))
                return false;
            return true;
        }
        ...
        void HandleAfterEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pUnit = GetCaster())
            {
                ...
                if (AuraEffect* pAuraEffect = pUnit->GetAuraEffectOfRankedSpell(WARLOCK_TALENT_IMPROVED_HEALTH_FUNNEL, EFFECT_0))
                {
                    if (pAuraEffect->GetAmount() == 10) // Rank 1
                        pUnit->AddAura(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_1, pUnitTarget);
                    else if (pAuraEffect->GetAmount() == 20) // Rank 2
                        pUnit->AddAura(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_2, pUnitTarget);
                }
            }
        }
        ...
        {
            if (Unit* pUnitTarget = GetTarget())
            {
                pUnitTarget->RemoveAurasDueToSpell(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_1);
                pUnitTarget->RemoveAurasDueToSpell(WARLOCK_SPELL_IMPROVED_HEALTH_FUNNEL_2);
            }
        }
        ...
        void HandleEffectCalcAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pUnit = GetCaster())
                if (AuraEffect* pAuraEffect = pUnit->GetAuraEffectOfRankedSpell(WARLOCK_TALENT_IMPROVED_HEALTH_FUNNEL, EFFECT_0))
                    AddPctN(amount, pAuraEffect->GetAmount());
        }

        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(spell_warl_health_funnel_AuraScript::HandleAfterEffectApply, EFFECT_0, SPELL_AURA_PERIODIC_HEAL, AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(spell_warl_health_funnel_AuraScript::HandleAfterEffectRemove, EFFECT_0, SPELL_AURA_PERIODIC_HEAL, AURA_EFFECT_HANDLE_REAL);
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_health_funnel_AuraScript::HandleEffectCalcAmount, EFFECT_0, SPELL_AURA_PERIODIC_HEAL);
        }
    };
...
class spell_warl_seduce : public SpellScriptLoader
{
    public:

    spell_warl_seduce() : SpellScriptLoader("spell_warl_seduce") { }

    class spell_warl_seduce_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warl_seduce_SpellScript);

        void HandleBeforeHit()
        {
            if (GetCaster())
            {
                if (Unit* pUnitOwner = GetCaster()->GetOwner())
                {
                    if (pUnitOwner->HasAura(WARLOCK_GLYPH_SUCCUBUS))
                    {
                        if (Unit* pUnitTarget = GetHitUnit())
                        {
                            pUnitTarget->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE, 0, pUnitTarget->GetAura(32409)); // SW:D shall not be removed.
                            pUnitTarget->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE_PERCENT);
                            pUnitTarget->RemoveAurasByType(SPELL_AURA_PERIODIC_LEECH);
                        }
                    }
                }
            }
        ...
        void Register()
        {
            BeforeHit += SpellHitFn(spell_warl_seduce_SpellScript::HandleBeforeHit);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_warl_seduce_SpellScript();
    }
};
...
public:
  ...
  class spell_warl_immolate_SpellScript : public SpellScript {
    ...
    bool Validate(SpellInfo const * /*spellEntry*/) {
      if (!sSpellMgr->GetSpellInfo(WARLOCK_SPELL_CONFLAGRATE))
        return false;
      return true;
    }
    ...
    void HandleAfterHit() {
      if (GetCaster())
        if (Player *pPlayer = GetCaster()->ToPlayer())
          pPlayer->AddSpellCooldown(
              WARLOCK_SPELL_CONFLAGRATE, 0,
              time(NULL) + WARLOCK_SPELL_CONFLAGRATE_CD / IN_MILLISECONDS);
    }

    void Register() {
      AfterHit += SpellHitFn(spell_warl_immolate_SpellScript::HandleAfterHit);
    }
  };
