...
class spell_mage_incanters_absorbtion_base_AuraScript : public AuraScript
{
    public:
        enum Spells
        {
            ...
            TALENT_MAGE_SHATTERED_BARRIER               = 44745,
        };
        ...
        void Trigger(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32 & absorbAmount)
        {
            ...
            if (dmgInfo.GetDamage() > aurEff->GetAmount() && GetSpellInfo()->SpellFamilyFlags[1] & 0x1)
                if (const AuraEffect* pAuraEffect = target->GetAuraEffectOfRankedSpell(TALENT_MAGE_SHATTERED_BARRIER, EFFECT_0))
                    if (roll_chance_i(pAuraEffect->GetSpellInfo()->ProcChance))
                        GetCaster()->CastSpell(target, 55080, true, NULL, GetEffect(0));
        }
};
...
enum FingersOfFrost
{
    SPELL_FINGERS_OF_FROST_R1 = 44543,
    SPELL_FINGERS_OF_FROST_R2 = 44545,
    SPELL_FINGERS_OF_FROST_PROC = 44544,
};
...
public:
    ...
    class spell_mage_frostbite_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_mage_frostbite_AuraScript);

        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            if (Unit* owner = GetTarget())
            {
                if (owner->HasAura(SPELL_FINGERS_OF_FROST_R1))
                    owner->AddAura(SPELL_FINGERS_OF_FROST_PROC, owner);
                else if (owner->HasAura(SPELL_FINGERS_OF_FROST_R2))
                    owner->AddAura(SPELL_FINGERS_OF_FROST_PROC, owner);
            }
        }
        ...
        {
            OnProc += AuraProc(spell_mage_frostbite_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_ADD_TARGET_TRIGGER);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_mage_frostbite_AuraScript();
    }
};
*/
