...
enum PaladinSpells
{
    ...
    SPELL_PALADIN_SANCTIFIED_RETRIBUTION        = 63531,

    SPELL_PALADIN_SANCTIFIED_WRATH              = 57318,
    SPELL_PALADIN_SANCTIFIED_WRATH_TALENT_R1    = 53375,

    SPELL_PALADIN_AURA_SEAL_OF_VENGEANCE        = 31803,
    SPELL_PALADIN_AURA_SEAL_OF_CORRUPTION       = 53742
};
    ...
    public:
        ...
        class spell_pal_divine_storm_SpellScript : public SpellScript
        {
            ...
            void TriggerHeal()
            {
                Unit* caster = GetCaster();
                if (caster)
                    caster->CastCustomSpell(SPELL_DIVINE_STORM_DUMMY, SPELLVALUE_BASE_POINT0, (GetHitDamage() * healPct) / 100, caster, TRIGGERED_FULL_MASK);
            }
...
class spell_pal_auras : public SpellScriptLoader
{
    public:

    spell_pal_auras() : SpellScriptLoader("spell_pal_auras") { }

    class spell_pal_auras_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_auras_AuraScript);

        bool Validate(SpellInfo const* /*entry*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_SANCTIFIED_RETRIBUTION))
                return false;
            return true;
        }
        ...
        bool Load() override
        {
            ...
            if (spellInfo->SpellFamilyFlags[0] & 0x40) // Devotion aura
                auraType = SPELL_AURA_MOD_RESISTANCE;
            else if (spellInfo->SpellFamilyFlags[0] & 0x20000) // Concentration aura
                auraType = SPELL_AURA_REDUCE_PUSHBACK;
            else if (spellInfo->SpellFamilyFlags[0] & 0x4000000)
            {
                if (spellInfo->Id == 32223) // Crusader Aura
                    auraType = SPELL_AURA_MOD_MOUNTED_SPEED_NOT_STACK;
                else // Shadow/Ice/Fire resistance aura
                    auraType = SPELL_AURA_MOD_RESISTANCE_EXCLUSIVE;
            }
            else
                return false;
        ...
        void HandleAfterEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pUnit = GetCaster())
                pUnit->AddAura(SPELL_PALADIN_SANCTIFIED_RETRIBUTION, pUnit);
        }

        void HandleAfterEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pUnit = GetCaster())
                pUnit->RemoveAura(SPELL_PALADIN_SANCTIFIED_RETRIBUTION, pUnit->GetGUID());
        }

        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(spell_pal_auras_AuraScript::HandleAfterEffectApply, EFFECT_0, auraType, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            AfterEffectRemove += AuraEffectRemoveFn(spell_pal_auras_AuraScript::HandleAfterEffectRemove, EFFECT_0, auraType, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        }
    ...
    AuraScript* GetAuraScript() const
    {
        return new spell_pal_auras_AuraScript();
    }
};
