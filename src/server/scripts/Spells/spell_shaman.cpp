...
enum ShamanSpells
{
    ...
    SPIRIT_WOLF_SPIRIT_WOLF_LEAP           = 58867,
};
...
enum LesserHealingWave
{
    SPELL_AURA_TIDAL_WAVES = 53390,
};

class spell_sha_lesser_healing_wave : public SpellScriptLoader
{
    public:
        spell_sha_lesser_healing_wave() : SpellScriptLoader("spell_sha_lesser_healing_wave") { }

        class spell_sha_lesser_healing_wave_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_sha_lesser_healing_wave_SpellScript)
            ...
            {
                if (Player* caster = GetCaster()->ToPlayer())
                    if (Aura* aura = caster->GetAura(SPELL_AURA_TIDAL_WAVES))
                    {
                        if (Spell* spell = GetSpell())
                            caster->RestoreSpellMods(spell, SPELL_AURA_TIDAL_WAVES, aura);
                        aura->DropCharge();
                    }
            }
            ...
            void Register()
            {
                AfterHit += SpellHitFn(spell_sha_lesser_healing_wave_SpellScript::RestoreEffect);
            }
        ...
        SpellScript* GetSpellScript() const
        {
            return new spell_sha_lesser_healing_wave_SpellScript();
        }
};
...
class spell_sha_spirit_wolf_leap : public SpellScriptLoader
{
    public:
        spell_sha_spirit_wolf_leap() : SpellScriptLoader("spell_sha_spirit_wolf_leap") { }

        class spell_sha_spirit_wolf_leap_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_sha_spirit_wolf_leap_SpellScript)

            bool Validate(SpellInfo const* /*spell*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPIRIT_WOLF_SPIRIT_WOLF_LEAP))
                    return false;
                return true;
            };
            ...
            {
                if (!GetExplTargetUnit() || !GetExplTargetUnit()->IsHostileTo(GetCaster()))
            ...
            void Register()
            {
                OnCheckCast += SpellCheckCastFn(spell_sha_spirit_wolf_leap_SpellScript::CheckCast);
            };
        ...
        SpellScript* GetSpellScript() const
        {
            return new spell_sha_spirit_wolf_leap_SpellScript();
        }
};
...
void AddSC_shaman_spell_scripts()
{
    ...
    new spell_sha_lesser_healing_wave();
    new spell_sha_flame_shock();
    new spell_sha_sentry_totem();
    new spell_sha_spirit_wolf_leap();
}
