...
{
    SPELL_ILLIDARI_AGENT_ILLUSION  = 38224,
    ITEM_ENCHANTED_ILLIDARI_TABARD = 31279,
    MODEL_ID_MALE_1                = 20351,
    MODEL_ID_MALE_2                = 20355,
    MODEL_ID_FEMALE_1              = 20353,
    MODEL_ID_FEMALE_2              = 20357
};
...
public:
    ...
    class spell_q10769_dissension_amongst_the_Ranks_SpellScript : public SpellScript
    {
        ...
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_ILLIDARI_AGENT_ILLUSION))
                return false;
            return true;
        }
        ...
        SpellCastResult CheckCast()
        {
            ...
            if (pPlayer)
            {
                if (!pPlayer->HasItemOrGemWithIdEquipped(ITEM_ENCHANTED_ILLIDARI_TABARD, 1))
                    return SPELL_FAILED_DONT_REPORT;

                return sSpellMgr->GetSpellInfo(SPELL_ILLIDARI_AGENT_ILLUSION)->CheckLocation(pPlayer->GetMapId(), pPlayer->GetZoneId(), pPlayer->GetAreaId());
            }
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_q10769_dissension_amongst_the_Ranks_SpellScript::CheckCast);
        }
    };
        ...
        void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (pPlayer)
            {
                if (pPlayer->getGender()== GENDER_MALE)
                    pPlayer->SetDisplayId(rand()%2==1?MODEL_ID_MALE_1:MODEL_ID_MALE_2);
                else
                    pPlayer->SetDisplayId(rand()%2==1?MODEL_ID_FEMALE_1:MODEL_ID_FEMALE_2);
            }
        }
...
{
    SPELL_FEL_REAVER_CONTROLLER_TRIGGER = 38002,
    SPELL_FEL_REAVER_CONTROLLER_CHANNEL = 38003,
    ENTRY_FEL_REAVER_SENTINAL           = 21949,
};

const Position Horde_Spawn_Position       ={ -2713.387f, 2680.526f, 85.320863f,  5.468f };
const Position Alliance_Spawn_Position    ={ -2679.010f, 1971.615f, 142.075379f, 1.580f };

class spell_q10612_q10613_the_fel_and_the_furious : public SpellScriptLoader
{
    public:
        spell_q10612_q10613_the_fel_and_the_furious() : SpellScriptLoader("spell_q10612_q10613_the_fel_and_the_furious") { }

        class spell_q10612_q10613_the_fel_and_the_furious_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q10612_q10613_the_fel_and_the_furious_SpellScript);

            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_FEL_REAVER_CONTROLLER_TRIGGER))
                    return false;
                return true;
            }
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (pPlayer = GetCaster()->ToPlayer())
                {
                    Position SpawnPosition;

                    if (pPlayer->GetTeam()== HORDE)
                        SpawnPosition = Horde_Spawn_Position;
                    else
                        SpawnPosition = Alliance_Spawn_Position;

                    Creature* pCreature = pPlayer->SummonCreature(ENTRY_FEL_REAVER_SENTINAL, SpawnPosition, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300*IN_MILLISECONDS);
                    if (pCreature)
                        pPlayer->CastSpell(pCreature, SPELL_FEL_REAVER_CONTROLLER_CHANNEL, true);
                }
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_q10612_q10613_the_fel_and_the_furious_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_q10612_q10613_the_fel_and_the_furious_SpellScript();
        };
};

class spell_q10612_q10613_the_fel_and_the_furious_channel : public SpellScriptLoader
{
public:
    spell_q10612_q10613_the_fel_and_the_furious_channel() : SpellScriptLoader("spell_q10612_q10613_the_fel_and_the_furious_channel") { }

    class spell_q10612_q10613_the_fel_and_the_furious_channel_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_q10612_q10613_the_fel_and_the_furious_channel_AuraScript);

        bool Validate(SpellInfo const* /*spell*/)
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_FEL_REAVER_CONTROLLER_CHANNEL))
                return false;
            return true;
        }
        ...
        void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                Unit* pUnit = NULL;
                if (pUnit = pPlayer->GetCharm())
                {
                    pPlayer->RemoveCharmAuras();
                    pUnit->DeleteCharmInfo();
        ...
        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_q10612_q10613_the_fel_and_the_furious_channel_AuraScript::HandleEffectApply, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(spell_q10612_q10613_the_fel_and_the_furious_channel_AuraScript::HandleEffectRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_q10612_q10613_the_fel_and_the_furious_channel_AuraScript();
    }
};
...
// CreatureEntry: 21195
// ObjectId:      184701
enum IWasALotOfThings
{
    QUEST_I_WAS_A_LOT_OF_THINGS = 10514,
    SPELL_TUBER_WHISTLE         = 36652,
    ENTRY_FELBOAR               = 21195,
    OBJECT_TUBER_MOUND          = 184701
};

class spell_q10514_i_was_a_lot_of_things : public SpellScriptLoader
{
    public:
        spell_q10514_i_was_a_lot_of_things() : SpellScriptLoader("spell_q10514_i_was_a_lot_of_things") { }

        class spell_q10514_i_was_a_lot_of_things_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q10514_i_was_a_lot_of_things_SpellScript);

            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_TUBER_WHISTLE))
                    return false;
                return true;
            }
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (pPlayer = GetCaster()->ToPlayer())
                {
                    Creature* pCreature = pPlayer->FindNearestCreature(ENTRY_FELBOAR, 15.0f);

                    Position RandomPosition;
                    pPlayer->GetNearPosition(RandomPosition, 10.0f, (float)rand_norm() * static_cast<float>(2 * M_PI));

                    if (!pCreature)
                        pCreature = pPlayer->SummonCreature(ENTRY_FELBOAR, RandomPosition, TEMPSUMMON_TIMED_DESPAWN, 20*IN_MILLISECONDS);
                    if (pCreature && !pCreature->isInCombat() && !pCreature->isMoving())
                    {
                        pCreature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        pCreature->SetDefaultMovementType(POINT_MOTION_TYPE);
                        pCreature->GetMotionMaster()->Initialize();

                        GameObject* pObject = pPlayer->FindNearestGameObject(OBJECT_TUBER_MOUND, 5.0f);
                        if (pObject)
                        {
                            pObject->GetContactPoint(pObject, RandomPosition.m_positionX, RandomPosition.m_positionY, RandomPosition.m_positionZ, 1.5f);
                            pCreature->GetMotionMaster()->Clear();
                            pCreature->GetMotionMaster()->MovePoint(1, RandomPosition);
                        }
                    }
                }
            }
            ...
            void Register()
            {
                OnEffectLaunch += SpellEffectFn(spell_q10514_i_was_a_lot_of_things_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_q10514_i_was_a_lot_of_things_SpellScript();
        };
};

class spell_q8149_8150_harvest_festival : public SpellScriptLoader
{
    public:

    spell_q8149_8150_harvest_festival() : SpellScriptLoader("spell_item_harvest_festival_tribute") { }

    class spell_q8149_8150_harvest_festival_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q8149_8150_harvest_festival_SpellScript);
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            if (Unit* pUnit = GetCaster())
            {
                if (Item* pItem = GetCastItem())
                {
                    ...
                    switch (pUnit->getRace())
                    {
                        // Alliance item 19850 - 24194
                        case RACE_HUMAN:
                            spellID = 24105;
                            break;
                        case RACE_DWARF:
                    ...
                    if (spellID)
                        pUnit->CastSpell(pUnit, spellID, true, pItem);
                }
            }
        }
        ...
        {
            OnEffectHit += SpellEffectFn(spell_q8149_8150_harvest_festival_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_q8149_8150_harvest_festival_SpellScript();
    }
};

enum RamRacingReins
{
    RRR_AURA_RAM_BASE = 42924,

    RRR_AURA_RAM_TROT = 42992,
    RRR_AURA_RAM_CANTER = 42993,
    RRR_AURA_RAM_GALOPP = 42994,

    RRR_AURA_RAM_EXHAUSTED = 43332,

    RRR_KILL_CREDIT_FIRST = 24263,
    RRR_KILL_CREDIT_SECOND = 24264,
    RRR_KILL_CREDIT_THIRD = 24265
};

class spell_q11318_11409_brewfest : public SpellScriptLoader
{
    public:

    spell_q11318_11409_brewfest() : SpellScriptLoader("spell_item_brewfest_ram_racing_reins") {}

    class spell_q11318_11409_brewfest_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q11318_11409_brewfest_SpellScript);

        bool Validate(SpellInfo const* /*spellEntry*/) override
        {
            if (!sSpellMgr->GetSpellInfo(RRR_AURA_RAM_TROT) || !sSpellMgr->GetSpellInfo(RRR_AURA_RAM_CANTER)
                || !sSpellMgr->GetSpellInfo(RRR_AURA_RAM_GALOPP) || !sSpellMgr->GetSpellInfo(RRR_AURA_RAM_BASE))
                return false;
            return true;
        }
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            Unit* pUnit = GetCaster();
            if (!pUnit || pUnit->GetTypeId() != TYPEID_PLAYER)
                return;

            if (pUnit->HasAura(RRR_AURA_RAM_EXHAUSTED))
                return;

            Aura* pAura = pUnit->GetAura(RRR_AURA_RAM_BASE);
            if (!pAura)
                return;

            int8 value = GetValue(pAura->GetStackAmount());

            ApplySpeedAurasDueToValue(pUnit, value);
        }

        void Register() override
        {
            OnEffectHit += SpellEffectFn(spell_q11318_11409_brewfest_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };

    class spell_q11318_11409_brewfest_AuraScript : public AuraScript
    {
        public:

        PrepareAuraScript(spell_q11318_11409_brewfest_AuraScript);

        bool Load() override
        {
            memset(&speedStackTimes, NULL, sizeof(speedStackTimes));
            speedTimes = 0;
            return true;
        }
        ...
        bool Validate(SpellInfo const* /*spellEntry*/) override
        {
            if (!sSpellMgr->GetSpellInfo(RRR_AURA_RAM_TROT) || !sSpellMgr->GetSpellInfo(RRR_AURA_RAM_CANTER) || !sSpellMgr->GetSpellInfo(RRR_AURA_RAM_GALOPP))
                return false;
            return true;
        }
        ...
        void HandleOnEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            int8 value = GetValue(GetStackAmount());

            if (value != -1)
            {
                if (value == 0 && speedTimes > 0)
                    speedTimes -= 1;
                else
                    speedTimes += value;

                ++speedStackTimes[value];

                // Need 4 times periodic update = 8 seconds
                for (uint8 i = 0; i < 3; ++i)
                {
                    if (i != value)
                        speedStackTimes[i] = 0;

                    if (speedStackTimes[i] == 5)
                        pUnit->ToPlayer()->KilledMonsterCredit(RRR_KILL_CREDIT_FIRST + i, NULL);
                }
            }
            else
                speedTimes = 0;
            ...
            if (speedTimes >= 14)
            {
                pUnit->AddAura(RRR_AURA_RAM_EXHAUSTED, pUnit);
                ModStackAmount(1 - GetStackAmount());
                speedTimes = 0;
            }
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_q11318_11409_brewfest_AuraScript::HandleOnEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }

        private:

        uint8 speedStackTimes[3];
        uint8 speedTimes;
    ...
    SpellScript* GetSpellScript() const override
    {
        return new spell_q11318_11409_brewfest_SpellScript();
    }

    AuraScript* GetAuraScript() const override
    {
        return new spell_q11318_11409_brewfest_AuraScript();
    }

    static int8 GetValue(uint8 stackAmount)
    {
        // Ranges: 2 -> 4 : 5 -> 8 : 9 -> 11
        if (stackAmount > 1 && stackAmount <= 4)
            return 0;
        else if (stackAmount > 4 && stackAmount <= 8)
            return 1;
        else if (stackAmount > 8)
            return 2;
        else
            return -1;
    }

    static void ApplySpeedAurasDueToValue(Unit* pUnit, int8 value)
    {
        if (!pUnit)
            return;
        ...
        for (uint8 i = 0; i < 3; ++i)
        {
            if (i != value || pUnit->HasAura(RRR_AURA_RAM_EXHAUSTED))
                pUnit->RemoveAurasDueToSpell(RRR_AURA_RAM_TROT + i);
            else if (i != -1)
                pUnit->AddAura(RRR_AURA_RAM_TROT + value, pUnit);
        }
    }
};

uint32 KillCredits_12736[] = { 29056, 29057, 29058, 29069 };

class spell_q12736_sholazar_basin : public SpellScriptLoader
{
    public:

    spell_q12736_sholazar_basin() : SpellScriptLoader("spell_q12736_sholazar_basin") { }

    class spell_q12736_sholazar_basin_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q12736_sholazar_basin_SpellScript);
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            ...
            if (Unit* pUnit = GetCaster())
            {
                ...
                Unit* pUnitTarget = GetExplTargetUnit();

                if (!pUnitTarget)
                {
                    for (uint8 i = 0; i < 4; ++i)
                        if (pUnitTarget = pUnit->FindNearestCreature(KillCredits_12736[i], 100.0f))
                            break;
                }

                if (pUnitTarget)
                    pUnit->ToPlayer()->KilledMonsterCredit(pUnitTarget->GetEntry(), NULL);
            }
        }

        void Register() override
        {
            OnEffectHit += SpellEffectFn(spell_q12736_sholazar_basin_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_q12736_sholazar_basin_SpellScript();
    }
};
...
void AddSC_quest_spell_scripts()
{
    ...
    new spell_q10612_q10613_the_fel_and_the_furious();
    new spell_q10612_q10613_the_fel_and_the_furious_channel();
    new spell_q10514_i_was_a_lot_of_things();
    new spell_q8149_8150_harvest_festival();
    new spell_q11318_11409_brewfest();
    new spell_q12736_sholazar_basin();
}
