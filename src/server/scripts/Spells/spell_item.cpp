...
// Generic script for handling item dummy effects with two or more trigger spells.
class spell_item_trigger_spells : public SpellScriptLoader
{
    public:

    spell_item_trigger_spells(const char* name, SpellEffIndex effectIndex, SpellEffects effect, uint32 spellID_1, uint32 spellID_2,
                              uint32 spellID_3 = NULL, uint32 spellID_4 = NULL, uint32 spellID_5 = NULL)
                              : SpellScriptLoader(name), spellEffectIndex(effectIndex), spellEffect(effect)
    {
        // Initialize vector
        triggeredSpellIDVector.clear();

        triggeredSpellIDVector.push_back(spellID_1);
        triggeredSpellIDVector.push_back(spellID_2);
        triggeredSpellIDVector.push_back(spellID_3);
        triggeredSpellIDVector.push_back(spellID_4);
        triggeredSpellIDVector.push_back(spellID_5);
    }

    SpellScript* GetSpellScript() const override
    {
        return new spell_item_trigger_spells_SpellScript(triggeredSpellIDVector, spellEffectIndex, spellEffect);
    }

    class spell_item_trigger_spells_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_trigger_spells_SpellScript);

        public:

            spell_item_trigger_spells_SpellScript(const std::vector<uint32>& spellIDVector, SpellEffIndex effectIndex, SpellEffects effect)
                : SpellScript(), triggeredSpellIDVector(spellIDVector), spellEffect(effect), spellEffectIndex(effectIndex) { }

            bool Validate(SpellInfo const* /*spellEntry*/) override
            {
                if (triggeredSpellIDVector.empty())
                    return false;

                for (std::vector<uint32>::const_iterator itr = triggeredSpellIDVector.begin(); itr != triggeredSpellIDVector.end(); ++itr)
                    if ((*itr) != NULL)
                        if (!sSpellMgr->GetSpellInfo(*itr))
                            return false;
                return true;
            }

            void HandleTriggerSpells(SpellEffIndex /*effIndex*/)
            {
                Unit* pUnit = GetCaster();
                if (Item* pItem = GetCastItem())
                    for (std::vector<uint32>::const_iterator itr = triggeredSpellIDVector.begin(); itr != triggeredSpellIDVector.end(); ++itr)
                        if ((*itr) != NULL)
                            pUnit->CastSpell(pUnit, *itr, true, pItem);
            }

            void Register() override
            {
                OnEffectHit += SpellEffectFn(spell_item_trigger_spells_SpellScript::HandleTriggerSpells, spellEffectIndex, spellEffect);
            }

        private:

            const std::vector<uint32> triggeredSpellIDVector;
    ...
    private:
        ...
        std::vector<uint32> triggeredSpellIDVector;
        SpellEffects spellEffect;
        SpellEffIndex spellEffectIndex;
};
...
class spell_item_arena_queue : public SpellScriptLoader
{
public:
    spell_item_arena_queue() : SpellScriptLoader("spell_item_arena_queue") { }

    class spell_item_arena_queue_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_arena_queue_SpellScript);

        void HandleDummy()
        {
            BattlegroundTypeId bgTypeId = sBattlegroundMgr->GetBattleMasterBG(29533);
            if (!GetCaster()->ToPlayer()->GetBGAccessByLevel(bgTypeId))
            {
                // temp, must be gossip message...
                GetCaster()->ToPlayer()->GetSession()->SendNotification(LANG_YOUR_BG_LEVEL_REQ_ERROR);
                return;
            }
            GetCaster()->ToPlayer()->GetSession()->SendBattleGroundList(0, bgTypeId);
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_item_arena_queue_SpellScript::HandleDummy);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_item_arena_queue_SpellScript();
    }
};
