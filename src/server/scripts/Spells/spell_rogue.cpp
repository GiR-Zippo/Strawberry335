...
enum RogueSpells
{
    ...
    ROGUE_SPELL_KILLING_SPREE_PORT               = 57840,
    ROGUE_SPELL_KILLING_SPREE_DMG                = 57841,
    ROGUE_SPELL_KILLING_SPREE_DESEL              = 61851,
};
...
// Killing Spree
class KillingSpreeTargetFilter
{
    public:

        bool operator()(Unit* pUnit) const
        {
            if (pUnit->GetOwner())
                return true;
            return false;
...
class spell_killing_spree : public SpellScriptLoader
{
    public:
        spell_killing_spree() : SpellScriptLoader("spell_killing_spree") { }

        class spell_killing_spree_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_killing_spree_AuraScript);

            bool Validate(const SpellInfo*)
            {
                if (!sSpellMgr->GetSpellInfo(ROGUE_SPELL_KILLING_SPREE_PORT))
                    return false;
                if (!sSpellMgr->GetSpellInfo(ROGUE_SPELL_KILLING_SPREE_DMG))
                    return false;
                return true;
            }
            ...
            void HandleDummy (AuraEffect const* aurEff)
            {
                UnitList targets;
                {
                    ...
                    CellCoord p(Trinity::ComputeCellCoord(GetCaster()->GetPositionX(), GetCaster()->GetPositionY()));
                    Cell cell(p);

                    Trinity::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(GetCaster(), radius);
                    Trinity::UnitListSearcher<Trinity::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(GetCaster(), targets, u_check);
                    ...
                    cell.Visit(p, grid_object_checker, *GetCaster()->GetMap(), *GetCaster(), radius);
                    cell.Visit(p, world_object_checker, *GetCaster()->GetMap(), *GetCaster(), radius);
                }

                targets.remove_if(KillingSpreeTargetFilter());
                if (targets.empty())
                    return;

                if (Unit* spellTarget = Trinity::Containers::SelectRandomContainerElement(targets))
                {
                    GetTarget()->CastSpell(spellTarget, ROGUE_SPELL_KILLING_SPREE_PORT, true);
                    GetTarget()->CastSpell(spellTarget, ROGUE_SPELL_KILLING_SPREE_DMG, true);
                }
            }

            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_killing_spree_AuraScript::HandleDummy, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
            }
        };

        class spell_killing_spree_SpellScript : public SpellScript
        {
           PrepareSpellScript(spell_killing_spree_SpellScript);

           void HandleHitTarget(SpellEffIndex /*effIndex*/)
           {
               GetCaster()->RemoveAurasByType(SPELL_AURA_MOD_ROOT);
               GetCaster()->CastSpell(GetCaster(), ROGUE_SPELL_KILLING_SPREE_DESEL, true);
           }
           ...
           void Register()
           {
               OnEffectHitTarget += SpellEffectFn(spell_killing_spree_SpellScript::HandleHitTarget, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
               OnCheckCast += SpellCheckCastFn(spell_killing_spree_SpellScript::HandleCheckCast);
           }
        };

        SpellScript* GetSpellScript() const
        {
           return new spell_killing_spree_SpellScript();
        }

        AuraScript* GetAuraScript() const
        {
            return new spell_killing_spree_AuraScript();
        }
};
