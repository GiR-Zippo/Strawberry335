...
class spell_dk_anti_magic_shell_raid : public SpellScriptLoader
{
    public:
        ...
        class spell_dk_anti_magic_shell_raid_AuraScript : public AuraScript
        {
            ...
            bool Load()
            {
                Unit* pUnitOwner = GetCaster()->GetOwner() ? GetCaster()->GetOwner() : GetCaster();
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32 & amount, bool & /*canBeRecalculated*/)
            {
                ...
                amount = talentSpell->Effects[EFFECT_0].CalcValue(pUnitOwner);
                if (Player* pPlayer = pUnitOwner->ToPlayer())
                    amount += int32(2 * pPlayer->GetTotalAttackPowerValue(BASE_ATTACK));
            }

            void Absorb(AuraEffect* /*aurEff*/, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                absorbAmount = std::min(CalculatePctN(dmgInfo.GetDamage(), absorbPct), absorbAmount);
            }
...
class spell_dk_anti_magic_zone : public SpellScriptLoader
{
    public:
        ...
        class spell_dk_anti_magic_zone_AuraScript : public AuraScript
        {
            ...
            {
                Unit* pUnitOwner = GetCaster()->GetOwner() ? GetCaster()->GetOwner() : GetCaster();
                absorbPct = GetSpellInfo()->Effects[EFFECT_0].CalcValue(pUnitOwner);
                return true;
            }
    ...
    public:
        ...
        class spell_dk_scourge_strike_SpellScript : public SpellScript
        {
            ...
            void HandleAfterHit()
            {
                ...
                if (Unit* unitTarget = GetHitUnit())
                {
                    int32 bp = GetCleanDamage() * multiplier;
...
public:
    ...
    class spell_dk_death_grip_dummySpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_death_grip_dummySpellScript);
        ...
        SpellCastResult CheckCast()
        {
            ...
            if (target && target->GetTypeId() == TYPEID_PLAYER && target->IsHostileTo(caster))
        ...
        void HandleDummy(SpellEffIndex effIndex)
        {
            if (Unit* target = GetHitUnit())
            {
                if (target->HasAuraType(SPELL_AURA_REFLECT_SPELLS)) // Spellreflection (Warrior) hackfix @todo
                {
                    target->CastSpell(GetCaster(), 49560, true);
                    target->RemoveAurasByType(SPELL_AURA_REFLECT_SPELLS);
                }
                else
                    GetCaster()->CastSpell(target, 49560, true);
            }
        }

        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_dk_death_grip_dummySpellScript::CheckCast);
            OnEffectHitTarget += SpellEffectFn(spell_dk_death_grip_dummySpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    ...
    SpellScript* GetSpellScript() const
    {
        return new spell_dk_death_grip_dummySpellScript();
    }
};

class spell_dk_ghoul_taunt : public SpellScriptLoader
{
    public:

    spell_dk_ghoul_taunt() : SpellScriptLoader("spell_dk_ghoul_taunt") { }

    class spell_dk_ghoul_taunt_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_ghoul_taunt_SpellScript);

        SpellCastResult CheckCast()
        {
            if (Unit* pUnit = GetExplTargetUnit())
                if (Creature* pCreature = pUnit->ToCreature())
                    if (!pCreature->IsDungeonBoss() && !pCreature->isWorldBoss())
                        return SPELL_CAST_OK;
            return SPELL_FAILED_BAD_TARGETS;
        }
        ...
        {
            OnCheckCast += SpellCheckCastFn(spell_dk_ghoul_taunt_SpellScript::CheckCast);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_dk_ghoul_taunt_SpellScript();
    }
};
...
public:
    ...
    class spell_dk_chains_of_iceSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_chains_of_iceSpellScript);
        
        void HandleApply()
        {
            if (Player* target = GetHitPlayer())
                target->RemoveAura(DK_SPELL_CHAINS_OF_ICE);
        }

        void Register()
        {
            BeforeHit += SpellHitFn(spell_dk_chains_of_iceSpellScript::HandleApply);
        }
    ...
    SpellScript* GetSpellScript() const
    {
        return new spell_dk_chains_of_iceSpellScript();
    }
};
        ...
        void OnHit(SpellEffIndex /*effIndex*/)
        {
            ...
            if (victim != hitUnit || caster->HasAura(SPELL_DK_GLYPH_OF_DISEASE))
            {
                if (Aura* aurOld = victim->GetAura(SPELL_DK_BLOOD_PLAGUE, caster->GetGUID())) // Check Blood Plague application on victim.
                {
                    ...
                    {
                        caster->CastSpell(hitUnit, SPELL_DK_BLOOD_PLAGUE, TRIGGERED_FULL_MASK); // Spread the disease to hitUnit.
                        
                        if (Aura* aurNew = hitUnit->GetAura(SPELL_DK_BLOOD_PLAGUE, caster->GetGUID())) // Check Blood Plague application on hitUnit.
                            aurNew->SetDuration(aurOld->GetDuration());
                    }
                }
                ...
                if (Aura* aurOld = victim->GetAura(SPELL_DK_FROST_FEVER, caster->GetGUID())) // Check Frost Fever application on victim.
                {
                    if (AuraEffect* aurEffOld = aurOld->GetEffect(EFFECT_0))
                    {
                        caster->CastSpell(hitUnit, SPELL_DK_FROST_FEVER, TRIGGERED_FULL_MASK); // Spread the disease to hitUnit.
...
// ID - 50526 Wandering Plague (AOE Damage)
class spell_dk_wandering_plague_aoe : public SpellScriptLoader
{
public:
    spell_dk_wandering_plague_aoe() : SpellScriptLoader("spell_dk_wandering_plague_aoe") { }

    class spell_dk_wandering_plague_aoe_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_wandering_plague_aoe_SpellScript);
        
        void OnTargetSelect(std::list<WorldObject*>& targets)
        {
            erase_if(targets, [](WorldObject *target) {
                Unit *unit = target->ToUnit();
                return unit ? unit->HasBreakableByDamageCrowdControlAura() || unit->HasStealthAura() || unit->HasInvisibilityAura()
                            : false;
            });
        }
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_dk_wandering_plague_aoe_SpellScript::OnTargetSelect, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_dk_wandering_plague_aoe_SpellScript();
    }
};
...
void AddSC_deathknight_spell_scripts()
{
    ...
    new spell_dk_ghoul_taunt();
    new spell_dk_chains_of_ice();
    new spell_dk_pestilence();
    new spell_dk_wandering_plague_aoe();
}
