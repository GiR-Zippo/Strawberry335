...
enum HunterSpells
{
    ...
    HUNTER_SPELL_GIFT_OF_THE_NAARU               = 59543,
};
        ...
        class spell_hun_masters_call_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                if (Player* caster = GetCaster()->ToPlayer())
                {
                    if (Pet* pet = caster->GetPet())
                    {
                        ...
                        if (pet->HasUnitState(UNIT_STATE_FLEEING) || pet->HasUnitState(UNIT_STATE_CONFUSED))
                        {
            ...
            void HandleScriptEffect(SpellEffIndex /*effIndex*/)
            {
                if (Unit* target = GetHitUnit())
                {
                    ...
                    //target->CastSpell(target, GetEffectValue(), castMask); // Adds Immunity-Mask for Stuns
                    ...
                    {
                        //target->CastSpell(ally, GetEffectValue(), castMask); // Adds Immunity-Mask for Stuns
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                for (SpellCooldowns::const_iterator itr = cm.begin(); itr != cm.end();)
                {
                    ...
                    if (spellInfo &&
                        ...
                        spellInfo->Id != HUNTER_SPELL_GIFT_OF_THE_NAARU &&
            ...
            void HandlePeriodic(AuraEffect const* aurEff)
            {
                ...
                if (aurEff->GetAmount() <= 0)
                {
                    ...
                    if (Unit* target = GetTarget())
                    {
                        ...
                        if (Player* pPlayerOwner = GetUnitOwner()->ToPlayer())
                        {
                            int32 baseAmount = aurEff->GetBaseAmount();
                            int32 amount = pPlayerOwner->CalculateSpellDamage(pPlayerOwner, GetSpellInfo(), aurEff->GetEffIndex(), &baseAmount);
        ...
        class spell_hun_pet_heart_of_the_phoenix_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* owner = caster->GetOwner())
                    ...
                    {
                        owner->CastCustomSpell(HUNTER_PET_HEART_OF_THE_PHOENIX_TRIGGERED, SPELLVALUE_BASE_POINT0, 100, caster, TRIGGERED_FULL_MASK);
