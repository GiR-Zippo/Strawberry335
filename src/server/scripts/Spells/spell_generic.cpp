...
enum VehicleScaling
{
    SPELL_WG_GEAR_SCALING   = 65635,
    SPELL_SOTA_GEAR_SCALING = 65636
};
    ...
    public:
        ...
        class spell_gen_vehicle_scaling_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
            {
                ...
                switch (GetId())
                {
                    case SPELL_WG_GEAR_SCALING:
                    case SPELL_SOTA_GEAR_SCALING:
                        factor = 1.0f;
                        baseItemLevel = 200;
                        break;
                    default:
                ...
                float avgILvl = pCaster->ToPlayer()->GetAverageItemLevel();
                if (avgILvl < baseItemLevel)
                    return;                     // TODO: Research possibility of scaling down

                amount = int32((avgILvl - baseItemLevel) * factor);
            }
...
class spell_gen_ds_flush : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_ds_flush_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_ds_flush_SpellScript::FilterTargets, EFFECT_ALL, TARGET_UNIT_DEST_AREA_ENTRY);
            }
        };
...
class spell_gen_ds_spout : public SpellScriptLoader
{
    public:
        spell_gen_ds_spout() : SpellScriptLoader("spell_gen_ds_spout") { }

        class spell_gen_ds_spout_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_ds_spout_SpellScript);
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                if (Map* map = GetCaster()->GetMap())
                {
                    if (map->IsBattleArena())
                    {
                        ...
                        for (Map::PlayerList::const_iterator itr = map->GetPlayers().begin(); itr != map->GetPlayers().end(); ++itr)
                            if (Player* player = itr->getSource())
                                if (player->GetDistance(GetCaster()) <= 7.0f)
                                    unitList.push_back(player);
                    }
                }
            }
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_ds_spout_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_ds_spout_SpellScript();
        }
};
...
// SpellScript Ulduar - Halls of Stone - Tribunal of the Ages
// SpellName:       Searing Gaze
// SpellId:         51136
// H_SpellId:       59867
// NeededData:      OnObjectAreaTargetSelect
// NeededData:      OnEffectRemove
// Type:            SPELL_AURA_DUMMY
// CreatureEntry:   30899
// H_CreatureEntry: 31878
enum SearingGazeDummy
{
    SPELL_SEARING_GAZE       = 51136,
    H_SPELL_SEARING_GAZE     = 59867,
    NPC_CHECK_TARGET_ENTRY   = 30899,
};

class SearingGazeTargetFilter
{
    public:
        bool operator()(WorldObject* target) const
        {
            if (Unit* unit = target->ToUnit())
                return unit->GetEntry() != NPC_CHECK_TARGET_ENTRY;
            return false;
        }
};

class spell_gen_searing_gaze_dummy : public SpellScriptLoader
{
    public:
        spell_gen_searing_gaze_dummy() : SpellScriptLoader("spell_gen_searing_gaze_dummy"){}

        class spell_gen_searing_gaze_dummy_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_searing_gaze_dummy_SpellScript);

            void FilterTargets(std::list<WorldObject*>& targets)
            {
                targets.remove_if(SearingGazeTargetFilter());
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_searing_gaze_dummy_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
            }
        };

        class spell_gen_searing_gaze_dummy_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_gen_searing_gaze_dummy_AuraScript);
            ...
            void HandleEffectRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
            {
                ...
                if (pUnit)
                {
                    pUnit->RemoveAurasDueToSpell(SPELL_SEARING_GAZE);
                    pUnit->RemoveAurasDueToSpell(H_SPELL_SEARING_GAZE);
                }
            }

            void Register()
            {
                OnEffectRemove += AuraEffectApplyFn(spell_gen_searing_gaze_dummy_AuraScript::HandleEffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_searing_gaze_dummy_SpellScript();
        }

        AuraScript* GetAuraScript() const
        {
            return new spell_gen_searing_gaze_dummy_AuraScript();
        }
};

// SpellScript Ulduar - Halls of Stone - Tribunal of the Ages
// SpellName:       Kill Tribunal Add
// SpellId:         51288
// NeededData:      OnObjectAreaTargetSelect
// KillPlayer:      true
// CreatureEntry:   27983, 27984, 27985
enum KillTribunalAdds
{
    SPELL_KILL_TRIBUNAL      = 51288,
    NPC_CHECK_TARGET_ENTRY_1 = 27983,
    NPC_CHECK_TARGET_ENTRY_2 = 27984,
    NPC_CHECK_TARGET_ENTRY_3 = 27985,
};

class KillTribunalAddsTargetFilter
{
    public:
        bool operator()(WorldObject* target) const
        {
            if (Unit* unit = target->ToUnit())
            {
                ...
                {
                    case NPC_CHECK_TARGET_ENTRY_1:
                    case NPC_CHECK_TARGET_ENTRY_2:
                    case NPC_CHECK_TARGET_ENTRY_3:
                        return false;
                    break;
                }
            }
...
class spell_gen_kill_tribunal_add : public SpellScriptLoader
{
    public:
        spell_gen_kill_tribunal_add() : SpellScriptLoader("spell_gen_kill_tribunal_add"){}

        class spell_gen_kill_tribunal_add_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_kill_tribunal_add_SpellScript);

            void FilterTargets(std::list<WorldObject*>& targets)
            {
                targets.remove_if(KillTribunalAddsTargetFilter());
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_kill_tribunal_add_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_kill_tribunal_add_SpellScript();
        }
};

// Bug #2552 Triggered Spell BladeWarding
enum BladeWarding
{
...
class spell_gen_blade_warding_trigger : public SpellScriptLoader
{
    public:

    spell_gen_blade_warding_trigger() : SpellScriptLoader("spell_gen_blade_warding_trigger") { }

    class spell_gen_blade_warding_trigger_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_blade_warding_trigger_SpellScript);

        bool Validate(SpellInfo const* /*spell*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_BLADE_WARDING_PROC) || !sSpellMgr->GetSpellInfo(SPELL_BLADE_WARDING_DAMAGE))
                return false;
            return true;
        }
        ...
        void HandleEffectHitTarget(SpellEffIndex effIndex)
        {
            ...
            {
                float factor = pUnit->GetAura(SPELL_BLADE_WARDING_PROC)->GetStackAmount() ? pUnit->GetAura(SPELL_BLADE_WARDING_PROC)->GetStackAmount() : 1;
                damage = GetSpellInfo()->Effects[effIndex].CalcValue(GetCaster(), NULL, NULL, factor);
            }

            SetHitDamage(damage ? damage : 1);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_gen_blade_warding_trigger_SpellScript::HandleEffectHitTarget, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_gen_blade_warding_trigger_SpellScript();
    }
};


class spell_gen_player_scaling : public SpellScriptLoader
{
    public:

    spell_gen_player_scaling() : SpellScriptLoader("spell_gen_player_scaling") { }

    class spell_gen_player_scaling_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_player_scaling_SpellScript);
        ...
        void HandleEffectHitTarget(SpellEffIndex effIndex)
        {
            ...
            if (Unit* pUnit = GetHitUnit())
            {
                ...
                {
                    int32 stackAmount = GetHitDamage();
                    pPlayer->SetAuraStack(GetSpellInfo()->Id, pPlayer, stackAmount);
                }
            }
        }
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_gen_player_scaling_SpellScript::HandleEffectHitTarget, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
        }
    };

    class spell_gen_player_scaling_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_player_scaling_AuraScript);

        void HandleAfterEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pUnit = GetUnitOwner())
                if (Player* pPlayer = pUnit->ToPlayer())
                    if (uint32 itemlevel = GetStackAmount() + PSM::CONFIG_PSM_MIN_ITEMLEVEL - 1)
                        pPlayer->GetStatsModifier()->SetModifierDueToItemLevel(itemlevel);
        }
        ...
        void HandleAfterEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pUnit = GetUnitOwner())
                if (Player* pPlayer = pUnit->ToPlayer())
                    pPlayer->GetStatsModifier()->ResetModifier();
        }

        void Register()
        {
            AfterEffectApply += AuraEffectApplyFn(spell_gen_player_scaling_AuraScript::HandleAfterEffectApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAPPLY);
            AfterEffectRemove += AuraEffectRemoveFn(spell_gen_player_scaling_AuraScript::HandleAfterEffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_player_scaling_SpellScript();
    }

    AuraScript* GetAuraScript() const
    {
        return new spell_gen_player_scaling_AuraScript();
    }
};
...
class spell_gen_inherit_masters_threat_list : public SpellScriptLoader
{
    public:

    spell_gen_inherit_masters_threat_list() : SpellScriptLoader("spell_gen_inherit_masters_threat_list") { }

    class spell_gen_inherit_masters_threat_list_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_inherit_masters_threat_list_SpellScript);

        void HandleDummy()
        {
            if (Unit* pUnitCaster = GetCaster())
            {
                if (Unit* pUnitTarget = GetHitUnit())
                {
                    if (pUnitTarget->GetOwner() == pUnitCaster)
                    {
                        // Copy threat list
                        HostileReference* ref = pUnitCaster->getHostileRefManager().getFirst();
                        while (ref)
                        {
                            if (Unit* pUnit = ref->getSource()->getOwner())
                                pUnitTarget->AddThreat(pUnit, ref->getThreat());
                            ref = ref->next();
                        }
                    }
                }
        ...
        void Register()
        {
            OnHit += SpellHitFn(spell_gen_inherit_masters_threat_list_SpellScript::HandleDummy);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_inherit_masters_threat_list_SpellScript();
    }
};
...
void AddSC_generic_spell_scripts()
{
    ...
    new spell_gen_ds_spout();
    new spell_gen_upper_deck_create_foam_sword();
    new spell_gen_bonked();
    new spell_gen_vendor_bark_trigger();
    new spell_gen_spectator_cheer_trigger();
    new spell_gen_searing_gaze_dummy();
    new spell_gen_kill_tribunal_add();
    new spell_gen_blade_warding_trigger();
    new spell_gen_player_scaling();
    new spell_gen_shadowmeld();
    new spell_gen_inherit_masters_threat_list();
}
