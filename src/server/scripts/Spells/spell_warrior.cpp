...
enum WarriorSpells
{
    ...
    WARRIOR_INTERVENE_TRIGGER           = 59667
};
    ...
    public:
        ...
        class spell_warr_slam_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                ...
                if (Aura * aura = GetCaster()->GetAura(AURA_BLOODSURGE))
                    if (aura->GetCharges())
                    {
                        if (Player* caster = GetCaster()->ToPlayer())
                        {
                            caster->RestoreSpellMods(GetSpell(), AURA_BLOODSURGE); 
                            caster->RestoreSpellMods(GetSpell(), AURA_GCD_SLAM);
                        }
                        aura->DropCharge();
                    }
            }
...
enum Execute
{
    ...
    AURA_SUDDEN_DEATH           = 52437,
    AURA_GCD_EXECUTE            = 71072,
};
            ...
            void HandleDummy(SpellEffIndex effIndex)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_GLYPH_OF_EXECUTION, EFFECT_0))
                     ...
                     // Item - Warrior T10 Melee 4P Bonus
                    if (Aura * aura = GetCaster()->GetAura(AURA_SUDDEN_DEATH))
                        if (aura->GetCharges())
                            if (Player* caster = GetCaster()->ToPlayer())
                                caster->RestoreSpellMods(GetSpell(), AURA_GCD_EXECUTE);
    ...
    class spell_warr_overpower_SpellScript : public SpellScript
    {
        ...
        void HandleEffect(SpellEffIndex /* effIndex */)
        {
            ...
            if (Player* target = GetHitPlayer())
                if (target->IsNonMeleeSpellCasted(false, false, true)) // UNIT_STATE_CASTING should not be used here, it's present during a tick for instant casts
...
class spell_warr_t10_4_bonus : public SpellScriptLoader
{
    public:

    spell_warr_t10_4_bonus() : SpellScriptLoader("spell_warr_t10_4_bonus") { }

    class spell_warr_t10_4_bonus_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_warr_t10_4_bonus_AuraScript);

        void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool & /*canBeRecalculated*/)
        {
            if (Unit* pUnit = GetCaster())
                amount = CalculatePctN(pUnit->GetMaxHealth(), 20);
        }

        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warr_t10_4_bonus_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_warr_t10_4_bonus_AuraScript();
    }
};

class spell_warr_intervene_trigger : public SpellScriptLoader
{
    public:

    spell_warr_intervene_trigger() : SpellScriptLoader("spell_warr_intervene_trigger") { }

    class spell_warr_intervene_trigger_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_intervene_trigger_SpellScript);

        bool Validate(SpellInfo const* /*spell*/) override
        {
            if (!sSpellMgr->GetSpellInfo(WARRIOR_INTERVENE_TRIGGER))
                return false;
            return true;
        }
        ...
        void HandleAfterHit()
        {
            if (Unit* pUnit = GetHitUnit())
                pUnit->CastSpell(pUnit, WARRIOR_INTERVENE_TRIGGER, true);
        }

        void Register() override
        {
            AfterHit += SpellHitFn(spell_warr_intervene_trigger_SpellScript::HandleAfterHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_warr_intervene_trigger_SpellScript();
    }
};
...
void AddSC_warrior_spell_scripts()
{
    ...
    new spell_warr_t10_4_bonus();
    new spell_warr_intervene_trigger();
}
