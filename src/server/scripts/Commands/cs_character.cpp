...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand characterCommandTable[] =
        {
            ...
            { "itemlevel", SEC_GAMEMASTER, true, &HandleCharacterItemlevelCommand, "", NULL },
            { "removeprofession", SEC_GAMEMASTER, true, &HandleCharacterRemoveProfessionCommand, "", NULL },
            { NULL, 0, false, NULL, "", NULL }
        };
...
*/
    ...
    static bool HandleCharacterItemlevelCommand(ChatHandler* handler, char const* args)
    {
        Player* target = handler->getSelectedPlayer();
        if (target)
        {
            handler->PSendSysMessage("Your average itemlevel is %f", target->GetAverageItemLevel());
            return true;
        }
        else
            target = handler->GetSession()->GetPlayer();
        handler->PSendSysMessage("The average itemlevel of %s is %f", target->GetName(), target->GetAverageItemLevel());
        return true;
    }

    static bool HandleCharacterRemoveProfessionCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        // number or [name] Shift-click form |color|Htrade:skillId|h[name]|h|r
        uint32 skillId = handler->extractSpellIdFromLink((char*)args);
        if (!skillId)
            return false;
        ...
        if (!sSkillLineStore.LookupEntry(skillId) || index == -1)
        {
        ...
        pPlayer->SetSkill(skillId, 0, 0, 0);
        handler->PSendSysMessage(LANG_COMMAND_REMOVE_PROFESSION, handler->GetTrinityString(LANG_COMMAND_PROFESSION_ALCHEMY + index), ProfessionSkills[index]);
        return true;
    }
