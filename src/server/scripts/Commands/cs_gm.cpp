...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand gmCommandTable[] =
        {
            ...
            { "developer",      SEC_ADMINISTRATOR,  false, &HandleGMSetDevCommand,            "", NULL },
            { "",               SEC_MODERATOR,      false, &HandleGMCommand,                  "", NULL },
            { NULL,             0,                  false, NULL,                              "", NULL }
        };
        static ChatCommand vipCommandTable[] =
        {
            { "ingame",         SEC_MODERATOR,      false, &HandleVIPIngameCommand,           "", NULL },
			{ "list",           SEC_MODERATOR,      false, &HandleVIPListFullCommand,         "", NULL },
            { NULL,             0,                  false, NULL,                              "", NULL }
        };
    ...
    static bool HandleVIPIngameCommand(ChatHandler* handler, const char* /*args*/)
    {
        bool first = true;
        bool footer = false;
        ...
        for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
        {
            ...
            if (((itr_sec == SEC_VIP )) && (!handler->GetSession() ))
            {
                if (first)
                {
                    first = false;
                    footer = true;
                    handler->SendSysMessage("VIPs Online:");
                ...
                if (handler->GetSession())
                    handler->PSendSysMessage("| VIP: %s", name);
                else
        ...
        if (footer)
            handler->SendSysMessage("========================");
        if (first)
            handler->SendSysMessage("Keine VIPs Ingame");
        return true;
    }
    
    /// Display the list of VIPs
    static bool HandleVIPListFullCommand(ChatHandler* handler, const char* /*args*/)
    {
        ///- Get the accounts with VIP Level 1
        ...
        if (result)
        {
            handler->SendSysMessage("VIP Liste:");
            handler->SendSysMessage("========================");
            ///- Cycle through them. Display username and VIP
            do
            {
    ...
    static bool HandleGMSetDevCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            handler->GetSession()->SendNotification(".gm developer on/off");
        else
        {
    ...
    static bool HandleGMChatCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            ...
            if (!AccountMgr::IsVIPorPlayer(session->GetSecurity()) && session->GetPlayer()->isGMChat())
    ...
    static bool HandleGMListIngameCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
        {
            ...
            if ((itr->second->isGameMaster() || (!AccountMgr::IsVIPorPlayer(itrSec) && itrSec <= AccountTypes(sWorld->getIntConfig(CONFIG_GM_LEVEL_IN_GM_LIST)))) &&
