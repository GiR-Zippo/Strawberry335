...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand gobjectSetCommandTable[] =
        {
            ...
            { "scale",          SEC_ADMINISTRATOR,  false, &HandleGameObjectSetScaleCommand,  "", NULL },
            { NULL,             0,                  false, NULL,                              "", NULL }
        };
    ...
    static bool HandleGameObjectAddTempCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (GameObject* pObject = player->SummonGameObject(objectId, x, y, z, ang, 0, 0, rot2, rot3, spawntm))
            handler->PSendSysMessage(LANG_GAMEOBJECT_ADD, objectId, pObject->GetName(), pObject->GetGUIDLow(), x, y, z);
    ...
    static bool HandleGameObjectSetScaleCommand(ChatHandler* handler, char const* args)
    {
        ...
        float numScale = atof(scale);

        object->SetObjectScale(numScale);

        handler->PSendSysMessage("Set gobject lowguid %d to scale %f", guidLow, numScale);
        return true;
    }
};
