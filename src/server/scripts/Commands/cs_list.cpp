...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand listCommandTable[] =
        {
            ...
            { "professions",    SEC_ADMINISTRATOR,  false, &HandleListProfessionsCommand,       "", NULL },
    ...
    static bool HandleListProfessionsCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            if (pPlayer->HasSkill(ProfessionSkills[i]))
            {
                handler->PSendSysMessage(LANG_COMMAND_TARGET_PROFESSIONS, ProfessionSkills[i], handler->GetTrinityString(LANG_COMMAND_PROFESSION_ALCHEMY + i),
                                         pPlayer->GetSkillValue(ProfessionSkills[i]), pPlayer->GetMaxSkillValue(ProfessionSkills[i]));
                ++profCount;
            }
        }

        handler->PSendSysMessage(LANG_COMMAND_PROFESSION_COUNT, profCount);
        return true;
    }
};
