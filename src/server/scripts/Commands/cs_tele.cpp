...
public:
    ...
    static bool HandleTeleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (me->isInCombat() || (!me->TeleportAllowed() && handler->GetSession()->GetSecurity() < SEC_VIP))
        {
        ...
        uint32 zoneId = sMapMgr->GetZoneId(tele->mapId, tele->position_x, tele->position_y, tele->position_z);
        if (!me->isGameMaster() && (zoneId == 4197 || me->GetZoneId() == 4197)) // Wintergrasp
        {
            Battlefield* pBF = sBattlefieldMgr->GetBattlefieldToZoneId(4197);
            if (pBF)
            {
                if (pBF->IsWarTime())
                {
                    handler->SendSysMessage(LANG_WG_CANNOT_TELE_DURING_BATTLE);
                    handler->SetSentErrorMessage(true);
                    return false;
                }
            }
        ...
        // Leave vehicle - Check if vehicle handles function
        me->ExitVehicle();
