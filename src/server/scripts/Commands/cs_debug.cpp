...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand debugSendCommandTable[] =
        {
            ...
            { "test",           SEC_ADMINISTRATOR,  false, &HandleDebugSendTestCommand,           "", NULL },
            { "typeopcode",     SEC_ADMINISTRATOR,  false, &HandleDebugSendTypeOpcodeCommand,     "", NULL },
            { NULL,             SEC_PLAYER,         false, NULL,                                  "", NULL }
        };
        static ChatCommand debugCommandTable[] =
        {
            ...
            { "test",           SEC_ADMINISTRATOR,  false, &HandleDebugTestCommand,            "", NULL },
            { NULL,             SEC_PLAYER,         false, NULL,                               "", NULL }
        };
    ...
    static bool HandleDebugTestCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        uint32 mapId = 0;
        uint32 diff = 0;
        char* mapId_str = strtok((char*)args, " ");
        if (!mapId_str)
            return false;
        ...
        mapId = atoi(mapId_str);
        diff = atoi(diff_str);

        sInstanceSaveMgr->_ResetOrWarnAll(mapId, Difficulty(diff), false, time(NULL));
        return true;
    }

    static bool HandleDebugSendTestCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
    ...
    static bool HandleDebugSendTypeOpcodeCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (!pUnit || (pUnit->GetTypeId() != TYPEID_PLAYER))
            pPlayer = handler->GetSession()->GetPlayer();
        else
            pPlayer = pUnit->ToPlayer();

        if (!pPlayer)
            return false;

        std::istringstream sourceArgs(args);
        if (!sourceArgs)
            return false;

        // Token store <type, value>
        std::vector<std::pair<std::string, uint32> > tokenVector;
        ...
        while (!sourceArgs.eof())
        {
            // Get type
            std::string subType;
            sourceArgs >> subType;
            if (!sourceArgs || subType.empty())
                return false;
            ...
            // Get size
            if (subType.find("bool") != std::string::npos)
                size += sizeof(bool);
            else if (subType.find("uint8") != std::string::npos)
                size += sizeof(uint8);
            else if (subType.find("uint16") != std::string::npos)
                size += sizeof(uint16);
            else if (subType.find("uint32") != std::string::npos)
                size += sizeof(uint32);
            else
                return false;

            // Convert to pair
            std::pair<std::string, uint32> pairObj;
            pairObj.first = subType;
            pairObj.second = subValue;

            // Create message
            std::stringstream messageStream;
            messageStream << "Type: " << subType << " Value: " << subValue;

            tokenVector.push_back(pairObj);
            messageVector.push_back(messageStream.str());
        }
        ...
        // Write worldpacket
        for (std::vector<std::pair<std::string, uint32> >::const_iterator itr = tokenVector.begin(); itr != tokenVector.end(); ++itr)
        {
            if (itr->first.find("bool") != std::string::npos)
                data << bool(itr->second);
            else if (itr->first.find("uint8") != std::string::npos)
                data << uint8(itr->second);
            else if (itr->first.find("uint16") != std::string::npos)
                data << uint16(itr->second);
            else if (itr->first.find("uint32") != std::string::npos)
        ...
        // Send messages
        handler->PSendSysMessage("Data input:");

        for (std::vector<std::string>::const_iterator itr = messageVector.begin(); itr != messageVector.end(); ++itr)
            handler->PSendSysMessage(itr->c_str());

        handler->PSendSysMessage("Opcode with size: %d and valuecount: %d was successfully sent.", size, tokenVector.size());
        return true;
    }
