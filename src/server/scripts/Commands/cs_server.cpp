...
public:
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        handler->PSendSysMessage("%s%s%s", _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B);
...
private:
    ...
    static bool ShutdownServer(char const* args, uint32 shutdownMask, int32 defaultExitCode)
    {
        ...
        char* parsedString[2] = { NULL, NULL };
        std::string reason(args);

        // delay
        parsedString[0] = strtok((char*)args, " ");
        if (!parsedString[0])
            return false;
        ...
        // reason
        uint8 delfound = 0;
        std::string::iterator reasonItr = reason.end();
        for (std::string::iterator itr = reason.begin(); itr != reason.end() && delfound != 2; ++itr)
        {
            if (*itr == ' ')
            {
                if (delfound == 1)
                    reason.erase(reason.begin(), itr);
        ...
        int32 delay = atoi(parsedString[0]);

        // Prevent interpret wrong arg value as 0 secs shutdown time
        if ((delay == 0 && (parsedString[0][0] != '0' || parsedString[0][1] != '\0')) || delay < 0)
            return false;

        int32 exitCode = defaultExitCode;
        if (parsedString[1])
            if (!ParseExitCode(parsedString[1], exitCode))
                return false;
