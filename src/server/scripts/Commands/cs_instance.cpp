...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand instanceCommandTable[] =
        {
            ...
            { "doaction",       SEC_ADMINISTRATOR,  false,  &HandleInstanceDoActionCommand,     "", NULL },
            { "setbossstate",   SEC_ADMINISTRATOR,  false,  &HandleInstanceSetBossStateCommand, "", NULL },
    ...
    static bool HandleInstanceDoActionCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (InstanceScript* pInstance = map->ToInstanceMap()->GetInstanceScript())
        {
            int32 action = atoi((char*)args);
            pInstance->DoAction(action);

            handler->PSendSysMessage("Action %d sent to instance.", action);
        }
        else
        {
            handler->PSendSysMessage("Map has no instance data.");
    ...
    static bool HandleInstanceSetBossStateCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 bossId = 0;
        uint32 state = 0;
        char* bossId_str = strtok((char*)args, " ");
        if (!bossId_str)
            return false;
        ...
        if (InstanceScript* pInstance = map->ToInstanceMap()->GetInstanceScript())
        {
            pInstance->SetBossState(bossId, EncounterState(state));
            handler->PSendSysMessage("Set state of boss %d to %d.", bossId, state);
        }
        else
        {
            handler->PSendSysMessage("Map has no instance data.");
