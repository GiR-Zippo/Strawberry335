...
public:
    ...
    static bool HandleGuildRenameCommand(ChatHandler* handler, char const* args)
    {
        ...
        CharacterDatabase.PExecute("UPDATE guild SET name = '%s' WHERE guildid = %u", newname, guild->GetId());
        handler->PSendSysMessage(LANG_GUILD_RENAMED, guild->GetId(), oldname, newname);
        return true;
    }
};
