...
SDLaenalith: Quest support: 10514(#2861)
...
// CreatureEntry:     19823
// KillCredit:        22051
// RequiredSpellAura: 38224
enum Colossus
{
    COLOSSUS_KILL_CREDIT    = 22051,
    COLOSSUS_AURA_REQ       = 38224
};
...
public:
    ...
    struct npc_enraged_spiritAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer)
        {
            Player* pPlayer = killer->ToPlayer();

            if (pPlayer)
            {
                if (pPlayer->HasAura(COLOSSUS_AURA_REQ))
                {
                    Creature* pCreature = me->SummonCreature(COLOSSUS_KILL_CREDIT, me->GetHomePosition());
                    if (pCreature)
                    {
                        ...
                        if (playerList.size() > 1)
                        {
                            ...
                            for (itr = playerList.begin(); itr != playerList.end(); ++itr)
                            {
                                if ((*itr))
                                {
                                    if (((*itr)->IsInSameGroupWith(pPlayer) && (*itr)->HasAura(COLOSSUS_AURA_REQ)) || (*itr) == pPlayer)
                                        (*itr)->KilledMonsterCredit(COLOSSUS_KILL_CREDIT, pCreature->GetGUID());
                                }
                            }
                        }
                        else
                            pPlayer->KilledMonsterCredit(COLOSSUS_KILL_CREDIT, pCreature->GetGUID());
...
// CreatureEntry: 21195
// SpellId:       36462
enum IWasALotOfThings_Creature
{
    SPELL_SHADOWMOON_TUBER      = 36462,
    OBJECT_TUBER_MOUND          = 184701,
    QUEST_I_WAS_A_LOT_OF_THINGS = 10976
};

class npc_q10514_dom_felboar : public CreatureScript
{
public:
    npc_q10514_dom_felboar() : CreatureScript("npc_q10514_dom_felboar") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_q10514_dom_felboarAI(creature);
    }

    struct npc_q10514_dom_felboarAI : public ScriptedAI
    {
        npc_q10514_dom_felboarAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        void UpdateAI(const uint32 diff)
        {
            if (timer)
            {
                if (_timer <= diff)
                {
                    GameObject* pObject = me->FindNearestGameObject(OBJECT_TUBER_MOUND, 5.0f);
                    if (pObject)
                    {
                        std::list<Player*> PlList = me->GetNearestPlayersList(15.0f);
                        std::list<Player*>::iterator itr;
                        ...
                        if (!PlList.empty())
                        {
                            for (itr = PlList.begin(); itr != PlList.end(); ++itr)
                            {
                                if (*itr)
                                {
                                    if ((*itr)->IsActiveQuest(QUEST_I_WAS_A_LOT_OF_THINGS))
                                    {
                                        me->CastSpell(me, SPELL_SHADOWMOON_TUBER, true);
                                        pObject->DestroyForPlayer((*itr));
                                        break;
                                    }
                                }
                            }
