...
SDLaenalith: Quest support: 11089(#2860)
...
// Akolyth Earth:    23098
// Akolyth Fire:     23099
// Arcane Elementar: 23100
// SayId:            -1900001 -/- -1900010
enum TheSoulCannonOfRethhedron
{
    QUEST_THE_SOUL_CANNON_OF_RETHHEDRON = 11089,
    NPC_FACTION_ENEMY                   = 17,

    NPC_AKOLYTH_AIR                     = 23096,
    NPC_AKOLYTH_WATER                   = 23097,
    NPC_AKOLYTH_EARTH                   = 23098,
    NPC_AKOLYTH_FIRE                    = 23099,
    NPC_ARCAN_ELEMENTAR                 = 23100,

    WP_ID_REACHED_EMOTE                 = 1,
    WP_ID_REACHED_SAY                   = 3,
    WP_ID_SUMMON_AIR                    = 4,
    WP_ID_SUMMON_WATER                  = 5,
    WP_ID_SUMMON_EARTH                  = 7,
    WP_ID_SUMMON_FIRE                   = 10,
    WP_ID_SUMMON_ARCAN                  = 13,
    WP_ID_REACHED_HOME                  = 15,

    SARTHIS_EVENT_EMOTE                 = -1900001,
    SARTHIS_EVENT_SAY_1                 = -1900002,
    SARTHIS_EVENT_SAY_2                 = -1900003,
    SARTHIS_EVENT_SAY_DONE              = -1900004,
    SARTHIS_EVENT_SAY_WATER             = -1900005,
    SARTHIS_EVENT_SAY_EARTH             = -1900006,
    SARTHIS_EVENT_SAY_FIRE              = -1900007,
    SARTHIS_EVENT_SAY_ARCANE            = -1900008,
    SARTHIS_EVENT_SAY_NPC_ARCANE        = -1900009,
    SARTHIS_EVENT_SAY_FINAL             = -1900010,

    GOSSIP_OPTIONID_START               = 1,
    EVENT_WAIT_COMBAT_END               = 1,
    EVENT_FINAL                         = 2,
    EVENT_GO_HOME                       = 3,
    EVENT_MAX_TIME                      = 4,
    EVENT_TIMER_COMBAT_END              = 5,

    TIMER_MAX_EVENT_TIME                = 8*MINUTE*IN_MILLISECONDS,
    TIMER_MAX_COMBAT_TIME               = 1*MINUTE*IN_MILLISECONDS
};

const Position SpawnPositionElementals[5] =
{
    { -2472.98, 4652.60, 159.90, 1.47 },
    { -2441.02, 4651.58, 160.63, 1.60 },
    { -2413.41, 4648.32, 161.22, 3.83 },
    { -2410.18, 4594.34, 161.40, 3.27 },
    { -2378.54, 4575.61, 166.14, 3.57 },
};

class npc_sarthis : public CreatureScript
{
public:
    npc_sarthis() : CreatureScript("npc_sarthis") { }
    ...
    struct npc_sarthisAI : public npc_escortAI
    {
        ...
        void ResetMe()
        {
            ...
            me->FarTeleportTo(me->GetMap(), HomePosition);
        ...
        void JustSummoned(Creature* summoned)
        {
            if (pPlayer)
            {
                if (summoned->GetEntry() != NPC_ARCAN_ELEMENTAR)
                {
                ...
                else
                {
                    ...
                    DoScriptText(SARTHIS_EVENT_SAY_NPC_ARCANE, summoned);
                }
            }
        }
        ...
        void WaypointReached(uint32 waypointId)
        {
            switch (waypointId)
            {
                case WP_ID_REACHED_EMOTE: { DoScriptText(SARTHIS_EVENT_EMOTE, me);    }break;
                case WP_ID_REACHED_SAY:   { DoScriptText(SARTHIS_EVENT_SAY_2, me);    }break;
                case WP_ID_SUMMON_AIR:    { StopAndSummonElemental(WP_ID_SUMMON_AIR); }break;

                case WP_ID_SUMMON_WATER:
                {
                    DoScriptText(SARTHIS_EVENT_SAY_WATER, me);
                    StopAndSummonElemental(WP_ID_SUMMON_WATER);
                }break;

                case WP_ID_SUMMON_EARTH:
                {
                    DoScriptText(SARTHIS_EVENT_SAY_EARTH, me);
                    StopAndSummonElemental(WP_ID_SUMMON_EARTH);
                }break;

                case WP_ID_SUMMON_FIRE:
                {
                    DoScriptText(SARTHIS_EVENT_SAY_FIRE, me);
                    StopAndSummonElemental(WP_ID_SUMMON_FIRE);
                }break;

                case WP_ID_SUMMON_ARCAN: { StopAndSummonElemental(WP_ID_SUMMON_ARCAN); }break;
                case WP_ID_REACHED_HOME: { ResetMe();                                  }break;
            }
        }
        ...
        void StartWP(Player* player)
        {
            if (!player)
                return;

            DoScriptText(SARTHIS_EVENT_SAY_1, me);
            SetEscortPaused(false);
            Start(false, false, player->GetGUID());
            pPlayer = player;

            events.ScheduleEvent(EVENT_MAX_TIME, TIMER_MAX_EVENT_TIME);
        }

        void StopAndSummonElemental(uint32 uiStep)
        {
            ...
            switch (uiStep)
            {
                case WP_ID_SUMMON_AIR:   { creatureEntry = NPC_AKOLYTH_AIR;                 }break;
                case WP_ID_SUMMON_WATER: { creatureEntry = NPC_AKOLYTH_WATER; spawnId = 1;   }break;
                case WP_ID_SUMMON_EARTH: { creatureEntry = NPC_AKOLYTH_EARTH; spawnId = 2;   }break;
                case WP_ID_SUMMON_FIRE:  { creatureEntry = NPC_AKOLYTH_FIRE; spawnId = 3;    }break;
                case WP_ID_SUMMON_ARCAN: { creatureEntry = NPC_ARCAN_ELEMENTAR; spawnId = 4; }break;
                default: return;
            }

            currentElemental = NULL;

            if (currentElemental = me->SummonCreature(creatureEntry, SpawnPositionElementals[spawnId], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 70*IN_MILLISECONDS))
            {
                me->SetTarget(currentElemental->GetGUID());
                currentElemental->setFaction(NPC_FACTION_ENEMY);

                if (uiStep != WP_ID_SUMMON_ARCAN)
                {
                    events.ScheduleEvent(EVENT_WAIT_COMBAT_END, 5*IN_MILLISECONDS);
                    events.ScheduleEvent(EVENT_TIMER_COMBAT_END, 1*MINUTE*IN_MILLISECONDS);
                }
                else
                    events.ScheduleEvent(EVENT_FINAL, 3*IN_MILLISECONDS);
            }
        }
        ...
        void ExecuteEvents()
        {
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_WAIT_COMBAT_END:
                    {
                        if (!currentElemental)
                            ResetMe();
                        else
                        {
                            if (currentElemental->isAlive())
                            {
                                if (currentElemental->getFaction()== 35)
                                    currentElemental->setFaction(NPC_FACTION_ENEMY);
                                events.ScheduleEvent(EVENT_WAIT_COMBAT_END, 5*IN_MILLISECONDS);
                            }
                            else
                            {
                                DoScriptText(SARTHIS_EVENT_SAY_DONE, me);
                                currentElemental->DespawnOrUnsummon();
                                events.CancelEvent(EVENT_TIMER_COMBAT_END);
                                NextStep();
                            }
                        }
                    }break;

                    case EVENT_TIMER_COMBAT_END:
                    {
                        if (currentElemental)
                            currentElemental->DespawnOrUnsummon();
                        ResetMe();
                    ...
                    case EVENT_FINAL:
                    {
                        DoScriptText(SARTHIS_EVENT_SAY_FINAL, me, pPlayer);
                        me->HandleEmoteCommand(EMOTE_ONESHOT_BOW);
                        events.ScheduleEvent(EVENT_GO_HOME, 3*IN_MILLISECONDS);
                    }break;
                    ...
                    case EVENT_GO_HOME:
                    {
                        ...
                        currentElemental->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        currentElemental->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                        currentElemental->GetMotionMaster()->Initialize();

                        currentElemental->SetInCombatWith(pPlayer);
                        pPlayer->SetInCombatWith(currentElemental);
                        currentElemental->AddThreat(pPlayer, 0.0f);

                        SetRun(true);
                        NextStep();
                    }break;

                    case EVENT_MAX_TIME:{ ResetMe(); }break;
                }
            }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->isGameMaster())
            player->ADD_GOSSIP_ITEM_DB(gossipMenuId, GOSSIP_OPTIONID_START, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        else if (player->IsActiveQuest(QUEST_THE_SOUL_CANNON_OF_RETHHEDRON))
            player->ADD_GOSSIP_ITEM_DB(gossipMenuId, GOSSIP_OPTIONID_START, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF:
                {
                    CAST_AI(npc_sarthis::npc_sarthisAI, creature->AI())->StartWP(player);
