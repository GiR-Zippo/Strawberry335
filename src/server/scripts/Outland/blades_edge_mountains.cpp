...
// QuestId:       10976
// CreatureEntry: 20889
// GoEntry:       185461 -/- 185467
enum TheMarkOfTheNexusKing
{
    NPC_FACTION_ENEMY  = 17,
    NPC_PRISONER_DUMMY = 20889
};
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* pObj)
    {
        int Random = rand() % (sizeof(NpcStasisEntry) / sizeof(uint32));

        Creature* pCreature = NULL;
        if (pCreature = player->SummonCreature(NpcStasisEntry[Random], pObj->GetPositionX(), pObj->GetPositionY(), pObj->GetPositionZ(), pObj->GetAngle(player),
            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
        {
            pCreature->setFaction(NPC_FACTION_ENEMY);
            pCreature->SetInCombatWith(player);
            player->SetInCombatWith(pCreature);
            pCreature->AddThreat(player, 0.0f);
        }
