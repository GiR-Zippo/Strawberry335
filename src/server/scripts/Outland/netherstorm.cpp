...
SDLaenalith: Quest support: 10857(#2863), 10310 (#2801), 10309 (#2800)
...
enum TeleportThis
{
    QUEST_TELEPORT_THIS = 10857,
    CREATURE_TRIGGER_1  = 22348,
    CREATURE_TRIGGER_2  = 22350,
    CREATURE_TRIGGER_3  = 22351
};

class npc_q10857_trigger : public CreatureScript
{
public:
    npc_q10857_trigger() : CreatureScript("npc_q10857_trigger") { }

    struct npc_q10857_triggerAI : public ScriptedAI
    {
        npc_q10857_triggerAI(Creature* creature) : ScriptedAI(creature) {}

        void JustDied(Unit* killer)
        {
            if (me->GetEntry() != CREATURE_TRIGGER_1 
                && me->GetEntry() != CREATURE_TRIGGER_2
                && me->GetEntry() != CREATURE_TRIGGER_3)
                return;

            if (killer->isCharmed())
            {
                Player* pPlayer = NULL;
                if (pPlayer = killer->GetCharmerOrOwner()->ToPlayer())
                {
                    if (pPlayer->IsActiveQuest(QUEST_TELEPORT_THIS))
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_q10857_triggerAI(creature);
    }
};
...
// QuestGiver:     20281
// Creature:       20404
// CreatureVisual: 20296
// SpellVisualNPC: 45850
// SpellVisual:    59084
enum SabotageTheWarpGate
{
    QUEST_SABOTAGE_THE_WARP_GATE        = 10310,
    NPC_FACTION_ENEMY                   = 17,
    GOSSIP_OPTIONID_START               = 1,

    DRIJYA_SAY_1                        = -1900016,
    DRIJYA_SAY_2                        = -1900017,
    DRIJYA_SAY_3                        = -1900018,
    DRIJYA_SAY_4                        = -1900019,
    ...
    WAYPOINTID_WAVES_BEGIN              = 10,
    WAYPOINTID_END                      = 11,

    WAVE_COUNT                          = 3,
    CREATURE_TELEPORT_TRIGGER           = 20296,
    CREATURE_WAVES                      = 20404,

    SPELL_VISUAL_NPC                    = 45850,
    SPELL_VISUAL_DESTROY                = 59084,

    TIMER_MAX_EVENT_TIME                = 5*MINUTE*IN_MILLISECONDS
};

const Position WaveSpawnPosition ={ 3041.05f, 2693.13f, 114.27f, 2.35f };

class npc_q10310_escort : public CreatureScript
{
public:
    npc_q10310_escort() : CreatureScript("npc_q10310_escort"){}

    struct npc_q10310_escortAI : public npc_escortAI
    {
        npc_q10310_escortAI(Creature* creature) : npc_escortAI(creature){}
        ...
        void WaypointReached(uint32 waypointId)
        {
            switch (waypointId)
            {
                case WAYPOINTID_WAVES_BEGIN:
                {
                    if (pPlayer)
                    {
                        SetEscortPaused(true);
                        DoScriptText(DRIJYA_SAY_2, me);
                        me->SetReactState(REACT_PASSIVE);
                        me->CastSpell(me, SPELL_VISUAL_NPC);
                ...
                case WAYPOINTID_END: { ResetMe(); }break;
            }
        };

        void UpdateEscortAI(const uint32 diff)
        {
            if (events.Empty() || !HasEscortState(STATE_ESCORT_ESCORTING))
                return;
        ...
        void StartWP(Player* player)
        {
            ...
            DoScriptText(DRIJYA_SAY_1, me);
            events.ScheduleEvent(EVENT_MAX_TIME, TIMER_MAX_EVENT_TIME);
        };

        void SummonWave()
        {
            DoScriptText(DRIJYA_SAY_3, me);

            _summons.clear();
            Creature* pCreature = NULL;
            Position SpawnPosition = WaveSpawnPosition;

            for (uint8 i = 0; i < 3+_currentWave; ++i)
            {
                SpawnPosition.m_positionX += 2.0f*i;
                SpawnPosition.m_positionY += 2.0f*i;

                if (pCreature = DoSummon(CREATURE_WAVES, SpawnPosition, 30000, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT))
                    _summons.push_back(pCreature);
            }
        };
        ...
        bool CheckAlive()
        {
            ...
            for (itr = _summons.begin(); itr != _summons.end(); ++itr)
            {
                if ((*itr)->isAlive())
                    return true;
            }
        ...
        void ExecuteEvents()
        {
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        SummonWave();
                        events.ScheduleEvent(EVENT_WAIT_WAVE, 10*IN_MILLISECONDS);
                    }break;

                    case EVENT_WAIT_WAVE:
                    {
                        if (CheckAlive())
                            events.ScheduleEvent(EVENT_WAIT_WAVE, 3*IN_MILLISECONDS);
                        else
                        {
                            ++_currentWave;
                            if (_currentWave == WAVE_COUNT)
                                events.ScheduleEvent(EVENT_FINISH, 1*IN_MILLISECONDS);
                            else
                    ...
                    case EVENT_FINISH:
                    {
                        ...
                        {
                            Creature* pCreature = NULL;
                            if (pCreature = me->FindNearestCreature(CREATURE_TELEPORT_TRIGGER, 100.0f))
                                pCreature->CastSpell(pCreature, SPELL_VISUAL_DESTROY, true);
                            DoScriptText(DRIJYA_SAY_4, me);
                            pPlayer->GroupEventHappens(QUEST_SABOTAGE_THE_WARP_GATE, me);
                            me->CastStop();
                        }
                        SetEscortPaused(false);
                    }break;
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_q10310_escortAI(creature);
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->IsActiveQuest(QUEST_SABOTAGE_THE_WARP_GATE) || player->isGameMaster())
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
    {
        ...
        switch (action)
        {
            ...
            {
                CAST_AI(npc_q10310_escort::npc_q10310_escortAI, creature->AI())->StartWP(player);
...
// QuestId:        10309
// SpellId:        35282
// Needed:         SPELL_EFFECT_DUMMY
enum ItsAFelReaverButWithHeart
{
    SPELL_FEL_ZAPPER = 35282
};

class spell_q10309_dummy : public SpellScriptLoader
{
    public:
        spell_q10309_dummy() : SpellScriptLoader("spell_q10309_dummy") { }

        class spell_q10309_dummySpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q10309_dummySpellScript);

            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_FEL_ZAPPER))
                    return false;
                return true;
            }
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                Unit* pUnit = NULL;
                if (pUnit = GetHitUnit())
                    pUnit->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC);
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_q10309_dummySpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_q10309_dummySpellScript();
        }
};
...
void AddSC_netherstorm()
{
    ...
    new npc_q10857_trigger();
    new npc_q10310_escort();
    new spell_q10309_dummy();
}
