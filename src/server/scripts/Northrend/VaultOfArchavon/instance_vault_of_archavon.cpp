...
class instance_archavon : public InstanceMapScript
{
    public:
        ...
        struct instance_archavon_InstanceMapScript : public InstanceScript
        {
            ...
            {
                if (WintergraspCheckTimer <= diff)
                {
                    if (sWorld->getBoolConfig(CONFIG_WINTERGRASP_ENABLE))
                    {
                        Battlefield* pBF = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_BATTLEID_WG);
                        if (!pBF)
                            return;

                        if (pBF->IsWarTime())
                            DoCastSpellOnPlayers(SPELL_TELEPORT_DALARAN);
                        else if (TeamInInstance != TEAM_NEUTRAL)
                        {
                            if (pBF->GetDefenderTeam() != TeamInInstance)
                                DoCastSpellOnPlayers(SPELL_TELEPORT_DALARAN);
                        }
                    }
                    WintergraspCheckTimer = 5000;
