...
Position const EventLeaderPos2 = {1062.94592f, 99.0246f, 631.005f, 2.049f};
// position slave leader pos at end Gorkun Ironskull (37581)
Position const SlaveLeaderEndPos = {982.987488f, 168.280182f, 628.155273f, 6.239910f};
Position const SindragosaEndPos = {895.892761f, 164.136551f, 575.395691f, 0.0f};
    ...
    public:
        ...
        struct instance_pit_of_saron_InstanceScript : public InstanceScript
        {
            instance_pit_of_saron_InstanceScript(Map* map) : InstanceScript(map)
            {
                ...
                _teamInInstance = 0;
                _tyrannusEventState = NOT_STARTED;
            }
                        ...
                        if (state == DONE)
                        {
                            if (Creature* summoner = instance->GetCreature(_tyrannusGUID))
                            {
                                if (_teamInInstance == ALLIANCE)
                                {
                                    ...
                                    summoner->SummonCreature(NPC_MARTIN_VICTUS_2, SlaveLeaderEndPos, TEMPSUMMON_MANUAL_DESPAWN);
                                }
                                else
                                {
                                    summoner->SummonCreature(NPC_SYLVANAS_PART2, EventLeaderPos2, TEMPSUMMON_MANUAL_DESPAWN);
                                    summoner->SummonCreature(NPC_GORKUN_IRONSKULL_1, SlaveLeaderEndPos, TEMPSUMMON_MANUAL_DESPAWN);
                                }
                                summoner->SummonCreature(NPC_SINDRAGOSA_END, SindragosaEndPos, TEMPSUMMON_MANUAL_DESPAWN);
                            }
                            _tyrannusEventState = DONE;
                        }
                        break;
                    default:
                        break;
            ...
            std::string GetSaveData()
            {
                ...
                saveStream << "P S " << GetBossSaveData() << _tyrannusEventState;
            ...
            void Load(const char* in)
            {
                ...
                if (dataHead1 == 'P' && dataHead2 == 'S')
                {
                    ...
                    loadStream >> _tyrannusEventState;
                }
                else
                    OUT_LOAD_INST_DATA_FAIL;
