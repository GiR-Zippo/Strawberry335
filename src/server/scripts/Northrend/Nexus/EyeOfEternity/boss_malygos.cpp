...
public:
    ...
    struct boss_malygosAI : public BossAI
    {
        ...
        void Initialize()
        {
            ...
            memset(&_surgeTargetGUID, NULL, sizeof(_surgeTargetGUID));
        ...
        void EnterEvadeMode() override
        {
            ...
            setZoneOverrideLight(LIGHT_GET_DEFAULT_FOR_MAP, 1 * IN_MILLISECONDS);
        ...
        void KilledUnit(Unit* victim) override
        {
            if (victim->GetTypeId() == TYPEID_UNIT)
                if (Player* pPlayer = victim->GetCharmerOrOwnerPlayerOrPlayerItself())
                    me->Kill(pPlayer);
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            switch (id)
            {
                ...
                case POINT_PHASE_ONE_TO_TWO_TRANSITION:
                    ...
                    if (Creature* alexstraszaBunny = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_ALEXSTRASZA_BUNNY_GUID)))
                        me->SetFacingToObject(alexstraszaBunny);
                    setZoneOverrideLight(LIGHT_ARCANE_RUNES, 5 * IN_MILLISECONDS);
                ...
                case POINT_DESTROY_PLATFORM_P_TWO:
                    setZoneOverrideLight(LIGHT_OBSCURE_SPACE, 1 * IN_MILLISECONDS);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_LIGHT_DIMENSION_CHANGE:
                        setZoneOverrideLight(LIGHT_CHANGE_DIMENSIONS, 2 * IN_MILLISECONDS);
                        break;
                    ...
                    case EVENT_START_P_THREE:
                        setZoneOverrideLight(LIGHT_OBSCURE_ARCANE_RUNES, 1 * IN_MILLISECONDS);
    ...
    private:
        ...
        void setZoneOverrideLight(uint32 overrideId, uint32 fadeInTime)
        {
            WorldPacket data(SMSG_OVERRIDE_LIGHT, 12);
            data << uint32(AREA_EYE_OF_ETERNITY);       // Light.dbc entry (map default)
            data << uint32(overrideId); // Light.dbc entry (override)
            data << uint32(fadeInTime);
            sendPacketToPlayers(&data);
        }

        void sendPacketToPlayers(const WorldPacket* data) const
        {
...
public:
    ...
    struct npc_power_sparkAI : public ScriptedAI
    {
        ...
        void MoveToMalygos()
        {
            ...
            if (Creature* malygos = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_MALYGOS)))
                me->GetMotionMaster()->MoveChase(malygos);
        }
    ...
    struct npc_melee_hover_diskAI : public VehicleAI
    {
        ...
        void DoAction(const int32 /*action*/) override
        {
            if (Vehicle* vehicleTemp = me->GetVehicleKit())
            {
                ...
                else if (vehicleTemp->GetPassenger(0))
                    vehicleTemp->GetPassenger(0)->ToCreature()->DespawnOrUnsummon();
            }
...
class spell_wyrmrest_skytalon_summon_red_dragon_buddy : public SpellScriptLoader
{
    public:
        ...
        class spell_wyrmrest_skytalon_summon_red_dragon_buddy_SpellScript : public SpellScript
        {
            ...
            void SetDest()
            {
                if (const WorldLocation* explTargetDest = GetExplTargetDest())
                {
                    WorldLocation wLocation;
                    wLocation.Relocate(explTargetDest);

                    // Adjust effect summon position to lower Z
                    const Position offset = { 0.0f, 0.0f, -80.0f, 0.0f };
                    wLocation.RelocateOffset(offset);
                    SetExplTargetDest(wLocation);
                }
            }

            void Register() override
            {
                OnCast += SpellCastFn(spell_wyrmrest_skytalon_summon_red_dragon_buddy_SpellScript::SetDest);
            }
        };
