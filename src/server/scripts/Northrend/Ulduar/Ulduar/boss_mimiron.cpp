...
#define EMOTE_LEVIATHAN                         "Leviathan MK II beginnt Plasmaeruption zu wirken!"
...
enum Spells
{
    ...
    SPELL_SUMMON_FIREBOT                        = 64622,
    SPELL_SUMMON_FIREBOT_VISUAL                 = 64621,

    SPELL_SUMMON_SCRAP_BOT_TRIGGER              = 63820,
    SPELL_SUMMON_ASSAULT_BOT_TRIGGER            = 64425,
    SPELL_SUMMON_FIREBOT_TRIGGER                = 64620,
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                else
                {
                    ...
                    if (_botTimer > 15000) // spell 64383
                    {
                        ...
                        if (Creature* AerialUnit = me->GetCreature(*me, instance->GetData64(DATA_AERIAL_COMMAND_UNIT)))
                    ...
                    else
                    {
                        ...
                        Creature* AerialUnit = me->GetCreature(*me, instance->GetData64(DATA_AERIAL_COMMAND_UNIT));
        ...
        void DoAction(int32 const action)
        {
            switch (action)
            {
                case DO_START_ENCOUNTER:
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_SERVER_CONTROLLED);
...
public:
    ...
    struct boss_vx_001AI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            events.ScheduleEvent(EVENT_PRE_LASER_BARRAGE, urand(35000, 40000));
        ...
        void DoAction(int32 const action)
        {
            switch (action)
            {
                ...
                case DO_VX001_ASSEMBLED:
                    ...
                    events.RescheduleEvent(EVENT_PRE_LASER_BARRAGE, 360000);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PRE_LASER_BARRAGE:
                        {
                            ...
                            if (events.IsInPhase(PHASE_VX001_ASSEMBLED)) {
                                if (Creature* leviathan = me->GetVehicleCreatureBase()) {
    ...
    struct boss_aerial_unitAI : public BossAI
    {
        ...
        void spawnAdd()
        {
            switch (spawnedAdds)
            {
                case 0:
                    for (uint8 n = 0; n < 2; n++)
                        DoCastAOE(SPELL_SUMMON_SCRAP_BOT_TRIGGER);
                    break;
                case 1:
                    DoCastAOE(SPELL_SUMMON_ASSAULT_BOT_TRIGGER);
                    if (MimironHardMode)
                        for (uint8 i = 0; i < 2; ++i)
                            DoCastAOE(SPELL_SUMMON_FIREBOT_TRIGGER);
                    break;
                case 2:
                    events.RescheduleEvent(EVENT_PLASMA_BALL, 3000);
                    DoCastAOE(SPELL_BOOM_BOT_SUMMON);
                    break;
            }
...
class spell_summon_scrapbot_trigger : public SpellScriptLoader
{
    public:
        spell_summon_scrapbot_trigger() : SpellScriptLoader("spell_summon_scrapbot_trigger") { }

        class spell_summon_scrapbot_trigger_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_summon_scrapbot_trigger_SpellScript);
            ...
            {
                Unit* trigger = GetHitUnit();
                trigger->CastSpell(trigger, SPELL_SUMMON_SCRAP_BOT_VISUAL);
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_summon_scrapbot_trigger_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
                OnEffectHitTarget += SpellEffectFn(spell_summon_scrapbot_trigger_SpellScript::HandleHit, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_summon_scrapbot_trigger_SpellScript();
        }
};

class spell_summon_assaultbot_trigger : public SpellScriptLoader
{
    public:
        spell_summon_assaultbot_trigger() : SpellScriptLoader("spell_summon_assaultbot_trigger") { }

        class spell_summon_assaultbot_trigger_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_summon_assaultbot_trigger_SpellScript);
            ...
            {
                Unit* trigger = GetHitUnit();
                trigger->CastSpell(trigger, SPELL_SUMMON_ASSAULT_BOT_VISUAL);
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_summon_assaultbot_trigger_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
                OnEffectHitTarget += SpellEffectFn(spell_summon_assaultbot_trigger_SpellScript::HandleHit, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_summon_assaultbot_trigger_SpellScript();
        }
};

class spell_summon_firebot_trigger : public SpellScriptLoader
{
    public:
        spell_summon_firebot_trigger() : SpellScriptLoader("spell_summon_firebot_trigger") { }

        class spell_summon_firebot_trigger_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_summon_firebot_trigger_SpellScript);
            ...
            {
                Unit* trigger = GetHitUnit();
                trigger->CastSpell(trigger, SPELL_SUMMON_FIREBOT_VISUAL);
            }

            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_summon_firebot_trigger_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
                OnEffectHitTarget += SpellEffectFn(spell_summon_firebot_trigger_SpellScript::HandleHit, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_summon_firebot_trigger_SpellScript();
        }
};

class spell_summon_scrap_bot_visual : public SpellScriptLoader
{
    public:
        spell_summon_scrap_bot_visual() : SpellScriptLoader("spell_summon_scrap_bot_visual") { }

        class spell_summon_scrap_bot_visual_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_summon_scrap_bot_visual_AuraScript);

            void HandleRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
            {
                GetTarget()->CastSpell(GetTarget(), SPELL_SUMMON_SCRAP_BOT);
            }

            void Register()
            {
                OnEffectRemove += AuraEffectRemoveFn(spell_summon_scrap_bot_visual_AuraScript::HandleRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_summon_scrap_bot_visual_AuraScript();
        }
};

class spell_summon_assault_bot_visual : public SpellScriptLoader
{
    public:
        spell_summon_assault_bot_visual() : SpellScriptLoader("spell_summon_assault_bot_visual") { }

        class spell_summon_assault_bot_visual_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_summon_assault_bot_visual_AuraScript);

            void HandleRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
            {
                GetTarget()->CastSpell(GetTarget(), SPELL_SUMMON_ASSAULT_BOT);
            }

            void Register()
            {
                OnEffectRemove += AuraEffectRemoveFn(spell_summon_assault_bot_visual_AuraScript::HandleRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_summon_assault_bot_visual_AuraScript();
        }
};

class spell_summon_fire_bot_visual : public SpellScriptLoader
{
    public:
        spell_summon_fire_bot_visual() : SpellScriptLoader("spell_summon_fire_bot_visual") { }

        class spell_summon_fire_bot_visual_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_summon_fire_bot_visual_AuraScript);

            void HandleRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
            {
                GetTarget()->CastSpell(GetTarget(), SPELL_SUMMON_FIREBOT);
            }

            void Register()
            {
                OnEffectRemove += AuraEffectRemoveFn(spell_summon_fire_bot_visual_AuraScript::HandleRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new spell_summon_fire_bot_visual_AuraScript();
        }
};
...
class npc_mimiron_bot_trigger : public CreatureScript
{
    public:
        npc_mimiron_bot_trigger() : CreatureScript("npc_mimiron_bot_trigger") { }

        struct npc_mimiron_bot_triggerAI : public ScriptedAI
        {
            npc_mimiron_bot_triggerAI(Creature* creature) : ScriptedAI(creature)
            {
            }
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_mimiron_bot_triggerAI(creature);
        }
};
...
void AddSC_boss_mimiron()
{
    ...
    new spell_summon_scrapbot_trigger();
    new spell_summon_firebot_trigger();
    new spell_summon_assaultbot_trigger();
    new spell_summon_scrap_bot_visual();
    new spell_summon_fire_bot_visual();
    new spell_summon_assault_bot_visual();
    new npc_mimiron_bot_trigger();
}
