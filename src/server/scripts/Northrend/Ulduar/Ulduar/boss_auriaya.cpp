...
class boss_auriaya : public CreatureScript
{
    public:
        ...
        struct boss_auriayaAI : public BossAI
        {
            ...
            {
                for (uint8 i = 0; i == 3; i++)
                    SanctumSentryGUIDs[i]= 0;
            }

            void Reset()
            {
                _Reset();
                _RespawnSanctumSentrys();
                DefenderGUID = 0;
                defenderLives = 8;
                crazyCatLady = true;
                nineLives = false;
            ...
            void _RespawnSanctumSentrys()
            {
                uint8 mode = RAID_MODE(1,2);

                if (SanctumSentryGUIDs[0] != 0)
                    if (Creature* tempsummon = ObjectAccessor::GetCreature(*me, SanctumSentryGUIDs[0]))
                        tempsummon->DespawnOrUnsummon(0);

                if (SanctumSentryGUIDs[1] != 0)
                    if (Creature* tempsummon = ObjectAccessor::GetCreature(*me, SanctumSentryGUIDs[1]))
                        tempsummon->DespawnOrUnsummon(0);

                if (SanctumSentryGUIDs[2] != 0 && mode == 2)
                    if (Creature* tempsummon = ObjectAccessor::GetCreature(*me, SanctumSentryGUIDs[2]))
                        tempsummon->DespawnOrUnsummon(0);

                if (SanctumSentryGUIDs[3] != 0 && mode == 2)
                    if (Creature* tempsummon = ObjectAccessor::GetCreature(*me, SanctumSentryGUIDs[3]))
                        tempsummon->DespawnOrUnsummon(0);
                
                TempSummon* i_ts;
                TempSummon* b_ts;
                if (i_ts = me->SummonCreature(NPC_SANCTUM_SENTRY,me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()))
                {
                    i_ts->ToUnit()->GetMotionMaster()->MoveFollow(me->ToUnit(), 0.5f, 0.5f);
                    SanctumSentryGUIDs[0] = i_ts->GetGUID();
                    if (mode == 2)
                    {
                        b_ts = i_ts->SummonCreature(NPC_SANCTUM_SENTRY,i_ts->GetPositionX(),i_ts->GetPositionY(), i_ts->GetPositionZ());
                        b_ts->ToUnit()->GetMotionMaster()->MoveFollow(i_ts->ToUnit(), 4.0f, 0.1f);
                        SanctumSentryGUIDs[2] = b_ts->GetGUID();
                    }
                }
                ...
                {
                    i_ts->ToUnit()->GetMotionMaster()->MoveFollow(me->ToUnit(), 0.5f, -0.5f);
                    SanctumSentryGUIDs[1] = i_ts->GetGUID();
                    if (mode == 2)
                    {
                        b_ts = i_ts->SummonCreature(NPC_SANCTUM_SENTRY,i_ts->GetPositionX(),i_ts->GetPositionY(), i_ts->GetPositionZ());
                        b_ts->ToUnit()->GetMotionMaster()->MoveFollow(i_ts->ToUnit(), 4.0f, -0.1f);
                        SanctumSentryGUIDs[3] = b_ts->GetGUID();
                    }
                }
            }

            void _RespawnSanctumSentry(uint8 id, float dist, float angle, bool bigmode)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DEFENDER:
                            ...
                            DoCast(me, SPELL_ACTIVATE_DEFENDER, true);
                            break;
