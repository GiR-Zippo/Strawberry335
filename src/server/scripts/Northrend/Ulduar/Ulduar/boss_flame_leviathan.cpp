...
class boss_flame_leviathan : public CreatureScript
{
    public:
        ...
        struct boss_flame_leviathanAI : public BossAI
        {
            ...
            {
                if (!me->getVictim() || !me->getVictim()->isAlive() || !CheckInRoom())
                    if (!UpdateVictim())
                        return;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_PURSUE:
                            ...
                            events.ScheduleEvent(EVENT_PURSUE, 65*IN_MILLISECONDS);
                            break;
                        case EVENT_MISSILE:
...
class npc_liquid_pyrite : public CreatureScript
{
    public:
        ...
        struct npc_liquid_pyriteAI : public PassiveAI
        {
            ...
            void SpellHit(Unit* caster, SpellInfo const* spell) 
            {
                //sLog->outString("ii %u",spell->Id);
                if (spell->Id == 62479)
                {
                    if (!caster->GetVehicleCreatureBase())
                        return;
                    me->CastSpell(caster->GetVehicleCreatureBase(),62496,false);
                    target = caster;
                    me->DisappearAndDie();
                    me->SetVisible(false);
                }
            ...
            {
                /*if (target)
                    if (me->GetPosition() == target->GetPosition())
                    {
                        me->DisappearAndDie();
                        sLog->outString("DEAD");
                    }*/
...
#define GOSSIP_ITEM_3  "Deactivate secondary defensive systems"
#define GOSSIP_ITEM_4  "Confirmed"
        ...
        switch (uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                if (player)
                {
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_4, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
                    player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
                }
                break;
            case GOSSIP_ACTION_INFO_DEF+2:
                ...
                instance->ProcessEvent(creature, EVENT_TOWER_OF_STORM_DESTROYED);
                instance->ProcessEvent(creature, EVENT_TOWER_OF_FROST_DESTROYED);
                instance->ProcessEvent(creature, EVENT_TOWER_OF_FLAMES_DESTROYED);
                instance->ProcessEvent(creature, EVENT_TOWER_OF_LIFE_DESTROYED);
                break;
        }
        return true;
    }
    ...
    public:
        ...
        class spell_pursue_SpellScript : public SpellScript
        {
            ...
            void FilterTargetsSubsequently(std::list<WorldObject*>& targets)
            {
                targets.clear();
                if (_target)
                    targets.push_front(_target);
            }

            void HandleScript(SpellEffIndex /*eff*/)
            {
                Creature* caster = GetCaster()->ToCreature();
                if (!caster && !_target->GetVehicleKit())
                    return;

                caster->AI()->AttackStart(_target);    // Chase target

                for (SeatMap::const_iterator itr = _target->GetVehicleKit()->Seats.begin(); itr != _target->GetVehicleKit()->Seats.end(); ++itr)
                {
                    if (IS_PLAYER_GUID(itr->second.Passenger))
                    {
...
/*62363
class spell_air_rocket : public SpellScriptLoader
{
    public:
        spell_air_rocket() : SpellScriptLoader("spell_air_rocket") {}

        class spell_air_rocket_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_air_rocket_SpellScript);
            ...
            void HandleTarget(std::list<Unit*>& unitList)
            {
                sLog->outString("%u", GetTargetUnit()->GetAreaId());
            }
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_air_rocket_SpellScript::HandleTarget, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
                //OnEffectLaunch
                OnEffectHitTarget += SpellEffectFn(spell_air_rocket_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_TRIGGER_MISSILE);
            }
        ...
        SpellScript* GetSpellScript() const
        {
            return new spell_air_rocket_SpellScript();
        }
};*/
...
class npc_ulduar_colossus : public CreatureScript
{
    public:
        ...
        struct npc_ulduar_colossusAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*victim*/)
            {
                me->GetInstanceScript()->SetData(DATA_COLOSSUS,1);
            }
        };
