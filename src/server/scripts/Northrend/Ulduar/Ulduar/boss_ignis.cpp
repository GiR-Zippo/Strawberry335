    ...
    public:
        ...
        struct boss_ignis_AI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (!used_construct_list.empty())
                    for (std::vector<Creature*>::iterator itr = used_construct_list.begin(); itr != used_construct_list.end(); itr++)
                        if ((*itr))
                            if ((*itr)->isAlive())
                                (*itr)->RemoveFromWorld();

                if (!construct_list.empty())
                    for (std::vector<Creature*>::iterator itr = construct_list.begin(); itr != construct_list.end(); itr++)
                        (*itr)->RemoveFromWorld();

                used_construct_list.clear();
                construct_list.clear();
                for (uint8 n = 0; n < CONSTRUCT_SPAWN_POINTS -1; n++)
                {
                    if (Creature* Construct = me->SummonCreature(NPC_IRON_CONSTRUCT, ConstructSpawnPosition[n], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 3000))
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SLAG_POT:
                            ...
                            // + 11Secs
                            events.ScheduleEvent(EVENT_SLAG_POT, RAID_MODE(41000, 26000));
                            break;
                        case EVENT_GRAB_POT:
                        ...
                        case EVENT_CHANGE_POT:
                            if (Unit* slagPotTarget = ObjectAccessor::GetUnit(*me, _slagPotGUID))
                            {
                                ...
                                slagPotTarget->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PLAYER_VEHICLE);
                        ...
                        case EVENT_CONSTRUCT:
                            if (!construct_list.empty())
                            {
                                ...
                                if (pTarget)
                                {
                                    ...
                                    used_construct_list.push_back((*itr));
        ...
        private:
            ...
            std::vector<Creature *> used_construct_list;
