...
class boss_algalon : public CreatureScript
{
    public:
        ...
        struct boss_algalonAI : public BossAI
        {
            ...
            {
                uint32 chestEntry;
                Position loot = { 1632.099976f, -306.561005f, 417.321014f };
                chestEntry = RAID_MODE(194821, 194822);
            ...
            void DoMeleeAttacksIfReady() // we need this function because DoMeleeAttackIfReady causes the mob to hit with both weapons at the same time (not blizzlike)
            {
                if (me->IsWithinMeleeRange(me->getVictim()) && !me->IsNonMeleeSpellCasted(false))
                {
                    if (me->isAttackReady() && me->getVictim())
                    {
                        ...
                        me->ResetAttackTimer();
                        me->SetAttackTimer(OFF_ATTACK, me->GetBaseAttackTime(BASE_ATTACK)/2);
                    }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                EntryCheckPredicate b_hole(CREATURE_BLACK_HOLE); //move this to an other position
                events.Update(diff);
                ...
                if (me->HasUnitState(UNIT_STATE_CASTING))
                    ...
                    case EVENT_SUMMON_UNLEASHED_DARK_MATTER:
                        summons.DoAction(ACTION_BLACKHOLE_SUMMON, b_hole);
    ...
    public:
        ...
        struct mob_living_constellationAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (Creature* BH = me->FindNearestCreature(CREATURE_BLACK_HOLE, 0.5f, true))
                {
        ...
        struct mob_algalon_asteroid_triggerAI : public PassiveAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if (Event_Timer <= diff)
                {
                    if (Event_Phase == 1)
                    {
                        if (Creature* Stalker = me->FindNearestCreature(CREATURE_ALGALON_ASTEROID_2, 100))
...
{
    public:
        spell_algalon_cosmic_smash() : SpellScriptLoader("spell_algalon_cosmic_smash_initial") { }
...
public:
    ...
    class spell_algalon_summon_asteroid_stalkers_SpellScript : public SpellScript
    {
        ...
        void SpellEffect(SpellEffIndex effIndex)
        {
            ...
            caster->SummonCreature(entry, pos, TEMPSUMMON_TIMED_DESPAWN, duration);
...
public:
    ...
    class spell_algalon_cosmic_smash_initial_SpellScript : public SpellScript
    {
        ...
        void FillTargetsSubsequential(std::list<WorldObject*>& unitList)
        {
            ...
            for (std::list<Unit*>::const_iterator itr = m_unitList.begin(); itr != m_unitList.end(); ++itr)
            {
