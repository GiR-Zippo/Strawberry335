...
public:
    ...
    struct instance_drak_tharon_InstanceScript : public InstanceScript
    {
        ...
        void Update(uint32 diff){
            ...
            if ((GetData(DATA_TROLLGORE_EVENT) == DONE) && (GetData(DATA_NOVOS_EVENT) == DONE)) {
                if (Creature* pTharon = instance->GetCreature(uiTharonJa)){
                    pTharon->SetReactState(REACT_AGGRESSIVE);
                    pTharon->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC);
                }
            }
        }
    };
