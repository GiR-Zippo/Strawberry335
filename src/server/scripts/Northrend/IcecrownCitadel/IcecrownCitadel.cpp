            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    case ACTION_KILL_CAPTAIN:
                        me->CastCustomSpell(SPELL_CARESS_OF_DEATH, SPELLVALUE_MAX_TARGETS, 1, me, TRIGGERED_FULL_MASK);
                        break;
                ...
                switch (spell->Id)
                {
                    ...
                    case SPELL_IMPALING_SPEAR:
                        if (TempSummon* summon = target->SummonCreature(NPC_IMPALING_SPEAR, *target))
                        {
                            ...
                            summon->CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED, SPELLVALUE_BASE_POINT0, 1, target, TRIGGERED_NONE);
...
class npc_wrynn_or_garrosh : public CreatureScript
{
    public:

    npc_wrynn_or_garrosh() : CreatureScript("npc_wrynn_or_garrosh") { }
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature) override
    {
        ...
        {
            if (pInstance->GetData(DATA_ICC_BUFF))
            {
                if (pPlayer->isGameMaster())
                    pPlayer->ADD_GOSSIP_ITEM_DB(gossipMenuId, GOSSIP_OPTION_REMOVE_ICC_BUFF, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                else if (sWorld->getBoolConfig(CONFIG_CUSTOM_ICC_BUFF_PLAYERCHOOSE))
                    if (pInstance->GetData(DATA_ICC_BUFF))
                        pPlayer->ADD_GOSSIP_ITEM_DB(gossipMenuId, GOSSIP_OPTION_REMOVE_ICC_BUFF, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            }
            else
                gossipMenuId += GOSSIPICCBUFFENDPAGE;
        }

        pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(gossipMenuId, pCreature), pCreature->GetGUID());
        return true;
    }
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 sender, uint32 action) override
    {
        ...
        uint32 gossipMenuId = Player::GetDefaultGossipMenuForSource(pCreature) + (action - GOSSIP_ACTION_INFO_DEF);

        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF + 1:
            {
                pPlayer->ADD_GOSSIP_ITEM_DB(gossipMenuId, GOSSIP_OPTION_REMOVE_ICC_BUFF, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(gossipMenuId, pCreature), pCreature->GetGUID());
            }break;
            ...
            case GOSSIP_ACTION_INFO_DEF + 2:
            {
                ...
                if (pCreature->GetInstanceScript())
                    pCreature->GetInstanceScript()->SetData(DATA_ICC_BUFF, NULL);
            }break;
            ...
            void HandleEvent(SpellEffIndex effIndex)
            {
                ...
                switch (GetSpellInfo()->Effects[effIndex].MiscValue)
                {
                    ...
                    case EVENT_AWAKEN_WARD_4:
                        ...
                        break;
                    case EVENT_AWAKEN_WARD_5:
                        trapId = GO_PLAGUE_TRAP;
                        break;
                    default:
                        return;
                }
                ...
                else if (trapId == GO_PLAGUE_TRAP)
                {
                    Position spawnPoint = { 4355.876953f, 3026.773926f, 360.52f, 3.14f };
                    for (uint32 i = 0; i < 10; i++)
                    {
                        Position point;
                        GetCaster()->GetRandomPoint(spawnPoint, 10.0f, point);
                        Creature* vengeful = GetCaster()->SummonCreature(NPC_VENGEFUL_FLESHREAPER, point);
                        if (i == 0)
                            vengeful->AI()->Talk(0);
                        if (Unit* target = vengeful->SelectNearestTarget(150.0f))
                            vengeful->AI()->AttackStart(target);
                    }
                }
            }
...
class go_orb_crimsonhalls : public GameObjectScript
{
public:
    go_orb_crimsonhalls() : GameObjectScript("go_orb_crimsonhalls") { }

    bool OnGossipHello(Player* player, GameObject* go)
    {
        player->AddAura(70227, player);
        
        go->SetGoState(GO_STATE_ACTIVE);

        if (Creature* controller = go->FindNearestCreature(38463, 5.0f))
...
void LoadIcecrownCitadel()
{
    ...
    new SpellTriggerSpellFromCaster("spell_svalna_caress_of_death", SPELL_IMPALING_SPEAR_KILL);
