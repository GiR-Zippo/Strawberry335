...
DoorData const doorData[] =
{
    ...
    {GO_KRIFFEL_AREA_RESTRICTION,            DATA_KRIFFEL,               DOOR_TYPE_ROOM,       BOUNDARY_NONE},
...
WeeklyQuest const WeeklyQuestData[WeeklyNPCs] =
{
    ...
    {NPC_INFILTRATOR_MINCHAR_BQ_10,      {QUEST_BLOOD_QUICKENING_10,              QUEST_BLOOD_QUICKENING_25             }}, // Blood Quickening
...
class InstanceIcecrownCitadel : public InstanceMapScript
{
    public:
        InstanceIcecrownCitadel() : InstanceMapScript(ICCScriptName, 631) { }
        ...
        struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
        {
            instance_icecrown_citadel_InstanceMapScript(InstanceMap* map) : InstanceScript(map)
            {
                ...
                memset(BloodPrinceGateGUIDs, 0, 2 * sizeof(uint64));
                PutricideCollisionGUID = 0;
                PutricideScientistEntrance = 0;
                FestergutGUID = 0;
                RotfaceGUID = 0;
                ...
                KriffelSummonerGUID = 0;
                skipCheckLichKing = false;
                frostmourneTeleportLichKing = 1;
            ...
            void OnPlayerEnter(Player* player)
            {
                if (!TeamInInstance)
                {
                    ...
                    if (ICCBuffEntry != -1)
                    {
                        ...
                        {
                            ICCBuffEntry = sWorld->getIntConfig(CONFIG_CUSTOM_ICC_BUFF_ALLIANCE);
                            ICCBuffLKEntry = sWorld->getIntConfig(CONFIG_CUSTOM_ICC_BUFF_LK_ALLIANCE);
                        }
                        else if (TeamInInstance == HORDE)
                        {
                            ICCBuffEntry = sWorld->getIntConfig(CONFIG_CUSTOM_ICC_BUFF_HORDE);
                            ICCBuffLKEntry = sWorld->getIntConfig(CONFIG_CUSTOM_ICC_BUFF_LK_HORDE);
                        }
                    }
                }
                ...
                if (HeroicAttempts && MaxHeroicAttempts)
                {
                    DoUpdateWorldState(WORLDSTATE_SHOW_ATTEMPTS, uint32(instance->IsHeroic()));
                    DoUpdateWorldState(WORLDSTATE_ATTEMPTS_REMAINING, uint32(HeroicAttempts));
                    DoUpdateWorldState(WORLDSTATE_ATTEMPTS_MAX, uint32(MaxHeroicAttempts));
                }

                #ifdef SKIP_UPPER_SPIRE_HC
                if (instance->IsHeroic())
                    if (GetBossState(DATA_DEATHBRINGER_SAURFANG) == DONE)
                        player->TeleportTo(631, 76.8638f, 2211.370117f, 30.0, 3.14965f);
                #endif
                ...
                if (instance->IsHeroic())
                    if (GetBossState(DATA_PROFESSOR_PUTRICIDE) == DONE && GetBossState(DATA_BLOOD_QUEEN_LANA_THEL) == DONE && GetBossState(DATA_SINDRAGOSA) == DONE)
                        if (checkLowerSpireForHcKill() && checkUpperSpireForHcKill() && checkWingBossesForHcKill())
                            player->TeleportTo(631, 76.8638f, 2211.370117f, 30.0, 3.14965f);
                #endif
            }
            ...
            void OnCreatureCreate(Creature* creature)
            {
                ...
                switch (entry)
                {
                    ...
                    case NPC_SINDRAGOSAS_WARD:
                        ...
                        break;
                    case NPC_KRIFFEL_SUMMONER:
                        KriffelSummonerGUID = creature->GetGUID();
                        break;
                }

				// update npcs if crossfaction is activ
				// ToDo: Support for Gunship Battle
				switch (entry)
				{
					case NPC_KOR_KRON_GENERAL:
					...
					case NPC_SE_SKYBREAKER_MARINE:
						if (ServerAllowsTwoSideGroups())
							creature->setFaction(FACTION_FRIENDLY_FOR_ALL);
						break;
					case NPC_SKYBREAKER_ASSASSIN:
					case NPC_SKYBREAKER_DREADBLADE:
					case NPC_SKYBREAKER_HIEROPHANT:
					case NPC_SKYBREAKER_LUMINARY:
					...
					case NPC_SKYBREAKER_VINDICATOR:
						...
						break;
					case NPC_KOR_KRON_DEFENDER:
					case NPC_KOR_KRON_INVOKER:
					case NPC_KOR_KRON_NECROLYTE:
					case NPC_KOR_KRON_ORACLE:
					case NPC_KOR_KRON_PRIMALIST:
					case NPC_KOR_KRON_REAVER:
					case NPC_KOR_KRON_SNIPER:
					case NPC_KOR_KRON_STALKER:
					case NPC_KOR_KRON_TEMPLAR:
					case NPC_KOR_KRON_VANQUISHER:
						if (ServerAllowsTwoSideGroups()){
            ...
            void OnCreatureRemove(Creature* creature)
            {
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_MUTATED_ABOMINATION_25:
                        creature->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false);
                        creature->GetOwner()->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false);
                        break;
                    default:
                        break;
                }
            ...
            uint32 GetCreatureEntry(uint32 /*guidLow*/, CreatureData const* data)
            {
                ...
                switch (entry)
                {
                    ...
                    case NPC_INFILTRATOR_MINCHAR_BQ_10:
            ...
            void OnGameObjectCreate(GameObject* go)
            {
                switch (go->GetEntry())
                {
                    case GO_SCIENTIST_ENTRANCE:
                        PutricideScientistEntrance = go->GetGUID();
                        AddDoor(go, true);
                        break;
                    ...
                    case GO_ICE_WALL:
                    case GO_KRIFFEL_AREA_RESTRICTION:
                        AddDoor(go, true);
                        break;
                    ...
                    case GO_BLOOD_ELF_COUNCIL_DOOR:
                        BloodPrinceGateGUIDs[0] = go->GetGUID();
                        AddDoor(go, true);
                        break;
                    case GO_BLOOD_ELF_COUNCIL_DOOR_RIGHT:
                        BloodPrinceGateGUIDs[1] = go->GetGUID();
                        AddDoor(go, true);
                        break;
            ...
            uint32 GetData(uint32 type)
            {
                switch (type)
                {
                    ...
                    case DATA_KINETIC_BOMB_COUNT:
                        return KineticBombCount;
                    case DATA_SKIP_CHECK_LICHKING:
                        return skipCheckLichKing;
                    case DATA_FROSTMOURNE_TELEPORT:
                        return frostmourneTeleportLichKing;
                    case DATA_ICC_BUFF:
                        if (ICCBuffEntry == -1)
                            return 0;
                        else
                            return uint32(ICCBuffEntry);
                    default:
                        break;
                }
            ...
            uint64 GetData64(uint32 type)
            {
                switch (type)
                {
                    ...
                    case DATA_PUTRICIDE_GATE_1:
                        return PutricideGateGUIDs[0];
                    case DATA_PUTRICIDE_GATE_2:
                        return PutricideGateGUIDs[1];
                    case DATA_PUTRICIDE_GATE_3:
                        return PutricideCollisionGUID;
                    case DATA_PUTRICIDE_ENTRANCE:
                        return PutricideScientistEntrance;
            ...
            void setKilledInHc(uint32 type)
            {
                killedInHc[type] = true;
            }

            bool getKilledInHc(uint32 type) const
            {
                return killedInHc[type];
            }

            bool checkLowerSpireForHcKill() const
            {
                bool check = getKilledInHc(DATA_LORD_MARROWGAR);
                check &= getKilledInHc(DATA_LADY_DEATHWHISPER);
                check &= getKilledInHc(DATA_GUNSHIP_BATTLE);
                check &= getKilledInHc(DATA_DEATHBRINGER_SAURFANG);
                return check;
            }

            bool checkUpperSpireForHcKill() const
            {
                bool check = getKilledInHc(DATA_ROTFACE);
                check &= getKilledInHc(DATA_FESTERGUT);
                check &= getKilledInHc(DATA_VALITHRIA_DREAMWALKER);
                check &= getKilledInHc(DATA_BLOOD_PRINCE_COUNCIL);
                return check;
            }

            bool checkWingBossesForHcKill() const
            {
                bool check = getKilledInHc(DATA_PROFESSOR_PUTRICIDE);
                check &= getKilledInHc(DATA_SINDRAGOSA);
                check &= getKilledInHc(DATA_BLOOD_QUEEN_LANA_THEL);
                return check;
            }
                ...
                switch (type)
                {
                    case DATA_LORD_MARROWGAR:
                        if (state == DONE)
                        {
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_LORD_MARROWGAR, NULL);
                        }
                        break;
                    case DATA_LADY_DEATHWHISPER:
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_LADY_DEATHWHISPER, NULL);
#ifdef SKIP_GUNSHIP
                    ...
                    case DATA_GUNSHIP_EVENT:
                        if (state == DONE)
                        {
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, 72959, NULL);
                        }
                        break;
                    case DATA_DEATHBRINGER_SAURFANG:
                        switch (state)
                        {
                            case DONE:
                                ...
                                if (instance->IsHeroic())
                                    setKilledInHc(type);
                                UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_DEATHBRINGER_SAURFANG, NULL);
                                // no break
                            case NOT_STARTED:
                    ...
                    case DATA_FESTERGUT:
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_ROTFACE, NULL);
                        }
                        break;
                    case DATA_ROTFACE:
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_FESTERGUT, NULL);
                        }
                        break;
                    case DATA_PROFESSOR_PUTRICIDE:
                        ...
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_PROFESSOR_PUTRICIDE, NULL);
                        }
                        if (instance->IsHeroic())
                        {
                    ...
                    case DATA_BLOOD_PRINCES_CONTROL:
                        if (state == DONE)
                        {
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_PRINCE_VALANAR, NULL);
                        }
                        break;
                    case DATA_BLOOD_QUEEN_LANA_THEL:
                        ...
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_BLOOD_QUEEN_LANA_THEL, NULL);
                        }
                        if (instance->IsHeroic())
                        {
                            if (state == FAIL && HeroicAttempts)
                    ...
                    case DATA_VALITHRIA_DREAMWALKER:
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_VALITHRIA_DREAMWALKER, NULL);
                        }
                        break;
                    case DATA_SINDRAGOSA_GAUNTLET:
                        if (state == DONE)
                        {
                            HandleGameObject(SindragosaEntranceDoorGUID, false);
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_SINDRAGOSAS_WARD, NULL);

                            instance->SummonCreature(NPC_KRIFFEL_SUMMONER, GunshipErsatzSummonerPos);
                        }
                    case DATA_SINDRAGOSA:
                        ...
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_SINDRAGOSA, NULL);
                        }
                        if (instance->IsHeroic())
                        {
                            if (state == FAIL && HeroicAttempts)
                    ...
                    case DATA_THE_LICH_KING:
                    {
                        ...
                        if (state == DONE)
                        {
                            ...
                            if (instance->IsHeroic())
                                setKilledInHc(type);
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_THE_LICH_KING, NULL);
                        }
                        break;
                    }
                    case DATA_KRIFFEL:
                    {
                        if (state == DONE)
                        {
                            UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, NPC_KRIFFEL, NULL);

                            if (Creature *creature = instance->GetCreature(KriffelSummonerGUID))
            ...
            void SetData(uint32 type, uint32 data)
            {
                switch (type)
                {
                    ...
                    case DATA_KINETIC_BOMB_COUNT:
                        KineticBombCount = data;
                        break;
                    case DATA_SKIP_CHECK_LICHKING:
                        skipCheckLichKing = true;
                        break;
                    case DATA_FROSTMOURNE_TELEPORT:
                        frostmourneTeleportLichKing = data;
                        break;
                    case DATA_ICC_BUFF:
                        if (data == 0)
                        {
            ...
            bool CheckRequiredBosses(uint32 bossId, Player const* player = NULL) const
            {
                ...
                switch (bossId)
                {
                    case DATA_THE_LICH_KING_HC:
                        if (!checkWingBossesForHcKill())
                            return false;
                        // no break
                    case DATA_THE_LICH_KING:
            ...
            bool CheckLowerSpire(uint32 bossId) const
            {
                switch (bossId)
                {
                    ...
                    case DATA_FESTERGUT:
                        if (GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE)
                        {
                            // Extended check for saurfang (workaround) @todo
                            Unit* pUnit = sObjectAccessor->FindUnit(DeathbringerSaurfangGUID);
                            if (pUnit && pUnit->isAlive() && !pUnit->HasAura(70628)) // SPELL_PERMANENT_FEIGN_DEATH
                                return false;
                            else if (pUnit)
                            {
                                sLog->outError("SAURFANG: Workaround activated with state %u alive %s aura %s",
                                               GetBossState(DATA_DEATHBRINGER_SAURFANG),
                                               pUnit->isAlive() ? "true" : "false",
                                               pUnit->HasAura(70628) ? "true" : "false");
                            }
                            else
                                sLog->outError("SAURFANG: Workaround activated with invalid unit pointer - not spawned?");
                        }
                        // no break
                    case DATA_DEATHBRINGER_SAURFANG:
            ...
            std::string GetSaveData()
            {
                ...
                << ColdflameJetsState << ' ' << BloodQuickeningState << ' ' << BloodQuickeningMinutes << ' ' << SpireFrostwyrmState;

                for (uint32 i = 0; i < EncounterCount; ++i)
                {
                    saveStream << ' ' << killedInHc[i];
                }
            ...
            void Load(const char* str)
            {
                ...
                if (dataHead1 == 'I' && dataHead2 == 'C')
                {
                    ...
                    if (GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE)
                    {
                        for (uint32 i = (DATA_DEATHBRINGER_SAURFANG + 1); i < EncounterCount; ++i)
                        {
                    ...
                    loadStream >> temp;
                    SpireFrostwyrmState = temp ? DONE : NOT_STARTED;

                    for (uint32 i = 0; i < EncounterCount; ++i)
                    {
                        loadStream >> killedInHc[i];
                    }
                }
                else
                    OUT_LOAD_INST_DATA_FAIL;
            ...
            {/*
                ...
                if (!isPrepared)
                {
                    ...
                    th->UpdatePassengerPositions();
                    th->UpdatePlayerPositions();
                }
                else
                {
            ...
            */}
            ...
            void ApplyICCBuff(Player* pPlayer)
            {
                if (!pPlayer || pPlayer->isGameMaster())
                    return;

                if (ICCBuffEntry)
                    if (!pPlayer->HasAura(ICCBuffEntry))
                        pPlayer->AddAura(ICCBuffEntry, pPlayer);
            }

            void ApplyICCBuffLK(Player* pPlayer)
            {
                if (!pPlayer || pPlayer->isGameMaster())
                    return;

                RemoveICCBuff(pPlayer);

                // Apply icc buff for lk if RAID_DIFFICULTY_25MAN_HEROIC
                if (instance->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC && ICCBuffLKEntry)
                {
                    if (!pPlayer->HasAura(ICCBuffLKEntry))
                        pPlayer->AddAura(ICCBuffLKEntry, pPlayer);
                }
                else
                    ApplyICCBuff(pPlayer);
            }

            void RemoveICCBuff(Player* pPlayer)
            {
                if (!pPlayer || pPlayer->isGameMaster())
                    return;
                ...
                for (uint8 i = 0; i < (sizeof(ICCBuffEntries[index]) / sizeof(uint32)); ++i)
                    pPlayer->RemoveAurasDueToSpell(ICCBuffEntries[index][i]);
            }
        ...
        protected:
            ...
            uint64 PutricideScientistEntrance;
            uint64 BloodPrinceGateGUIDs[2];
            ...
            uint8 KineticBombCount;
            uint8 frostmourneTeleportLichKing;
            bool IsBonedEligible;
            bool IsOozeDanceEligible;
            bool IsNauseaEligible;
            bool IsOrbWhispererEligible;
            bool killedInHc[EncounterCount];
            bool skipCheckLichKing;
            ...
            uint64 KriffelSummonerGUID;
            int32 ICCBuffEntry;
            int32 ICCBuffLKEntry;
        };
