        ...
        void SummonChampions(Team playerTeam)
        {
            ...
            for (uint8 i = 0; i < vChampionEntries.size(); ++i)
            {
                ...
                if (Creature* temp = me->SummonCreature(vChampionEntries[i], vChampionJumpOrigin[urand(0, vChampionJumpOrigin.size()-1)], TEMPSUMMON_MANUAL_DESPAWN))
                {
                    ...
                    {
                        temp->SetHomePosition((TrialCrusaderLoc[0].GetPositionX()*2)-vChampionJumpTarget[pos].GetPositionX(), vChampionJumpTarget[pos].GetPositionY(), vChampionJumpTarget[pos].GetPositionZ(), 3);
                        temp->GetMotionMaster()->MoveJump((TrialCrusaderLoc[0].GetPositionX()*2)-vChampionJumpTarget[pos].GetPositionX(), vChampionJumpTarget[pos].GetPositionY(), vChampionJumpTarget[pos].GetPositionZ(), 20.0f, 20.0f);
                    ...
                    switch (uiData)
                    {
                        ...
                        case DONE:
                            ...
                            if (m_uiChampionsKilled == 1)
                                instance->SetData(TYPE_ENCOUNTER_STATUS, SPECIAL);
...
void AddEncounterFactionChampions()
{
