        ...
        void UpdateCorporeality()
        {
            ...
            for (uint8 itr = DATA_HALION; itr <= DATA_TWILIGHT_HALION; itr++)
            {
                if (Creature* halion = ObjectAccessor::GetCreature(*me, _instance->GetData64(itr)))
                {
                    ...
                    if (itr == DATA_TWILIGHT_HALION)
                        halion->AI()->Talk(oldValue < _materialCorporealityValue ? EMOTE_CORPOREALITY_TOT : EMOTE_CORPOREALITY_TIT, _instance->GetData64(DATA_HALION));
                    else // if (itr == DATA_HALION)
                        halion->AI()->Talk(oldValue > _materialCorporealityValue ? EMOTE_CORPOREALITY_POT : EMOTE_CORPOREALITY_PIP, _instance->GetData64(DATA_HALION));
                }
            }
        }
    ...
    public:
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (GetTarget()->HasAura(_markSpell))
                GetTarget()->RemoveAurasDueToSpell(_markSpell, NULL, 0, AURA_REMOVE_BY_EXPIRE);
        }
    ...
    public:
        ...
        void BeforeDispel(DispelInfo* dispelData)
        {
            ...
            if (dispelledUnit->HasAura(_removeSpellId))
                dispelledUnit->RemoveAurasDueToSpell(_removeSpellId, NULL, 0, AURA_REMOVE_BY_EXPIRE);
        }
...
public:
    ...
    public:
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*handle*/)
        {
            ...
            target->RemoveAurasDueToSpell(_beforeHitSpellId, NULL, 0, AURA_REMOVE_BY_ENEMY_SPELL);
