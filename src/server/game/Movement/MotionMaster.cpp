...
void MotionMaster::UpdateMotion(uint32 diff)
{
    ...
    // Crash fix if player has invalid map coords (workaround - research + fix needed) @todo 29.03.2015 <TehKevin>
    if (_owner->GetTypeId() == TYPEID_PLAYER && !MapManager::IsValidMapCoord(_owner->GetMapId(), _owner->GetPositionX(), _owner->GetPositionY(), _owner->GetPositionZ()))
    {
        if (Player* pPlayer = _owner->ToPlayer())
        ...
        sLog->outError("Player %u has invalid map coords (map: %u, x: %f, y: %f, z: %f) - ported to homebind",
                       _owner->GetGUIDLow(), _owner->GetMapId(), _owner->GetPositionX(), _owner->GetPositionY(), _owner->GetPositionZ());
        return;
    }
