...
void ConfusedMovementGenerator<T>::Initialize(T &unit)
{
    ...
    for (uint8 idx = 0; idx < MAX_CONF_WAYPOINTS + 1; ++idx)
    {
        ...
        {
            float checkZ = idx > 0 ? i_waypoints[idx-1][2] : z;
            bool is_water = map->IsInWater(wanderX, wanderY, z);

            if ((is_water && !is_water_ok) || (!is_water && !is_land_ok) || ((fabs(checkZ - z) > INVALID_THRESHOLD)  && !unit.IsFalling() && !unit.IsFlying() && !unit.HasUnitState(UNIT_STATE_JUMPING)))
            {
