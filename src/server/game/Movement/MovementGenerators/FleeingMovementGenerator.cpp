...
bool FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    float temp_x, temp_y, angle = 0.0f;
    ...
    for (uint8 i = 0; i < 15; ++i)
    {
        ...
        switch (i % 3)
        {
            case 0:
                ...
                if (!i_only_forward)
                {
                    ...
                    {
                        relax++;
                        angle += frand(-M_PI, M_PI) * float(relax) / 4.0f;
                    }
                    else
                        angle += frand(-M_PI, M_PI);
                }
                break;
            case 1:
                distance /= 2.0f;
        ...
        if (owner.IsWithinLOS(temp_x, temp_y, z))
        {
            ...
            if (new_z <= INVALID_HEIGHT || (fabs(new_z - z) > INVALID_THRESHOLD))
                continue;
...
void FleeingMovementGenerator<T>::Initialize(T &owner)
{
    ...
    i_to_distance_from_caster = 0.0f;
    if (owner.IsFalling() || owner.IsFlying() || owner.HasUnitState(UNIT_STATE_JUMPING))
        return;
