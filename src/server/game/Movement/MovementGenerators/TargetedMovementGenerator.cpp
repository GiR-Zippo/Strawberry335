...
//     }
//     else
    if (i_target->GetTransport())
    {
        if (i_target->IsWithinDistInMap(&owner, i_offset + 1.0f) || !i_targetReached)
            return;
        x = i_target->m_movementInfo.transport.pos.m_positionX;
        y = i_target->m_movementInfo.transport.pos.m_positionY;
        z = i_target->m_movementInfo.transport.pos.m_positionZ;
        float o = i_target->GetOrientation();
        i_target->GetTransport()->CalculatePassengerPosition(x,y,z,o);
