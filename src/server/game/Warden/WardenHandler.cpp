...
void Warden::Update()
{
    if (m_initialized)
    {
        ...
        if (m_WardenDataSent)
        {
            ...
            if (m_WardenKickTimer > sWorld->getIntConfig(CONFIG_WARDEN_CLIENT_RESPONSE_DELAY) * IN_MILLISECONDS)
            {
...
void Warden::HandleData(ByteBuffer &buff)
{
    ...
    if (found)
        sWorld->BanAccountbyId(Client->GetAccountId(), sWorld->getIntConfig(CONFIG_WARDEN_CLIENT_BAN_DURATION), "Anticheat warning", "Warden");
}
