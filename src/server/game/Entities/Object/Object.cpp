...
void Object::_BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    uint32* flags = NULL;
    uint32 visibleFlag = _GetUpdateFieldData(target, flags);
...
uint32 Object::_GetUpdateFieldData(Player const* target, uint32*& flags) const
{
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT && ToUnit()->IsIgnoredByLoS())
        return true;
...
Player* Object::ToPlayer() { return dynamic_cast<Player*>(this); }
Player const* Object::ToPlayer() const { return dynamic_cast<Player const*>(this); }

Creature* Object::ToCreature() { return dynamic_cast<Creature*>(this); }
Creature const* Object::ToCreature() const { return dynamic_cast<Creature const*>(this); }

Unit* Object::ToUnit() { return dynamic_cast<Unit*>(this); }
Unit const* Object::ToUnit() const { return dynamic_cast<Unit const*>(this); }

GameObject* Object::ToGameObject() { return dynamic_cast<GameObject*>(this); }
GameObject const* Object::ToGameObject() const { return dynamic_cast<GameObject const*>(this); }

Corpse* Object::ToCorpse() { return dynamic_cast<Corpse*>(this); }
Corpse const* Object::ToCorpse() const { return dynamic_cast<Corpse const*>(this); }

DynamicObject* Object::ToDynObject() { return dynamic_cast<DynamicObject*>(this); }
DynamicObject const* Object::ToDynObject() const { return dynamic_cast<DynamicObject const*>(this); }
...
void WorldObject::SetMap(Map* map)
{
    ...
    if (m_currMap && !(m_currMap->GetId() == map->GetId() && m_currMap->GetDifficulty() != map->GetDifficulty()) && map->GetId() != 631)
    {
...
{
    bool alt = false;
    float tempX, tempY, tempZ, tempO;
    if (spellId && summoner)
    {
        float unitX, unitY, unitZ, unitO;
        summoner->GetPosition(unitX, unitY, unitZ, unitO);
        pos.GetPosition(tempX, tempY, tempZ, tempO);
        
        if (!isInLineOfSight(unitX, unitY, unitZ, tempX, tempY, tempZ, summoner->GetPhaseMask()) && !(sSpellStore.LookupEntry(spellId)->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS))
        {
            alt = true;
            summoner->GetPosition(tempX, tempY, tempZ, tempO);
        }
    }
    ...
    if (summoner)
    {
        ...
        if (summoner->GetTypeId() == TYPEID_PLAYER)
            team = summoner->ToPlayer()->GetTeam(true);
    }
    ...
    if (alt)
    {
        if (!summon->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), this, phase, entry, vehId, team, tempX, tempY, tempZ, tempO))
        {
            delete summon;
            return NULL;
        }
    ...
    if (alt)
        summon->SetHomePosition(tempX, tempY, tempZ, tempO);
    else
...
{
    uint32 count = 100;
    float distance = distance2d+searcher_size;
    float angle = absAngle;
    GetNearPoint2D(x, y, distance, angle);
    z = GetPositionZ();
    UpdateGroundPositionZ(x, y, z);

    while (!GetMap()->isInLineOfSight(GetPositionX(), GetPositionY(), GetPositionZ() + 0.5f, x, y, z + 0.5f, GetPhaseMask()) && count > 0)
    {
        if (distance >= 0.01f)
            distance -= 0.01f;
        GetNearPoint2D(x, y, distance, angle);
    ...
    if (distance < distance2d+searcher_size)
    {
        if (distance >= 0.1f)
            distance = 0.0f;
        else
