...
enum ItemLevelItemCount
{
    ITEMLEVEL_ITEM_COUNT_WITH_OFFHAND     = 17,
    ITEMLEVEL_ITEM_COUNT_WITHOUT_OFFHAND  = 16
};
...
struct InstancePlayerBind
{
    public:
        ...
        uint8 GetExtended() { return extended; }
        void SetExtended(uint8 state, uint32 lowGUID, bool onLoad = false);
...
enum PlayerRatesOverride
{
    RATE_OVERRIDE_XP_KILL,
    RATE_OVERRIDE_XP_QUEST,
    RATE_OVERRIDE_XP_EXPLORE,
    MAX_OVERRIDE_RATES
};

const uint32 ProfessionSkills[] = { 171, 164, 333, 202, 182, 773, 755, 165, 186, 393, 197 };
...
class PlayerTaxi
{
    public:
        ...
        /** Returns the last node of m_TaxiDestinations (Destination node of path)*/
        uint32 GetTaxiLastDestination() const {
            return m_TaxiDestinations.empty() ? 0 : m_TaxiDestinations.back();
        }
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        bool TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options = 0, bool forceLoad = false);
        bool TeleportTo(WorldLocation const &loc, uint32 options = 0, bool forceLoad = false)
        {
            return TeleportTo(loc.GetMapId(), loc.GetPositionX(), loc.GetPositionY(), loc.GetPositionZ(), loc.GetOrientation(), options, forceLoad);
        }
        ...
        std::string GetPlayerLink() const
        {
            return GetPlayerLink(GetName());
        }

        static std::string GetPlayerLink(std::string name)
        {
        ...
        Item* GetNotPermittedItemInArena();
        void RemoveItemBonus(Item* pItem);
        void AddItemBonus(Item* pItem);
        ...
        bool hasQuest(uint32 questId) const override;
        ...
        /** Checks primary profession count and removes profession with lowest skill */
        void CheckPrimaryProfessions();
        ...
        template <class TYPE>
        TYPE ApplySpellMod(uint32 spellId, SpellModOp op, TYPE &basevalue, Spell* spell = NULL);
        ...
        float GetRatingModPCT(CombatRating cr) const;
        ...
        float GetMeleeCritFromAgility(bool onlyPetBonus = false);
        void GetDodgeFromAgility(float &diminishing, float &nondiminishing);
        float GetMissPercentageFromDefence() const;
        float GetSpellCritFromIntellect(bool onlyPetBonus = false);
        ...
        /** @param force : Ignore crossfaction battlegrounds specific teams */
        uint32 GetTeam(bool force = false) const;
        TeamId GetTeamId(bool force = false) const;
        ...
        void CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, const SpellInfo* procSpellInfo);
        void CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex);
        void CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto, const SpellInfo* procSpellInfo);
        ...
        bool HandleWithCrossFactionBG(const Player* pPlayer = NULL) const;
        ...
        bool InBattlefield() const {
            return sBattlefieldMgr->GetBattlefieldToZoneId(GetZoneId());
        }
        ...
        void InitializeRndJoinCount(void);
        uint8 GetRndJoinCount(void);
        void IncrementRndJoinCount(void);
        void ResetRndJoinCount(void);

        void SetNewInGrp(bool value);
        bool IsNewInGrp(void);
        void SwapInstanceOut(uint32 oldInstance);
        void SwapInstanceIn(uint32 newInstance);
        ...
        InstancePlayerBind* BindToInstance(InstanceSave* save, bool permanent, bool load = false, uint8 extended = 0);
        void BindToInstance();
        ...
        */
        ...
        bool ItemlevelChanged()
        {
            float newItemLevel = GetAverageItemLevel();
            if (m_lastItemLevel != newItemLevel)
            {
                m_lastItemLevel = newItemLevel;
                return true;
            }
            else
                return false;
        ...
        void SetCooldownMod(int32 time, bool apply)
        {
            if (apply && m_spellCooldownModTime == 0)
                m_spellCooldownModTime += time;
            else if (m_spellCooldownModTime == time)
                m_spellCooldownModTime -= time;
        }

        void SendModCooldownIfAny(const SpellInfo* spellInfo);

        void SetOverrideRate(PlayerRatesOverride mod, uint32 val) { m_overrideRates[mod] = val; }
        uint32 GetOverrideRate(PlayerRatesOverride mod) { return m_overrideRates[mod]; }
        float CalculateOverrideRate(PlayerRatesOverride mod, float val) { return (m_overrideRates[mod] ? (float)m_overrideRates[mod] : val); }
    ...
    protected:
        ...
        */
        ...
        void _LoadMails(PreparedQueryResult  mail_result, PreparedQueryResult item_result);
        void _LoadMailedItems(std::map<uint32, Mail *> &mailMap, PreparedQueryResult item_result);
        ...
        void ActivateActions(uint8 spec);
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
    ...
    private:
        ...
        uint8 m_RndJoinCount;
        bool m_RndJoinCountExists;
        bool m_NewInGrp;
        ...
        //Personal Ranking
        uint32 m_pr_weekly;
        uint32 m_pr_lifetime;

        float m_itemLevelSum;
        bool m_wearingOffhand;
        bool m_needLFGUpdate;

        int32 m_spellCooldownModTime;

        uint32 m_overrideRates[MAX_OVERRIDE_RATES];

        float m_lastItemLevel;

        PlayerStatsModifier playerStatsModifier;

public:
    bool IsSaveCommited() const { return _transactions.empty(); }
    void AddTransaction(SQLTransaction transaction) { _transactions.push_back(transaction); }

    PlayerTransmog* GetTransmog() { return &Transmog; }

    PlayerStatsModifier* GetStatsModifier() { return &playerStatsModifier; }

private:
    std::list<SQLTransaction> _transactions;
    PlayerTransmog Transmog;
};
...
template <class TYPE>
TYPE Player::ApplySpellMod(uint32 spellId, SpellModOp op, TYPE &basevalue, Spell* spell)
{
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            if (mod->op == SPELLMOD_CASTING_TIME && basevalue >= TYPE(10000) && mod->value <= -100)
                continue;
    ...
    basevalue = TYPE((float)basevalue + diff);
    return TYPE(diff);
}
#endif
