...
bool Item::LoadFromDB(uint32 guid, uint64 owner_guid, Field* fields, uint32 entry)
{
    ...
    if (GemsFitSockets())
        SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 + BONUS_ENCHANTMENT_SLOT * MAX_ENCHANTMENT_OFFSET + ENCHANTMENT_ID_OFFSET, GetTemplate()->socketBonus);
...
void Item::DeleteFromDB(SQLTransaction& trans, uint32 itemGuid)
{
    ...
    // item_loot
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEM_LOOT);
    stmt->setUInt32(0, itemGuid);
    trans->Append(stmt);
}
...
/*static*/
void Item::DeleteFromInventoryDB(SQLTransaction& trans, uint32 itemGuid, uint32 playerGuid)
{
...
void Item::SaveItemLootToDB(SQLTransaction* givenTrans)
{
    if (!loot.isLooted())
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_REP_ITEM_LOOT);
        stmt->setUInt32(0, GetGUIDLow());

        // Gold
        stmt->setUInt32(1, loot.gold);
        ...
        if (!loot.items.empty())
        {
            for (std::vector<LootItem>::const_iterator itr = loot.items.begin();
                 itr != loot.items.end();
                 ++itr)
            {
                if (!itr->is_looted)
...
uint32 Item::GetPermittedItemInArena() const
{
    // Get replacement from array if item should be replaced
    for (uint8 i = 0; i < ITEM_NOT_PERMITTED_COUNT; ++i)
        if (ArenaNotPermittedItems[i][0] == GetEntry())
            return ArenaNotPermittedItems[i][1];
    return GetEntry();
}
