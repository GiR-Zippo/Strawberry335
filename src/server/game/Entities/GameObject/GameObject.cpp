...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_SPELLCASTER:                   //22
        {
            ...
            // Special case SotA 'Seaforium Bomb' (Crossfaction)
            if (GetEntry() == OBJECT_SEAFORIUM_BOMB && spellCaster && spellCaster->GetTypeId() == TYPEID_PLAYER
                && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS))
            {
                if (Player* pPlayer = spellCaster->ToPlayer())
                    if (Battleground* pBattleground = pPlayer->GetBattleground())
                        if (pBattleground->GetData(BG_SA_DATA_ATTACKER_TEAM) != pPlayer->GetTeamId())
                            return;
            }
...
void GameObject::TakenDamage(uint32 damage, Unit *who)
{
    ...
    if (HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_DAMAGED)) // from damaged to destroyed
    {
        if (!m_goValue->Building.Health)
        {
            ...
            sScriptMgr->OnGameObjectDestroyed(this, pwho);
        }
    }
    else // from intact to damaged
    {
        ...
        if (m_goValue->Building.Health <= m_goInfo->building.damagedNumHits)
        {
            ...
            EventInform(m_goInfo->building.damagedEvent);
            sScriptMgr->OnGameObjectDamaged(this, pwho);
            if (pwho)
                if (Battleground* bg = pwho->GetBattleground())
...
void GameObject::_BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == GAMEOBJECT_FLAGS && forcedFlags))
        {
            ...
            else if (index == GAMEOBJECT_FACTION && target->HandleWithCrossFactionBG())
            {
                ...
                switch (GetGoType())
                {
                    ...
                    case GAMEOBJECT_TYPE_FLAGSTAND:
                        if (const FactionTemplateEntry* entry = sFactionTemplateStore.LookupEntry(m_uint32Values[GAMEOBJECT_FACTION]))
                            faction = Battleground::GetFactionForCrossfactionBG(target, entry, true);
                        break;
                }

                if (faction)
                    fieldBuffer << uint32(faction);
                else
