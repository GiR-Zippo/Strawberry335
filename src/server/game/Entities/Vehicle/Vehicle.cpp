...
void Vehicle::ApplyAllImmunities()
{
    ...
    // Define clean custom immuns
    switch (_me->ToCreature()->GetCreatureTemplate()->Entry)
    {
        case 27881:
        case 28094:
        case 28312:
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    unit->m_movementInfo.transport.pos.m_positionX = veSeat->m_attachmentOffsetX;
    unit->m_movementInfo.transport.pos.m_positionY = veSeat->m_attachmentOffsetY;
    unit->m_movementInfo.transport.pos.m_positionZ = veSeat->m_attachmentOffsetZ;
    unit->m_movementInfo.transport.pos.m_orientation = 0;
    unit->m_movementInfo.transport.time = 0; // 1 for player
...
bool Vehicle::IsPassenger(Unit* pUnit)
{
    if (Seats.empty())
        return false;
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    {
        if (itr->second.Passenger == pUnit->GetGUID())
            return true;
    }
    return false;
}
