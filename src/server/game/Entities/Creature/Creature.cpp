...
void Creature::GetRespawnPosition(float &x, float &y, float &z, float* ori, float* dist) const
{
    ...
    if (ori)
        *ori = m_homePosition.GetOrientation();
    if (dist)
        *dist = 0;
}
...
void Creature::FarTeleportTo(Map* map, const Position& destination)
{
    CleanupBeforeRemoveFromMap(false);
    GetMap()->RemoveFromMap(this, false);
    Relocate(destination.GetPositionX(), destination.GetPositionX(), destination.GetPositionX(), destination.GetOrientation());
    SetMap(map);
    GetMap()->AddToMap(this);
}
