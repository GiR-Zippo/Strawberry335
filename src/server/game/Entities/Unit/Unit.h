...
enum TriggerCastFlags
{
    ...
    // Triggered mask to exclude some triggers (has to be updated at TriggerCastFlags change
    TRIGGERED_CHANGE_MASK                           = ((TRIGGERED_DONT_REPORT_CAST_ERROR * 2) - 1),

    // Triggered mask for item procs (everything without TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD)
    TRIGGERED_ITEM_PROC_MASK                        = TRIGGERED_CHANGE_MASK - TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD
};
...
{
    DiminishingReturn()
        : Applications(0), RemoveTime(0), Level(DIMINISHING_LEVEL_1) { }
...
class Unit : public WorldObject
{
    public:
        ...
        typedef std::map<DiminishingGroup, DiminishingReturn> Diminishing;
        ...
        bool IsAffectedByDiminishing(DiminishingGroup group);
        DiminishingLevels GetDiminishingLevel(DiminishingGroup group);
        float ApplyDiminishingToDuration(const SpellInfo* spellproto, int32 &duration, Unit* caster, bool triggeredByAura);
        void ApplyDiminishingAura(DiminishingGroup  group, bool apply);
        void IncreaseDiminishingLevel(const SpellInfo* spellproto, bool triggeredByAura);
        ...
        uint32 GetAttackTimer(WeaponAttackType type) const { return _attackTimer[type]; }
        void SetAttackTimer(WeaponAttackType type, uint32 time) { _attackTimer[type] = time; }
        void ResetAttackTimer(WeaponAttackType type = BASE_ATTACK) { _attackTimer[type] = uint32(GetBaseAttackTime(type) * _totalCombatSpeedMod[type]); }
        bool isAttackReady(WeaponAttackType type = BASE_ATTACK) const { return _attackTimer[type] == 0; }

        uint32 GetBaseAttackTime(WeaponAttackType att) const{ return _baseAttackTime[att]; }
        void SetBaseAttackTime(WeaponAttackType att, uint32 val) { _baseAttackTime[att] = val; SetFloatValue(UNIT_FIELD_BASEATTACKTIME+att, val * _totalCombatSpeedMod[att]); }

        float GetHaste(CombatType ctype) const { return _hasteCombatSpeedMod[ctype]; }

        void UpdateHaste(CombatType ctype, float value);
        void ApplyCombatSpeedMod(CombatType ctype, float value, bool apply);
        void UpdateCombatSpeed(CombatType ctype);
        ...
        void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, uint32 effectiveAmount = 0, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL);
        void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 gainedHealOrCleanDamage = 0, SpellInfo const* procAura = NULL);
        ...
        float MeleeSpellDodgeChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const;
        float MeleeSpellParryChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const;
        float MeleeSpellBlockChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const;
        float MeleeGlancingChance(const Unit* victim, WeaponAttackType attType) const;
        float MeleeCritChance(const Unit* victim, WeaponAttackType attType) const;
        float MeleeCrushingChance(const Unit* victim, WeaponAttackType attType) const;
        ...
        uint32 GetShieldBlockValue(uint32 soft_cap, uint32 hard_cap) const
        {
            ...
            if (value >= soft_cap)
                value = (sqrt(8.0f *(value/(float)soft_cap) + 1.0f)-1.0f) * soft_cap /2.0f;
        ...
        void SetInCombatFor(uint32 time = 5000);
        void ClearInCombat();
        ...
        void CastCustomSpell(Unit* Victim, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, TriggerCastFlags triggerFlags, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void CastCustomSpell(uint32 spellId, CustomSpellValues const &value, Unit* Victim = NULL, TriggerCastFlags triggerFlag = TRIGGERED_NONE, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void DelayedCastSpell(uint32 delay, Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags = TRIGGERED_FULL_MASK, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        void DelayedCastCustomSpell(uint32 delay, Unit* Victim, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, TriggerCastFlags triggerFlags = TRIGGERED_FULL_MASK, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void NearTeleportTo(const Position& destination, bool casting = false);
        ...
        void SetShouldBeFeared() { m_shouldBeFeared = true; }
        bool ShouldBeFeared() { return m_shouldBeFeared; }
        ...
        */
        ...
        Unit* GetHealthPctLowestRaidMemberOrPet(float radius, uint32 excludeAura = 0);
        bool SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp = NULL);
        void RemoveCharmedBy(Unit* charmer, bool inRemove=false);
        void RestoreFaction();
        ...
        void RemoveAurasDueToItemSpell(uint64 castItemGUID, uint32 spellId);
        ...
        /**
         * Returns modifier/multiplier of auras which are permanent and without classmask
         * @param includeAreaAura : Consider auras with spell effect AreaAura (true)
         */
        int32 GetBaseAuraModifier(AuraType auratype, bool includeAreaAura = true) const;
        float GetBaseAuraMultiplier(AuraType auratype, bool includeAreaAura = true) const;

        /**
        * Returns modifier/multiplier of auras which are permanent and without classmask
        * @param includeAreaAura : Consider auras with spell effect AreaAura
        */
        int32 GetBaseAuraModifierByMiscMask(AuraType auratype, uint32 misc_mask, bool includeAreaAura = true) const;
        float GetBaseAuraMultiplierByMiscMask(AuraType auratype, uint32 misc_mask, bool includeAreaAura = true) const;

        /**
        * Returns modifier/multiplier of auras which are permanent and without classmask
        * @param includeAreaAura : Consider auras with spell effect AreaAura
        */
        int32 GetBaseAuraModifierByMiscValue(AuraType auratype, int32 misc_value, bool includeAreaAura = true) const;
        float GetBaseAuraMultiplierByMiscValue(AuraType auratype, int32 misc_value, bool includeAreaAura = true) const;
        ...
        void ShareThreat(UnitList victims, float fThreat, SpellSchoolMask schoolMask = SPELL_SCHOOL_MASK_NORMAL, SpellInfo const* threatSpell = NULL);
        ...
        uint32 CalculateBaseWeaponDamage(WeaponAttackType attType, bool normalized, bool addTotalPct);
        ...
        float CalculateDamageBonusDonePct();
        std::pair<int32, float> CalculateSpellDamageBonusPair(Unit *victim, SpellInfo const *spellProto);
        int32 CalculateCoefficientBonus(Unit *victim, SpellInfo const *spellProto, DamageEffectType damagetype, uint32 stack = 1, Aura *aura = NULL);
        uint32 SpellDamageBonusDone(Unit* victim, SpellInfo const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack = 1, Aura *aura = NULL);
        ...
        bool CanCritWithSpell(SpellInfo const* spellProto) const;
        bool IsSureCritAgainst(Unit* victim, SpellInfo const* spellProto) const;

        float CalculateCriticalChanceDone(WeaponAttackType attackType) const;
        float ApplyCriticalChanceTaken(float crit_chance, const Unit *attacker, WeaponAttackType attackType) const;

        float CalculateCriticalChanceSpellDone(SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const;
        float ApplyCriticalChanceSpellTaken(float crit_chance, Unit const *caster, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const;

        bool isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK, Aura *aura = NULL) const;
        ...
        float CalcResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, SpellInfo const* spellInfo = NULL);
        ...
        bool IsFalling() const  { return m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING); }
        ...
        Guardian* ToGuardian(){ if (isGuardian()) return reinterpret_cast<Guardian*>(this); else return NULL; }
        ...
        void SetIgnoreLoS(bool value);
        bool IsIgnoredByLoS() const;
        ...
        bool TeleportAllowed() const { return (m_TeleDelay == 0); }

    protected:
        explicit Unit (bool isWorldObject);

        void _BuildValuesUpdate(uint8 updatetype, ByteBuffer* data, Player* target) const override;
        ...
        uint32 _attackTimer[MAX_ATTACK];
        uint32 _baseAttackTime[MAX_ATTACK];

        float _hasteCombatSpeedMod[MAX_CTYPE];
        float _auraCombatSpeedMod[MAX_CTYPE];
        float _totalCombatSpeedMod[MAX_CTYPE];
    ...
    private:
        ...
        bool HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 gainedHealOrCleanDamage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown);
        ...
        bool m_isIgnoredByLoS; // every spell hits this unit if set, regardless of los
        bool m_shouldBeFeared; // apply fear only if player not is falling or jumping
};
...
namespace Trinity
{
    ...
    class PowerPctOrderPred
    {
        public:
            ...
            bool operator() (const WorldObject* objA, const WorldObject* objB)
            {
                const Unit* a = objA->ToUnit();
                const Unit* b = objB->ToUnit();
                return order(a, b);
            }
        ...
        private:
            ...
            bool order(const Unit* a, const Unit* b) const
            {
                float rA = (a && a->GetMaxPower(m_power)) ? float(a->GetPower(m_power)) / float(a->GetMaxPower(m_power)) : 0.0f;
                float rB = (b && b->GetMaxPower(m_power)) ? float(b->GetPower(m_power)) / float(b->GetMaxPower(m_power)) : 0.0f;
