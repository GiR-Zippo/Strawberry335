...
m_HostileRefManager(this), m_isIgnoredByLoS(false), m_shouldBeFeared(false), m_TeleDelay(0)
{
#ifdef _MSC_VER
#pragma warning(default:4355)
    ...
    _attackTimer[BASE_ATTACK] = 0;
    _attackTimer[OFF_ATTACK] = 0;
    _attackTimer[RANGED_ATTACK] = 0;
    
    _baseAttackTime[BASE_ATTACK] = BASE_ATTACK_TIME;
    _baseAttackTime[OFF_ATTACK] = BASE_ATTACK_TIME;
    _baseAttackTime[RANGED_ATTACK] = BASE_ATTACK_TIME;
    
    for (uint32 i = 0; i < MAX_CTYPE; ++i)
    {
        _hasteCombatSpeedMod[i] = 1.0f;
        _auraCombatSpeedMod[i] = 1.0f;
        _totalCombatSpeedMod[i] = 1.0f;
    }
...
void Unit::Update(uint32 p_time)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER || (ToCreature()->isPet() && IsControlledByPlayer()))
    {
        if (isInCombat())
        {
            ...
            m_TeleDelay = (m_TeleDelay > p_time) ? m_TeleDelay - p_time : 0;
    }
    ...
    if (isAlive())
    {
        if (m_shouldBeFeared && !(IsFalling() || HasUnitState(UNIT_STATE_JUMPING)))
        {
            // Prevent fear bug if player has dispelled fear while jumping
            if (HasAuraWithMechanic((1 << MECHANIC_FEAR)) || HasAuraWithMechanic((1 << MECHANIC_HORROR)))
                SetControlled(true, UNIT_STATE_FLEEING);
            m_shouldBeFeared = false;
        }
    }
}
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getPowerType() == POWER_RAGE)
    {
        ...
        switch (cleanDamage->attackType)
        {
            case BASE_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetBaseAttackTime(cleanDamage->attackType) / 1000.0f * 3.5f);
                if (cleanDamage->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor *= 2;

                // Rage should only be generated when damage != 0 as attacker
                if (rage_damage != 0)
            ...
            case OFF_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetBaseAttackTime(cleanDamage->attackType) / 1000.0f * 1.75f);
                if (cleanDamage->hitOutCome == MELEE_HIT_CRIT)
                    weaponSpeedHitFactor *= 2;
    ...
    if (!damage)
    {
        ...
        // Damage from absorbed damage in BG Stats
        if (victim->GetTypeId() == TYPEID_PLAYER)
        {
            if (Player* killer = ToPlayer())
                if (Battleground* bg = killer->GetBattleground())
                    if (cleanDamage && cleanDamage->absorbed_damage)
                        bg->UpdatePlayerScore(killer, SCORE_DAMAGE_DONE, cleanDamage->absorbed_damage);
        }
    ...
    else
    {
        ...
        if (victim->GetTypeId() != TYPEID_PLAYER)
        {
            // Special handling mirror images
            if ((GetTypeId() == TYPEID_PLAYER && HasAura(55342)) || GetEntry() == ENTRY_MIRROR_IMAGE)
            {
                Player* pPlayer = ToPlayer();
                if (!pPlayer && GetOwner())
                    pPlayer = GetOwner()->ToPlayer();

                if (pPlayer)
                {
                    UnitList victims;
                    victims.push_back(this);

                    for (ControlList::const_iterator itr = pPlayer->m_Controlled.begin(); itr != pPlayer->m_Controlled.end(); ++itr)
                        if (Unit* pUnit = (*itr))
                            if (pUnit->GetEntry() == ENTRY_MIRROR_IMAGE)
                                victims.push_back(pUnit);

                    // Threat is shared between mirror images and player
                    victim->ShareThreat(victims, float(damage), damageSchoolMask, spellProto);
                }
            }
            else
...
{
    CastCustomSpell(target, spellId, bp0, bp1, bp2, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster);
}
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    damage += CalculateBaseWeaponDamage(damageInfo->attackType, false, true);
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->CastItemCombatSpell(victim, damageInfo->attackType, damageInfo->procVictim, damageInfo->procEx, NULL);
...
bool Unit::IsDamageReducedByArmor(SpellSchoolMask schoolMask, SpellInfo const* spellInfo, uint8 effIndex)
{
    ...
    if (spellInfo)
    {
        ...
        if (uint32 difficultyId = sSpellMgr->GetSpellDifficultyId(spellInfo->Id))
            if (SpellDifficultyEntry const* difficultyEntry = sSpellDifficultyStore.LookupEntry(difficultyId))
                if (difficultyEntry->SpellID[REGULAR_DIFFICULTY] > 0)
                    baseSpellId = uint32(difficultyEntry->SpellID[REGULAR_DIFFICULTY]);

        if (!baseSpellId)
            baseSpellId = spellInfo->Id;

        switch (baseSpellId)
        {
            case 66331: // Impale - Gormok
            case 71623: // Delirious Slash - Blood-Queen
            case 69409: // Soul Reaper - Lich King
                return (effIndex != 1);
            case 69075: // Bonestorm - Marrowgar
            case 72385: // Boiling Blood - Saurfang
            case 51422: // Wintergrasp Cannon
            case 49872: // SotA Cannon
                return false;
        }
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        float maxArmorPen = victim->getLevel();
        if (maxArmorPen < 60)
            maxArmorPen = float(400 + 85 * maxArmorPen);
        else
            maxArmorPen = float(400 + 85 * maxArmorPen + 4.5f * 85 * (maxArmorPen - 59));
    ...
    float newdamage = getLevel();
    if (newdamage < 60)
        newdamage = float(400 + 85 * newdamage);
    else
        newdamage = float(400 + 85 * newdamage + 4.5f * 85 * (newdamage - 59));

    newdamage = newdamage / (armor + newdamage);

    if (newdamage > 1.0f)
        newdamage = 1.0f;
    else if (newdamage < 0.25f)
        newdamage = 0.25f;

    newdamage = float(newdamage * damage);

    return (newdamage > 1.0f) ? uint32(newdamage) : 1;
}

float Unit::CalcResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, SpellInfo const* spellInfo)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0
        && (!spellInfo || (spellInfo->AttributesEx4 & SPELL_ATTR4_IGNORE_RESISTANCES) == 0)) // Ignore spells that cant be resisted)
    {
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    // Non Binary partial spell resist - pve currently disabled @todo
    if (spellInfo && !spellInfo->Binary ||
        (GetTypeId() == TYPEID_UNIT &&
         (!GetOwner() || GetOwner()->GetTypeId() != TYPEID_PLAYER)) ||
        (victim->GetTypeId() == TYPEID_UNIT &&
         (!victim->GetOwner() ||
          victim->GetOwner()->GetTypeId() != TYPEID_PLAYER)))
        dmgInfo.ResistDamage(uint32(CalcResist(victim, schoolMask, damagetype, damage, spellInfo)));

    // Ignore Absorption Auras
    float ignore_absorb_mod = 0;
    ...
    for (AuraEffectList::const_iterator itr = AbsIgnoreAurasA.begin(); itr != AbsIgnoreAurasA.end(); ++itr)
    {
        ...
        if ((*itr)->GetAmount() > ignore_absorb_mod)
            ignore_absorb_mod = float((*itr)->GetAmount());
    }
    ...
    for (AuraEffectList::const_iterator itr = AbsIgnoreAurasB.begin(); itr != AbsIgnoreAurasB.end(); ++itr)
    {
        ...
        if (((*itr)->GetAmount() > ignore_absorb_mod) && (*itr)->IsAffectedOnSpell(spellInfo))
            ignore_absorb_mod = float((*itr)->GetAmount());
    }
    RoundToInterval(ignore_absorb_mod, 0.0f, 100.0f);

    auto absorb_ignoring_damage = CalculatePctF(dmgInfo.GetDamage(), ignore_absorb_mod);
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        ProcDamageAndSpell(damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, damageInfo.procEx, damageInfo.damage, (damageInfo.damage + damageInfo.absorb + damageInfo.resist), damageInfo.attackType);
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType) const
{
    ...
    int32 sum = 0, tmp = 0;
    int32 roll = static_cast<int32>(urand(0, 10000));

    tmp = static_cast<int32>(MeleeSpellMissChance(victim, attType, 0) * 100.0f);

    if (tmp > 0 && roll < (sum += tmp))
    {
        sLog->outStaticDebug("RollMeleeOutcomeAgainst: MISS <%d)", tmp);
        return MELEE_HIT_MISS;
    }

    float critChance = static_cast<int32>(MeleeCritChance(victim, attType) * 100.0f);
    ...
    else
    {
        ...
        tmp = static_cast<int32>(MeleeSpellDodgeChance(victim, attType, 0) * 100.0f);
        if ((tmp > 0) && roll < (sum += tmp))
        {
    ...
    else
    {
        ...
        {
            tmp = static_cast<int32>(MeleeSpellParryChance(victim, attType, 0) * 100.0f);
            if ((tmp > 0) && roll < (sum += tmp))
            {
    ...
    {
        tmp = static_cast<int32>(MeleeGlancingChance(victim, attType) * 100.0f);
        if ((tmp > 0) && roll < (sum += tmp))
        {
            sLog->outStaticDebug("RollMeleeOutcomeAgainst: GLANCING <%d, %d)", sum - tmp, sum);
    ...
    else
    {
        ...
        {
            tmp = static_cast<float>(MeleeSpellBlockChance(victim, attType, 0) * 100.0f);
            if ((tmp > 0) && roll < (sum += tmp))
            {
    ...
    {
        tmp = static_cast<int32>(MeleeCrushingChance(victim, attType) * 100.0f);
...
uint32 Unit::CalculateBaseWeaponDamage(WeaponAttackType attType, bool normalized, bool addTotalPct)
{
...
bool Unit::isSpellBlocked(Unit* victim, SpellInfo const* spellProto, WeaponAttackType attackType)
{
    ...
    if (victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION) || victim->HasInArc(M_PI, this))
    {
        ...
        float blockChance = victim->MeleeSpellBlockChance(victim, attackType, spellProto->Id);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    bool victimIsPlayerOrPetOfAPlayer = (victim->GetTypeId() == TYPEID_PLAYER);

    if (!victimIsPlayerOrPetOfAPlayer && victim->isPet())
        if (Unit *owner = victim->GetOwner())
            victimIsPlayerOrPetOfAPlayer = (owner->GetTypeId() == TYPEID_PLAYER);
    ...
    {
        // Roll dodge
        int32 dodgeChance = static_cast<int32>(MeleeSpellDodgeChance(victim, attType, spell->Id) * 100.0f);
    ...
    {
        // Roll parry
        int32 parryChance = int32(MeleeSpellParryChance(victim, attType, spell->Id) * 100.0f);

        if (roll < (tmp += parryChance))
            return SPELL_MISS_PARRY;
    }

    if (canBlock)
    {
        int32 blockChance = int32(MeleeSpellBlockChance(victim, attType, spell->Id) * 100.0f);

        if (roll < (tmp += blockChance))
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    // Calc binary resistance - pve currently disabled @todo
    if (spell->Binary &&
        (GetTypeId() != TYPEID_UNIT ||
         (GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER)) &&
        (victim->GetTypeId() != TYPEID_UNIT &&
         (victim->GetOwner() &&
          victim->GetOwner()->GetTypeId() == TYPEID_PLAYER))) {
        float maxChance = 75.0f * 100.0f; // Maximum mitigation is 75%
        tmp += CalcResist(victim, schoolMask, SPELL_DIRECT_DAMAGE, uint32(maxChance), spell);
    }
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, casterGUID, (newAura->AttributesCu & SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID : 0, 0))
        {
            ...
            foundAura->InitSnapShot(caster ? caster : sObjectAccessor->FindPlayer(casterGUID), this);
            return foundAura;
        }
    }
...
void Unit::RemoveAurasDueToItemSpell(uint64 castItemGUID, uint32 spellId)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.lower_bound(spellId); iter != m_appliedAuras.upper_bound(spellId);)
    {
        if (!castItemGUID || iter->second->GetBase()->GetCastItemGUID() == castItemGUID)
        {
            RemoveAura(iter);
...
void Unit::RemoveAurasWithInterruptFlags(uint32 flag, uint32 except)
{
    ...
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        if ((spell->getState() == SPELL_STATE_CASTING
            // AuraInterruptFlags != ChannelInterruptFlags for CHANNEL_INTERRUPT_FLAG_INTERRUPT = 0x08
            && (spell->m_spellInfo->ChannelInterruptFlags & (flag & ~CHANNEL_INTERRUPT_FLAG_INTERRUPT))
            && spell->m_spellInfo->Id != except)
            // In case AURA_INTERRUPT_FLAG_MOVE was given, we check for the AuraInterruptFlags and not the ChannelInterruptFlags
            || (flag & AURA_INTERRUPT_FLAG_MOVE
            && spell->m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_MOVE))
...
int32 Unit::GetBaseAuraModifier(AuraType auratype, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        const SpellInfo* pSpellInfo = (*i)->GetSpellInfo();
        if (!pSpellInfo || (pSpellInfo->Effects[(*i)->GetEffIndex()].SpellClassMask && pSpellInfo->SpellIconID != 3220)) // Exclude "Demonic Pact" (Has SpellClassMask)
            continue;

        if (!includeAreaAura)
        {
            switch (pSpellInfo->Effects[(*i)->GetEffIndex()].Effect)
            {
        ...
        const SpellDurationEntry* pSpellDurationEntry = pSpellInfo->DurationEntry;

        // Check if the Aura Effect has a the Same Effect Stack Rule and if so, use the highest amount of that SpellGroup
        // If the Aura Effect does not have this Stack Rule, it returns false so we can add to the modifier as usual
        if (!pSpellDurationEntry || pSpellDurationEntry->ID == 21) // DurationEntry->ID 21 = permanent
            if (!sSpellMgr->AddSameEffectStackRuleSpellGroups(pSpellInfo, (*i)->GetAmount(), SameEffectSpellGroup))
                modifier += (*i)->GetAmount();
    }
...
float Unit::GetBaseAuraMultiplier(AuraType auratype, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    float multiplier = 1.0f;
...
int32 Unit::GetBaseAuraModifierByMiscMask(AuraType auratype, uint32 misc_mask, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
...
float Unit::GetBaseAuraMultiplierByMiscMask(AuraType auratype, uint32 misc_mask, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    float multiplier = 1.0f;
...
int32 Unit::GetBaseAuraModifierByMiscValue(AuraType auratype, int32 misc_value, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    int32 modifier = 0;
...
float Unit::GetBaseAuraMultiplierByMiscValue(AuraType auratype, int32 misc_value, bool includeAreaAura) const
{
    std::map<SpellGroup, int32> SameEffectSpellGroup;
    float multiplier = 1.0f;
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && ToCreature()->IsAIEnabled)
        ToCreature()->AI()->JustSummoned(gameObj);
}
...
void Unit::RemoveGameObject(GameObject* gameObj, bool del)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && ToCreature()->IsAIEnabled)
        ToCreature()->AI()->SummonedGameObjectRemoved(gameObj);

    if (del)
    {
        if (GetTypeId() == TYPEID_UNIT && ToCreature()->IsAIEnabled)
            ToCreature()->AI()->SummonedGameObjectDeleted(gameObj);
...
void Unit::ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 amount, uint32 effectiveAmount, WeaponAttackType attType, SpellInfo const* procSpell, SpellInfo const* procAura)
{
     // Not much to do if no flags are set.
    if (procAttacker)
        ProcDamageAndSpellFor(false, victim, procAttacker, procExtra, attType, procSpell, amount, effectiveAmount, procAura);
    // Now go on with a victim's events'n'auras
    // Not much to do if no flags are set or there is no victim
    if (victim && victim->isAlive() && procVictim)
        victim->ProcDamageAndSpellFor(true, this, procVictim, procExtra, attType, procSpell, amount, effectiveAmount, procAura);
}
...
//victim may be NULL
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 gainedHealOrCleanDamage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    basepoints0 = CalculatePctN(int32(damage), triggerAmount);
                    CastCustomSpell(70872, SPELLVALUE_BASE_POINT0, basepoints0, this, TRIGGERED_FULL_MASK);
                    return true;
                }
                case 65032: // Boom aura (321 Boombot)
                {
                    if (victim && victim->GetEntry() != 33343)   // Scrapbot
                        return false;
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (dummySpell->SpellFamilyFlags[1] & 0x8)
            {
                ...
                if (victim && (!HasInArc(M_PI, victim) || HasUnitState(UNIT_STATE_STUNNED)))
                    return false;
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (dummySpell->SpellIconID == 2820)
            {
                ...
                if (AuraEffect* aurEff = target->GetAuraEffectOfRankedSpell(47586, EFFECT_0))
                    basepoints0 += ( basepoints0 / 100 ) * aurEff->GetAmount();
            ...
            switch (dummySpell->Id)
            {
                ...
                case 54815:
                {
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00800000, 0x0, 0x0, GetGUID()))
                        ...
                        {
                            int32 modDuration = aura->GetModDuration();
                            if (modDuration < 6000)
                            {
                                aura->SetDuration(aura->GetDuration() + 2000);
                                aura->SetMaxDuration(aura->GetMaxDuration() + 2000);
                                aura->SetModDuration(modDuration + 2000);
                                return true;
                            }
                        }
                    return false;
                ...
                case 24932:
                {
                   if (triggerAmount <= 0)
                    ...
                    if (triggeredByAura->GetCasterGUID() != GetGUID())
                        break;
                    int32 basepoints1 = CalculatePctN(GetMaxPower(Powers(POWER_MANA)), triggerAmount * 2);
                ...
                case 70664:
                {
                    ...
                    if (victim)
                        CastCustomSpell(70691, SPELLVALUE_BASE_POINT0, damage, victim, TRIGGERED_FULL_MASK);
                    return true;
                }
            }
            // Eclipse
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31801:
                {
                    // Only proc if damage was dealed
                    if (gainedHealOrCleanDamage <= 0)
                        return false;
                }break;
            }
            ...
            if (dummySpell->SpellFamilyFlags[0] & 0x8000000)
            {
                ...
                float holy = SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY);
                if (victim)
                    holy += victim->SpellBaseDamageBonusTaken(SPELL_SCHOOL_MASK_HOLY);

                basepoints0 = int32(GetBaseAttackTime(BASE_ATTACK) * (ap * 0.022f + 0.044f * holy) / 1000.0f);
                break;
            }
            // Light's Beacon - Beacon of Light
            if (dummySpell->Id == 53651)
            {
                // victim is the unit that cast the heal spell, we need it here
                if (!victim)
                    return false;

                triggered_spell_id = 0;
                if (auto beacon_target = triggeredByAura->GetCaster())
                {
                    // healing the beacon should have no effect
                    if (beacon_target == this)
                        return false;

                    // we need to get the aura effect of the actual beacon that was applied by the caster
                    if (auto beacon_effect = beacon_target->GetAuraEffect(53563, EFFECT_0, victim->GetGUID()))
                    {
                        basepoints0 = CalculatePctN(damage, beacon_effect->GetAmount());
                        triggered_spell_id = procSpell->IsRankOf(sSpellMgr->GetSpellInfo(635)) ? 53652 : 53654;
                        victim->CastCustomSpell(beacon_target, triggered_spell_id, &basepoints0, NULL, NULL, true);
            ...
            {
                if (procFlag & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS || damage == 0)
                    return false;
            ...
            if (dummySpell->SpellIconID == 3025)
            {
                ...
                // Add remaining ticks to damage done
                basepoints0 += victim ? victim->GetRemainingPeriodicAmount(GetGUID(), triggered_spell_id, SPELL_AURA_PERIODIC_DAMAGE) : 0;
                break;
            }
            // Sheath of Light
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31801:
                {
                    ...
                    // Melee attack or paladin spells beneath
                    bool stacker = !procSpell || procSpell->Id == 53595 || procSpell->Id == 71433;

                    // Spells with SPELL_DAMAGE_CLASS_MELEE
                    bool damager = procSpell && procSpell->DmgClass == SPELL_DAMAGE_CLASS_MELEE;
                    ...
                    // On target with spell or melee strike at 5 stacks Holy Vengeance direct damage is done
                    if (Aura* pAura = victim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        if (damager || pAura->GetStackAmount() == 5)
                        {
                            // Weapon damage should be scaled with stack amount
                            int32 bp = float((pAura->GetStackAmount() / 5.0f) * 33.0f);
                            CastCustomSpell(victim, 42463, &bp, NULL, NULL, true);
                        }
                    }
                ...
                case 53736:
                {
                    ...
                    // On target with spell or melee strike at 5 stacks Blood Corruption direct damage is done
                    if (Aura* pAura = victim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        if (damager || pAura->GetStackAmount() == 5)
                        {
                            // Weapon damage should be scaled with stack amount
                            int32 bp = float((pAura->GetStackAmount() / 5.0f) * 33.0f);
                            CastCustomSpell(victim, 53739, &bp, NULL, NULL, true);
                        }
                    }
                ...
                case 33776:
                {
                    ...
                    // heal amount
                    basepoints0 = int32(CalculatePctN(gainedHealOrCleanDamage, triggerAmount));
                    target = this;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49028)
            {
                ...
                {
                    if (procSpell->SpellFamilyFlags[0] & 0x1) // "Plague Strike" -> "Blood Plague" disease should be casted on victim
                        pPet->CastSpell(pPet->getVictim(), procSpell->Effects[EFFECT_2].TriggerSpell, true);
                    else if (procSpell->SpellFamilyFlags[0] & 0x2) // "Icy Touch" -> "Frost Fever" disease should be casted on victim
                        pPet->CastSpell(pPet->getVictim(), procSpell->Effects[EFFECT_1].TriggerSpell, true);
                ...
                if (pPet && pPet->getVictim() && damage && !procSpell)
                {
                        ...
                        pPet->ProcDamageAndSpell(damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, damageInfo.procEx, damageInfo.cleanDamage, damageInfo.attackType);
            ...
            // Unholy Blight
            if (victim && dummySpell->Id == 49194)
            {
                triggered_spell_id = 50536;
                const SpellInfo* unholyBlight = sSpellMgr->GetSpellInfo(triggered_spell_id);
                if (!unholyBlight)
                    return false;
            ...
            {
                if (!roll_chance_f(GetFloatValue(PLAYER_CRIT_PERCENTAGE)))
                    return false;
                basepoints0 = CalculatePctN(int32(damage), triggerAmount);
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellFamilyFlags[1] & 0x40000)
            {
                ...
                basepoints0 = CalculatePctF(maxmana, GetBaseAttackTime(RANGED_ATTACK) / 1000.0f);
                target = this;
...
bool Unit::HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PRIEST:
        {
            if ((dummySpell->SpellFamilyFlags[0] & 0x2      // Inner Fire
                || dummySpell->SpellFamilyFlags[1] & 0x20)  // Prayer of Mending
                && dummySpell->SpellFamilyFlags[2] & 0x400)
            {
                if (damage <= 0 || GetHealth() <= damage) // should not proc if no damage dealed / damage bigger health
                    *handled = true;
            }
            break;
        }
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                // Moonkin Passive Passive - Mana restore
                case 24905:
                {
                    if (procSpell->IsAffectingArea() // AOE Effects
                        || procSpell->SpellFamilyFlags[1] & 0x800000) // Starfall special handling
                        *handled = true;
                    break;
                }
            }
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
            ...
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 5202:
                    {
                        ...
                        {
                            if (procSpell->Effects[i].IsAura(SPELL_AURA_PERIODIC_DAMAGE))
                            {
                                if (AuraEffect* aura = victim->GetAuraEffect(originalSpellId, i, GetGUID()))
                                {
                                    bp = aura->GetAmount();
                                    if (victim && victim->isAlive())
                                        CastCustomSpell(victim, 48856, &bp, NULL, NULL, true);
                                }
                            }
                        }

                        if (victim && victim->isAlive())
                            CastSpell(victim, originalSpellId, true);
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 67758: // Item - Coliseum 25 Heroic Caster Trinket
                    {
                        ...
                        if (Player* caster = this->ToPlayer())
                            if (caster->HasSpellCooldown(stack_spell_id))
                                return false;

                        CastSpell(this, stack_spell_id, true, NULL, triggeredByAura);

                        if (Player* caster = this->ToPlayer())
                            caster->AddSpellCooldown(stack_spell_id,0,time(NULL) + cooldown);
                        ...
                        if (Player *plr = ToPlayer())
                            if (plr->GetRuneCooldown(i) == 0)
                            {
                                if (plr->GetCurrentRune(i) == RUNE_DEATH)
                                    usedR++;
                                else if (runeCost[plr->GetCurrentRune(i)])
                                {
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 70107:
        {
            if (!procEx & auraSpellInfo->ProcFlags || trigger_spell_id != 70106 || !roll_chance_i(20))
                return false;
            break;
        }
        case 51528: // Maelstrom Weapon (Dont proc if max stack amount reached)
        case 51529:
        case 51530:
        case 51531:
        case 51532:
        {
            if (Aura* pAura = GetAura(trigger_spell_id))
    ...
    switch (trigger_spell_id)
    {
        ...
        case 48090:
        {
            ...
            if (isPet())
            {
                if (Unit* owner = GetOwner())
                {
                    ...
                    {
                        basepoints0 = int32((aurEff->GetAmount() * owner->SpellBaseDamageBonusDone(SpellSchoolMask(SPELL_SCHOOL_MASK_MAGIC))) / 100.0f);
        ...
        case 50452:
        {
            // triggeramount is calculated once at apply - need manual random calculate
            uint8 count = urand(2, 4);
    ...
    bool useOrigCaster = false;

    switch (trigger_spell_id)
    {
    case 66636: // Rising Anger
    case 67721: // Expose Weakness 10 Man
    case 67847: // Expose Weakness 25 Man
    case 65775: // Acid-Drenched Mandibles 10 Man
    case 67861: // Acid-Drenched Mandibles 25 Man
    case 67862: // Acid-Drenched Mandibles 10 Man Heroic
    case 67863: // Acid-Drenched Mandibles 25 Man Heroic
    case 66320: // Firebomb 10 Man
    case 67472: // Firebomb 25 Man
    case 67473: // Firebomb 10 Man Heroic
    case 67475: // Firebomb 25 Man Heroic
        useOrigCaster = true;
    }

    uint64 origCaster = 0;
    if (useOrigCaster && target)
        if (Aura* aura = target->GetAura(sSpellMgr->GetSpellIdForDifficulty(trigger_spell_id, this)))
            origCaster = aura->GetCasterGUID();

    if (basepoints0)
        CastCustomSpell(target, trigger_spell_id, &basepoints0, NULL, NULL, true, castItem, triggeredByAura, origCaster);
    else
        CastSpell(target, trigger_spell_id, true, castItem, triggeredByAura, origCaster);
...
ReputationRank Unit::GetReactionTo(Unit const* target) const
{
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
    {
        if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
        {
            if (selfPlayerOwner && targetPlayerOwner)
            {
                ...
                // BG Crossfaction handling
                if (selfPlayerOwner->HandleWithCrossFactionBG(targetPlayerOwner))
                {
    ...
    // BG Crossfaction handling
    if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS) && getFaction() != 35) // Exclude always friendly units
    {
        Player* pPlayer = GetAffectingPlayer();
        Player* pPlayerTarget = target->GetAffectingPlayer();

        if ((pPlayer && pPlayer->HandleWithCrossFactionBG()) || (pPlayerTarget && pPlayerTarget->HandleWithCrossFactionBG()))
        {
            uint32 faction = Battleground::GetFactionForCrossfactionBG(pPlayer ? pPlayer : pPlayerTarget, pPlayer ? target->getFactionTemplateEntry() : getFactionTemplateEntry());

            if (faction != 1 && faction != 2)
                return GetFactionReactionTo(getFactionTemplateEntry(), target);

            ReputationRank repRank = REP_FRIENDLY;

            if (!pPlayer)
                pPlayer = pPlayerTarget;

            switch (pPlayer->GetTeam(true))
            {
                case ALLIANCE:
                    if (faction == 2)
                        repRank = REP_HOSTILE;
...
Unit* Unit::GetHealthPctLowestRaidMemberOrPet(float radius, uint32 excludeAura)
{
    Player* player = NULL;
    if (GetTypeId() == TYPEID_PLAYER)
        player = ToPlayer();
    ...
    for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        if (Player* Target = itr->getSource())
        {
            // IsHostileTo check duel and controlled by enemy
            if (Target != this && Target->isAlive() && IsWithinDistInMap(Target, radius) && !IsHostileTo(Target))
                if (!excludeAura || !Target->HasAura(excludeAura))
                    nearMembers.push_back(Target);

            // Push player's pet to vector
            if (Unit* pet = Target->GetGuardianPet())
                if (pet != this && pet->isAlive() && IsWithinDistInMap(pet, radius) && !IsHostileTo(pet))
                    if (!excludeAura || !pet->HasAura(excludeAura))
                        nearMembers.push_back(pet);
        }
    }
...
void Unit::EnergizeBySpell(Unit* victim, uint32 spellID, uint32 damage, Powers powerType)
{
    ...
    // Exclude aggro generation with hunter pet happiness
    if (powerType != POWER_HAPPINESS)
        victim->getHostileRefManager().threatAssist(this, float(damage) * 0.5f, spellInfo);
}

std::pair<int32, float> Unit::CalculateSpellDamageBonusPair(Unit *victim, SpellInfo const *spellProto)
{
        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                if (spellProto->SpellFamilyFlags[1] & 0x6000000 && spellProto->SpellFamilyFlags[1] & 0x800)
                    AddPctN(DoneTotalMod, 15);

                break;
        }
    }

    return std::pair<int32, float> (DoneTotal, DoneTotalMod);
}

uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, Aura *aura)
{
    ...
    std::pair<int32, float> dmgPair = CalculateSpellDamageBonusPair(victim, spellProto);
    if (aura)
        if (float tempMult = aura->GetSnapShotValue(SSVAL_PCT, victim))
            dmgPair.second = tempMult;

    dmgPair.first += CalculateCoefficientBonus(victim, spellProto, damagetype, stack, aura);
    ...
    float tmpDamage = (int32(pdamage) + dmgPair.first) * dmgPair.second;
...
{
    if (!spellProto || damagetype == DIRECT_DAMAGE || (spellProto ? spellProto->AttributesEx4 & SPELL_ATTR4_FIXED_DAMAGE : false))
        return pdamage;
    ...
    if (uint32 mechanicMask = spellProto->GetAllEffectsMechanicMask())
    {
        ...
        for (std::map<SpellGroup, int32>::const_iterator itr = SameEffectSpellGroup.begin(); itr != SameEffectSpellGroup.end(); ++itr)
            AddPctN(TakenTotalMod, itr->second);
    }
...
int32 Unit::CalculateCoefficientBonus(Unit *victim, SpellInfo const *spellProto, DamageEffectType damagetype, uint32 stack, Aura *aura)
{
    ...
    int32 DoneAPBonus = 0;
    int32 DoneAdvertisedBenefit = aura ? aura->GetSnapShotValue(SSVAL_SP, victim) : 0;
    if (DoneAdvertisedBenefit <= 0)
        DoneAdvertisedBenefit = SpellBaseDamageBonusDone(spellProto->GetSchoolMask());

    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->GetStatsModifier()->ApplyModifier(DoneAdvertisedBenefit);
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            ...
            if (bonus->ap_dot_bonus > 0)
            {
                ...
                float APbonus = aura ? aura->GetSnapShotValue(SnapShotValue(SSVAL_AP_MAIN + attType), victim) : 0;
                if (APbonus <= 0)
                    APbonus = GetTotalAttackPowerValue(attType);
                APbonus += float(victim->GetTotalAuraModifier(attType == BASE_ATTACK ? SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS : SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS));
                DoneAPBonus = int32(bonus->ap_dot_bonus * stack * ApCoeffMod * APbonus);
            }
        }
        else
        {
            ...
            if (bonus->ap_bonus > 0)
            {
                ...
                APbonus += GetTotalAttackPowerValue(attType);
                DoneAPBonus = int32(bonus->ap_bonus * stack * ApCoeffMod * APbonus);
            }
        }
    }
    ...
    // Pyroblast Dot, Fireball Dot and FrostFireBolt Dot must not profit from Empowered Fire
    if (!(spellProto->SpellFamilyName == SPELLFAMILY_MAGE && damagetype == DOT &&
        (spellProto->SpellFamilyFlags[0] & 0x400001 || spellProto->SpellFamilyFlags[1] & 0x1000)))
    {
        if (Player *modOwner = GetSpellModOwner()) {
            coeff *= 100.0f;
    ...
    return DoneAPBonus + int32(DoneAdvertisedBenefit * coeff * factorMod);
}
...
bool Unit::CanCritWithSpell(SpellInfo const* spellProto) const
{
...
bool Unit::IsSureCritAgainst(Unit* victim, SpellInfo const* spellProto) const
{
    if (!victim)
        return false;
            ...
            switch (spellProto->SpellFamilyName)
            {
                case SPELLFAMILY_PALADIN:
                {
                    ...
                    if (spellProto->Category == 19)
                        return (victim->GetCreatureTypeMask() & CREATURE_TYPEMASK_DEMON_OR_UNDEAD);
                    break;
                }
                case SPELLFAMILY_SHAMAN:
                {
...
float Unit::CalculateCriticalChanceDone(WeaponAttackType attackType) const
{
...
float Unit::ApplyCriticalChanceTaken(float crit_chance, const Unit *attacker, WeaponAttackType attackType) const
{
    float full_crit_chance = crit_chance;

    if (attackType == RANGED_ATTACK)
        full_crit_chance += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_CHANCE);
    else
        full_crit_chance += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE);

    full_crit_chance += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE);

    if (attackType != RANGED_ATTACK && HasAura(63057) && HasAura(22812))
        full_crit_chance -= 25.0f;

    full_crit_chance += 0.04f * (float(attacker->GetWeaponSkillValue(attackType)) - float(GetDefenseSkillValue()));

    if (GetTypeId() == TYPEID_UNIT)
        full_crit_chance -= 1.6f * std::max(0.0f, float(getLevel()) - attacker->getLevel());

    attacker->ApplyResilience(this, &full_crit_chance, NULL, false, (attackType != RANGED_ATTACK) ? CR_CRIT_TAKEN_MELEE : CR_CRIT_TAKEN_RANGED);

    return std::max(0.0f, full_crit_chance);
}

float Unit::CalculateCriticalChanceSpellDone(SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    float crit_chance = 0.0f;
    switch (spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            crit_chance += CalculateCriticalChanceDone(attackType);
            crit_chance += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_SPELL_CRIT_CHANCE_SCHOOL, schoolMask);
...
float Unit::ApplyCriticalChanceSpellTaken(float crit_chance, Unit const *caster, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    float full_crit_chance = crit_chance;
    ...
    switch (spellProto->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            if (!spellProto->IsPositive())
            {
                ...
                full_crit_chance += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
            ...
            if (GetTypeId() == TYPEID_UNIT)
                full_crit_chance -= 0.6 * std::max(0.0f, float(getLevel()) - caster->getLevel());
                ...
                case SPELLFAMILY_PALADIN:
                    ...
                    if (spellProto->SpellFamilyFlags[0] & 0x40000000)
                    {
                        ...
                        if (AuraEffect const* aura = GetAuraEffect(58597, 1, GetGUID()))
                            full_crit_chance += aura->GetAmount();
                    }
                    break;
            }
        }
        ...
        {
            if (!spellProto->IsPositive())
                caster->ApplyResilience(this, &full_crit_chance, NULL, false, CR_CRIT_TAKEN_SPELL);
            break;
        }
        case SPELL_DAMAGE_CLASS_MELEE:
        {
                ...
                case SPELLFAMILY_DRUID:
                {
                    ...
                    if (spellProto->SpellFamilyFlags[0] & 0x00800000 && spellProto->SpellIconID == 1680 && HasAuraState(AURA_STATE_BLEEDING))
                        if (AuraEffect const* rendAndTear = caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 2859, 1))
                            full_crit_chance += rendAndTear->GetAmount();
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            full_crit_chance = ApplyCriticalChanceTaken(full_crit_chance, caster, attackType);
        }
    }
...
bool Unit::isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, Aura *aura) const
{
    if (!CanCritWithSpell(spellProto))
        return false;

    if (IsSureCritAgainst(victim, spellProto))
        return true;

    float crit_chance = aura ? aura->GetSnapShotValue(SSVAL_CRIT, victim) :
        CalculateCriticalChanceSpellDone(spellProto, schoolMask, attackType);
    crit_chance = victim->ApplyCriticalChanceSpellTaken(crit_chance, this, spellProto, schoolMask, attackType);
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    {
        flag96 spellClassMask = (*i)->GetSpellInfo()->Effects[(*i)->GetEffIndex()].SpellClassMask;

        if (!spellClassMask || spellClassMask & spellProto->SpellFamilyFlags)
            AddPctN(DoneTotalMod, (*i)->GetAmount());
    }
...
{
    if (!spellInfo || !spellInfo->Effects[index].IsEffect() || spellInfo->Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY)
        return false;
    ...
    if (spellProto)
        if (!(spellProto->AttributesEx6 & SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS) && !spellProto->IsRankOf(sSpellMgr->GetSpellInfo(12162)))
        {
            ...
            for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
            {
                ...
                if ((*i)->GetMiscValue() & spellProto->GetSchoolMask() && !(((*i)->GetMiscValue() | spellProto->GetSchoolMask()) & SPELL_SCHOOL_MASK_NORMAL))
                {
                    if ((*i)->GetSpellInfo()->EquippedItemClass == -1)
                        apply = true;
                    else if (!((*i)->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK) && ((*i)->GetSpellInfo()->EquippedItemSubClassMask == 0))
...
{
    if (pdamage == 0 || (spellProto ? spellProto->AttributesEx4 & SPELL_ATTR4_FIXED_DAMAGE : false))
        return pdamage;
...
void Unit::SetInCombatFor(uint32 time)
{
...
{
    m_TeleDelay = sWorld->getIntConfig(CONFIG_TELE_DELAY);
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    if (slow)
    {
        ...
        if (float minSpeedMod = (float)GetMaxPositiveAuraModifier(SPELL_AURA_MOD_MINIMUM_SPEED))
        {
            ...
            // Min speed of units (creatures) should be calculated with base speed of creature_template
            if (GetTypeId() == TYPEID_UNIT)
            {
                switch (mtype)
                {
                    ...
                    case MOVE_SWIM:
                    case MOVE_FLIGHT:
                        min_speed *= ToCreature()->GetCreatureTemplate()->speed_run;
                        break;
                }
            }
...
void Unit::ShareThreat(UnitList victims, float fThreat, SpellSchoolMask schoolMask, SpellInfo const* threatSpell)
{
    // Only mobs can manage threat lists
    if (CanHaveThreatList())
    {
        if (!victims.empty())
        {
            float threat = float(fThreat / victims.size());
            for (UnitList::const_iterator itr = victims.begin(); itr != victims.end(); ++itr)
                if (Unit* pUnit = (*itr))
                    m_ThreatManager.addThreat(pUnit, threat, schoolMask, threatSpell);
        }
    }
}
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->Attributes & (SPELL_ATTR0_ABILITY|SPELL_ATTR0_TRADESPELL)) && ((GetTypeId() == TYPEID_PLAYER && spellProto->SpellFamilyName) || GetTypeId() == TYPEID_UNIT))
        ...
        castTime = int32(float(castTime) * _totalCombatSpeedMod[CTYPE_RANGED]);
    else if (spellProto->SpellVisual[0] == 3881 && HasAura(67556)) // cooking with Chef Hat.
        castTime = 500;
}

bool Unit::IsAffectedByDiminishing(DiminishingGroup group)
{
    if (GetTypeId() == TYPEID_PLAYER) // Player should always be affected
        return true;

    if (GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER // Summon, Pet... of player should always be affected
        && (isSummon() || isGuardian() || isPet() || isHunterPet()))
        return true;

    if (GetTypeId() == TYPEID_UNIT) // Creature with extra flag should also be affected
    {
        if (group == DIMINISHING_TAUNT)
            return ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH;
        else
            return ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH;
    }

    return false;
}

DiminishingLevels Unit::GetDiminishingLevel(DiminishingGroup group)
{
    // Only limit duration
    if (group == DIMINISHING_LIMITONLY)
        return DIMINISHING_LEVEL_1;

    // Checking for existing in the table
    if (m_Diminishing.find(group) != m_Diminishing.end())
    {
        uint32 lastDimReset = sWorld->GetLastDiminishingReturnReset();

        if (m_Diminishing[group].Applications == 0 && m_Diminishing[group].RemoveTime < lastDimReset
            && getMSTimeDiff(m_Diminishing[group].RemoveTime, lastDimReset) >= 15 * IN_MILLISECONDS)
            m_Diminishing[group].Level = DIMINISHING_LEVEL_1;

        return DiminishingLevels(m_Diminishing[group].Level);
    }
    else
    {
        //ASSERT(false); // Entry should exist => called after spellhit and increase
        sLog->outError("Problem with Diminishing group: %u", group);
        return DIMINISHING_LEVEL_1;
    }
}

float Unit::ApplyDiminishingToDuration(const SpellInfo* spellproto, int32 &duration, Unit* caster, bool triggeredByAura)
{
    DiminishingGroup group = spellproto->DiminishingReturn() ? spellproto->DiminishingReturn()->GetGroup(triggeredByAura) : DIMINISHING_NONE;
    DiminishingReturnsType type = spellproto->DiminishingReturn()->GetReturnsType();

    if (duration == -1 || group == DIMINISHING_NONE || (!IsAffectedByDiminishing(group) && type != DRTYPE_ALL))
        return 1.0f;

    // Duration of crowd control abilities is limited by 10 sec (PvP)
    int32 limitDuration = spellproto->DiminishingReturn()->GetLimitDuration();
    ...
    if (group == DIMINISHING_TAUNT)
    {
        if (GetTypeId() == TYPEID_UNIT)
        {
            switch (GetDiminishingLevel(group))
            {
...
void Unit::ApplyDiminishingAura(DiminishingGroup group, bool apply)
{
    ...
    if (m_Diminishing.find(group) != m_Diminishing.end())
    {
        if (apply)
        {
            m_Diminishing[group].Applications++;
            m_Diminishing[group].RemoveTime = getMSTime();
        }
        else if (m_Diminishing[group].Applications)
        {
            m_Diminishing[group].Applications--;

            // Remember time after last aura from group removed
            if (m_Diminishing[group].Applications == 0)
                m_Diminishing[group].RemoveTime = getMSTime();
        }
    }
}

void Unit::IncreaseDiminishingLevel(const SpellInfo* spellproto, bool triggeredByAura)
{
    DiminishingGroup group = spellproto->DiminishingReturn() ? spellproto->DiminishingReturn()->GetGroup(triggeredByAura) : DIMINISHING_NONE;
    if (group == DIMINISHING_NONE)
        return;

    // Checking for existing in the table - if not -> create
    if (m_Diminishing.find(group) == m_Diminishing.end())
        m_Diminishing[group] = DiminishingReturn();
    else
    {
        if (m_Diminishing[group].Level < spellproto->DiminishingReturn()->GetMaxLevel())
            m_Diminishing[group].Level++;
    }
}
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 gainedHealOrCleanDamage, SpellInfo const* procAura)
{
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "ProcDamageAndSpell:HandleAuraProc: casting spell %u (triggered with value by %s aura of spell %u)", spellInfo->Id, (isVictim?"a victim's":"an attacker's"), Id);
            takeCharges = true;
        }
            ...
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_DUMMY:
                    {
                        ...
                        if (HandleDummyAuraProc(target, damage, gainedHealOrCleanDamage, triggeredByAura, procSpell, procFlag, procExtra, cooldown))
                            takeCharges = true;
                        break;
                    }
                    case SPELL_AURA_OBS_MOD_POWER:
                    ...
                    {
                        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "ProcDamageAndSpell:HandleProcTriggerSpell: casting spell %u (triggered with value by %s aura of spell %u)", spellInfo->Id, (isVictim?"a victim's":"an attacker's"), triggeredByAura->GetId());
...
void Unit::UpdateHaste(CombatType ctype, float value)
{
    _hasteCombatSpeedMod[ctype] = 1.0f;
    ApplyPercentModFloatVar(_hasteCombatSpeedMod[ctype], value, false);
    UpdateCombatSpeed(ctype);
}

void Unit::ApplyCombatSpeedMod(CombatType ctype, float value, bool apply)
{
    if (value > 0)
        ApplyPercentModFloatVar(_auraCombatSpeedMod[ctype], value, !apply);
    else
        ApplyPercentModFloatVar(_auraCombatSpeedMod[ctype], -value, apply);
...
void Unit::UpdateCombatSpeed(CombatType ctype)
{
    float _oldTotalCombatSpeedMod = _totalCombatSpeedMod[ctype];
    _totalCombatSpeedMod[ctype] = _hasteCombatSpeedMod[ctype] * _auraCombatSpeedMod[ctype];
    ...
    switch (ctype)
    {
        ...
        case CTYPE_RANGED:
        {
            SetAttackTimer(WeaponAttackType(ctype), float(GetAttackTimer(WeaponAttackType(ctype)) * _totalCombatSpeedMod[ctype] / _oldTotalCombatSpeedMod));

            SetFloatValue(UNIT_FIELD_BASEATTACKTIME + ctype, _baseAttackTime[ctype] * _totalCombatSpeedMod[ctype]);
            break;
        }
        case CTYPE_CAST_TIME:
            SetFloatValue(UNIT_MOD_CAST_SPEED, _totalCombatSpeedMod[ctype]);
            break;
    }
}
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    // Has to be done before spell init
    SetMinion(pet, true);
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = triggeredByAura->GetCaster())
        {
            ...
            if (Unit* target = GetHealthPctLowestRaidMemberOrPet(radius))
            {
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (GetTypeId() == TYPEID_UNIT)
            ...
            victim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_PLAYER, 1, ToPlayer()->GetTeam(true));
    }
...
void Unit::SetControlled(bool apply, UnitState state)
{
    ...
    else
    {
        ...
        if (HasUnitState(UNIT_STATE_JUMPING))
...
void Unit::SetFeared(bool apply)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        if (!ToPlayer()->HasUnitState(UNIT_STATE_POSSESSED))
...
void Unit::RemoveCharmedBy(Unit* charmer, bool inRemove)
{
    if (!isCharmed())
        return;
...
//        ASSERT(false);
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if (type != CHARM_TYPE_VEHICLE) // Vehicles' AI is never modified
        {
            ...
            if (creature->AI() && charmer && charmer->isAlive() && !inRemove)
...
float Unit::MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const
{
    ...
    if (attType == RANGED_ATTACK && (getClass() == CLASS_PALADIN || getClass() == CLASS_WARRIOR))
        attType = BASE_ATTACK;
    ...
    int32 skillDifference = victim->GetMaxSkillValueForLevel(this) - GetWeaponSkillValue(attType, victim);

    if (!victimIsPlayerOrPetOfAPlayer && skillDifference >= 0)
        missChance += skillDifference > 10 ? 1.0f + static_cast<float>(skillDifference - 10) * 0.4f : static_cast<float>(skillDifference) * 0.1f;
    else
        missChance += static_cast<float>(skillDifference) * 0.04f;
    ...
    return std::min(60.0f, std::max(0.0f, missChance));
}

float Unit::MeleeSpellDodgeChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const
{
    if (victim->isTotem())
        return 0.0f;
    ...
    float dodgeChance = victim->GetUnitDodgeChance();

    dodgeChance += static_cast<float>(GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_COMBAT_RESULT_CHANCE, VICTIMSTATE_DODGE));
    dodgeChance *= GetTotalAuraMultiplier(SPELL_AURA_MOD_ENEMY_DODGE);

    if (GetTypeId() == TYPEID_PLAYER)
        dodgeChance -= ToPlayer()->GetExpertiseDodgeOrParryReduction(attType);
    else
        dodgeChance -= static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE)) * 0.25f;

    int32 skillDifference = victim->GetMaxSkillValueForLevel(this) - GetWeaponSkillValue(attType, victim);

    if (!victimIsPlayerOrPetOfAPlayer && skillDifference >= 0)
        dodgeChance += skillDifference * 0.1f;
    else
        dodgeChance += skillDifference * 0.04f;

    return std::max(0.0f, dodgeChance);
}

float Unit::MeleeSpellParryChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const
{
    if (victim->isTotem())
        return 0.0f;
    ...
    float parryChance = victim->GetUnitParryChance();
    // Reduce parry chance by attacker expertise rating

    if (GetTypeId() == TYPEID_PLAYER)
        parryChance -= ToPlayer()->GetExpertiseDodgeOrParryReduction(attType);
    else
        parryChance -= static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE)) * 0.25f;

    int32 skillDifference = victim->GetMaxSkillValueForLevel(this) - GetWeaponSkillValue(attType, victim);

    if (!victimIsPlayerOrPetOfAPlayer && skillDifference >= 0)
        parryChance += skillDifference > 10 ? 1.0f + static_cast<float>(skillDifference - 10) * 2.0f : static_cast<float>(skillDifference) * 0.1f;
    else
        parryChance += static_cast<float>(skillDifference) * 0.04f;

    return std::max(0.0f, parryChance);
}

float Unit::MeleeSpellBlockChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const
{
    if (victim->isTotem())
        return 0.0f;
    ...
    if (!victimIsPlayerOrPetOfAPlayer && skillDifference >= 0)
        blockChance += static_cast<float>(skillDifference) * 0.1f;
    else
        blockChance += static_cast<float>(skillDifference) * 0.04f;

    return std::max(0.0f, blockChance);
}

float Unit::MeleeGlancingChance(const Unit* victim, WeaponAttackType attType) const
{
    if (attType == RANGED_ATTACK)
        return 0.0f;

    bool isPlayerOrPetOfAPlayer = (GetTypeId() == TYPEID_PLAYER);

    if (!isPlayerOrPetOfAPlayer && isPet())
        if (Unit *owner = GetOwner())
            isPlayerOrPetOfAPlayer = (owner->GetTypeId() == TYPEID_PLAYER);

    if (!isPlayerOrPetOfAPlayer)
        return 0.0f;
    ...
    if (victimIsPlayerOrPetOfAPlayer)
        return 0.0f;

    int32 cappedWeaponSkill = std::min(static_cast<int32>(GetWeaponSkillValue(attType, victim)),
        static_cast<int32>(GetMaxSkillValueForLevel(victim)));

    int32 skillDifference = victim->GetDefenseSkillValue(this) - cappedWeaponSkill;

    return std::min(40.0f, std::max(0.0f, static_cast<float>(10 + skillDifference)));
}

float Unit::MeleeCritChance(const Unit* victim, WeaponAttackType attType) const
{
    float critChance = CalculateCriticalChanceDone(attType);
    critChance = victim->ApplyCriticalChanceTaken(critChance, this, attType);

    return std::max(0.0f, critChance);
}

float Unit::MeleeCrushingChance(const Unit* victim, WeaponAttackType attType) const
{
    // mobs can score crushing blows if they're 4 or more levels above victim
    if (getLevelForTarget(victim) < victim->getLevelForTarget(this) + 4)
        return 0.0f;

    int32 cappedDefenseSkill = std::min(static_cast<int32>(victim->GetDefenseSkillValue(this)),
    static_cast<int32>(victim->GetMaxSkillValueForLevel(victim)));

    int32 skillDifference = GetWeaponSkillValue(attType, victim) - cappedDefenseSkill;

    return std::max(0.0f, 15.0f + static_cast<float>(skillDifference - 15) * 2.0f);
}
...
{
    CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED, SPELLVALUE_BASE_POINT0, seatId + 1, base, TRIGGERED_FULL_MASK);
}
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (GetVehicleKit() == vehicle)
        return;

    if (vehicle->GetBase()->IsOnVehicle(this))
        return;
...
// Add support parameter type Position
void Unit::NearTeleportTo(const Position& destination, bool casting /*= false*/)
{
    DisableSpline();
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->TeleportTo(GetMapId(), destination.GetPositionX(), destination.GetPositionY(), destination.GetPositionZ(), destination.GetOrientation(), 
                                TELE_TO_NOT_LEAVE_TRANSPORT | TELE_TO_NOT_LEAVE_COMBAT | TELE_TO_NOT_UNSUMMON_PET | (casting ? TELE_TO_SPELL : 0));
    else
    {
        UpdatePosition(destination.GetPositionX(), destination.GetPositionY(), destination.GetPositionZ(), destination.GetOrientation(), true);
        SendMovementFlagUpdate();
    }
}
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == UNIT_FIELD_AURASTATE && HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK)))
        {
            ...
            else if (index == UNIT_FIELD_FLAGS)
            {
                ...
                if (target->isGameMaster() || target->isDeveloper())
            ...
            {
                const Player* pPlayer = ToPlayer();

                if (IsControlledByPlayer() && target != this && IsInRaidWith(target)
                ...
                else if (index == UNIT_FIELD_FACTIONTEMPLATE && target->HandleWithCrossFactionBG()) // BG Crossfaction handling
                    fieldBuffer << Battleground::GetFactionForCrossfactionBG(target, (pPlayer || (GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER)) ? NULL : getFactionTemplateEntry(),
                                                                                false, (GetOwner() && GetOwner()->GetTypeId() == TYPEID_PLAYER) ? GetAffectingPlayer() : pPlayer);
                else
                    fieldBuffer << m_uint32Values[index];
            }
            else
...
void Unit::SetIgnoreLoS(bool value)
{
    m_isIgnoredByLoS = value;
}

bool Unit::IsIgnoredByLoS() const
{
    return m_isIgnoredByLoS;
}
