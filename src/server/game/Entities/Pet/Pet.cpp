...
Guardian::Guardian(SummonPropertiesEntry const* properties, Unit* owner, bool isWorldObject)
    : Minion(properties, owner, isWorldObject), bonusSpellDamage(0), ownerScaling(NULL)
{
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ownerScaling = new OwnerScaling(MAX_PET_TYPE, GetEntry());
        if (!ownerScaling->FindScalingContainer())
        {
            sLog->outDebug(LOG_FILTER_PETS, "Guardian::InitStatsForLevel: No scaling container found for entry %u", GetEntry());
            delete ownerScaling;
        }
    }

    previosValidStatStorage.clear();
    ...
    {
        sLog->outDebug(LOG_FILTER_PETS, "Guardian::InitStatsForLevel: Create stats found in db, use PetLevelInfo for entry %u and level %u",
                       cinfo->Entry, petlevel);
    ...
    {
        sLog->outDebug(LOG_FILTER_PETS, "Guardian::InitStatsForLevel: Create stats couldn't be found in db, use fake data for entry %u and level %u",
                       cinfo->Entry, petlevel);
    ...
    float bonusStats[MAX_STATS];
    memset(bonusStats, 0.0f, sizeof(bonusStats));
    ...
    float minDMG = 1.0f;
    float maxDMG = 1.0f;
    float createHealth = 0.0f;
    float createMana = 0.0f;

    // Get spell and attack power
    if (ownerScaling)
    {
        for (uint8 i = 0; i < MAX_STATS; ++i)
            bonusStats[i] = ownerScaling->CalculateScaling(STAT_IDENTIFIER_UNIT_MOD, UnitMods(i), GetOwnerStat(Stats(i)));

        bonusAP = ownerScaling->CalculateScaling(STAT_IDENTIFIER_ATTACK_POWER, BASE_ATTACK, GetOwnerAttackPower(BASE_ATTACK));
        for (uint8 i = 0; i < MAX_SPELL_SCHOOL; ++i)
            bonusSP = std::max(ownerScaling->CalculateScaling(STAT_IDENTIFIER_SPELL_POWER, SpellSchools(i), GetOwnerSpellPower(SpellSchools(i))), bonusSP);
        armor = ownerScaling->CalculateScaling(STAT_IDENTIFIER_RESISTANCE, SPELL_SCHOOL_MASK_NORMAL, GetOwnerResistance(SPELL_SCHOOL_MASK_NORMAL));
    }

    // Set base weapon damages + misc
    switch (GetEntry())
    {
        case ENTRY_TREANT:
        {
            if (!pInfo)
                createHealth = 30 + 30 * petlevel;

            minDMG = float(petlevel * 2.5f - (petlevel / 2) + bonusSP);
            maxDMG = float(petlevel * 2.5f + (petlevel / 2) + bonusSP);
        }break;

        case ENTRY_EARTH_ELEMENTAL:
        {
            if (!pInfo)
                createHealth = 100 + 120 * petlevel;

            bonusAP = bonusSP;
            bonusSP = 0;

            minDMG = float(petlevel - (petlevel / 4));
            maxDMG = float(petlevel + (petlevel / 4));
        }break;
        ...
        case ENTRY_FIRE_ELEMENTAL:
        {
            if (!pInfo)
            {
                createHealth = 40 * petlevel;
                createMana = 28 + 10 * petlevel;
            }

            minDMG = float(petlevel * 4 - petlevel);
            maxDMG = float(petlevel * 4 + petlevel);
        }break;
        ...
        case ENTRY_SHADOWFIEND:
        {
            if (!pInfo)
            {
                createHealth = 28 + 10 * petlevel;
                createMana = 28 + 30 * petlevel;
            }

            minDMG = float((petlevel * 4 - petlevel) + bonusSP);
            maxDMG = float((petlevel * 4 + petlevel) + bonusSP);
        }break;

        case ENTRY_FERAL_SPIRIT:
        {
            if (!pInfo)
                createHealth = 30 * petlevel;

            // Bug #2889 fix
            SetReactState(REACT_PASSIVE);

            float damageFactor = 0.3f;
            if (m_owner->GetAuraEffect(63271, EFFECT_0)) // Glyph of Feral Spirit
                damageFactor = 0.6f;

            // 14AP == 1dps, wolf's strike speed == 2s so dmg = basedmg + AP / 14 * 2
            minDMG = float((petlevel * 4 - petlevel) + (bonusAP * damageFactor * 2 / 14));
            maxDMG = float((petlevel * 4 + petlevel) + (bonusAP * damageFactor * 2 / 14));
        ...
        case ENTRY_ARMY_OF_THE_DEAD_GHOUL:
            createHealth = float(GetOwner()->GetHealth() / 8.0f);
        case ENTRY_GHOUL:
        {
        ...
        {
            if (!pInfo)
                createHealth = 4 * petlevel;

            minDMG = float(petlevel - 30 - (petlevel / 4));
            maxDMG = float(petlevel - 30 + (petlevel / 4));
        }break;

        case 19833: //Snake Trap - Venomous Snake
        {
            minDMG = float((petlevel / 2) - 25);
            maxDMG = float((petlevel / 2) - 18);
        }break;

        case 19921: //Snake Trap - Viper
        {
            minDMG = float(petlevel / 2 - 10);
            maxDMG = float(petlevel / 2);
        }break;
    }
    ...
    if (GetEntry() == ENTRY_GHOUL)
    {
        ...
        if (Aura* pAura = GetOwner()->GetAuraOfRankedSpell(48965)) // TALENT_DK_RAVENOUS_DEAD
            add += pAura->GetSpellInfo()->Effects[EFFECT_1].BasePoints;

        if (const AuraEffect* pAuraEffect = GetOwner()->GetAuraEffect(58686, EFFECT_0)) // GLYPH_DK_GLYPH_OF_GHOUL
            add += pAuraEffect->GetAmount();

        if (ownerScaling)
            bonusStats[STAT_STAMINA] = ownerScaling->CalculateScaling(STAT_IDENTIFIER_UNIT_MOD, UNIT_MOD_STAT_STAMINA,
                                                                        GetOwnerStat(STAT_STAMINA), add);

        // DK Ghoul only increases stamina if player bonus stat is higher than (basehealth / multiplicator) of Ghoul
        bonusStats[STAT_STAMINA] -= (GetCreateHealth() / 10.0f);

        if (bonusStats[STAT_STAMINA] < 0.0f)
            bonusStats[STAT_STAMINA] = 0.0f;
    }

    // Base stats
    for (uint8 i = 0; i < MAX_STATS; ++i)
        SetModifierValue(UnitMods(i), BASE_VALUE, bonusStats[i]);

    // Create health + mana
    if (createHealth)
        SetCreateHealth(createHealth);
    if (createMana)
        SetCreateMana(createMana);
    ...
    // Attack power
    SetModifierValue(UNIT_MOD_ATTACK_POWER, TOTAL_VALUE, bonusAP);

    // Armor
    SetModifierValue(UNIT_MOD_ARMOR, BASE_VALUE, armor);

    // Spell power
    SetBonusDamage(int32(bonusSP));

    sLog->outDebug(LOG_FILTER_PETS, "Guardian::InitStatsForLevel: Bonus for Guardian with entry %u is createHealth: %f createMana: %f minDMG: %f maxDMG: %f bonusAP: %f armor: %f bonusSP: %f",
                   GetEntry(), createHealth, createMana, minDMG, maxDMG, bonusAP, armor, bonusSP);

    UpdateAllStats();

    // Has to be done hardcoded
    UpdateHasteRating();

    // Regenerate health + power
    RegenerateAll();
...
void Guardian::LearnPetPassives()
{
    CreatureTemplate const* cInfo = GetCreatureTemplate();
    if (!cInfo)
        return;
    ...
    // Spells with no skill entry
    switch (GetEntry())
    {
        case ENTRY_ARMY_OF_THE_DEAD_GHOUL:
            addSpell(43264, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Periodic Taunt
        case ENTRY_GHOUL:
            addSpell(62137, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Avoidance
            break;

        case ENTRY_SHADOWFIEND:
            addSpell(63623, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Avoidance
            break;

        default:
            addSpell(32233, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Avoidance for any guardian
            break;
    }
}
...
bool Guardian::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (spellInfo->IsPassive() && (!spellInfo->CasterAuraState || HasAuraState(AuraStateType(spellInfo->CasterAuraState))))
        DelayedCastSpell(100, this, spellId, TRIGGERED_FULL_MASK);

    return true;
}

void Guardian::RegenerateAll()
{
    SetFullHealth();
...
bool Pet::InitStatsForLevel(uint8 petlevel)
{
    CreatureTemplate const* cinfo = GetCreatureTemplate();
    ASSERT(cinfo);
    ...
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ownerScaling = new OwnerScaling(m_petType, GetEntry());
        if (!ownerScaling->FindScalingContainer())
            delete ownerScaling;
    }
    ...
    uint32 creature_ID = (m_petType == HUNTER_PET) ? 1 : cinfo->Entry;
    ...
    // Scale
    CreatureFamilyEntry const* cFamily = sCreatureFamilyStore.LookupEntry(cinfo->family);
    if (cFamily && cFamily->minScale > 0.0f && m_petType == HUNTER_PET)
    {
        float scale;
    ...
    if (pInfo) // exist in DB
    {
        sLog->outDebug(LOG_FILTER_PETS, "Pet::InitStatsForLevel: Create stats found in db, use PetLevelInfo for entry %u and level %u",
                       creature_ID, petlevel);

        SetCreateHealth(pInfo->health);
        if (m_petType != HUNTER_PET) // Hunter pet uses focus
            SetCreateMana(pInfo->mana);

        if (pInfo->armor > 0)
        {
            if (IsPetGhoul())
                SetArmor(float(pInfo->armor));
            else
                SetModifierValue(UNIT_MOD_ARMOR, BASE_VALUE, float(pInfo->armor));
        }
    ...
    {
        sLog->outDebug(LOG_FILTER_PETS, "Pet::InitStatsForLevel: Create stats couldn't be found in db, use fake data for entry %u and level %u",
                       creature_ID, petlevel);
    ...
    if (Player* pPlayer = m_owner->ToPlayer())
    {
        switch (m_petType)
        {
            case HUNTER_PET:
    ...
    // Set stats - pet is scaled by auras
    for (uint8 i = STAT_STRENGTH; i < MAX_STATS; ++i)
        SetStat(Stats(i), int32(GetTotalStatValue(Stats(i))));
...
void Pet::_LoadAuras(uint32 timediff)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                if (!aura->CanBeSaved() || spellInfo->AttributesEx & SPELL_ATTR1_UNAUTOCASTABLE_BY_PET)
                {
                    aura->Remove();
                    continue;
                }
...
void Pet::LearnPetPassives()
{
    ...
    {
        case ENTRY_GHOUL:
            addSpell(61697, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Death Knight Pet Scaling 03
            addSpell(62137, ACT_DECIDE, PETSPELL_NEW, PETSPELL_FAMILY); // Avoidance
            break;
    }
}
