...
bool ArenaTeam::LoadArenaTeamFromDB(QueryResult result)
{
    ...
    Stats.Inactivity  = fields[15].GetUInt8();
...
void ArenaTeam::MassInviteToEvent(WorldSession* session)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Sent SMSG_CALENDAR_ARENA_TEAM");
}
...
void ArenaTeam::UpdateArenaPointsHelper(std::map<uint32, uint32>& playerPoints)
{
    ...
    if (Stats.WeekGames < 10)
    {
        ...
        //{
        //    int32 modPCT = pow(2, Stats.Inactivity);
        //
        //    Stats.Rating = std::max(Stats.Rating - CalculatePctN(Stats.Rating, modPCT), 1500);
        //
        //    sLog->outString("Arena Team %u has been inactive for %u weeks resulting in a loss of %d%% (= %u)Rating, now has %u Rating.",
        //        GetId(), Stats.Inactivity, modPCT, CalculatePctN(Stats.Rating, modPCT), Stats.Rating);
        //
        //    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
        //        itr->PersonalRating = std::max(itr->PersonalRating - CalculatePctN(itr->PersonalRating, modPCT), 1500);
        //
        //    Stats.Inactivity += 1;
        //}
        return;
...
void ArenaTeam::SaveToDB()
{
    ...
    stmt->setUInt8(6, Stats.Inactivity);
