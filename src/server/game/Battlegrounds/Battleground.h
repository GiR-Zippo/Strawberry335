...
// Custom pvp game events
enum PvPGameEvents
{
    PVP_GAME_EVENT_ARENA_3VS3_PERIOD_1      = 111,
    PVP_GAME_EVENT_ARENA_3VS3_PERIOD_2      = 112,
    PVP_GAME_EVENT_ARENA_3VS3_PERIOD_3      = 113,
    PVP_GAME_EVENT_ARENA_3VS3_PERIOD_4      = 114,
    PVP_GAME_EVENT_ARENA_3VS3_AP_AMOUNT     = 10,
    PVP_GAME_EVENT_BG_HOLIDAY_AP_AMOUNT     = 5
};

// Items which are not allowed in arena and those replacements
enum ArenaNotPermittedItemEntries
{
    ITEM_ENTRY_SHADOWMOURNE                 = 49623,
    ITEM_ENTRY_SHADOWMOURNE_REPLACEMENT     = 60001,
    ITEM_NOT_PERMITTED_COUNT                = 1
};

const uint32 ArenaNotPermittedItems[ITEM_NOT_PERMITTED_COUNT][2] =
{
    { ITEM_ENTRY_SHADOWMOURNE, ITEM_ENTRY_SHADOWMOURNE_REPLACEMENT }
};

// All vehicle should be immune against this effects
const uint32 BattlegroundVehicleSpellImmunity[] =
{
...
class Battleground
{
    public:
        ...
        void AddPlayerToTeleportQueue(uint64 npc_guid, uint64 player_guid);
        void RemovePlayerFromTeleportQueue(uint64 player_guid);
        ...
        void RewardReputationToTeam(uint32 faction_id, uint32 Reputation, uint32 TeamID, uint32 enemy_faction_id = 0);
        ...
        // Function to get uint32 Data from the Battleground (same as InstanceScript)
        virtual uint32 GetData(uint32) { return 0; }

        // Death related
        virtual WorldSafeLocsEntry const* GetClosestGraveYard(Player* player);
        virtual WorldSafeLocsEntry const* GetClosestGraveYard(Player* player, uint64& /*npc_guid*/)
        {
            return GetClosestGraveYard(player);
        }
        ...
        bool IsBgCreature(uint32 type);
        bool IsBgObject(uint32 type);
        ...
        static uint32 GetFactionForCrossfactionBG(const Player* pPlayer, const FactionTemplateEntry* pFactionEntry = NULL, bool isGameObject = false, const Player* pPlayerTarget = NULL);
    ...
    protected:
        ...
        // Used to set winner if battle has ended because of less players
        ...
        std::map<uint64, std::vector<uint64> >  m_TeleportQueue;
    ...
    private:
        ...
        /** Called after player leave */
        void balanceTeams();

        /** Called to change player team */
        void changeTeam(Player* pPlayer);
