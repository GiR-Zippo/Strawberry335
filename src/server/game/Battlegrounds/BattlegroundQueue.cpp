...
{
    for (uint32 i = 0; i < MAX_BATTLEGROUND_BRACKETS; i++){
        m_statusA[i] = 0;
        m_statusH[i] = 0;
    }

    currentBracket =  BattlegroundBracketId(0);
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    uint32 team = leader->GetTeam();

    // BG Crossfaction handling
    if (!ArenaType && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS))
    {
        uint32 queuedPlayers[2] =
        {
            (m_QueuedGroups[bracketId][TEAM_ALLIANCE].size() + m_QueuedGroups[bracketId][TEAM_ALLIANCE + BG_TEAMS_COUNT].size()),
            (m_QueuedGroups[bracketId][TEAM_HORDE].size() + m_QueuedGroups[bracketId][TEAM_HORDE + BG_TEAMS_COUNT].size()),
        };

        if (queuedPlayers[TEAM_ALLIANCE] > queuedPlayers[TEAM_HORDE]) // More alliance players
            team = HORDE;
        else if (queuedPlayers[TEAM_ALLIANCE] < queuedPlayers[TEAM_HORDE]) // More horde players
            team = ALLIANCE;
        else // Horde and alliance players are equal
            team = urand(0, 1) ? HORDE : ALLIANCE;
    }
    ...
    if (grp)
    {
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if (ginfo->BgTypeId == BATTLEGROUND_RB)
                BattlegroundMgr::AddActiveBGPlayer(member);
            if (!member)
    ...
    else
    {
        ...
        if (ginfo->BgTypeId == BATTLEGROUND_RB)
            BattlegroundMgr::AddActiveBGPlayer(leader);
    }
    ...
    if (!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
    {
        if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
        {
            ...
            m_statusA[bracketId] = qAlliance;

            for (itr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE].begin(); itr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE].end(); ++itr)
                if (!(*itr)->IsInvitedToBGInstanceGUID)
                    qHorde += (*itr)->Players.size();
            m_statusH[bracketId] = qHorde;

            if (ginfo->Team == ALLIANCE)
                ginfo->QueuePosition = qAlliance - ginfo->Players.size();
            else
                ginfo->QueuePosition = qHorde - ginfo->Players.size();

            currentBracket = BattlegroundBracketId(bracketId);
...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    GroupsQueueType groupQueue;

    bool decrease = false;

    if (group->Team == ALLIANCE)
    {
        m_statusA[group->BracketId]--;
        groupQueue = m_QueuedGroups[group->BracketId][BG_QUEUE_NORMAL_ALLIANCE];
    }
    else
    {
        m_statusH[group->BracketId]--;
        groupQueue = m_QueuedGroups[group->BracketId][BG_QUEUE_NORMAL_HORDE];
    }

    for (GroupsQueueType::iterator group_iter = groupQueue.begin(); group_iter != groupQueue.end();group_iter++)
    {
        if (decrease && (*group_iter))
            (*group_iter)->QueuePosition--;
        else if ((*group_iter) == group)
            decrease = true;
    }
...
int32 BattlegroundQueue::GetAStatus(uint64 guid){
    GroupQueueInfo ginfo;
    if (GetPlayerGroupInfoData(guid,&ginfo))
        return m_statusA[ginfo.BracketId];
    return m_statusA[currentBracket];
}

int32 BattlegroundQueue::GetHStatus(uint64 guid){
    GroupQueueInfo ginfo;
    if (GetPlayerGroupInfoData(guid,&ginfo))
        return m_statusH[ginfo.BracketId];
    return m_statusH[currentBracket];
}

int32 BattlegroundQueue::GetPlayerPosition(uint64 guid){
    GroupQueueInfo ginfo;
    if (GetPlayerGroupInfoData(guid,&ginfo))
        return ginfo.QueuePosition;
    return -1;
}