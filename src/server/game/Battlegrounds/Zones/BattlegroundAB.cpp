...
BattlegroundAB::BattlegroundAB() : Battleground()
{
    m_BuffChange = true;
            ...
            if (m_lastTick[team] > BG_AB_TickIntervals[points])
            {
                ...
                if (m_ReputationScoreTics[team] >= m_ReputationTics)
                {
                    if (team == BG_TEAM_ALLIANCE)
                        RewardReputationToTeam(BG_AB_FACTION_ALLIANCE, BG_AB_REPUTATION_AMOUNT, ALLIANCE, BG_AB_FACTION_HORDE);
                    else
                        RewardReputationToTeam(BG_AB_FACTION_HORDE, BG_AB_REPUTATION_AMOUNT, HORDE, BG_AB_FACTION_ALLIANCE);
...
void BattlegroundAB::_NodeDeOccupied(uint8 node)
{
    ...
    if (!ghost_list.empty())
    {
        ...
        for (std::vector<uint64>::const_iterator itr = ghost_list.begin(); itr != ghost_list.end(); ++itr)
        {
            ...
            if (!ClosestGrave)                              // cache
                ClosestGrave = GetClosestGraveYard(player, unused);
    ...
    std::vector<uint64> tele_list = m_TeleportQueue[BgCreatures[node]];
    if (!tele_list.empty())
    {
        WorldSafeLocsEntry const* ClosestGrave = NULL;
        uint64 unused = 0;
        for (std::vector<uint64>::const_iterator itr = tele_list.begin(); itr != tele_list.end(); ++itr)
        {
            Player* player = ObjectAccessor::FindPlayer(*itr);
            if (!player)
                continue;
...
WorldSafeLocsEntry const* BattlegroundAB::GetClosestGraveYard(Player* player, uint64& npc_guid)
{
    ...
    if (!good_entry)
    {
        ...
        npc_guid = BgCreatures[teamIndex+5];
    }
