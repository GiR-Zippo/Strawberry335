...
enum BattlegroundRVTimer
{
    BG_RV_TIMER_PILAR_ANIMATION,
    BG_RV_TIMER_PILAR_TOGGLE,
    BG_RV_TIMER_FLAMEWALL_CLOSE,
    BG_RV_TIMER_MAX,
};

enum BattlegroundRVData
{
    BG_RV_PILLAR_SWITCH_TIMER_MIN                = 15000,
    BG_RV_PILLAR_SWITCH_TIMER_MAX                = 25000,
    BG_RV_PILLAR_SWITCH_TIMER                    =  5000,
    BG_RV_CLOSE_FIRE_TIMER                       = 20000,
...
class BattlegroundRV : public Battleground
{
    ...
    private:
        ...
        std::list<BattlegroundRVTimer> timerList;
        bool    PillarCollision;

        void PostUpdateImpl(uint32 diff);

    protected:
        void SetTimer(BattlegroundRVTimer timer, uint32 time);
        bool UpdateTimer(BattlegroundRVTimer timer, uint32 diff);
