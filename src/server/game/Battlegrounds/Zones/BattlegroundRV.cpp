...
void BattlegroundRV::ResetBGSubclass()
{
}
...
void BattlegroundRV::PostUpdateImpl(uint32 diff)
{
    ...
    std::list<BattlegroundRVTimer>::iterator itr = timerList.begin();
    BattlegroundRVTimer l_timer;
    while (itr != timerList.end())
    {
        l_timer = (*itr);
        itr++;
        if (!UpdateTimer(l_timer,diff))
            continue;
        switch (l_timer)
        {
        case BG_RV_TIMER_PILAR_ANIMATION:
            SetTimer(BG_RV_TIMER_PILAR_TOGGLE,BG_RV_PILLAR_SWITCH_TIMER);
            for (uint8 i = BG_RV_OBJECT_GEAR_1; i <= BG_RV_OBJECT_PULLEY_2; ++i)
            {
                if (GetBGObject(i)->GetGoState() == GO_STATE_ACTIVE)
                {
                    DoorClose(i);
                } else {
                    DoorOpen(i);
            ...
            break;
        case BG_RV_TIMER_PILAR_TOGGLE:
            SetTimer(BG_RV_TIMER_PILAR_ANIMATION,urand(BG_RV_PILLAR_SWITCH_TIMER_MIN,BG_RV_PILLAR_SWITCH_TIMER_MAX));
            ...
            break;
        case BG_RV_TIMER_FLAMEWALL_CLOSE:
...
void BattlegroundRV::StartingEventOpenDoors()
{
    ...
    DoorOpen(BG_RV_OBJECT_PILAR_1);
    DoorOpen(BG_RV_OBJECT_PILAR_3);
    DoorOpen(BG_RV_OBJECT_GEAR_1);
    DoorOpen(BG_RV_OBJECT_GEAR_2);

    SetTimer(BG_RV_TIMER_FLAMEWALL_CLOSE,BG_RV_CLOSE_FIRE_TIMER);
...
void BattlegroundRV::SetTimer(BattlegroundRVTimer timer, uint32 time)
{
...
bool BattlegroundRV::UpdateTimer(BattlegroundRVTimer timer, uint32 diff)
{
    if (diff < timers[timer])
    {
        timers[timer] -= diff;
        return false;
    }
