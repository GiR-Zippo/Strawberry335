...
WorldSafeLocsEntry const* BattlegroundEY::GetClosestGraveYard(Player* player, uint64& npc_guid)
{
    uint32 g_id = 0;
    ...
    switch (player->GetTeam())
    {
        case ALLIANCE: 
            ...
            npc_guid = BgCreatures[EY_SPIRIT_MAIN_ALLIANCE];
            break;
        case HORDE:
            g_id = EY_GRAVEYARD_MAIN_HORDE;
            npc_guid = BgCreatures[EY_SPIRIT_MAIN_HORDE];
            break;
        default:       return NULL;
    }
...
void BattlegroundEY::_PointDeCaptured(uint8 Point)
{
    if (Point >= EY_DYNAMIC_NODES_COUNT)
        return;

    // Those who are waiting to resurrect at this node are taken to the closest own node's graveyard
    std::vector<uint64> ghost_list = m_ReviveQueue[BgCreatures[Point]];
    if (!ghost_list.empty())
    {
    ...
    std::vector<uint64> tele_list = m_TeleportQueue[BgCreatures[Point]];
    if (!tele_list.empty())
    {
        WorldSafeLocsEntry const* ClosestGrave = NULL;
        uint64 unused = 0;
