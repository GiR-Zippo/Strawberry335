...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (_bothFlagsKept)
        {
            ...
            {
                if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_HORDE)))
                {
                    p->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
                    p->RemoveAurasDueToSpell(WS_SPELL_BRUTAL_ASSAULT);
                    p->CastSpell(p, WS_SPELL_FOCUSED_ASSAULT, true);
                }
                if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_ALLIANCE)))
                {
                    p->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
                    p->RemoveAurasDueToSpell(WS_SPELL_BRUTAL_ASSAULT);
                    p->CastSpell(p, WS_SPELL_FOCUSED_ASSAULT, true);
            ...
            else if (_flagDebuffState == 1 && _flagSpellForceTimer >= 15*MINUTE*IN_MILLISECONDS) //15 minutes
            {
                if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_HORDE)))
                {
                    p->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
                    p->RemoveAurasDueToSpell(WS_SPELL_BRUTAL_ASSAULT);
                    p->CastSpell(p, WS_SPELL_BRUTAL_ASSAULT, true);
                }
                if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_ALLIANCE)))
                {
                    p->RemoveAurasDueToSpell(WS_SPELL_FOCUSED_ASSAULT);
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* Source)
{
    ...
    if (Source->GetTeam() == ALLIANCE)
    {
        ...
        RewardReputationToTeam(BG_WS_FACTION_ALLIANCE, m_ReputationCapture, ALLIANCE, BG_WS_FACTION_HORDE);
    }
    else
    {
        ...
        RewardReputationToTeam(BG_WS_FACTION_HORDE, m_ReputationCapture, HORDE, BG_WS_FACTION_ALLIANCE);
    }
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
        ...
        && BgObjects[BG_WS_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        if (GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_PLAYER)
            _bothFlagsKept = true;
        if (_bothFlagsKept)
        {
            if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_HORDE)))
            {
                if (_flagDebuffState == 1)
                    p->CastSpell(p, BG_WS_SPELL_FOCUSED_ASSAULT, true);
                else if (_flagDebuffState == 2)
                    p->CastSpell(p, BG_WS_SPELL_BRUTAL_ASSAULT, true);
            }
            if (_flagDebuffState == 1)
        ...
        && BgObjects[BG_WS_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        ...
        if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_PLAYER)
            _bothFlagsKept = true;
        if (_bothFlagsKept)
        {
            if (Player* p = ObjectAccessor::FindPlayer(GetFlagPickerGUID(BG_TEAM_ALLIANCE)))
