...
std::map<uint64, uint32> BattlegroundMgr::m_ActiveHordeBGPlayers;
std::map<uint64, uint32> BattlegroundMgr::m_ActiveAllianceBGPlayers;
...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if ((sWorld->getBoolConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS)) &&
        (sWorld->getIntConfig(CONFIG_CORE_TYPE) <= NODE_TYPE_MASTER))
    {
        if (m_AutoDistributionTimeChecker < diff)
        {
            if (time(NULL) > m_NextAutoDistributionTime)
            {
                ...
                if (CustomArenaResetTimer* pCustomArenaResetTimer = sWorld->GetCustomArenaResetTimer())
                    m_NextAutoDistributionTime = pCustomArenaResetTimer->CalculateNextResetTime();
                else
...
// Choose a BG Type for Ranbom BG
BattlegroundTypeId BattlegroundMgr::GetRandomBattlegroundType(BattlegroundSelectionWeightMap* weights)
{
    uint32 Weight = 0;
    uint32 selectedWeight = 0;
    BattlegroundTypeId bgTypeId = BATTLEGROUND_TYPE_NONE;
    // Get sum of all weights
    for (BattlegroundSelectionWeightMap::const_iterator it = weights->begin(); it != weights->end(); ++it)
        Weight += it->second;
    if (!Weight)
    ...
    // Do not Open a huge BG when it doesnt make sense
    if (!CanOpenLargeBG() && (bgTypeId == BATTLEGROUND_AV || bgTypeId == BATTLEGROUND_IC))
        return GetRandomBattlegroundType(weights);
    return bgTypeId;
}

// Check if there are enough Players actively playing BG to open 40A+40H BGs (AV + IC)
bool BattlegroundMgr::CanOpenLargeBG()
{
    if (GetCurrentActiveBGPlayers() < 20)
        return false;
    return true;
}
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    if (selectionWeights)
    {
        if (selectionWeights->empty())
           return NULL;
        bgTypeId = GetRandomBattlegroundType(selectionWeights);
        if (bgTypeId == BATTLEGROUND_TYPE_NONE)
            return NULL;
...
void BattlegroundMgr::UpdateActiveBGPlayers()
{
    ActiveBGPlayers::iterator ii = m_ActiveHordeBGPlayers.begin();
    while (ii != m_ActiveHordeBGPlayers.end())
    {
        if ((getMSTime()-(*ii).second) < 21*60*1000)
        {
            m_ActiveHordeBGPlayers.erase(ii++);
        } else {
            ++ii;
        }
    }

    ii = m_ActiveAllianceBGPlayers.begin();
    while (ii != m_ActiveAllianceBGPlayers.end())
    {
        if ((getMSTime()-(*ii).second) < 21*60*1000)
        {
            m_ActiveAllianceBGPlayers.erase(ii++);
        } else {
            ++ii;
        }
    }
...
uint32 BattlegroundMgr::GetCurrentActiveBGPlayers()
{
    if (m_ActiveHordeBGPlayers.size() > m_ActiveAllianceBGPlayers.size())
        return m_ActiveAllianceBGPlayers.size();
    return m_ActiveHordeBGPlayers.size();
}

void BattlegroundMgr::AddActiveBGPlayer(Player* player)
{
    UpdateActiveBGPlayers();
    if (player->GetTeam() == HORDE) {
        m_ActiveHordeBGPlayers[player->GetGUID()] = getMSTime();
    } else {
        m_ActiveAllianceBGPlayers[player->GetGUID()] = getMSTime();
    }
}

int32 BattlegroundMgr::GetHStatus(uint64 guid){
    return m_BattlegroundQueues[BATTLEGROUND_QUEUE_RB].GetHStatus(guid);
}
int32 BattlegroundMgr::GetAStatus(uint64 guid){
    return m_BattlegroundQueues[BATTLEGROUND_QUEUE_RB].GetAStatus(guid);
}
int32 BattlegroundMgr::GetPosition(uint64 guid){
    return m_BattlegroundQueues[BATTLEGROUND_QUEUE_RB].GetPlayerPosition(guid);
}
