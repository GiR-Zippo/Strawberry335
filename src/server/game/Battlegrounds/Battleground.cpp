...
inline void Battleground::_ProcessProgress(uint32 diff)
{
    ...
    {
        // time's up!
        uint32 winner = getPrematureWinner();
        if (!winner) // tide -> compare player count
            winner = (m_PlayersCount[BG_TEAM_ALLIANCE] > m_PlayersCount[BG_TEAM_HORDE] ? ALLIANCE : HORDE);
...
void Battleground::RewardReputationToTeam(uint32 faction_id, uint32 Reputation, uint32 TeamID, uint32 enemy_faction_id)
{
    if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(faction_id))
    {
        for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        {
            ...
            {
                if (enemy_faction_id && player->HandleWithCrossFactionBG() && player->GetTeam(true) != TeamID)
                    if (FactionEntry const* enemyFactionEntry = sFactionStore.LookupEntry(enemy_faction_id))
                        factionEntry = enemyFactionEntry;
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (isArena() && isRated())
        {
            ...
            if (m_ArenaType == 3 && CanAwardArenaPoints()
                && (sGameEventMgr->IsActiveEvent(PVP_GAME_EVENT_ARENA_3VS3_PERIOD_1) || sGameEventMgr->IsActiveEvent(PVP_GAME_EVENT_ARENA_3VS3_PERIOD_2)
                || sGameEventMgr->IsActiveEvent(PVP_GAME_EVENT_ARENA_3VS3_PERIOD_3) || sGameEventMgr->IsActiveEvent(PVP_GAME_EVENT_ARENA_3VS3_PERIOD_4))
                && ((player->GetArenaPoints() + PVP_GAME_EVENT_ARENA_3VS3_AP_AMOUNT) <= sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS)))
                player->ModifyArenaPoints(PVP_GAME_EVENT_ARENA_3VS3_AP_AMOUNT);
        }
        ...
        if (isBattleground())
        {
            if (team == winner)
            {
                ...
                if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID()))
                {
                    ...
                    {
                        if (BattlegroundMgr::IsBGWeekend(GetTypeID()) && player->GetRandomWinner())
                            winner_arena = PVP_GAME_EVENT_BG_HOLIDAY_AP_AMOUNT;
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
        ...
        if (player)
        {
            ...
            if (isArena())
            {
                ...
                if (isRated() && GetStatus() == STATUS_IN_PROGRESS)
                {
                    ...
                    if (itr2->second->DamageDone == 0 && itr2->second->HealingDone == 0)
                        if (Aura* pAura = player->AddAura(SPELL_BG_DESERTER_DEBUFF, player))
                            pAura->SetDuration(300000); // 5 Minuten debuff
                }
            }
            if (SendPacket)
            {
            ...
            if (Item* pItem = player->GetNotPermittedItemInArena())
                player->RemoveItemBonus(pItem);
        }
        else
    ...
    /* Currently disabled
    if (!isArena() && GetStatus() == STATUS_IN_PROGRESS && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS))
        balanceTeams();*/
...
void Battleground::AddPlayer(Player* player)
{
    ...
    if (isArena())
    {
        ...
        if (GetStatus() == STATUS_WAIT_JOIN)                 // not started yet
        {
            ...
            {
                case CLASS_MAGE:
                    player->CastSpell(player, 58661, true);
                    break;
                case CLASS_WARLOCK:
                    player->CastSpell(player, 58889, true);
                    break;
            }
        }
...
void Battleground::AddPlayerToResurrectQueue(uint64 npc_guid, uint64 player_guid)
{
    ...
    RemovePlayerFromTeleportQueue(player_guid);
...
void Battleground::AddPlayerToTeleportQueue(uint64 npc_guid, uint64 player_guid)
{
    m_TeleportQueue[npc_guid].push_back(player_guid);
...
void Battleground::RemovePlayerFromTeleportQueue(uint64 player_guid)
{
    for (std::map<uint64, std::vector<uint64> >::iterator itr = m_TeleportQueue.begin(); itr != m_TeleportQueue.end(); ++itr)
    {
        for (std::vector<uint64>::iterator itr2 = (itr->second).begin(); itr2 != (itr->second).end(); ++itr2)
        {
            if (*itr2 == player_guid)
...
bool Battleground::IsBgCreature(uint32 type)
{
    if (BgCreatures[type])
        return true;
    return false;
...
bool Battleground::IsBgObject(uint32 type)
{
    if (BgObjects[type])
        return true;
    return false;
...
int32 Battleground::GetCreatureType(uint64 guid)
{
    for (uint32 i = 0; i < BgCreatures.size(); ++i)
        if (BgCreatures[i] == guid)
            return i;
    sLog->outError("Battleground::GetCreatureType: guid %u couldn't been found in BgCreatures", GUID_LOPART(guid));
    return -1;
}
...
uint32 Battleground::GetFactionForCrossfactionBG(const Player* pPlayer, const FactionTemplateEntry* pFactionEntry, bool isGameObject, const Player* pPlayerTarget)
{
    ASSERT(pPlayer && (pFactionEntry || pPlayerTarget)); // Player and one arg has to be given

    uint32 faction = pPlayer->GetTeam(true) == ALLIANCE ? 2 : 1; // Default is hostile

    // Player - Player handling
    if (pPlayerTarget && !isGameObject)
    {
        if (pPlayer->GetBGTeam() == pPlayerTarget->GetBGTeam())
        {
            if (pPlayer->GetTeam(true) == ALLIANCE)
                faction = 1; // Alliance (Human) faction entry
            else
                faction = 2; // Horde (Orc) faction entry
        }
        return faction;
    }

    // Player - Creature/GameObject handling
    if (pFactionEntry->IsNeutralToAll() || pFactionEntry->IsHostileToPlayers())
        return pFactionEntry->ID;

    if (const FactionTemplateEntry* pFaction = pPlayer->getFactionTemplateEntry())
    {
        bool friendly = pFactionEntry->IsFriendlyTo(*pFaction) || !pFactionEntry->IsHostileTo(*pFaction);

        switch (pPlayer->GetTeam(true))
        {
            case ALLIANCE:
                if (isGameObject)
                    faction = 1997; // Horde faction entry (useable)
                else if ((!friendly && pPlayer->GetBGTeam() == HORDE) || (friendly && pPlayer->GetBGTeam() == ALLIANCE))
                    faction = 1; // Alliance (Human) faction entry
                break;

            case HORDE:
                if (isGameObject)
                    faction = 1995; // Alliance faction entry (useable)
                else if ((!friendly && pPlayer->GetBGTeam() == ALLIANCE) || (friendly && pPlayer->GetBGTeam() == HORDE))
                    faction = 2; // Horde (Orc) faction entry
                break;
        }
    }
...
void Battleground::balanceTeams()
{/* Not functional!!!
    if (m_PlayersCount[BG_TEAM_ALLIANCE] <= MAX_DIFFERENCE_TEAMS
        && m_PlayersCount[BG_TEAM_HORDE] <= MAX_DIFFERENCE_TEAMS)
        return;

    float players[BG_TEAMS_COUNT] =
    {
        float(m_PlayersCount[BG_TEAM_ALLIANCE] - m_PlayersCount[BG_TEAM_HORDE]),
        float(m_PlayersCount[BG_TEAM_HORDE] - m_PlayersCount[BG_TEAM_ALLIANCE])
    };

    for (uint8 i = 0; i < BG_TEAMS_COUNT; ++i)
    {
        if (players[i] >= MAX_DIFFERENCE_TEAMS)
        {
            uint8 count = players[i];
            if ((players[i] - count) >= 0.5f)
                ++count;

            uint8* playerToBalance = new uint8[count];

            // Generate randoms
            for (uint8 j = 0; j < count; ++j, ++playerToBalance)
                *playerToBalance = urand(0, m_PlayersCount[i]);

            // Change team
            for (uint8 k = 0; k < count; ++k)
            {
                BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
                std::advance(itr, urand(0, (m_Players.size() - 1)));
                if (itr == m_Players.end())
                    continue;
            ...
            delete[] playerToBalance;
        }
    }
*/}

void Battleground::changeTeam(Player* pPlayer)
{
    if (!pPlayer)
        return;

    // Change in map
    if (BattlegroundPlayer* currentBP = &m_Players[pPlayer->GetGUID()])
    {
        // Team
        UpdatePlayersCountByTeam(currentBP->Team, true); // -1 player
        currentBP->Team = currentBP->Team == BG_TEAM_ALLIANCE ? BG_TEAM_HORDE : BG_TEAM_ALLIANCE;
        pPlayer->SetBGTeam(currentBP->Team == BG_TEAM_ALLIANCE ? ALLIANCE : HORDE);
        UpdatePlayersCountByTeam(currentBP->Team, false); // +1 player

        WorldPacket data;
        sBattlegroundMgr->BuildPlayerJoinedBattlegroundPacket(&data, pPlayer);
        SendPacketToTeam(currentBP->Team, &data, pPlayer, false);

        // Group
        AddOrSetPlayerToCorrectBgGroup(pPlayer, pPlayer->GetBGTeam());

        // Misc
        RemovePlayerFromResurrectQueue(pPlayer->GetGUID());
        ...
        if (!pPlayer->IsBeingTeleported())
        {
            if (!pPlayer->isAlive())
            {
                if (const WorldSafeLocsEntry* closestGraveYard = GetClosestGraveYard(pPlayer))
                    pPlayer->TeleportTo(closestGraveYard->map_id, closestGraveYard->x, closestGraveYard->y, closestGraveYard->z, 0.f);
            }
            else
        ...
        // Log
        sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: Player %s changed team to %s", pPlayer->GetName(), (pPlayer->GetBGTeam() == ALLIANCE ? "alliance" : "horde"));
    }
}