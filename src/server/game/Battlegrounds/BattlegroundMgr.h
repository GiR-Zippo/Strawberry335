...
class BattlegroundMgr
{
    ...
    public:
        ...
        BattlegroundTypeId GetRandomBattlegroundType(std::map<BattlegroundTypeId, uint8>* weights);
        bool CanOpenLargeBG();
        ...
        static uint32 GetCurrentActiveBGPlayers();
        static void AddActiveBGPlayer(Player* player);

        int32 GetHStatus(uint64 guid);
        int32 GetAStatus(uint64 guid);
        int32 GetPosition(uint64 guid);
    private:
        static void UpdateActiveBGPlayers();

        BattleMastersMap    mBattleMastersMap;

        typedef std::map<BattlegroundTypeId, uint8> BattlegroundSelectionWeightMap; // TypeId and its selectionWeight
        typedef std::map<uint64, uint32> ActiveBGPlayers; // Player GUID and the Player's queueing time
        /* Battlegrounds */
        ...
        static ActiveBGPlayers m_ActiveHordeBGPlayers;
        static ActiveBGPlayers m_ActiveAllianceBGPlayers;
