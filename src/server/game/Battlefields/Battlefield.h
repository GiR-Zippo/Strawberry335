 ...
 * @section DESCRIPTION
 *
 * The class Battlefield is a base class for all not instanceable battlegrounds
...
{
    BATTLEFIELD_SPELL_SPIRITUAL_IMMUNITY = 58729,
};
...
public:
    ...
    virtual bool HandlePlayerLeave(Player* player);
...
class Battlefield : public ZoneScript
{
    ...
    public:
        ...
        void KickPlayerFromBattlefield(Player* pPlayer);
        ...
        void HandlePlayerLeaveBfGroup(Player* player);
        ...
        void SendWarningToAllInZone(uint32 entry, uint32 additionalEntry = 0);
        void SendWarningToPlayer(Player* player, uint32 entry, uint32 additionalEntry = 0);

        void PlayerAcceptInviteToQueue(Player* player);
        void PlayerAcceptInviteToWar(Player* player);
        void PlayerDeclinedInviteToWar(Player* player);
        uint32 GetBattleId() { return m_BattleId; }
        void AskToLeaveQueue(Player* player);

        virtual void DoCompleteOrIncrementAchievement(uint32 /*achievement*/, Player* /*player*/, uint8 /*incrementNumber = 1*/) { }

        /// Send all worldstate data to all player in zone/server.
        virtual void SendInitWorldStatesToAll(bool toServer = false) = 0;
        virtual void FillInitialWorldStates(WorldPacket& /*data*/) = 0;

        /// Return if we can use mount in battlefield
        bool CanFlyIn() { return !m_isActive; }

        /** Return if player can join war */
        bool CanPlayerJoinWar(Player* pPlayer) {
            return ((m_PlayersInWar[pPlayer->GetTeamId()].size() + m_InvitedPlayers[pPlayer->GetTeamId()].size()) < m_MaxPlayer);
        }
    ...
    protected:
        ...
        /** Used for delayed invite to war because of taxi */
        GuidSet playersOnTaxi;
        ...
        void DelCapturePoint(BfCapturePoint* cp) { m_capturePoints.erase(cp->GetCapturePointEntry()); }
