...
/// World constructor
World::World() : m_customArenaResetTimer(NULL)
{
    m_playerLimit = 0;
...
World::~World()
{
    ...
    delete m_customArenaResetTimer;
...
void World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && AccountMgr::IsVIPorPlayer(s->GetSecurity()) && !HasRecentlyDisconnected(s))
    {
        AddQueuedPlayer (s);
...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_int_configs[CONFIG_CORE_TYPE] = ConfigMgr::GetIntDefault("CoreType", 0);
    
    ///- Read ticket system setting from the config file
    m_bool_configs[CONFIG_ALLOW_TICKETS] = ConfigMgr::GetBoolDefault("AllowTickets", true);

    ///- Read if gm-commands are bound at its char
    m_bool_configs[CONFIG_UNIQUE_GM] = ConfigMgr::GetBoolDefault("GM.Unique", true);
    ...
    m_bool_configs[CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS]       = ConfigMgr::GetBoolDefault("AllowTwoSide.Battlegrounds", false);
    ...
    m_int_configs[CONFIG_ARENA_SEASON_TRANSMOG_ID]                   = ConfigMgr::GetIntDefault ("Arena.ArenaSeason.TransmogID", 0);
    ...
    sLog->outString("VMap support included. LineOfSight: %i, getHeight: %i, indoorCheck: %i PetLOS: %i", enableLOS, enableHeight, enableIndoor, enablePetLOS);
    ...
    m_bool_configs[CONFIG_RAID_FINDER_ENABLE] = ConfigMgr::GetBoolDefault("RaidFinder.Enable", false);
    m_int_configs[CONFIG_LFG_OPTIONSMASK] = m_bool_configs[CONFIG_DUNGEON_FINDER_ENABLE] ? (m_bool_configs[CONFIG_RAID_FINDER_ENABLE] ? 0x02 : 0x01) : 0x00;
    m_bool_configs[CONFIG_DUNGEON_FINDER_COOLDOWN] = ConfigMgr::GetBoolDefault("DungeonFinder.Cooldown", true);
    m_bool_configs[CONFIG_DUNGEON_FINDER_DESERTER] = ConfigMgr::GetBoolDefault("DungeonFinder.Deserter", true);
    m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_TIMES] = ConfigMgr::GetIntDefault("DungeonFinder.MaxJoinTimes",15);
    if (m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_TIMES] < 0)
        m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_TIMES] = 15;
    m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_SIZE] = ConfigMgr::GetIntDefault("DungeonFinder.MaxJoinSize", 5);
    if (m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_SIZE] > 5 || m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_SIZE] < 0)
        m_int_configs[CONFIG_DUNGEON_FINDER_MAX_JOIN_SIZE] = 5;
    ...
    m_int_configs[CONFIG_FACTION_TRANSFER_FACTIONS_ENABLED] = ConfigMgr::GetIntDefault("FactionTransfer.EnabledFactions", 0);

    m_int_configs[CONFIG_TELE_DELAY] = ConfigMgr::GetIntDefault("Custom.TeleDelay", 2000);

    m_bool_configs[CONFIG_CUSTOM_ICC_BUFF_PLAYERCHOOSE] = ConfigMgr::GetBoolDefault("Custom.ICCBuff.PlayerChoose", false);
    m_int_configs[CONFIG_CUSTOM_ICC_BUFF_ALLIANCE] = ConfigMgr::GetIntDefault("Custom.ICCBuff.Alliance", 0);
    m_int_configs[CONFIG_CUSTOM_ICC_BUFF_HORDE] = ConfigMgr::GetIntDefault("Custom.ICCBuff.Horde", 0);
    m_int_configs[CONFIG_CUSTOM_ICC_BUFF_LK_ALLIANCE] = ConfigMgr::GetIntDefault("Custom.ICCBuff.LK.Alliance", 0);
    m_int_configs[CONFIG_CUSTOM_ICC_BUFF_LK_HORDE] = ConfigMgr::GetIntDefault("Custom.ICCBuff.LK.Horde", 0);

    m_bool_configs[CONFIG_CUSTOM_GM_QUALITY_MANAGER] = ConfigMgr::GetBoolDefault("Custom.GMQualityManager", false);

    m_int_configs[CONFIG_STARTER_GUILD_ALLIANCE] = ConfigMgr::GetFloatDefault("StarterGuild.Alliance", 0);
    m_int_configs[CONFIG_STARTER_GUILD_HORDE] = ConfigMgr::GetFloatDefault("StarterGuild.Horde", 0);
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString("Loading spell diminishing return...");
    sSpellMgr->LoadSpellDiminishingReturn();
    ...
    sLog->outString("Loading Spell Item Data...");
    sSpellMgr->LoadSpellItemData();                             // must be after item template load
...
//     sLFGMgr->LoadLFGDungeons();
    ...
    sLog->outString("Loading Transmog data...");
    sTransmogMgr->LoadFromDB();

    sLog->outString("Loading Survey data...");
    sSurvey.load_from_db();
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) == NODE_TYPE_SINGLE)
        LoginDatabase.PExecute("INSERT INTO uptime (realmid, starttime, uptime, revision) VALUES('%u', " UI64FMTD ", 0, '%s%s%s')",
                            realmID, uint64(m_startTime), _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B);       // One-time query
    else
        LogonDatabase.PExecute("INSERT INTO uptime (realmid, starttime, startstring, uptime, revision) VALUES('%u', " UI64FMTD ", '%s', 0, '%s%s%s')",
                            realmID, uint64(m_startTime), isoDate, _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B);       // One-time query
    ...
    m_timers[WUPDATE_DR].SetInterval(5 * IN_MILLISECONDS); // Global diminishing return reset timer
    ...
    /// Initialize CustomArenaResetTimer
    if (m_customArenaResetTimer = new CustomArenaResetTimer())
    {
        sLog->outString("Loading Custom Arena Reset Timers...");
        if (!m_customArenaResetTimer->Load())
        {
            delete m_customArenaResetTimer;
            m_customArenaResetTimer = NULL;
            sLog->outString(">> No Custom Arena Reset Timer loaded");
        }
        sLog->outString();
    }
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
    {
        sLog->outString("Loading MapRelations...");
        sMapRelations->LoadNodeMap();
    }
...
void World::RecordTimeDiff(const char *text, ...)
{
    ...
    if (diff > 0)
    {
        ...
        va_end(ap);
        sLog->outPerformance("Difftime %s: %u.", str, diff);
    }
...
void World::Update(uint32 diff)
{
    ...
    //sLog->outPerformance("Update time diff: %u.", m_updateTime);
    ...
    _UpdateGameTime();
    //RecordTimeDiff("_UpdateGameTime");
    ...
    if (m_gameTime > m_NextDailyQuestReset)
    {
        ...
        //RecordTimeDiff("ResetDailyQuests");
    }

    if (m_gameTime > m_NextWeeklyQuestReset)
    {
        ResetWeeklyQuests();
        //RecordTimeDiff("ResetWeeklyQuests");
    }

    if (m_gameTime > m_NextRandomBGReset)
    {
        ResetRandomBG();
        //RecordTimeDiff("ResetRandomBG");
    }
    ...
    if (m_timers[WUPDATE_WEATHERS].Passed())
    {
        ...
        //RecordTimeDiff("WeatherUpdate");
    }

    //Update DiminishingReturn reset timer
    if (m_timers[WUPDATE_DR].Passed())
    {
        m_timers[WUPDATE_DR].Reset();
        m_lastDiminishingReturnReset = getMSTime();
    }

    //Update the MasterSessions
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
    {
        sPoolSessionMgr->Update(diff);
        //RecordTimeDiff("UpdatePoolSessions");
    }
    ...
    if (m_timers[WUPDATE_DELETECHARS].Passed())
    {
        ...
        Player::DeleteOldCharacters();
        //RecordTimeDiff("DeleteOldCharacters");
    }
    ...
    if (m_timers[WUPDATE_EVENTS].Passed())
    {
        ...
        //RecordTimeDiff("GameEventUpdate");
    ...
    if (m_timers[WUPDATE_PINGDB].Passed())
    {
        ...
        //RecordTimeDiff("DatabaseKeepAlive");
    }

    // update the instance reset times
    sInstanceSaveMgr->Update();
    //RecordTimeDiff("InstanceSaveUpdate");
    ...
    //RecordTimeDiff("OnWorldUpdateScripts");
}
...
void World::SendGlobalMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second &&
            ...
            itr->second != self &&
            (team == 0 || itr->second->GetPlayer()->GetTeam(true) == team))
        {
...
void World::SendGlobalGMMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second &&
            ...
            itr->second->GetPlayer()->IsInWorld() &&
            itr->second != self &&
            !AccountMgr::IsVIPorPlayer(itr->second->GetSecurity()) &&
            (team == 0 || itr->second->GetPlayer()->GetTeam(true) == team))
        {
            itr->second->SendPacket(packet);
        }
...
/// Ban an account or ban an IP address, duration will be parsed using TimeStringToSecs if it is positive, otherwise permban
BanReturn World::BanAccountbyId(uint32 accountId, int32 duration, std::string reason, std::string author)
{
    PreparedStatement* stmt = NULL;
...
void World::ShutdownMsg(bool show, Player* player, const std::string& reason)
{
        ...
        (m_ShutdownTimer > 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0)) // > 12 h ; every 12 h
    {
        ...
        if (!reason.empty())
            str += "- [|cffff0000Anmerkung|r] |c1f40af20" + reason + "|r";
        else if (!m_ShutdownReason.empty())
            str += "- [|cffff0000Anmerkung|r] |c1f40af20" + m_ShutdownReason + "|r";
...
void World::_UpdateRealmCharCount(PreparedQueryResult resultCharCount)
{
    if (resultCharCount)
    {
        ...
        {
            PreparedStatement* stmt = LogonDatabase.GetPreparedStatement(LOGON_DEL_REALM_CHARACTERS);
            stmt->setUInt32(0, accountId);
            stmt->setUInt32(1, realmID);        
            LogonDatabase.Execute(stmt);         
            stmt = LogonDatabase.GetPreparedStatement(LOGON_INS_REALM_CHARACTERS);
            stmt->setUInt8(0, charCount);
            stmt->setUInt32(1, accountId);
            stmt->setUInt32(2, realmID);
            LogonDatabase.Execute(stmt); 
...
{
    PreparedStatement* stmt = LogonDatabase.GetPreparedStatement(LOGON_SEL_NODE_SECURITY_LEVEL);
...
bool World::isWorldState(uint32 index) const
{
    WorldStatesMap::const_iterator it = m_worldstates.find(index);
