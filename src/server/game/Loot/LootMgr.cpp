...
LootStoreItem const* LootStore::FindLootStoreItemByEntry(uint32 item_entry) const
{
    for (auto lootTemplate : m_LootTemplates)
    {
        if (lootTemplate.second)
        {
            const LootStoreItemList &list = lootTemplate.second->GetLootStoreItemList();
            auto itr = std::find_if(list.begin(), list.end(),
                                 [item_entry](const LootStoreItem& item) {
                                   return item.itemid == item_entry;
                                 });
...
bool player_in_mixed_group_or_raid(Player const* player) {
...
bool LootItem::AllowedForPlayer(Player const* player, bool mixd_group_check /*= false*/) const
{
    // DB conditions check
    ...
    bool only_allow_alliance = mixd_group_check && player_in_mixed_group_or_raid(player) && sWorld->getBoolConfig(WorldBoolConfigs::CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && player->GetMap()->IsRaidOrHeroicDungeon();

    // not show loot for not own team
    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY) && ((player->GetTeam() != HORDE) || only_allow_alliance))
        return false;

    if ((pProto->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY) && ((player->GetTeam() != ALLIANCE) && !only_allow_alliance))
        return false;
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    tab->Process(*this, store.IsRatesAllowed(), lootMode, restorable_items);
...
QuestItemList* Loot::FillNonQuestNonFFAConditionalLoot(Player* player, bool presentAtLooting)
{
    ...
    {
        LootItem &item = items[i];
        if (!item.is_looted && !item.freeforall && (item.AllowedForPlayer(player, true) || (item.follow_loot_rules && player->GetGroup() && ((player->GetGroup()->GetLootMethod() == MASTER_LOOT && player->GetGroup()->GetLooterGuid() == player->GetGUID()) || player->GetGroup()->GetLootMethod() != MASTER_LOOT ))))
        {
            if (presentAtLooting)
...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
    ...
    switch (lv.permission)
    {
        case GROUP_PERMISSION:
        {
            ...
            {
                if (!l.items[i].is_looted && !l.items[i].freeforall && l.items[i].conditions.empty() && l.items[i].AllowedForPlayer(lv.viewer, true))
                {
                    uint8 slot_type;
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, LootRestoreVector &restoreable_items, uint8 groupId) const
{
    // Restore (only item loots)
    if (!restoreable_items.empty())
    {
        for (auto item_entry : restoreable_items)
        {
            auto lootStoreItem = LootTemplates_Item.FindLootStoreItemByEntry(item_entry);
            if (lootStoreItem)
        ...
        for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
        {
            ...
            if (i->mincountOrRef < 0)                             // References processing
            {
                ...
                for (uint32 loop = 0; loop < maxcount; ++loop)    // Ref multiplicator
                    Referenced->Process(loot, rate, lootMode, restoreable_items, i->group);
            }
