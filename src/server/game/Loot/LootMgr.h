...
struct LootItem
{
    ...
    bool AllowedForPlayer(Player const* player, bool mixed_group_check = false) const;
...
typedef std::vector<uint32> LootRestoreVector;
...
class LootStore
{
    public:
        ...
        LootStoreItem const* FindLootStoreItemByEntry(uint32 item_entry) const;
...
class LootTemplate
{
    ...
    public:
        ...
        void Process(Loot& loot, bool rate, uint16 lootMode, LootRestoreVector& restoreable_items, uint8 groupId = 0) const;
        ...
        LootStoreItemList const& GetLootStoreItemList() const { return Entries; }
...
struct Loot
{
    ...
    LootRestoreVector restorable_items;
    uint32 gold;
    uint8 unlootedCount;
    ...
    // Restore system
    void AddRestorableMoneyLoot(uint32 amount) {
        gold = amount;
        restored = true;
    }

    void AddRestorableItem(uint32 item_entry) {
        restorable_items.push_back(item_entry);
        restored = true;
    }
