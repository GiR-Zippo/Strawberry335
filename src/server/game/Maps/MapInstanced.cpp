...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
    ...
    else
    {
        ...
        if (mapId == 631)
        {
            ...
            //else
            //{
            //    newInstanceId = player->GetMap()->GetInstanceId();
            //}
            ...
            if (map)
            {
                if (map->GetDifficulty() != diff)
                {
                    map = CreateInstance(newInstanceId, pSave ? pSave : NULL, diff);
                }
            }
            else
            {
            ...
            if (player->GetSession()->GetSecurity() >= SEC_MODERATOR)
                if (!player->GetGroup() && pBind && !pBind->perm)
                    player->UnbindInstance(631, pBind->save->GetDifficulty());
            return map;
        }
        if (pSave)
        {
