...
class Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        TempSummon* SummonCreature(uint32 entry, const Position &pos, SummonPropertiesEntry const* properties = NULL, uint32 duration = 0, Unit* summoner = NULL, uint32 spellId = 0, uint32 vehId = 0);
        TempSummon* SummonCreatureOnMap(uint32 entry, const Position &pos, TempSummonType spwtype, uint32 duration);
        GameObject* SummonGameObjectOnMap(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime);
