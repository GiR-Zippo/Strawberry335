...
void Map::LoadMap(int gx, int gy, bool reload)
{
    ...
    if (GridMaps[gx][gy])
    {
        sLog->outDebug(LOG_FILTER_MAPS, "Unloading previously loaded map %u before reloading.", GetId());
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "Loading map %s", tmp);
    // loading data
...
void Map::SwitchGridContainers(GameObject* obj, bool on)
{
    ...
    if (!p.IsCoordValid())
    {
        sLog->outError("Map::SwitchGridContainers: Object %u has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUIDLow(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "Switch object %u from grid[%u, %u] %u", obj->GetGUIDLow(), cell.data.Part.grid_x, cell.data.Part.grid_y, on);
    NGridType *ngrid = getNGrid(cell.GridX(), cell.GridY());
    ASSERT(ngrid != NULL);

    GridType &grid = ngrid->GetGridType(cell.CellX(), cell.CellY());

    if (!obj->IsDynTransport()) // DynTransports are not added to grid
        obj->RemoveFromGrid(); //This step is not really necessary but we want to do ASSERT in remove/add
...
bool Map::AddPlayerToMap(Player* player)
{
    ...
    if (player->IsInGrid())
    {
        ...
        sLog->outError("Map::AddPlayerToMap: !player->IsInGrid()");
    }
...
{
    return VMAP::VMapFactory::createOrGetVMapManager()->isInLineOfSight(GetId(), x1, y1, z1, x2, y2, z2) && _dynamicTree.isInLineOfSight(x1, y1, z1, x2, y2, z2, phasemask);
}
...
void Map::RemoveAllObjectsInRemoveList()
{
    ...
    while (!i_objectsToRemove.empty())
    {
        ...
        case TYPEID_DYNAMICOBJECT:
            ...
            break;
        case TYPEID_GAMEOBJECT:
            if (obj->ToGameObject()->IsTransport())
                RemoveFromMap((Transport*)obj, true);
            else
...
{
    if (player->GetMapRef().getTarget() == this && player->GetMapId() != 631)
    {
                ...
                if (uint32 dungeonId = sLFGMgr->GetDungeon(group->GetGUID(), true))
                    if (LFGDungeonEntry const* dungeon = sLFGDungeonStore.LookupEntry(dungeonId))
                        if (LFGDungeonEntry const* randomDungeon = sLFGDungeonStore.LookupEntry(*(sLFGMgr->GetSelectedDungeons(player->GetGUID()).begin())))
                            if (uint32(dungeon->map) == GetId() && dungeon->difficulty == uint32(GetDifficulty()) && randomDungeon->type == uint32(lfg::LFG_TYPE_RANDOM))
...
GameObject* Map::SummonGameObjectOnMap(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry, this, 0xFF, x, y, z, ang, rotation0, rotation1, rotation2, rotation3, 100, GO_STATE_READY))
    {
        delete go;
        return NULL;
    }
...
TempSummon* Map::SummonCreatureOnMap(uint32 entry, const Position &pos, TempSummonType spwtype, uint32 duration)
{
    if (TempSummon* summon = SummonCreature(entry, pos, NULL, duration, NULL))
    {
