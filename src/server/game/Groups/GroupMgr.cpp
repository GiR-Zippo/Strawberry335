...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    uint32 const deltaValue = sObjectMgr->GetHiRange();
    uint32 const maxValue = 0xFFFFFFFF - deltaValue;

    for (uint32 i = NextGroupDbStoreId + deltaValue; i < maxValue; i += deltaValue)
    {
        if ((i < GroupDbStore.size() && GroupDbStore[i] == NULL) || i >= GroupDbStore.size())
        {
            NextGroupDbStoreId = i;
...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        sLog->outCrash("Group guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
