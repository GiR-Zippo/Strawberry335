...
m_subGroupsCounts(NULL), m_guid(0), m_counter(0), m_maxEnchantingLevel(0), m_dbStoreId(0), m_leader_team(ALLIANCE), m_mixed_group(false)
{
...
bool Group::Create(Player* leader)
{
    ...
    m_leader_team = sObjectMgr->GetPlayerTeamByGUID(leaderGuid);
...
bool Group::AddLeaderInvite(Player* player)
{
    ...
    m_leader_team = sObjectMgr->GetPlayerTeamByGUID(m_leaderGuid);
    return true;
}
...
bool Group::AddMember(Player* player)
{
    ...
    if (!m_mixed_group) {
        if (m_leader_team != sObjectMgr->GetPlayerTeamByGUID(player->GetGUID()))
            m_mixed_group = true;
    }
...
bool Group::RemoveMember(uint64 guid, const RemoveMethod &method /*= GROUP_REMOVEMETHOD_DEFAULT*/, uint64 kicker /*= 0*/, const char* reason /*= NULL*/)
{
    ...
    if (isBFGroup() && player)
        sBattlefieldMgr->HandlePlayerLeaveBfGroup(player, player->GetZoneId());
    ...
    if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
    {
        if (player)
        {
            // Battleground/field group handling
            if (isBGGroup() || isBFGroup())
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (!member || !member->GetSession())
                    continue;
                if (i->AllowedForPlayer(member, true))
                {
    ...
    for (std::vector<LootItem>::iterator i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                bool allowedForPlayer = i->AllowedForPlayer(playerToRoll, true);
        ...
        if (!roll->playerVote.empty())
        {
            ...
            if (player && player->GetSession())
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
                    ...
                    sScriptMgr->OnPlayerLootItem(player, roll->itemid, dest);
