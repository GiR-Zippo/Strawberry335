...
#include "PoolSocketAcceptor.h"
#include "ScriptMgr.h"

/**
* This is a helper class to PoolSocketMgr, that manages
...
class ReactorRunnable : protected ACE_Task_Base
{
    ...
    private:
        ...
        typedef std::set<PoolSocket*> SocketSet;
...
PoolSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
    ...
    m_Acceptor = new PoolSocketAcceptor;
...
int
PoolSocketMgr::StartNetwork (ACE_UINT16 port, const char* address)
{
    if (!sLog->IsOutDebug())
...
int
PoolSocketMgr::OnSocketOpen (PoolSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            sLog->outError ("PoolSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            sLog->outError ("PoolSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
