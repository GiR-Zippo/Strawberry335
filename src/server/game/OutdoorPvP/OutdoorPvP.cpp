...
void OPvPCapturePoint::HandlePlayerLeave(Player* player, uint64 guid)
{
    if (m_capturePoint && player)
...
OutdoorPvP::OutdoorPvP(Map *map) : m_map(map), m_sendUpdate(true)
{
}
...
bool OutdoorPvP::HandleOPvPGroupDisband(Group* /*group*/)
{
    // Placeholder - used by OutdoorPvPWG
    return false;
}

bool OutdoorPvP::HandleOPvPGroupRemoveMember(Player* /*player*/, const RemoveMethod &/*method*/)
{
    // Placeholder - used by OutdoorPvPWG
    return false;
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    {
        for (std::set<uint64>::iterator itr = m_activePlayers[team].begin(); itr != m_activePlayers[team].end(); ++itr)
        {
            if (Player* player = m_PvP->GetMap()->GetPlayer(*itr))
            {
                if (!m_capturePoint->IsWithinDistInMap(player, radius) || !player->IsOutdoorPvPActive())
                    HandlePlayerLeave(player);
            }
            else
                HandlePlayerLeave(NULL, *itr);
        }
    }
...
void OutdoorPvP::SendUpdateWorldState(uint32 field, uint32 value)
{
    if (m_sendUpdate)
        for (int i = 0; i < 2; ++i)
            for (std::set<uint64>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
                if (Player *player = m_map->GetPlayer(*itr))
...
void OutdoorPvP::BroadcastPacket(WorldPacket &data) const
{
    ...
    for (uint32 team = 0; team < 2; ++team)
        for (std::set<uint64>::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
        {
            if (Player *player = m_map->GetPlayer(*itr))
...
void OutdoorPvP::TeamCastSpell(TeamId team, int32 spellId)
{
    if (spellId > 0)
    {
        for (std::set<uint64>::iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
            if (Player *player = m_map->GetPlayer(*itr))
