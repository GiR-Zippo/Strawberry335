...
{
    sendEventMessage(event_id, true);
...
{
    sendEventMessage(event_id, false);
...
{
    {
        ...
        QueryResult result = WorldDatabase.Query("SELECT eventEntry, UNIX_TIMESTAMP(start_time), UNIX_TIMESTAMP(end_time), occurence, length, holiday, description, world_event, start_message, end_message FROM game_event");
        if (!result)
        {
            mGameEvent.clear();
        ...
        do
        {
            ...
            pGameEvent.start_message = fields[8].GetString();
            pGameEvent.end_message = fields[9].GetString();
...
void GameEventMgr::sendEventMessage(uint16 event_id, bool start)
{
    std::string* message;
    GameEventData& data = mGameEvent[event_id];
    if (start)
        message = &data.start_message;
    else
    ...
    if (message && !message->empty())
    {
        ...
        {
            if (auto session = itr->second)
            {
                if (session->GetPlayer())
                {
                    ChatHandler(session).PSendSysMessage(message->c_str());
                    session->SendAreaTriggerMessage(message->c_str());
                }
            }
        }
    }
