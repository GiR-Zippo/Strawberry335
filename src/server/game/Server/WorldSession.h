...
enum MailRequest
{
    MAIL_REQUEST_GET_MAIL_LIST                      = 1,
    MAIL_REQUEST_QUERY_NEXT_MAIL_TIME               = 2,
};
...
class WorldSession
{
    ...
    public:                                                 // opcodes handlers
        ...
        // GM Ticket opcodes <01.04.2015 - TehKevin>: Handling moved to Logon
        ...
        void _LoadMailsFor(MailRequest status);
        void HandleGetMailList(WorldPacket& recv_data);
        void HandleGetMailListCallback();
        ...
        void HandleQueryNextMailTimeCallback();
        ...
        void SendBfExitResponse(uint32 BattleId, uint32 ZoneId);
        void SendBfEntered(uint32 battleId);
        void SendBfLeaveMessage(uint32 battleId, BFLeaveReason reason = BF_LEAVE_REASON_EXITED);
        void SendBfPendingLeaveMessage(uint32 BattleId);
        void SendBfStateChange(uint32 BattleId, uint32 state);
        ...
        void HandleLfgGetStatusOpcode(WorldPacket& recvData);
        ...
        void SendLfgPlayerReward(uint32 rdungeonEntry, uint32 sdungeonEntry, uint8 done, const lfg::LfgReward* reward, const Quest *qRew);
        void SendLfgBootProposalUpdate(lfg::LfgPlayerBoot const* boot);
        void SendLfgUpdateProposal(uint32 proposalId, lfg::LfgProposal const* proposal);
    ...
    private:
        ...
        PreparedQueryResultFuture _requestMailCallback;
        PreparedQueryResultFuture _requestMailItemsCallback;
        uint32 _mailRequestStatus;
