...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    sLog->outDebug (LOG_FILTER_NETWORKIO, "SESSION: OUT opcode %s (0x%.4X) Size: %lu", LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode(), packet->size());
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    _mailRequestStatus = 0;
    _requestMailCallback.cancel();
    _requestMailItemsCallback.cancel();
...
void WorldSession::ProcessQueryCallbacks()
{
    ...
    //! HandleRequestMaol
    if (_requestMailCallback.ready() && _requestMailItemsCallback.ready())
    {
        PreparedQueryResult result2;
        _requestMailCallback.get(result);
        _requestMailItemsCallback.get(result2);
        GetPlayer()->_LoadMails(result, result2);

        if (_mailRequestStatus & (1 << (MAIL_REQUEST_GET_MAIL_LIST - 1)))
            HandleGetMailListCallback();

        if (_mailRequestStatus & (1 << (MAIL_REQUEST_QUERY_NEXT_MAIL_TIME - 1)))
