...
uint32 registerNewGuid(uint32 oldGuid, std::map<uint32, uint32> &guidMap, HighGuid highGuid)
{
    std::map<uint32, uint32>::const_iterator itr = guidMap.find(oldGuid);
    if (itr != guidMap.end())
        return itr->second;

    uint32 newGuid = sObjectMgr->GenerateLowGuid(highGuid);

    guidMap[oldGuid] = newGuid;
    return newGuid;
}

bool changeGuid(std::string &str, int n, std::map<uint32, uint32> &guidMap, HighGuid highGuid, bool nonzero = false)
{
    char chritem[20];
    ...
    uint32 newGuid = registerNewGuid(oldGuid, guidMap, highGuid);
    snprintf(chritem, 20, "%u", newGuid);

    return changenth(str, n, chritem, false, nonzero);
}

uint32 registerNewMailID(uint32 oldGuid, std::map<uint32, uint32> &guidMap)
{
    std::map<uint32, uint32>::const_iterator itr = guidMap.find(oldGuid);
    if (itr != guidMap.end())
        return itr->second;

    uint32 newGuid = sObjectMgr->GenerateMailID();

    guidMap[oldGuid] = newGuid;
    return newGuid;
}

bool changeMailID(std::string &str, int n, std::map<uint32, uint32> &guidMap, bool nonzero = false)
{
    char chritem[20];
    uint32 oldGuid = atoi(getnth(str, n).c_str());
    if (nonzero && oldGuid == 0)
    ...
    uint32 newGuid = registerNewMailID(oldGuid, guidMap);
    snprintf(chritem, 20, "%u", newGuid);
        ...
        */
        ...
        switch (type)
        {
            ...
            case DTT_INVENTORY:
            {
                ...
                if (!changeGuid(line, 2, items, HIGHGUID_ITEM, true))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_inventory.bag update
                if (!changeGuid(line, 4, items, HIGHGUID_ITEM))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_inventory.item update
                break;
            }
            case DTT_MAIL:                                  // mail
            {
                if (!changeMailID(line, 1, mails))
            ...
            case DTT_MAIL_ITEM:                             // mail_items
            {
                ...
                if (!changeGuid(line, 2, items, HIGHGUID_ITEM))
            ...
            case DTT_ITEM:
            {
                ...
                if (!changeGuid(line, 1, items, HIGHGUID_ITEM))
