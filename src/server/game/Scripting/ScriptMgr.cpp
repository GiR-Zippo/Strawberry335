...
void ScriptMgr::OnRelocate(Transport* transport, uint32 waypointId, uint32 mapId, float x, float y, float z, uint32 readId)
{
    GET_SCRIPT(TransportScript, transport->GetScriptId(), tmpscript);
    tmpscript->OnRelocate(transport, waypointId, mapId, x, y, z, readId);
}
...
void ScriptMgr::OnPlayerEquipChanged(Player* player)
{
    FOREACH_SCRIPT(PlayerScript)->OnEquipChanged(player);
}

void ScriptMgr::OnPlayerLootItem(Player* player, uint32& item_entry, ItemPosCountVec& dest)
{
    FOREACH_SCRIPT(PlayerScript)->OnPlayerLootItem(player, item_entry, dest);
}
