...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    float despositPercent = entry->depositPercent; // take real depositPercent

    // Select auctionhouse of stormwind (12), not neutral (horde/alliance auctionshouses have same depositPercent)
    if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
        if (AuctionHouseEntry const* auctionHouseEntry = AuctionHouseMgr::GetAuctionHouseEntry(12))
            despositPercent = auctionHouseEntry->depositPercent;

    float multiplier = CalculatePctN(despositPercent, 3);
...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry* auction, SQLTransaction& trans)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
        ...
        else
        {
            ...
            if (!AccountMgr::IsVIPorPlayer(bidder_security)) // not do redundant DB requests
            {
                if (!sObjectMgr->GetPlayerNameByGUID(bidder_guid, bidder_name))
                    bidder_name = sObjectMgr->GetTrinityStringForDBCLocale(LANG_UNKNOWN);
            }
        }
        if (!AccountMgr::IsVIPorPlayer(bidder_security))
        {
