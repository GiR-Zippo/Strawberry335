...
enum LfgJoinResult
{
    ...
    LFG_JOIN_OVER_GROUP_CAP                      = 19,     // You can not join the dungeon browser queue with more than $MaxJoinSize party members
    LFG_JOIN_OVER_JOIN_CAP                       = 20,     // You have a max cap on Times you can join the Dungeon Browser
    LFG_JOIN_PARTY_OVER_JOIN_CAP                 = 21      // Same as LFG_JOIN_TOO_MUCH, just for group
};
...
/// Invite declined counter state
enum LfgCooldownState
{
    LFG_COOLDOWN_STATE_SINGLE                   = 0, // 5min
    LFG_COOLDOWN_STATE_DOUBLE                   = 1, // 10min
    LFG_COOLDOWN_STATE_MAX                      = 2, // 15min
};
...
typedef std::map<uint32, AreaTrigger*> LfgSeasonMap;
...
typedef std::map<uint64, LfgCooldownState> LfgCooldownStateMap;
...
struct LfgReward
{
    ...
    LfgReward(uint32 _maxLevel = 0, uint32 firstQuest = 0, uint32 otherQuest = 0)
        : maxLevel(_maxLevel)
    {
        questId[0] = firstQuest;
        questId[1] = otherQuest;
    }
};
...
class LFGMgr
{
    friend class ACE_Singleton<lfg::LFGMgr, ACE_Null_Mutex>;
    ...
    public:
        ...
        void InitializeLockedDungeons(Player* player, bool update = false);
        ...
        inline bool ApplyDeserter(void){ return m_Deserter;};
        inline bool ApplyCooldown(void){ return m_Cooldown;};
        ...
        std::string DumpQueueInfo(bool full);
        void Clean();

        bool IsSeasonActive(uint32 dungeonId);

        std::string ConcatenateDungeons(lfg::LfgDungeonSet const& dungeons);
        std::string GetRolesString(uint8 roles);
        std::string GetStateString(lfg::LfgState state);
    ...
    private:
        ...
        // Extended deserter with counter
        void ApplyCooldownExtendedToPlayer(Player* pPlayer);
        void ApplyCooldownExtendedToPlayer(uint64 guid);

        // For extended DB handling
        void SendLfgJoinResultExtended(WorldSession* session, LfgJoinResultData joinData);

        // General variables
        bool m_update;                                     ///< Doing an update?
        bool m_Deserter;                                   ///< Apply Dungeon Deserter
        bool m_Cooldown;                                   ///< Apply Dungeon Cooldown
        ...
        uint32 m_MaxJoinSize;                              ///< Max size of group on initial Queue join
        uint8 m_RndJoinCap;                                ///< Cap on times you can join a random Dungeon per day
        LfgDungeonMap m_CachedDungeonMap;                  ///< Stores all dungeons by groupType
        LfgSeasonMap m_SeasonEntranceMap;                  ///< Stores entrance positions for seasonal events
        // Reward System
        ...
        LfgCooldownStateMap m_CooldownState;               ///< Cooldown counter data
