...
void ObjectMgr::HandleLostMailItems()
{
    sLog->outString("Handling Lost and Orphaned Mail Items");

    QueryResult result = CharacterDatabase.PQuery("SELECT item_guid, receiver FROM mail_items WHERE mail_id NOT IN (select id from mail) AND "
        "((SELECT guid FROM item_instance WHERE guid = item_guid) IS NOT NULL)");
    ...
    if (result)
    {
        do
        {
            ...
            trans->PAppend("UPDATE item_instance SET owner_guid = %u WHERE guid = %u", receiver, itemGUID);
        }
        while (result->NextRow());
    }

    // Cleanup the entries, where the item no longer exists
    trans->Append("DELETE FROM mail_items WHERE mail_id NOT IN (SELECT id FROM mail)");
...
void ObjectMgr::LoadGraveyardZones()
{
    ...
    count = 0;
    //                                   0   1      2  3  4      5             6        7
    result = WorldDatabase.Query("SELECT id, mapid, x, y, z, linkedmapid, linkedteam, revive FROM game_graveyards");
    if (!result)
    {
        sLog->outString(">> Loaded 0 graveyards. DB table `game_graveyards` is empty.");
        sLog->outString();
        return;
    }
    do {
        ...
        WorldSafeLocsEntry* entry = new WorldSafeLocsEntry();
        entry->ID = row[0].GetUInt32() + 10000;
        entry->map_id = row[1].GetUInt32();
        entry->x = row[2].GetFloat();
        entry->y = row[3].GetFloat();
        entry->z = row[4].GetFloat();
        uint32 instancemap = row[5].GetUInt32();
        uint32 team = row[6].GetUInt32();
        uint32 realteam;
        if (team == 1)
            realteam = HORDE;
        else if (team == 2)
        ...
        {
            sLog->outErrorDb("Table `game_graveyards` has a wrong value for team, skipped.");
            continue;
        }
        CustomGraveYardData data;
        data.entry = entry;
        data.team = realteam;
        data.revive = row[7].GetBool();
        _customWorldSafeLocs.insert(WorldSafeLocsContainer::value_type(instancemap, data));
    } while (result->NextRow());
    sLog->outString(">> Loaded %u custom graveyards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}

bool ObjectMgr::GetCustomGraveyardMustRevive(uint32 MapId, uint32 team)
{
    WorldSafeLocsContainer::const_iterator customgraveLow  = _customWorldSafeLocs.lower_bound(MapId);
    WorldSafeLocsContainer::const_iterator customgraveUp   = _customWorldSafeLocs.upper_bound(MapId);
    if (customgraveLow != customgraveUp)
    {
        for (WorldSafeLocsContainer::const_iterator itr = customgraveLow; itr != customgraveUp; ++itr)
        {
            if (itr->second.team == 0 || team == 0 || itr->second.team == team)
                return itr->second.revive;
        }
    }
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
    ...
    //     then check faction
    GraveYardContainer::const_iterator graveLow  = GraveYardStore.lower_bound(zoneId ? zoneId : MapId);
    GraveYardContainer::const_iterator graveUp   = GraveYardStore.upper_bound(zoneId ? zoneId : MapId);
...
void ObjectMgr::SetHighestGuids()
{
    ...
    if (result)
    {
        ...
        sLog->outError("ObjectMgr::SetHighestGuids _hiCreatureGuid:%u", _hiCreatureGuid);
    }
...
uint32 ObjectMgr::GenerateMailIDWhenNode()
{
    if (_mailId >= 0xFFFFFFFE)
    {
        sLog->outError("Mail ids overflow!! Can't continue, shutting down server. ");
    ...
    _mailId += _hiRange;
    sPoolSessionMgr->SendGUIDSync(ENTITYID_MAIL, _mailId);
    return _mailId;
}
...
{
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
        return GenerateLowGuidWhenNode(guidhigh);
    ...
    switch (guidhigh)
    {
        ...
        case HIGHGUID_UNIT:
        {
            ...
            sLog->outError("ObjectMgr::GenerateLowGuid _hiCreatureGuid:%u", _hiCreatureGuid);
        ...
        {
            ASSERT(_hiCharGuid < 0xFFFFFFFE && "Players guid overflow!");
            return _hiCharGuid++;
        }
...
uint32 ObjectMgr::GenerateLowGuidWhenNode(HighGuid guidhigh)
{
    switch (guidhigh)
    {
        case HIGHGUID_ITEM:
        {
            ASSERT(_hiItemGuid < 0xFFFFFFFE && "Item guid overflow!");
            _hiItemGuid += _hiRange;
            sPoolSessionMgr->SendGUIDSync(ENTITYID_ITEM, _hiItemGuid);
            return _hiItemGuid;
        }
        case HIGHGUID_UNIT:
        {
        ...
        {
            ASSERT(_hiCorpseGuid < 0xFFFFFFFE && "Corpse guid overflow!");
            _hiCorpseGuid += _hiRange;
            sPoolSessionMgr->SendGUIDSync(ENTITYID_CORPSE, _hiCorpseGuid);
            return _hiCorpseGuid;
        }
        case HIGHGUID_DYNAMICOBJECT:
        {
