...
class ObjectMgr
{
    ...
    public:
        ...
        bool GetCustomGraveyardMustRevive(uint32 MapId, uint32 team);
        ...
        void HandleLostMailItems();

        CreatureBaseStats const* GetCreatureBaseStats(uint8 level, uint8 unitClass);

        void SetHighestGuids();
        uint32 GenerateLowGuid(HighGuid guidhigh);
        uint32 GenerateLowGuidWhenNode(HighGuid guidhigh);
        uint32 GenerateAuctionID();
        uint64 GenerateEquipmentSetGuid();
        uint32 GenerateMailID();
        uint32 GenerateMailIDWhenNode();
        ...
        uint32 GetHiRange() { return _hiRange; }
    private:
        ...
        typedef std::multimap<uint32, CustomGraveYardData> WorldSafeLocsContainer;
        WorldSafeLocsContainer _customWorldSafeLocs;
