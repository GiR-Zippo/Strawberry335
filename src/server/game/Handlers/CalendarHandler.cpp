...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
    ...
    // @todo - possible lag fix
    sCalendarMgr->SendCalendarCommandResult(_player->GetGUID(), CALENDAR_ERROR_SYSTEM_DISABLED);
}
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& recvData)
{
    ...
    if (instanceBind->save->GetExtendedState() == EXTENDED_STATE_DONE)
        return;

    if (toggleExtend && (instanceBind->save->CanExtend() || instanceBind->save->GetExtendedState() == EXTENDED_STATE_NEEDED))
    {
        instanceBind->SetExtended(EXTENDED_STATE_NEEDED, GetPlayer()->GetGUIDLow());
        instanceBind->save->SetNeedExtendedState();
    }
    else if (!toggleExtend)
        instanceBind->SetExtended(EXTENDED_STATE_NONE, GetPlayer()->GetGUIDLow());
}
