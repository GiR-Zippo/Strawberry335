...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    {
        sLog->outError("Auctionhouse (itemsCount > MAX_AUCTION_ITEMS) : Player: %lu itemsCount: %u ", auctioneer, itemsCount);
    ...
    std::map<uint64, uint32> item_count_map;

    for (uint32 i = 0; i < itemsCount; ++i)
    {
        auto item_guid = itemGUIDs[i];

        sLog->outError("Auctionhouse item %u count %u %s", itemGUIDs[i], count[i], GetPlayerInfo().c_str());

        if (item_guid)
        {
            auto &map_entry = item_count_map[item_guid];

            if (map_entry > 0)
            {
                sLog->outError("Possible Auction House Hacker %s", GetPlayerInfo().c_str());
                SendAuctionCommandResult(0, AUCTION_SELL_ITEM, AUCTION_INTERNAL_ERROR);
                recvData.rfinish();
                return;
            }
    ...
    {
        sLog->outError("WORLD: HandleAuctionSellItem - Player %s (GUID %u) attempted to sell item with higher price than max gold amount.", _player->GetName(), _player->GetGUIDLow());
        SendAuctionCommandResult(0, AUCTION_SELL_ITEM, AUCTION_INTERNAL_ERROR);
        return;
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        {
            sLog->outError("Auctionhouse various Error: Player: %u", auctioneer);
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, AUCTION_INTERNAL_ERROR);
            return;
        }
    ...
    if (!finalCount)
    {
        sLog->outError("Auctionhouse (finalCountError) : Player: %lu finalCount: %u ", auctioneer, finalCount);
        SendAuctionCommandResult(0, AUCTION_SELL_ITEM, AUCTION_INTERNAL_ERROR);
        return;
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        {
            sLog->outError("Auctionhouse (item->GetMaxStackCount() < finalCount) : Player: %lu item->GetMaxStackCount(): %u finalCount: %u ", auctioneer, item->GetMaxStackCount(), finalCount);
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, AUCTION_INTERNAL_ERROR);
            return;
        }
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        if (itemsCount == 1 && item->GetCount() == count[i])
        {
            ...
            _player->AddTransaction(trans);
            item->DeleteFromInventoryDB(trans, _player->GetGUIDLow());
            item->SaveToDB(trans);
            AH->SaveToDB(trans);
        ...
        else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
        {
            ...
            for (uint32 j = 0; j < itemsCount; ++j)
            {
                ...
                if (item2->GetCount() == count[j])
                {
                    ...
                    SQLTransaction trans = CharacterDatabase.BeginTransaction();
                    item2->DeleteFromInventoryDB(trans, _player->GetGUIDLow());
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket & recvData)
{
    ...
    else
    {
        ...
        uint32 itemEntry = auction->item_template;
