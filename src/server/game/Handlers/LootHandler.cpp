...
void WorldSession::DoLootRelease(uint64 lguid)
{
    ...
    else if (IS_ITEM_GUID(lguid))
    {
        ...
        else if (pItem->loot.isLooted()) // Destroy item if nothing left to loot
            player->DestroyItem(pItem->GetBagSlot(), pItem->GetSlot(), true);
        else if (pItem->GetState() != ITEM_NEW)// Save item loot if item is not new and anything is left
            pItem->SaveItemLootToDB();
