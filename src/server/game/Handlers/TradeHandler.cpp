    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                ...
                if (!AccountMgr::IsVIPorPlayer(_player->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
            ...
            if (hisItems[i])
            {
                ...
                if (!AccountMgr::IsVIPorPlayer(trader->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    uint32 playerSecurity = 0;
    uint32 traderSecurity = 0;

    if (WorldSession *session = _player->GetSession())
        playerSecurity = session->GetSecurity();
    if (WorldSession *session = trader->GetSession())
        traderSecurity = session->GetSecurity();

    if ((AccountMgr::IsVIPorPlayer(playerSecurity) && !AccountMgr::IsVIPorPlayer(traderSecurity) && traderSecurity < 4) ||
        (!AccountMgr::IsVIPorPlayer(playerSecurity) && AccountMgr::IsVIPorPlayer(traderSecurity) && playerSecurity < 4))
    {
    ...
    if (his_trade->IsAccepted())
    {
        ...
        {
            if (!AccountMgr::IsVIPorPlayer(_player->GetSession()->GetSecurity()) && my_trade->GetMoney() > 0)
            {
            ...
            if (!AccountMgr::IsVIPorPlayer(trader->GetSession()->GetSecurity()) && his_trade->GetMoney() > 0)
            {
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    // Disable two side trade in battleground if crossfaction
    if (GetPlayer()->InBattleground() && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS)
        && GetPlayer()->GetTeam(true) != pOther->GetTeam(true))
    {
