...
void BuildPlayerLockDungeonBlock(WorldPacket& data, const lfg::LfgLockMap& lock)
{
...
void WorldSession::HandleLfgJoinOpcode(WorldPacket& recv_data)
{
    ...
    if (!sLFGMgr->isOptionEnabled(lfg::LFG_OPTION_ENABLE_DUNGEON_FINDER) && !sLFGMgr->isOptionEnabled(lfg::LFG_OPTION_ENABLE_RAID_BROWSER))
    {
        SendLfgDisabled();
        recv_data.rfinish();
        return;
...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(WorldPacket& /*recv_data*/)
{
    ...
    for (uint32 i = 0; i < sLFGDungeonStore.GetNumRows(); ++i)
    {
        if(LFGDungeonEntry const* dungeon = sLFGDungeonStore.LookupEntry(i))
            if ((dungeon->type == lfg::LFG_TYPE_RANDOM || (dungeon->grouptype == lfg::LFG_GROUP_TYPE_SEASONAL && sLFGMgr->IsSeasonActive(dungeon->ID)))
                && dungeon->expansion <= expansion && dungeon->minlevel <= level && level <= dungeon->maxlevel)
                randomDungeons.insert(dungeon->Entry());
    }

    // Get player locked Dungeons
    lfg::LfgLockMap lock = sLFGMgr->GetLockedDungeons(guid);
    ...
    for (lfg::LfgDungeonSet::const_iterator it = randomDungeons.begin(); it != randomDungeons.end(); ++it)
    {
        ...
        if (reward)
        {
            qRew = sObjectMgr->GetQuestTemplate(reward->questId[0]);
            if (qRew)
            {
                done = !GetPlayer()->CanRewardQuest(qRew, false);
                if (done)
                    qRew = sObjectMgr->GetQuestTemplate(reward->questId[1]);
            }
        }
        if (qRew)
        {
...
void WorldSession::SendLfgUpdatePlayer(const lfg::LfgUpdateData& updateData)
{
    bool queued = false;
    ...
    {
        case lfg::LFG_UPDATETYPE_JOIN_PROPOSAL:
...
void WorldSession::SendLfgUpdateParty(const lfg::LfgUpdateData& updateData)
{
    ...
    switch (updateData.updateType)
    {
        ...
        case lfg::LFG_UPDATETYPE_ADDED_TO_QUEUE:
            ...
            queued = true;
            break;
        case lfg::LFG_UPDATETYPE_CLEAR_LOCK_LIST:
...
void WorldSession::SendLfgRoleCheckUpdate(const lfg::LfgRoleCheck* pRoleCheck)
{
    ASSERT(pRoleCheck);
    ...
    data << uint8(pRoleCheck->state == lfg::LFG_ROLECHECK_INITIALITING);
    ...
    if (!pRoleCheck->roles.empty())
    {
        ...
        for (lfg::LfgRolesMap::const_iterator it = pRoleCheck->roles.begin(); it != pRoleCheck->roles.end(); ++it)
        {
...
void WorldSession::SendLfgJoinResult(const lfg::LfgJoinResultData& joinData)
{
    uint32 size = 0;
...
void WorldSession::SendLfgPlayerReward(uint32 rdungeonEntry, uint32 sdungeonEntry, uint8 done, const lfg::LfgReward* /*reward*/, const Quest* qRew)
{
...
void WorldSession::SendLfgBootProposalUpdate(const lfg::LfgPlayerBoot* pBoot)
{
    uint64 guid = GetPlayer()->GetGUID();
    lfg::LfgAnswer playerVote = pBoot->votes.find(guid)->second;
    uint8 votesNum = 0;
    uint8 agreeNum = 0;
    uint32 secsleft = uint8((pBoot->cancelTime - time(NULL)) / 1000);
    for (lfg::LfgAnswerMap::const_iterator it = pBoot->votes.begin(); it != pBoot->votes.end(); ++it)
    {
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "SMSG_LFG_BOOT_PROPOSAL_UPDATE [" UI64FMTD "] inProgress: %u - didVote: %u - agree: %u - victim: [" UI64FMTD "] votes: %u - agrees: %u - left: %u - needed: %u - reason %s",
        guid, uint8(pBoot->inProgress), uint8(playerVote != lfg::LFG_ANSWER_PENDING), uint8(playerVote == lfg::LFG_ANSWER_AGREE), pBoot->victim, votesNum, agreeNum, secsleft, pBoot->votedNeeded, pBoot->reason.c_str());
...
void WorldSession::SendLfgUpdateProposal(uint32 proposalId, const lfg::LfgProposal* pProp)
{
    if (!pProp)
        return;

    uint64 guid = GetPlayer()->GetGUID();
    lfg::LfgProposalPlayerMap::const_iterator itPlayer = pProp->players.find(guid);
    if (itPlayer == pProp->players.end())                  // Player MUST be in the proposal
        return;

    lfg::LfgProposalPlayer* ppPlayer = itPlayer->second;
    ...
    {
        uint64 gguid = grp->GetGUID();
        isContinue = grp->isLFGGroup() && sLFGMgr->GetState(gguid) != lfg::LFG_STATE_FINISHED_DUNGEON;
    ...
    {
        lfg::LfgDungeonSet playerDungeons = sLFGMgr->GetSelectedDungeons(guid);
    ...
    for (itPlayer = pProp->players.begin(); itPlayer != pProp->players.end(); ++itPlayer)
    {
        ...
        data << uint8(ppPlayer->accept != lfg::LFG_ANSWER_PENDING); // Answered
        data << uint8(ppPlayer->accept == lfg::LFG_ANSWER_AGREE); // Accepted
    }
    SendPacket(&data);
}
