...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    if (items_count > 0 || money > 0)
    {
        if (items_count > 0)
        {
            ...
            {
                Item* item = items[i];
                if (!AccountMgr::IsVIPorPlayer(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
                ...
                item->DeleteFromInventoryDB(trans, player->GetGUIDLow());     // deletes item from character's inventory
        ...
        if (money > 0 && !AccountMgr::IsVIPorPlayer(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
        {
...
void WorldSession::HandleMailDelete(WorldPacket & recv_data)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) > NODE_TYPE_MASTER)
    {
        GetPlayer()->SendMailResult(mailId, MAIL_DELETED, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
...
void WorldSession::HandleMailTakeItem(WorldPacket & recv_data)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) > NODE_TYPE_MASTER)
    {
        GetPlayer()->SendMailResult(mailId, MAIL_ITEM_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
...
void WorldSession::HandleMailTakeMoney(WorldPacket & recv_data)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) > NODE_TYPE_MASTER)
    {
        GetPlayer()->SendMailResult(mailId, MAIL_MONEY_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
...
void WorldSession::_LoadMailsFor(MailRequest request)
{
    if (!GetPlayer())
        return;

    bool in_progress = _mailRequestStatus != 0;

    _mailRequestStatus |= 1 << (request - 1);
    ...
    _requestMailCallback = CharacterDatabase.AsyncQuery(stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAILITEMS_BY_RECEIVER);
    stmt->setUInt32(0, GetPlayer()->GetGUIDLow());

    _requestMailItemsCallback = CharacterDatabase.AsyncQuery(stmt);
}
...
void WorldSession::HandleGetMailList(WorldPacket & recv_data)
{
    ...
    if (!_player->m_mailsLoaded)
        _LoadMailsFor(MAIL_REQUEST_GET_MAIL_LIST);
    else
        HandleGetMailListCallback();
}

void WorldSession::HandleGetMailListCallback()
{
    Player* player = _player;
...
void WorldSession::HandleMailCreateTextItem(WorldPacket & recv_data)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) > NODE_TYPE_MASTER)
    {
        GetPlayer()->SendMailResult(mailId, MAIL_MADE_PERMANENT, MAIL_ERR_INTERNAL_ERROR);
        return;
    }
...
void WorldSession::HandleQueryNextMailTime(WorldPacket & /*recv_data*/)
{
    ...
    if (!_player->m_mailsLoaded)
        _LoadMailsFor(MAIL_REQUEST_QUERY_NEXT_MAIL_TIME);
    else
        HandleQueryNextMailTimeCallback();
}

void WorldSession::HandleQueryNextMailTimeCallback()
{
