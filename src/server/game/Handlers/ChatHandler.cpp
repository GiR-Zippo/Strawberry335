...
bool WorldSession::processChatmessageFurtherAfterSecurityChecks(std::string& msg, uint32 lang)
{
    if (lang != LANG_ADDON)
    {
        ...
        if (sWorld->getIntConfig(CONFIG_CHAT_STRICT_LINK_CHECKING_SEVERITY) && AccountMgr::IsVIPorPlayer(GetSecurity())
            ...
            else
            {
                switch (type)
                {
                    ...
                    case CHAT_MSG_BATTLEGROUND_LEADER:
                        // BG Crossfaction chat handling
                        if (sender->InBattleground() && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS))
                            lang = LANG_UNIVERSAL;
                        break;
                    case CHAT_MSG_PARTY:
