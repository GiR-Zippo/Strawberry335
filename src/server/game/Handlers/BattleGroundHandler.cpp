...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recv_data)
{
    ...
    switch (action)
    {
        case 1:                                         // port to battleground
            ...
            if (Item* pItem = _player->GetNotPermittedItemInArena())
                _player->RemoveItemBonus(pItem);

            // set the destination instance id
            _player->SetBattlegroundId(bg->GetInstanceID(), bgTypeId);

            // BG Crossfaction handling
            if (ginfo.ArenaType == 0 && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_BATTLEGROUNDS))
            {
                if (bg->GetPlayersCountByTeam(ALLIANCE) > bg->GetPlayersCountByTeam(HORDE)) // More alliance players
                    ginfo.Team = HORDE;
                else if (bg->GetPlayersCountByTeam(ALLIANCE) < bg->GetPlayersCountByTeam(HORDE)) // More horde players
                    ginfo.Team = ALLIANCE;
                else // Horde and alliance players are equal
                    ginfo.Team = urand(0, 1) ? HORDE : ALLIANCE;
            }
