...
bool LoginQueryHolder::Initialize()
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_TRANSMOG_UNLOCKED);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_TRANSMOGUNLOCKED, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_TRANSMOG_ACTIVE);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_TRANSMOGACTIVE, stmt);

    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_TRANSMOG_HISTORY);
    stmt->setUInt32(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_TRANSMOGHISTORY, stmt);
...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (AccountMgr::IsVIPorPlayer(GetSecurity()) && sObjectMgr->IsReservedName(name))
    {
    ...
    if (heroic_free_slots == 0 && AccountMgr::IsVIPorPlayer(GetSecurity()) && class_ == CLASS_DEATH_KNIGHT)
    {
    ...
    if (AccountMgr::IsVIPorPlayer(GetSecurity()) && class_ == CLASS_DEATH_KNIGHT && req_level_for_heroic > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
    ...
    */
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 2:
        {
            ...
            bool allowTwoSideAccounts = !sWorld->IsPvPRealm() || sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) || !AccountMgr::IsVIPorPlayer(GetSecurity());
        ...
        case 3:
        {
            ...
            if (result)
            {
                ...
                if (AccountMgr::IsVIPorPlayer(GetSecurity()) && createInfo->Class == CLASS_DEATH_KNIGHT)
                {
            ...
            if (AccountMgr::IsVIPorPlayer(GetSecurity()) && createInfo->Class == CLASS_DEATH_KNIGHT && !hasHeroicReqLevel)
            {
                WorldPacket data(SMSG_CHAR_CREATE, 1);
                data << uint8(CHAR_CREATE_LEVEL_REQUIREMENT);
                SendPacket(&data);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket & recv_data)
{
    ...
    if ((sWorld->getIntConfig(CONFIG_CORE_TYPE) == NODE_TYPE_SINGLE) && !CharCanLogin(GUID_LOPART(playerGuid)))
    {
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    // Send MOTD
    {
        ...
        if (sWorld->getIntConfig(CONFIG_ENABLE_SINFO_LOGIN) == 1)
            chH.PSendSysMessage("%s%s%s", _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B);
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recv_data)
{
    ...
    // check name limitations
    if (AccountMgr::IsVIPorPlayer(GetSecurity()) && sObjectMgr->IsReservedName(newName))
    {
        WorldPacket data(SMSG_CHAR_RENAME, 1);
        data << uint8(CHAR_NAME_RESERVED);
        SendPacket(&data);
