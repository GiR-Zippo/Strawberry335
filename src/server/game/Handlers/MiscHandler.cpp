...
void WorldSession::HandleWhoOpcode(WorldPacket & recv_data)
{
    ...
    {
        if (AccountMgr::IsPlayerAccount(security) || AccountMgr::IsVIPAccount(security))
        {
        ...
        for (uint32 i = 0; i < zones_count; ++i)
        {
            // <TehKevin> If player is in arena, dont show with zone filter
            if (zoneids[i] == pzoneid && (!itr->second->InArena() || !AccountMgr::IsVIPorPlayer(GetSecurity())))
            {
                z_show = true;
                break;
            }
        ...
        for (uint32 i = 0; i < str_count; ++i)
        {
            if (!str[i].empty())
            {
                // <TehKevin> If player is in arena, dont show with string filter
                if (wgname.find(str[i]) != std::wstring::npos ||
                    wpname.find(str[i]) != std::wstring::npos ||
                    (Utf8FitTo(aname, str[i]) && (!itr->second->InArena() || !AccountMgr::IsVIPorPlayer(GetSecurity()))))
                {
                    s_show = true;
                    break;
                }
        ...
        // <TehKevin> If player is in arena, send dalaran
        if (itr->second->InArena() && AccountMgr::IsVIPorPlayer(GetSecurity()))
            pzoneid = 4395;
...
void WorldSession::HandleAddFriendOpcodeCallBack(PreparedQueryResult result, std::string friendNote)
{
    ...
    if (result)
    {
        ...
        if (!AccountMgr::IsVIPorPlayer(GetSecurity()) || sWorld->getBoolConfig(CONFIG_ALLOW_GM_FRIEND) || AccountMgr::IsVIPorPlayer(AccountMgr::GetSecurity(friendAccountId, realmID)))
        {
            if (friendGuid)
            {
                if (friendGuid == GetPlayer()->GetGUID())
                    friendResult = FRIEND_SELF;
                else if (GetPlayer()->GetTeam() != team && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ADD_FRIEND) && AccountMgr::IsVIPorPlayer(GetSecurity()) && !AccountMgr::IsVIPAccount(GetSecurity()))
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket & recv_data)
{
    ...
    // ICC is special, so we handle it seperately
    if (map->GetId() == 631)
    {
        if (!group)
        {
        ...
        if (!group->IsLeader(_player->GetGUID()))
        {
            group->SetRaidDifficulty(_player->GetRaidDifficulty());
            return; // Only Leader can switch mode
        }

        if ((map->Is25ManRaid() && !_player->HasAchieved(4597)) || (!map->Is25ManRaid() && !_player->HasAchieved(4530)))
        {
            _player->GetSession()->SendAreaTriggerMessage(_player->GetSession()->GetTrinityString(LANG_MISSING_ACHIEVEMENT_ICC));
            group->SetRaidDifficulty(_player->GetRaidDifficulty());
            return; // Player can only switch mode if he has the Kingslayer-Achievement
        }

        if (!_player->Satisfy(sObjectMgr->GetAccessRequirement(631, Difficulty(mode)), 631, true))
            return;
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if (groupGuy->GetMapId() != 631)
            {
                if (groupGuy->GetMap()->IsRaid() || groupGuy->GetMap()->IsDungeon())
                {
                    _player->SendResetInstanceFailed(0, 631); // Msg: There are players in the instance.
                    return; // Cant switch difficulty if players in the group are in a different raid or instance
                }
                continue;
            } else {
                ...
                {
                    group->SetRaidDifficulty(_player->GetRaidDifficulty());
                    return; // Cant switch if player in instance is infight
                }
            }
        }

        // All lights are green, beam me up, Scotty!

        for (const MapReference* itr = map->GetPlayers().getFirst(); itr != NULL; itr = itr->next())
        {
            Player* mapGuy = itr->getSource();
            ...
            if (!mapGuy->GetGroup())
            {
                if (mapGuy->GetSession()->GetSecurity() >= SEC_MODERATOR)
                {
                    if (group->GetBoundInstance(mapGuy->GetMap()))
                    {
                        mapGuy->BindToInstance(group->GetBoundInstance(mapGuy->GetMap())->save, group->GetBoundInstance(mapGuy->GetMap())->perm);
                        mapGuy->ResetInstances(INSTANCE_RESET_CHANGE_DIFFICULTY, true);
                        mapGuy->SetRaidDifficulty(Difficulty(mode));
                        mapGuy->SendRaidDifficulty(false, Difficulty(mode));
                    }
                }
                else
                {
                    mapGuy->RepopAtGraveyard();
                    continue;
                }
            }

            mapGuy->TeleportTo(*mapGuy, 0, true);
        ...
        for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* groupGuy = itr->getSource())
            {
                if (groupGuy->isDead())
                {
                    if (Corpse* corp = groupGuy->GetCorpse())
                    {
                        corp->SaveToDB();
                        corp->RemoveFromWorld();
                        corp->ResetMap();
                    }
                    else
                        continue;
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    _player->SendResetInstanceFailed(0, 631);
                    sLog->outError("WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while player %d inside!", _player->GetGUIDLow(), groupGuy->GetGUIDLow());
                    return;
                }
            }
            // the difficulty is set even if the instances can't be reset
...
void WorldSession::HandleAreaSpiritHealerQueueOpcode(WorldPacket & recv_data)
{
    ...
    if (!pCreature->isSpiritService())  // it's not spirit service
        return;

    if (Battleground* pBG = _player->GetBattleground())
        pBG->AddPlayerToResurrectQueue(guid, _player->GetGUID());
    else if (Battlefield* pBF = sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetZoneId()))
        pBF->AddPlayerToResurrectQueue(guid, _player->GetGUID());
}
...
void WorldSession::HandleHearthAndResurrect(WorldPacket& /*recv_data*/)
{
    ...
    if (!sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetZoneId()))
        return;

    // Currently only for wintergrasp
