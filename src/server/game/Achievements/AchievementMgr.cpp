...
void AchievementMgr::ResetAchievementCriteria(AchievementCriteriaTypes type, uint32 miscvalue1, uint32 miscvalue2, bool evenIfCriteriaComplete)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
    {
        ResetAchievementCriteriaAddition(type, miscvalue1, miscvalue2, evenIfCriteriaComplete);
        return;
    }
...
void AchievementMgr::CompletedCriteriaFor(AchievementEntry const* achievement)
{
    ...
    // Crossfaction BG achievement faction check
    uint8 team = m_player->GetTeam(true) == ALLIANCE ? 1 : 0;
    if (achievement->requiredFaction != -1 && achievement->requiredFaction != team)
        return;
...
void AchievementMgr::SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint32 changeValue, ProgressType ptype)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
    {
        SetCriteriaProgressAddition(entry,changeValue,ptype);
        return;
    }
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) != NODE_TYPE_SINGLE)
    {
        CompletedAchievementAddition(achievement);
        return;
    }
    ...
    if (uint32 titleId = reward->titleId[achievement->ID == 1793 ? GetPlayer()->getGender() : (GetPlayer()->GetTeam(true) == ALLIANCE ? 0 : 1)])
        if (CharTitlesEntry const* titleEntry = sCharTitlesStore.LookupEntry(titleId))
            GetPlayer()->SetTitle(titleEntry);
