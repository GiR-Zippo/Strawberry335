...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_OBJECT_ENTRY_GUID:
        {
            switch (cond->ConditionValue1)
            {
                case TYPEID_UNIT:
                    ...
                    {
                        sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has non existing creature template entry (%u), skipped", cond->ConditionValue2);
                        return false;
                    }
                    if (cond->ConditionValue3)
                    {
                        if (CreatureData const* creatureData = sObjectMgr->GetCreatureData(cond->ConditionValue3))
                        {
                            ...
                            {
                                sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has guid %u set but does not match creature entry (%u), skipped", cond->ConditionValue3, cond->ConditionValue2);
                                return false;
                            }
                        }
                        else
                        {
                            sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has non existing creature guid (%u), skipped", cond->ConditionValue3);
                            return false;
                        }
                    }
                    break;
                case TYPEID_GAMEOBJECT:
                    ...
                    {
                        sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has non existing gameobject template entry (%u), skipped", cond->ConditionValue2);
                        return false;
                    }
                    if (cond->ConditionValue3)
                    {
                        if (GameObjectData const* goData = sObjectMgr->GetGOData(cond->ConditionValue3))
                        {
                            ...
                            {
                                sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has guid %u set but does not match gameobject entry (%u), skipped", cond->ConditionValue3, cond->ConditionValue2);
                                return false;
                            }
                        }
                        else
                        {
                            sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has non existing gameobject guid (%u), skipped", cond->ConditionValue3);
                            return false;
                        }
                    }
                    break;
                ...
                case TYPEID_CORPSE:
                    if (cond->ConditionValue2)
                        sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has useless data in value2 (%u)!", cond->ConditionValue2);
                    if (cond->ConditionValue3)
                        sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has useless data in value3 (%u)!", cond->ConditionValue3);
                    break;
                default:
                    sLog->outErrorDb("sql.sql", "ObjectEntryGuid condition has wrong typeid set (%u), skipped", cond->ConditionValue1);
                    return false;
            }
            break;
        }
        ...
        case CONDITION_WORLD_STATE:
        {
            if (!sWorld->isWorldState(cond->ConditionValue1))
            {
