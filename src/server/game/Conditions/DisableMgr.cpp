        ...
        case DISABLE_TYPE_MAP:
            if (Player const* player = unit->ToPlayer())
            {
                ...
                if (mapEntry->IsDungeon())
                {
                    ...
                    uint8 tempdiff = player->GetDifficulty(mapEntry->IsRaid());
                    if (player->GetMapId() == mapEntry->MapID)
                    {
                        if (tempdiff > RAID_DIFFICULTY_25MAN_NORMAL)
                            tempdiff -= 2;
                        else
                            tempdiff += 2;
                    }
                    Difficulty targetDifficulty = Difficulty(tempdiff);
...
{
    return sWorld->getBoolConfig(CONFIG_ENABLE_MMAPS) && !IsDisabledFor(DISABLE_TYPE_MMAP, mapId, NULL, MMAP_DISABLE_PATHFINDING);
}*/
} // Namespace