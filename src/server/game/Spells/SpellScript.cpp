...
int32 SpellScript::GetCleanDamage()
{
    if (!IsInTargetHook())
    {
        sLog->outError("TSCR: Script: `%s` Spell: `%u`: function SpellScript::GetCleanDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
    if (!m_spell->m_cleanDamage)
        return m_spell->m_damage;
    return m_spell->m_cleanDamage;
}
