...
uint64 SpellCastTargets::GetOrigUnitTargetGUID() const
{
    switch (GUID_HIPART(m_origObjectTargetGUID))
    {
...
, m_spellValue(new SpellValue(m_spellInfo))
{
    ...
    // Hackfix Death grip reflected @todo a better solution
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && m_spellInfo->SpellFamilyFlags[0] == 0x2000000)
        m_canReflect = m_spellInfo->Effects[0].Mechanic == MECHANIC_GRIP;
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    // for compability with older code - add only unit and go targets
    // TODO: remove this
    bool skipLOS = (m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS || DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS));

    for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
    {
        if (skipLOS || (*itr)->IsWithinLOS(center->GetPositionX(), center->GetPositionY(), center->GetPositionZ()))
        {
            if (Unit* unitTarget = (*itr)->ToUnit())
                unitTargets.push_back(unitTarget);
            else if (GameObject* gObjTarget = (*itr)->ToGameObject())
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
            ...
            m_targets.GetUnitTargetGUID() == target->targetGUID)
    {
        ...
        if (missInfo == SPELL_MISS_MISS || missInfo == SPELL_MISS_DODGE || missInfo == SPELL_MISS_PARRY)
            if (Player *modOwner = m_caster->GetSpellModOwner())
                modOwner->RestoreSpellMods(this, 14177);
    }
    ...
    if (m_healing > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, addhealth, m_healing, m_attackType, m_spellInfo, m_triggeredByAuraSpell);
    }
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
        {
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, damageInfo.damage, (damageInfo.damage + damageInfo.absorb + damageInfo.resist), m_attackType, m_spellInfo, m_triggeredByAuraSpell);
            if (caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->Attributes & SPELL_ATTR0_STOP_ATTACK_TARGET) == 0 &&
               (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
                caster->ToPlayer()->CastItemCombatSpell(unitTarget, m_attackType, procVictim, procEx, GetSpellInfo());
        }

        m_damage = damageInfo.damage;
        m_cleanDamage = damageInfo.damage + damageInfo.absorb + damageInfo.blocked + damageInfo.resist;
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    // Diminishing return increase
    if (m_spellInfo->DiminishingReturn())
    {
        DiminishingGroup group = m_spellInfo->DiminishingReturn()->GetGroup(m_triggeredByAuraSpell);
        DiminishingReturnsType type = m_spellInfo->DiminishingReturn()->GetReturnsType();

        // Increase Diminishing on unit, current informations for actually casts will use values above
        if ((type == DRTYPE_PLAYER && unit->GetCharmerOrOwnerOrSelf()->IsAffectedByDiminishing(group)) || type == DRTYPE_ALL)
            unit->IncreaseDiminishingLevel(m_spellInfo, m_triggeredByAuraSpell);
    }
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                // Diminishing return
                if (aurSpellInfo->DiminishingReturn())
                {
                    float mod = unit->ApplyDiminishingToDuration(aurSpellInfo, duration, m_originalCaster, m_triggeredByAuraSpell);
                    ...
                    {
                        DiminishingGroup group = aurSpellInfo->DiminishingReturn()->GetGroup(m_triggeredByAuraSpell);
                        ((UnitAura*)m_spellAura)->SetDiminishGroup(group);
                        ...
                        if (m_spellInfo->IsChanneled())
                        {
                            if (m_spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION)
                            {
                                if (Player* modOwner = m_originalCaster->GetSpellModOwner())
                                    modOwner->ApplySpellMod(aurSpellInfo->Id, SPELLMOD_CASTING_TIME, duration, this);
                                duration = int32(duration * m_spellAura->GetSnapShotValue(SSVAL_HASTE, m_spellAura->GetUnitOwner()));
                            }
                        }
        ...
        if (ihit->missCondition == SPELL_MISS_NONE && (channelTargetEffectMask & ihit->effectMask))
        {
            ...
            if (IsValidDeadOrAliveTarget(unit))
            {
                if (channelAuraMask & ihit->effectMask)
                {
                    ...
                    {
                        if (m_caster != unit && (!m_caster->IsWithinDistInMap(unit, range) || !m_caster->canSeeOrDetect(unit)))
                        {
...
void Spell::cast(bool skipCheck)
{
    ...
    if (Player* playerCaster = m_caster->ToPlayer())
    {
        ...
        {
            playerCaster->GetAchievementMgr().StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_ITEM, m_CastItem->GetEntry());
            playerCaster->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM, m_CastItem->GetEntry());
        }

        playerCaster->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL, m_spellInfo->Id);
    }
...
void Spell::SendSpellCooldown()
{
    ...
    if (!IsTriggered())
        _player->SendModCooldownIfAny(GetSpellInfo());
}
...
void Spell::finish(bool ok)
{
    ...
    if (Player *modOwner = m_caster->GetSpellModOwner())
    {
        if (!m_triggeredByAuraSpell)
            modOwner->UpdatePotionCooldown(this);
...
void Spell::HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGOTarget, uint32 i, SpellEffectHandleMode mode)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell: %u Effect : %u unitTarget: %u itemTarget: %u gameObjTarget: %u destTarget: %u", m_spellInfo->Id, eff,
                   unitTarget ? unitTarget->GetEntry() : 0, itemTarget ? itemTarget->GetEntry() : 0, gameObjTarget ? gameObjTarget->GetEntry() : 0, destTarget ? 1 : 0);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    // check if unit should be feared
    if (m_caster->ShouldBeFeared())
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_RESURRECT_NEW:
            {
                if (Player* pPlayer = m_caster->ToPlayer())
                    if (pPlayer->HandleWithCrossFactionBG())
                        if (Corpse* pCorpse = m_targets.GetCorpseTarget())
                            if (Player* pPlayerTarget = sObjectAccessor->FindPlayer(pCorpse->GetOwnerGUID()))
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                // Bug #2550 Spell Effect 50362 and 50363 - no mounting allowed
                if(m_caster->HasAura(71484) || m_caster->HasAura(71491) || m_caster->HasAura(71492)
                    || m_caster->HasAura(71486) || m_caster->HasAura(71485) || m_caster->HasAura(71561)
                    || m_caster->HasAura(71559) || m_caster->HasAura(71560) || m_caster->HasAura(71558)
                    || m_caster->HasAura(71556))
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        ...
        default:                                            // normal case
            ...
            if (!caster)
                caster = m_caster;
            if (target != m_caster && !m_targets.HasDst() && !target->IsWithinLOSInMap(caster))
                return false;
            break;
    }
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (targetInfo.effectMask & (1<<i))
        {
            ...
            {
                if (m_spellInfo->Effects[i].IsTargetingArea()
                    || m_spellInfo->Effects[i].IsAreaAuraEffect()
                    || m_spellInfo->Effects[i].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA)
                    || m_spellInfo->Effects[i].ChainTarget > 10)
                {
                    ...
                    // Area-of-effect damage cap calculation
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    {
