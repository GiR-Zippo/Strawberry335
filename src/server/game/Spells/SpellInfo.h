...
enum SpellCustomAttributes
{
    ...
    SPELL_ATTR0_CU_DONT_RESET_PERIODIC_TIMER     = 0x00040000,
...
class SpellDiminishingReturn
{
    public:

        SpellDiminishingReturn(const SpellInfo* Spellproto);

        DiminishingGroup GetGroup(bool triggeredByAura, bool onLoad = false) const;
        DiminishingReturnsType GetReturnsType() const { return returnsType; }
        DiminishingLevels GetMaxLevel() const { return maxLevel; }
        int32 GetLimitDuration() const { return limitDuration; }

    private:

        DiminishingGroup loadGroup();
        DiminishingReturnsType loadReturnsType();
        DiminishingLevels loadMaxLevel();
        int32 loadLimitDuration();

        const SpellInfo* spellproto;

        DiminishingGroup group;
        DiminishingReturnsType returnsType;
        DiminishingLevels maxLevel;
        int32 limitDuration;
};

struct SpellDiminishingReturnInfo
{
    SpellDiminishingReturnInfo() : diminishingReturn(NULL), hasTriggeredSpell(false) { };
    ~SpellDiminishingReturnInfo()
    {
        if (diminishingReturn)
            delete diminishingReturn;
    }

    const SpellDiminishingReturn* operator() () const
    {
        return diminishingReturn;
    }

    const SpellDiminishingReturn* diminishingReturn;
    bool hasTriggeredSpell;
};
...
public:
    ...
    int32 CalcValue(Unit const* caster = NULL, int32 const* basePoints = NULL, Unit const* target = NULL, float factor = 1) const;
...
public:
    ...
    SpellDiminishingReturnInfo DiminishingReturn;
    bool Binary;
    ...
    // Misc
    bool IsSpellFamilyPlayerClass() const;
