...
SpellDiminishingReturn::SpellDiminishingReturn(const SpellInfo* spellInfo) : spellproto(spellInfo)
{
    ASSERT(spellproto);

    group = loadGroup();
    returnsType = loadReturnsType();
    maxLevel = loadMaxLevel();
    limitDuration = loadLimitDuration();
}

DiminishingGroup SpellDiminishingReturn::loadGroup()
{
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        {
            // Earthgrab shouldn't have DR group
...
DiminishingReturnsType SpellDiminishingReturn::loadReturnsType()
{
    switch (group)
    {
...
DiminishingLevels SpellDiminishingReturn::loadMaxLevel()
{
    switch (group)
    {
        case DIMINISHING_TAUNT:
...
int32 SpellDiminishingReturn::loadLimitDuration()
{
    switch (group)
    {
        // Apply limit duration only to this groups
...
DiminishingGroup SpellDiminishingReturn::GetGroup(bool triggeredByAura, bool onLoad) const
{
    if (spellproto->DiminishingReturn.hasTriggeredSpell && !onLoad)
        return DIMINISHING_NONE;
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/, float factor) const
{
    float basePointsPerLevel = RealPointsPerLevel;
    int32 basePoints = bp ? (*bp * (1000 * factor)) / 1000 : (BasePoints * (1000 * factor)) / 1000; // accuracy 10^-3
    int32 randomPoints = (DieSides * (1000 * factor)) / 1000; // accuracy 10^-3
...
SpellInfo::SpellInfo(SpellEntry const* spellEntry)
{
    ...
    // Check if binary spell
    Binary = false;
    if (!HasEffect(SPELL_EFFECT_SCHOOL_DAMAGE) || HasAnyEffectMechanic())
        Binary = true;
}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    if (saBounds.first != saBounds.second)
    {
         ...
         * <TehKevin>
         *
         * -Special case wintergrasp during battle (no ghost flying)
         * @todo : move that to conditions
         */
        if (zone_id == 4197 && Id == 55164)
        {
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
        ...
        if (caster != unitTarget)
        {
            if (caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (Mechanic == MECHANIC_DISARM)
                {
                    ...
                    if (const Player* pPlayer = unitTarget->ToPlayer())
                    {
                        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        {
                            switch (Effects[i].ApplyAuraName)
                            {
                                case SPELL_AURA_MOD_DISARM:
                                    if (pPlayer->GetWeaponForAttack(BASE_ATTACK) && pPlayer->IsUseEquipedWeapon(true))
                                        foundWeapon = true;
                                    break;

                                case SPELL_AURA_MOD_DISARM_OFFHAND:
                                    if (pPlayer->GetWeaponForAttack(OFF_ATTACK) && pPlayer->IsUseEquipedWeapon(true))
                                        foundWeapon = true;
                                    break;

                                case SPELL_AURA_MOD_DISARM_RANGED:
                                    if (pPlayer->GetWeaponForAttack(RANGED_ATTACK) && pPlayer->IsUseEquipedWeapon(true))
                                        foundWeapon = true;
                                    break;
                            }
    ...
    if ((Effects[0].TargetA.GetTarget() == TARGET_SRC_CASTER) && (Effects[0].TargetB.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY) && !(AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS))
    ...
    if ((Effects[0].TargetA.GetTarget() == TARGET_UNIT_CONE_ENEMY_104) && !(AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS))
        if (!caster->IsWithinLOSInMap(target))
            return SPELL_FAILED_LINE_OF_SIGHT;
...
bool SpellInfo::IsSpellFamilyPlayerClass() const
{
    return (SpellFamilyName < SPELLFAMILY_MAGE || SpellFamilyName > SPELLFAMILY_SHAMAN);
}
        ...
        case SPELLFAMILY_PRIEST:
            switch (Id)
            {
                ...
                case 47585: // Dispersion
                    return true;
                case 527: // Dispel Magic Rank 1
                case 988: // Dispel Magic Rank 2
                    return true;
                default:
                    break;
            }
