...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //169 SPELL_AURA_MOD_CRIT_PERCENT_VERSUS           implemented in Unit::DealDamageBySchool, Unit::DoAttackDamage, Unit::CalculateCriticalChanceSpellTaken
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //179 SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE implemented in Unit::CalculateCriticalChanceSpellTaken
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //187 SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE  implemented in Unit::CalculateCriticalChanceTaken
    &AuraEffect::HandleNoImmediateEffect,                         //188 SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_CHANCE implemented in Unit::CalculateCriticalChanceTaken
    ...
    &AuraEffect::HandleAuraModCooldown,                           //196 SPELL_AURA_MOD_COOLDOWN - flat mod of spell cooldowns
    &AuraEffect::HandleNoImmediateEffect,                         //197 SPELL_AURA_MOD_ATTACKER_SPELL_AND_WEAPON_CRIT_CHANCE implemented in Unit::CalculateCriticalChanceSpellTaken Unit::CalculateCriticalChanceTaken
    ...
    switch (GetAuraType())
    {
            ...
            switch (GetSpellInfo()->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_PRIEST:
                    ...
                    if (GetSpellInfo()->SpellFamilyFlags[0] & 0x1 && GetSpellInfo()->SpellFamilyFlags[2] & 0x400)
                    {
                        ...
                        int32 amountNew = amount;
                        int32 amountOld = amount;
                        ...
                        // Spiritual Heal holy Tree
                        if (AuraEffect* aurEff = caster->GetAuraEffectOfRankedSpell(14898, EFFECT_0))
                        {
                            AddPctN(amountNew, aurEff->GetAmount());
        ...
        case SPELL_AURA_PERIODIC_ENERGIZE:
            ...
            case 29166: // Innervate
                if (caster)
                    ApplyPctF(amount, float(caster->GetCreatePowers(POWER_MANA)) / GetTotalTicks());
                break;
        ...
        case SPELL_AURA_PERIODIC_HEAL:
            ...
            if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_PRIEST && m_spellInfo->SpellFamilyFlags[2] & 0x4000)
            {
                ...
                if (AuraEffect* pAurEff = caster->GetAuraEffect(55673, EFFECT_0))
                    AddPctN(amount, pAurEff->GetAmount());

                // Bonus from talent "spiritual healing"
                if (AuraEffect* pAurEff = caster->GetAuraEffectOfRankedSpell(14898, EFFECT_0))
                    AddPctN(amount, pAurEff->GetAmount());
            }
            break;
...
void AuraEffect::CalculatePeriodic(Unit* caster, bool create, bool load, bool refresh)
{
    ...
    if (m_amplitude)
    {
        ...
        if (caster)
        {
            if (caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, m_spellInfo) || (m_spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION))
                m_amplitude *= GetBase()->GetSnapShotValue(SSVAL_HASTE, GetBase()->GetUnitOwner());
        }
    }
    ...
    else if (!refresh) // aura just created or reapplied (excluding refreshing)
    {
        m_tickNumber = 0;
        // Reset periodic timer on aura create or on reapply, when aura isn't stackable.
        if (create || (m_spellInfo->StackAmount <= 1))
        {
            m_periodicTimer = 0;
...
bool AuraEffect::IsPeriodicTickCrit(Unit* target, Unit const* caster) const
{
    ...
    {
        if ((*itr)->IsAffectedOnSpell(m_spellInfo) && caster->isSpellCrit(target, m_spellInfo, m_spellInfo->GetSchoolMask(), BASE_ATTACK, GetBase()))
            return true;
    }

    // Rupture - since 3.3.3 can crit
    if (m_spellInfo->SpellIconID == 500 && m_spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE)
        return caster->isSpellCrit(target, m_spellInfo, m_spellInfo->GetSchoolMask(), BASE_ATTACK, GetBase());
...
void AuraEffect::HandleSpiritOfRedemption(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
        ...
        if (GetId() == 42016 && target->GetMountID() == 16314)
        {
            if (!target->GetAuraEffectsByType(SPELL_AURA_MOUNTED).empty())
            {
                ...
                if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(cr_id))
                {
                    uint32 team = 0;
                    if (target->GetTypeId() == TYPEID_PLAYER)
                        team = target->ToPlayer()->GetTeam(true);
...
void AuraEffect::HandleModFear(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // Bug #2827 fix
    if ((target->IsFalling() || target->HasUnitState(UNIT_STATE_JUMPING)) && apply)
        target->SetShouldBeFeared();
    else
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
        ...
        && GetSpellInfo()->IsPositive())                       //Only positive immunity removes auras
    {
        ...
        for (Unit::AuraApplicationMap::iterator iter = Auras.begin(); iter != Auras.end();)
        {
            ...
            if (spell->Id != GetId() && spell->GetSchoolMask() & school_mask)  // Don't remove self and check for school mask
            {
                if ((GetSpellInfo()->CanDispelAura(spell) && !iter->second->IsPositive()) // Don't remove positive spells if they havn't SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY
                    || iter->second->GetBase()->GetSpellInfo()->AttributesEx & SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY
                    || (iter->second->GetBase()->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_MAGE // Special handling living bomb
                    && iter->second->GetBase()->GetSpellInfo()->SpellFamilyFlags[1] & 0x20000))
                {
...
void AuraEffect::HandleModCastingSpeed(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    target->ApplyCombatSpeedMod(CTYPE_CAST_TIME, (float)GetAmount(), apply);
    target->ApplyCombatSpeedMod(CTYPE_SPELL, (float)GetAmount(), apply);
}
...
void AuraEffect::HandleModMeleeRangedSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    target->ApplyCombatSpeedMod(CTYPE_BASE, (float)GetAmount(), apply);
    target->ApplyCombatSpeedMod(CTYPE_OFF, (float)GetAmount(), apply);
    target->ApplyCombatSpeedMod(CTYPE_RANGED, (float)GetAmount(), apply);
}
...
void AuraEffect::HandleModDamageDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else
        {
            for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++)
            {
                // @todo : negative values shouldn't be displayed in character sheet
                if ((GetMiscValue() & (1 << i)) != 0)
                    target->ApplyModInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + i, GetAmount(), apply);
            }
        }
    }
    else if (Pet* pPet = target->ToPet())
        pPet->UpdateVisualBonusDamage();
}
        ...
        if (apply)
        {
            ...
            switch (GetId())
            {
                ...
                case 52916: // Honor Among Thieves
                    if (target->GetTypeId() == TYPEID_PLAYER)
                    {
                        Unit* pUnitOwner = aurApp->GetBase()->GetCaster();
                        if (pUnitOwner && pUnitOwner == target && pUnitOwner->isAlive())
                        {
                            Unit* spellTarget = sObjectAccessor->GetUnit(*target, target->ToPlayer()->GetComboTarget(), true);
                            if (!spellTarget)
                                spellTarget = sObjectAccessor->GetUnit(*target, target->GetUInt64Value(UNIT_FIELD_TARGET), true);
                ...
                case 59628: // Tricks of the Trade
                    ...
                    if (apply && caster->HasAura(70805))
                        caster->CastSpell(caster, 70804, true);
                    break;
                case 72232: // Valithria Corruption
                    if (target)
                    {
                        target->ApplySpellImmune(GetId(), IMMUNITY_ID, 47788, apply);
                        target->ApplySpellImmune(GetId(), IMMUNITY_ID, 48153, apply);
                    }
                    break;
                case 49440: // Racer Slam, Slamming
                    if (target)
                    {
                        // Achievement Crashin' & Thrashin'
                        Unit* pUnitOwner = target->GetOwner();
                        if (pUnitOwner && pUnitOwner->GetTypeId() == TYPEID_PLAYER)
                            if (Unit* pUnitOwnerTarget = sObjectAccessor->GetUnit(*pUnitOwner, pUnitOwner->GetUInt64Value(UNIT_FIELD_TARGET), true))
                                if (pUnitOwnerTarget->GetEntry() == 27664 || pUnitOwnerTarget->GetEntry() == 40281)
                                    pUnitOwner->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 49444);
                    }
                    break;
            }
        }
                ...
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        case 2584: // Waiting to Resurrect
                            ...
                            if (aurApp->GetRemoveMode() == AURA_REMOVE_BY_CANCEL)
                            {
                                if (target->GetTypeId() == TYPEID_PLAYER)
                                    if (Battleground* bg = target->ToPlayer()->GetBattleground())
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            {
                if (caster)
                    caster->CastCustomSpell(53352, SPELLVALUE_BASE_POINT0, m_amount, target, TRIGGERED_FULL_MASK, NULL, this);
                break;
            }
            switch (GetSpellInfo()->Id)
            {
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 27808:
                        if (caster)
                            caster->CastCustomSpell(29879, SPELLVALUE_BASE_POINT0, int32(target->CountPctFromMaxHealth(21)), target, TRIGGERED_FULL_MASK, NULL, this);
                        return;
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            case 46284:
                target->CastCustomSpell(triggerSpellId, SPELLVALUE_MAX_TARGETS, m_tickNumber / 10 + 1, NULL, TRIGGERED_FULL_MASK, NULL, this);
                return;
            // Poison (Grobbulus)
            case 28158:
            case 54362:
            ...
            case 66882:
                target->CastCustomSpell(triggerSpellId, SPELLVALUE_RADIUS_MOD, (int32)((((float)m_tickNumber / 60) * 0.9f + 0.1f) * 10000 * 2 / 3), NULL, TRIGGERED_FULL_MASK, NULL, this);
                return;
            // Beacon of Light
            case 53563:
            {
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), damage, DOT, GetBase()->GetStackAmount(), GetBase());
    ...
    caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx, damage, damage + cleanDamage.absorbed_damage, BASE_ATTACK, GetSpellInfo());
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (damage)
        procVictim |= PROC_FLAG_TAKEN_DAMAGE;
    if (caster->isAlive())
        caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx, damage, damage + absorb + resist, BASE_ATTACK, GetSpellInfo());
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    // ignore item heals
    if (!haveCastItem)
        caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx, damage, damage, BASE_ATTACK, GetSpellInfo());
}
...
void AuraEffect::HandlePeriodicEnergizeAuraTick(Unit* target, Unit* caster) const
{
    ...
    if ((target->isHunterPet() && powerType != POWER_HAPPINESS)
        && target->getPowerType() != powerType
        && !(m_spellInfo->AttributesEx7 & SPELL_ATTR7_CAN_RESTORE_SECONDARY_POWER))
        return;
    ...
    // Update scaling aura of hunter pet
    if (target->isHunterPet() && powerType == POWER_HAPPINESS)
    {
        if (Aura* pAura = target->GetAura(8875))
            pAura->RecalculateAmountOfEffects();
    }
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->ProcDamageAndSpell(damageInfo.target, procAttacker, procVictim, procEx, damageInfo.damage, damageInfo.cleanDamage, BASE_ATTACK, spellProto);
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            if (Unit* triggerTarget = target->GetHealthPctLowestRaidMemberOrPet(radius, GetId()))
            {
    ...
    target->CastCustomSpell(target, triggerSpellId, &value, NULL, NULL, TRIGGERED_FULL_MASK, NULL, this, GetCasterGUID());
}

void AuraEffect::HandleAuraModCooldown(const AuraApplication* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        target->ToPlayer()->SetCooldownMod(aurApp->GetBase()->GetEffect(0)->GetAmount(), apply);
    }
}
