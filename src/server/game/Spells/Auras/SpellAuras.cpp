...
{
    if (caster && owner->ToUnit())
        InitSnapShot(caster, owner->ToUnit());
...
void Aura::_InitEffects(uint8 effMask, Unit* caster, int32 *baseAmount)
{
    ...
    if (caster && caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->SpellFamilyName == SPELLFAMILY_POTION && caster->HasAura(53042))
    {
            ...
            sSpellMgr->IsSpellMemberOfSpellGroup(m_spellInfo->Id,SPELL_GROUP_ELIXIR_GUARDIAN))
        {
            ...
            {
                float multiplier = 1.3f; // hacky, since every elixir/flask has his own multiplier

                switch (m_spellInfo->Id) // for now we insert the correct multipliers for lvl 80 stuff and keep the rest at 1.3
                {
                    ...
                    case 53758: // Flask of Stoneblood
                        multiplier = 1.5f;
                        break;
                    // TODO: Elixirs and low lvl stuff 
                }

                for (uint8 i=0 ; i<MAX_SPELL_EFFECTS; ++i)
                {
                    if (effMask & (uint8(1) << i))
                        m_effects[i]->SetAmount((int32)(m_effects[i]->GetAmount() * multiplier));
                }
            }
        }
    }
...
{
    Unit *caster = GetCaster();
    int32 duration = CalcMaxDuration();

    UpdateSnapShot(caster, GetUnitOwner());
    ...
    if (caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, m_spellInfo) || (m_spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION))
        duration *= GetSnapShotValue(SSVAL_HASTE, GetUnitOwner());
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (HasEffect(i))
            GetEffect(i)->CalculatePeriodic(caster, false, false, true);
}
...
void Aura::RefreshTimers()
{
    ...
    SetMaxDuration(duration);
    SetDuration(duration);
    SetModDuration(0); // Reset mod duration

    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (HasEffect(i) && caster)
            GetEffect(i)->CalculatePeriodic(caster, false, false, false);
}
        ...
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_MAGE:
                switch (GetId())
                {
                    ...
                    case 57761: //Fireball!
                        if (removeMode == AURA_REMOVE_BY_EXPIRE || aurApp->GetBase()->IsExpired() || target->HasAura(44401) || target->HasAura(48108))
                            break;
                        else
...
bool Aura::CallScriptEffectRemoveHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        //Check if exists, else we'll get a crash
        if (!(*scritr))
            continue;
...
void UnitAura::_ApplyForTarget(Unit* target, Unit* caster, AuraApplication * aurApp)
{
    ...
    if (m_AuraDRGroup != DIMINISHING_NONE)
...
void Aura::InitSnapShot(Unit *caster, Unit *victim)
{
    SSStore *store = &(m_SSStores[victim->GetGUID()]);

    store->val[SSVAL_CRIT] = caster->CalculateCriticalChanceSpellDone(GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), BASE_ATTACK);
    store->val[SSVAL_PCT] = caster->CalculateSpellDamageBonusPair(victim, GetSpellInfo()).second;

    UpdateSnapShot(caster, victim);
}

void Aura::UpdateSnapShot(Unit *caster, Unit *victim)
{
    SSStore *store = &(m_SSStores[victim->GetGUID()]);

    for (int i = 0; i < MAX_ATTACK; ++i)
        store->val[SSVAL_AP_MAIN + i] = caster->GetTotalAttackPowerValue(WeaponAttackType(i));

    if (SpellInfo const *spellInfo = GetSpellInfo())
        store->val[SSVAL_SP] = caster->SpellBaseDamageBonusDone(spellInfo->GetSchoolMask());
    else
        store->val[SSVAL_SP] = .0f;

    store->val[SSVAL_HASTE] = caster->GetFloatValue(UNIT_MOD_CAST_SPEED);
}
