        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x200 && m_spellInfo->Category == 1209)
                {
                    ...
                    //Shield Block = 100% increase in block value over cap.
                    float block_value_multiplier = (m_caster->HasAura(2565))?2:1;

                    uint32 block_value = m_caster->GetShieldBlockValue(uint32(float(level) * 24.5f), uint32(float(level) * 34.5f)) * block_value_multiplier;
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x00000008)
                {
                    ...
                    {
                        uint32 combo = caster->GetComboPoints();

                        if (combo)
                        {
                            if (AuraEffect *deadlyPoison = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00010000, 0, 0, caster->GetGUID()))
                            {
                                uint32 doses = deadlyPoison->GetBase()->GetStackAmount();
                                combo = std::min(doses, combo);

                                if (!caster->HasAura(31226) && !caster->HasAura(31227) && !caster->HasAura(58410))
                                {
                                    if (combo < doses)
                                        deadlyPoison->GetBase()->SetStackAmount(doses - combo);
                                    else
                                        unitTarget->RemoveAura(deadlyPoison->GetBase());
                                }
                            }
                        }

                        damage += int32(caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.09f);
                        damage += caster->HasAura(37169) ? 40 : 0;
                ...
                else if (m_spellInfo->SpellFamilyFlags[0] & 0x00020000)
                {
                    if (Player *caster = GetCaster()->ToPlayer())
                    {
                        if (uint32 combo = caster->GetComboPoints())
                        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1]&0x00040000)
                {
                    ...
                    {
                        float min, max;
                        m_caster->ToPlayer()->CalculateMinMaxDamage(BASE_ATTACK, false, false, min, max);
                        average = (min + max) / 2.0f;
                    }
                    else
                        average = (m_caster->GetFloatValue(UNIT_FIELD_MINDAMAGE) + m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE)) / 2.0f;

                    average *= static_cast<float>(m_caster->CalculateSpellDamage(unitTarget, m_spellInfo, EFFECT_2) * IN_MILLISECONDS);
                    damage += static_cast<int32>(average / static_cast<float>(m_caster->GetFloatValue(UNIT_FIELD_BASEATTACKTIME)));
                    break;
                }
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                case 51582:                                 //Rocket Boots Engaged (Rocket Boots Xtreme and Rocket Boots Xtreme Lite)
                {
                    ...
                    if (Player* p_player = m_caster->ToPlayer())
                        p_player->RemoveSpellCooldown(30452, true);
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    // Don't apply arcane blast debuff
    if (triggered_spell_id == 36032)
        return;
            ...
            case 35729:
            {
                ...
                for (Unit::AuraApplicationMap::iterator iter = Auras.begin(); iter != Auras.end();)
                {
                    ...
                    if ((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC // only affect magic spells
						...
						&& !iter->second->IsPositive() && !iter->second->GetBase()->IsPassive() && (spell->Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) == 0)
                    {
        ...
        case 36941:
        {
            if (roll_chance_i(50))                        // 50% success
            {
                ...
                switch (rand_eff)
                {
                    ...
                    // Transform
                    {
                        if (m_caster->ToPlayer()->GetTeam(true) == ALLIANCE)
...
void Spell::EffectEnergizePct(SpellEffIndex effIndex)
{
    ...
    if ((unitTarget->isHunterPet() && power != POWER_HAPPINESS)
        && unitTarget->getPowerType() != power
        && !(m_spellInfo->AttributesEx7 & SPELL_ATTR7_CAN_RESTORE_SECONDARY_POWER))
        return;

    uint32 maxPower = unitTarget->GetMaxPower(power);
    if (m_spellInfo->Id == 54607 || m_spellInfo->Id == 59118) // Verbessertes Seele entziehen nimmt den Manapool des Hexers, nicht den des Pets
        maxPower = unitTarget->GetOwner()->GetMaxPower(power);
    ...
    if (unitTarget->isHunterPet() && power == POWER_HAPPINESS)
        if (Aura* pAura = unitTarget->GetAura(8875))
...
void Spell::EffectEnchantItemPerm(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        if (item_owner != p_caster && !AccountMgr::IsVIPorPlayer(p_caster->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
        {
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    int32 weaponDamage = m_caster->CalculateBaseWeaponDamage(m_attackType, normalized, true);
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 69961:
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
                    {
                        ...
                        // search for Diseases applied by the Deathknight
                        if (spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && aurEff->GetCasterGUID() == m_caster->GetGUID() && spellInfo->SpellFamilyFlags[2] & 0x2)
                            if (Aura * aura = aurEff->GetBase())
                            {
                                int32 modDuration = aura->GetModDuration();
                                ...
                                if (modDuration < 9000)
                                {
                                    ...
                                    aura->SetModDuration(modDuration + 3000);
                                }
                            }
                    }
                    if (AuraEffect* aurEff = unitTarget->GetAuraEffectOfRankedSpell(50508, EFFECT_0))
                    {
                        if (Aura * aura = aurEff->GetBase())
                        {
                            int32 modDuration = aura->GetModDuration();
                    ...
                    if (AuraEffect* aurEff = unitTarget->GetAuraEffectOfRankedSpell(51726, EFFECT_0))
                    {
                        if (Aura * aura = aurEff->GetBase())
                        {
                            int32 modDuration = aura->GetModDuration();
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/)
{
    ...
    for (Unit::AttackerSet::const_iterator itr = attackers.begin(); itr != attackers.end();)
    {
        if (Unit* pUnit = (*itr++))
        {
            if (!pUnit->canSeeOrDetect(unitTarget))
            {
                pUnit->AttackStop();
                pUnit->CastStop();
            }
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    // Disable Unstuck for Deathknights in Starting Area because it fucks up Phasing and Quests!
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        ...
        unitTarget->GetFirstCollisionPosition(pos, unitTarget->GetMeleeReach(), angle);

        m_caster->GetMotionMaster()->MoveCharge(pos.m_positionX, pos.m_positionY, pos.m_positionZ + 1.0f); // Avoiding falling through the map
    }
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_UNIT)
