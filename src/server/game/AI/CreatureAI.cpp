...
void CreatureAI::BossWhisper(uint8 id, uint64 WhisperGuid)
{
    sCreatureTextMgr->SendChat(me, id, WhisperGuid, CHAT_MSG_RAID_BOSS_WHISPER);
}

void CreatureAI::DoZoneInCombat(Creature* creature /*= NULL*/, float maxRangeToNearestTarget /*= 50.0f*/, bool includePets /*= true*/)
{
    if (!creature)
        creature = me;
    ...
    if (!creature->HasReactState(REACT_PASSIVE) && !creature->getVictim())
    {
        ...
        if (includePets)
            nearTarget = creature->SelectNearestTarget(maxRangeToNearestTarget);
        else
            nearTarget = creature->SelectNearestPlayer(maxRangeToNearestTarget);
