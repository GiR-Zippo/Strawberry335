...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SET_HP:
            if (e.action.setHP.percentage <= 0 || e.action.setHP.percentage > 100)
            {
                sLog->outErrorDb("SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses wrong value %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.setHP.percentage);
                return false;
            }
            break;
        ...
        {
            if (e.action.playerMark.symbol1 > RAID_TARGET_ICON_MAX || e.action.playerMark.symbol2 > RAID_TARGET_ICON_MAX || e.action.playerMark.symbol3 > RAID_TARGET_ICON_MAX || e.action.playerMark.symbol4 > RAID_TARGET_ICON_MAX || e.action.playerMark.symbol5 > RAID_TARGET_ICON_MAX)
            {
                sLog->outErrorDb("SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses higher value than expected, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                return false;
            }
            break;
        }
