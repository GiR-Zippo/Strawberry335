...
void SmartScript::_DespawnAtEvade()
{
    me->SetVisible(false);
    me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
    me->m_Events.AddEvent(new DespawnAtEvadeEvent(me->GetGUID()), me->m_Events.CalculateTime(30 * IN_MILLISECONDS));
}
    ...
    switch (e.GetActionType())
    {
                ...
                if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
                {
                    ...
                    if (e.action.morphOrMount.creature)
                    {
                        if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
                        {
                            ...
                            (*itr)->ToUnit()->SetDisplayId(display_id);
                    ...
                    {
                        (*itr)->ToUnit()->SetDisplayId(e.action.morphOrMount.model);
                        sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u set displayid to %u",
        ...
        case SMART_ACTION_ADD_AURA:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    (*itr)->ToUnit()->AddAura(e.action.addAura.aura1, (*itr)->ToUnit());

                    if (e.action.addAura.aura2 > 0)
                        (*itr)->ToUnit()->AddAura(e.action.addAura.aura2, (*itr)->ToUnit());
                    if (e.action.addAura.aura3 > 0)
                        (*itr)->ToUnit()->AddAura(e.action.addAura.aura3, (*itr)->ToUnit());
                    if (e.action.addAura.aura4 > 0)
                        (*itr)->ToUnit()->AddAura(e.action.addAura.aura4, (*itr)->ToUnit());
                    if (e.action.addAura.aura5 > 0)
                        (*itr)->ToUnit()->AddAura(e.action.addAura.aura5, (*itr)->ToUnit());
                    if (e.action.addAura.aura6 > 0)
                        (*itr)->ToUnit()->AddAura(e.action.addAura.aura6, (*itr)->ToUnit());

                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding aura %u to unit %u",
                        e.action.addAura.aura1, (*itr)->GetGUIDLow());
                }
            }
        ...
        case SMART_ACTION_REMOVEAURASFROMSPELL:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (e.action.addAura.aura1 == 0)
                {
                    (*itr)->ToUnit()->RemoveAllAuras();
                }
                else
                {
                    (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura1);

                    if (e.action.addAura.aura2 > 0)
                        (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura2);
                    if (e.action.addAura.aura3 > 0)
                        (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura3);
                    if (e.action.addAura.aura4 > 0)
                        (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura4);
                    if (e.action.addAura.aura5 > 0)
                        (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura5);
                    if (e.action.addAura.aura6 > 0)
                        (*itr)->ToUnit()->RemoveAurasDueToSpell(e.action.addAura.aura6);
                }
        ...
        case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
        {
            ...
            if (me)
            {
                if (e.action.combatWithZone.combatWithTarget > 0)
                {
                    ...
                    // Get nearest victim and start attack
                    if (!me->getVictim())
                        if (auto nearestTarget = me->SelectNearestTarget(50.0f))
                            me->AI()->AttackStart(nearestTarget);

                    // Get all targets and set in combat with me
        ...
        case SMART_ACTION_SUMMON_CREATURE:
        {
            float x, y, z, o;
            ObjectList* targets = GetTargets(e, unit);
            if (e.action.summonCreature.count == 0)
                e.action.summonCreature.count = 1;
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    z += e.target.z;
                    o += e.target.o;
                    for (uint32 i = 1; i <= e.action.summonCreature.count; i++)
            ...
            if (e.GetTargetType() == SMART_TARGET_NEAR_RANDOM_POSITION)
            {
                Position srcPos;
                if (e.target.nearbyRandomPosition.noPosition == 1)
                {
                    srcPos = me->GetPosition();
                } else {
                    srcPos.m_positionX = e.target.x;
                    srcPos.m_positionY = e.target.y;
                    srcPos.m_positionZ = e.target.z;
                }

                for (uint32 i = 1; i <= e.action.summonCreature.count; i++)
                {
                    Position randPos;
                    me->GetRandomPoint(srcPos, e.target.nearbyRandomPosition.radius, randPos);
                    if (Creature* summon = GetBaseObject()->SummonCreature(e.action.summonCreature.creature, randPos, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
        ...
        case SMART_ACTION_SUMMON_GO:
        {
            ...
            if (e.GetTargetType() == SMART_TARGET_NEAR_RANDOM_POSITION)
            {
                ...
                Position randPos;
                me->GetRandomPoint(srcPos, e.target.nearbyRandomPosition.radius, randPos);
                GetBaseObject()->SummonGameObject(e.action.summonGO.entry, randPos.GetPositionX(), randPos.GetPositionY(), randPos.GetPositionZ(), randPos.GetOrientation(), 0, 0, 0, 0, e.action.summonGO.despawnTime);
                break;
            }
        ...
        case SMART_ACTION_TELEPORT:
        {
            ...
            if (e.action.teleport.randomPoint > 0)
            {
                Position srcPos;
                if (e.action.teleport.noPos == 1)
                {
                    srcPos = me->GetPosition();
                }
                else {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    Position randPos;
                    me->GetRandomPoint(srcPos, e.action.teleport.radius, randPos);

                    (*itr)->ToPlayer()->TeleportTo(e.action.teleport.mapID, randPos.GetPositionX(), randPos.GetPositionY(), e.target.z, e.target.o);
                }
            } else {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
        ...
        case SMART_ACTION_SET_COUNTER:
        {
            if (ObjectList* targets = GetTargets(e, unit))
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsCreature(*itr))
                    {
                        if (SmartAI* ai = CAST_AI(SmartAI, (*itr)->ToCreature()->AI()))
                            ai->GetScript()->StoreCounter(e.action.setCounter.counterId, e.action.setCounter.incr, e.action.setCounter.decr, e.action.setCounter.reset);
                        else
                            sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript: Action target for SMART_ACTION_SET_COUNTER is not using SmartAI, skipping");
                    }
                    else if (IsGameObject(*itr))
                    {
                        if (SmartGameObjectAI* ai = CAST_AI(SmartGameObjectAI, (*itr)->ToGameObject()->AI()))
                            ...
                            sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript: Action target for SMART_ACTION_SET_COUNTER is not using SmartGameObjectAI, skipping");
                    }
                }
                ...
                StoreCounter(e.action.setCounter.counterId, e.action.setCounter.incr, e.action.setCounter.decr, e.action.setCounter.reset);
        ...
        case SMART_ACTION_SET_ORIENTATION:
        {
            ...
            ObjectList* targets = GetTargets(e, unit);
            if (e.action.setOrientation.random == 1)
                me->SetFacingTo(frand(0.0f, 2 * M_PI));
            else if (e.GetTargetType() == SMART_TARGET_SELF)
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            if (e.GetTargetType() == SMART_TARGET_CREATURE_RANGE || e.GetTargetType() == SMART_TARGET_CREATURE_GUID ||
                ...
                e.GetTargetType() == SMART_TARGET_HOSTILE_RANDOM || e.GetTargetType() == SMART_TARGET_HOSTILE_RANDOM_NOT_TOP || e.GetTargetType() == SMART_TARGET_FARTHEST ||
                e.GetTargetType() == SMART_TARGET_PLAYER_WITH_AURA || e.GetTargetType() == SMART_TARGET_PLAYER_WITHOUT_MULTI_AURA || e.GetTargetType() == SMART_TARGET_STORED )
            {
                ObjectList* targets = GetTargets(e, unit);
                if (!targets)
                    break;
        ...
        case SMART_ACTION_SET_HP:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (e.action.setHP.flat > 0)
                    (*itr)->ToUnit()->SetHealth(e.action.setHP.flat);
                else if (e.action.setHP.percentage > 0 && e.action.setHP.percentage <= 100)
                    (*itr)->ToUnit()->SetHealth((*itr)->ToUnit()->CountPctFromMaxHealth(e.action.setHP.percentage));
            }
        ...
        case SMART_ACTION_CONE_SUMMON:
        {
            ...
            uint32 summonId = e.action.coneSummon.summonEntry;
            uint32 duration = e.action.coneSummon.summonDuration;
            TempSummonType spawnType = (duration > 0) ? TEMPSUMMON_TIMED_DESPAWN : TEMPSUMMON_CORPSE_DESPAWN;
            ...
            for (uint32 radius = 0; radius <= e.action.coneSummon.coneLength; radius += e.action.coneSummon.distanceBetweenRings)
            {
                ...
                if (e.GetTargetType() == SMART_TARGET_SELF || e.GetTargetType() == SMART_TARGET_NONE)
                    currentAngle += (e.target.o > 0.0f) ? e.target.o : me->GetOrientation();
                else if (ObjectList* targets = GetTargets(e, unit))
                    currentAngle += (!targets->empty()) ? me->GetAngle(*targets->begin()) : me->GetOrientation();
                ...
                {
                    spawnPosition.Relocate(*me);
                    delta.RelocatePolar(currentAngle, radius);
                    spawnPosition.Add(delta);
                    currentAngle += deltaAngle;

                    me->SummonCreature(summonId, spawnPosition, spawnType, duration, 0);
                }
            }

            break;
        }
        case SMART_ACTION_RESET_THREATLIST:
        {
            if (!me->isInCombat())
                break;
        ...
        {
            ObjectList* targets = GetTargets(e, unit);
            char const* text = sObjectMgr->GetTrinityString(e.action.playerTalk.textId, DEFAULT_LOCALE);

            if (targets)
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                    if (IsPlayer((*itr)))
                        e.action.playerTalk.flag == 0 ? (*itr)->ToPlayer()->Say(text, 0) : (*itr)->ToPlayer()->Yell(text, 0);

            delete targets;
            break;
        }
        case SMART_ACTION_PLAYER_SEND_BOSSWHISPER:
        {
            ObjectList* targets = GetTargets(e, unit);
            ...
            if (targets)
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                    if (IsPlayer((*itr)))
                        me->AI()->BossWhisper(e.action.playerBossEmote.id, (*itr)->GetGUID());

            delete targets;
            break;
        }
        case SMART_ACTION_PLAYER_MARK:
        {
            ObjectList* targets = GetTargets(e, unit);
            uint32 counter = 1;
                    ...
                    if (Player* player = (*itr)->ToPlayer())
                        if (Group* pGroup = player->GetGroup())
                            if (pGroup->isRaidGroup())
                            {
                                ...
                                {
                                case 1:
                                    pGroup->SetTargetIcon(e.action.playerMark.symbol1, player->GetGUID(), player->GetGUID());
                                    break;
                                case 2:
                                    pGroup->SetTargetIcon(e.action.playerMark.symbol2, player->GetGUID(), player->GetGUID());
                                    break;
                                case 3:
                                    pGroup->SetTargetIcon(e.action.playerMark.symbol3, player->GetGUID(), player->GetGUID());
                                    break;
                                case 4:
                                    pGroup->SetTargetIcon(e.action.playerMark.symbol4, player->GetGUID(), player->GetGUID());
                                    break;
                                case 5:
                                    pGroup->SetTargetIcon(e.action.playerMark.symbol5, player->GetGUID(), player->GetGUID());
                                    break;
                                default:
                                    break;
                                }
        ...
        case SMART_ACTION_CALL_CHALLENGE_STOP:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                /*if (Group* pGroup = (*itr)->ToPlayer()->GetGroup())
                {
                    sChallengeMode->stop_challenge(sObjectAccessor->GetPlayer(**itr, pGroup->GetLeaderGUID()), e.action.callChallengeStop.challengeid);
                    delete targets;
                    break;
                }*/
            }
        ...
        case SMART_ACTION_JOIN_TARGETS_GROUP:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                Group* gr = (*itr)->ToPlayer()->GetGroup();
        ...
        case SMART_ACTION_GAMEOBJECT_REMOVE:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
        ...
        case SMART_ACTION_CUSTOM_CAST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    int32 bp0 = e.action.castCustom.bp1;
                    int32 bp1 = e.action.castCustom.bp2;
                    int32 bp2 = e.action.castCustom.bp3;

                    if (!(e.action.castCustom.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.castCustom.spell))
                        me->CastCustomSpell((*itr)->ToUnit(), e.action.castCustom.spell, &bp0, &bp1, &bp2, (e.action.castCustom.flags & SMARTCAST_TRIGGERED) ? true : false);
                    else
                        sLog->outDebug(LOG_FILTER_DATABASE_AI, "Spell %u not casted because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.castCustom.spell, (*itr)->GetGUID(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_CAST:: Creature %u casts spell %u on target %u with castflags %u",
                        me->GetGUIDLow(), e.action.castCustom.spell, (*itr)->GetGUIDLow(), e.action.castCustom.flags);
                }
            }
        ...
        case SMART_ACTION_VORTEX_SUMMON:
        {
            ...
            auto summon_entry = e.action.summonVortex.summonEntry;
            auto summon_duration = e.action.summonVortex.summonDuration;
            TempSummonType summon_type = (summon_duration > 0) ? TEMPSUMMON_TIMED_DESPAWN : TEMPSUMMON_CORPSE_DESPAWN;

            auto a = static_cast<float>(e.action.summonVortex.a);
            auto k = static_cast<float>(e.action.summonVortex.k) / 1000.0f;
            auto r_max = static_cast<float>(e.action.summonVortex.r_max);
            auto delta_phi = M_PI * static_cast<float>(e.action.summonVortex.phi_delta) / 180.0f;

            // r(phi) = a * e ^ (k * phi)
            // r(phi + delta_phi) = a * e ^ (k * (phi + delta_phi))
            // r(phi + delta_phi) = a * e ^ (k * phi) * e ^ (k * delta_phi)
            // r(phi + delta_phi) = r(phi) * e ^ (k * delta_phi)
            auto factor = powf(M_E, k * delta_phi);

            // r(0) = a * e ^ (k * 0) = a * e ^ 0 = a * 1 = a
            auto summon_radius = a;

            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                // Offset by orientation, should not count into radius calculation,
                // but is needed for vortex direction (polar coordinates)
                auto phi = (*itr)->GetOrientation();

                Position summon_pole(*(*itr));

                Position summon_position;
                do {
                    summon_position.RelocatePolar(phi, summon_radius);
                    summon_position.Add(summon_pole);

                    if (me->GetMapId() == 568) // Custom Kito
                        summon_position.m_positionZ += 1.0f;

                    me->SummonCreature(summon_entry, summon_position, summon_type, summon_duration, 0);

                    phi += delta_phi;
                    summon_radius *= factor;
                } while (summon_radius <= r_max);
            }

            delete targets;
            break;
        }
        case SMART_ACTION_PLAYER_JUMP_TO_POS:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            {
                if ((*itr)->ToPlayer())
                    (*itr)->ToPlayer()->GetMotionMaster()->MoveJump(e.target.x, e.target.y, e.target.z, e.action.PlayerJumpToPos.speed, 0);
            }
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_HOSTILE_RANDOM:
            if (me)
            {
                uint32 targets = (uint32)e.target.o;
                if (targets == 0)
                {
                    ...
                    for (std::list<HostileReference*>::const_iterator i = threatList.begin(); i != threatList.end(); ++i)
                        if (Unit* temp = Unit::GetUnit(*me, (*i)->getUnitGuid()))
                            if (IsPlayer(temp))
                                l->push_back(temp);
                    Trinity::Containers::RandomResizeList(*l, targets);
                }  
            }
            break;
        case SMART_TARGET_HOSTILE_RANDOM_NOT_TOP:
            if (me)
            {
                ...
                if (targets == 0)
                {
                    ...
                    for (std::list<HostileReference*>::const_iterator i = threatList.begin(); i != threatList.end(); ++i)
                        if (Unit* temp = Unit::GetUnit(*me, (*i)->getUnitGuid()))
                            if (IsPlayer(temp) && me->getVictim() != temp)
                                l->push_back(temp);
                    Trinity::Containers::RandomResizeList(*l, targets);
                }  
            }
        ...
        case SMART_TARGET_PLAYER_RANGE:
        {
            ...
            float onlyOne = (float)e.target.o;
            if (!units->empty() && GetBaseObject())
                for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); ++itr)
                {
                    if (IsPlayer(*itr) && !(*itr)->ToPlayer()->isGameMaster() && (*itr)->ToPlayer()->isAlive() && GetBaseObject()->IsInRange(*itr, (float)e.target.playerRange.minDist, (float)e.target.playerRange.maxDist))
                    {
                        l->push_back(*itr);
                    }
                }
            ...
            if (onlyOne > 0)
                Trinity::Containers::RandomResizeList(*l, onlyOne);
        ...
        case SMART_TARGET_PLAYER_DISTANCE:
        {
            ...
            for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); ++itr)
            if (IsPlayer(*itr) && !(*itr)->ToPlayer()->isGameMaster())
                    l->push_back(*itr);
        ...
        {
            // will always return a valid pointer, even if empty list
            ObjectList* units = GetWorldObjectsInDist((float)e.target.playerWithAura.distance);
            for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); ++itr)
                if (IsPlayer(*itr) && !(*itr)->ToPlayer()->isGameMaster())
                {
                    if (e.target.playerWithAura.negation != 0 && !(*itr)->ToPlayer()->HasAura(e.target.playerWithAura.spellId))
                        l->push_back(*itr);
                    else if (e.target.playerWithAura.negation == 0 && (*itr)->ToPlayer()->HasAura(e.target.playerWithAura.spellId))
                        l->push_back(*itr);
                }

            if (e.target.o > 0)
                Trinity::Containers::RandomResizeList(*l, e.target.o);

            delete units;
            break;
        }
        case SMART_TARGET_PLAYER_WITHOUT_MULTI_AURA:
        {
            // will always return a valid pointer, even if empty list
            ObjectList* units = GetWorldObjectsInDist((float)e.target.x);

            auto aura1 = e.target.playerWithoutMultiAura.spell1;
            auto aura2 = e.target.playerWithoutMultiAura.spell2;
            auto aura3 = e.target.playerWithoutMultiAura.spell1;

            std::for_each(units->begin(), units->end(), [aura1, aura2, aura3, &l](WorldObject *obj){
                if (auto player = obj->ToPlayer())
                    if (!player->isGameMaster() &&
                        (!aura1 || !player->HasAura(aura1)) &&
                        (!aura2 || !player->HasAura(aura2)) &&
                        (!aura3 || !player->HasAura(aura3)))
                            l->push_back(player);
            });
        ...
        { 
            ...
            uint32 targetsToHit = e.target.farthest.count;
            ObjectList* units = GetWorldObjectsInDist((float)e.target.farthest.radius);
        ...
        case SMART_TARGET_NEAR_RANDOM_POSITION:
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_COUNTER_SET:
            ...
            {
                RecalcTimer(e, e.event.counter.cooldownMin, e.event.counter.cooldownMax);
                ProcessAction(e);
            }
            break;
        case SMART_EVENT_EVADE:
        {
            if (e.event.reset.despawnAtEvade > 0)
                _DespawnAtEvade();

            // we do not break here so we can process actions
        }
        //no params
        ...
        case SMART_EVENT_NEAR_PLAYERS:
        {
            ...
            std::list<Player*> players = me->GetNearestPlayersList(range);

            if (players.size() >= e.event.nearPlayer.minCount)
                ProcessAction(e, unit);
