...
class SummonList : public std::list<uint64>
{
    public:
        ...
        {
            std::list<uint64> copy;
            Trinity::Containers::RandomResizeCopy<uint64, Predicate>(*this, copy, predicate, max);
            for (iterator i = copy.begin(); i != copy.end(); )
            {
...
class DespawnAtEvadeEvent : public BasicEvent
{
    public:

        explicit DespawnAtEvadeEvent(uint64 owner)
            : _owner(owner)
        {
        }
...
class BossAI : public ScriptedAI
{
    ...
    protected:
        ...
        // You can check any BoundaryMap with the current creature
        bool CheckBoundaryMap(const BossBoundaryMap* boundary);
