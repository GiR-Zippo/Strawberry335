...
enum ExtendedState
{
    EXTENDED_STATE_NONE     = 0,
    EXTENDED_STATE_DONE     = 1,
    EXTENDED_STATE_NEEDED   = 2
};
...
class InstanceSave
{
    ...
    public:
        ...
        InstanceSave(uint16 MapId, uint32 InstanceId, Difficulty difficulty, time_t resetTime,
                     bool canReset, ExtendedState extendedState);
        ...
        time_t GetResetTime(bool extended = false) const;
        ...
        void AddPlayer(Player* player) { m_playerList.insert(player); }
        bool RemovePlayer(Player* player)
        {
            PlayerListType::iterator iter = m_playerList.find(player);
            if (iter != m_playerList.end() && (*iter) == player)
        ...
        /** Extended save functions
            Used to check if extension is available and to set it  */
        ExtendedState GetExtendedState() const { return m_extendedState; }
        bool CanExtend() const;
        void SetNeedExtendedState();
        void SetExtendedDone(); // only intern

        typedef std::set<Player*> PlayerListType;
...
class InstanceSaveManager
{
    ...
    //private:
        ...
        bool _ResetSave(InstanceSaveHashMap::iterator &itr);
