...
{
    uint8 tempdiff = instance->GetDifficulty();
    if (instance->GetId() == 631 && tempdiff > RAID_DIFFICULTY_25MAN_NORMAL)
        tempdiff -= 2;
    DungeonEncounterList const* encounters = sObjectMgr->GetDungeonEncounterList(instance->GetId(), Difficulty(tempdiff));
    if (!encounters)
        return;
    ...
    for (DungeonEncounterList::const_iterator itr = encounters->begin(); itr != encounters->end(); ++itr)
    {
        ...
        if (encounter->creditType == type && encounter->creditEntry == creditEntry)
        {
            ...
            if (encounter->lastEncounterDungeon)
            {
                ...
                sLog->outDebug(LOG_FILTER_TSCR, "Instance %s (instanceId %u) completed encounter %s. Rewarded Dungeon: %u", instance->GetMapName(), instance->GetInstanceId(), (*itr)->dbcEntry->encounterName[0], dungeonId);
                break;
            }
        }
    }

    if (!dungeonId) // Special handling holiday encounters
    {
        switch (creditEntry)
        {
            case 23682: // Hallow's End - Headless Horseman
                dungeonId = 285;
                break;

            case 25740: // Midsummer Fire Festival - Ahune
                dungeonId = 286;
                break;

            case 23872: // Brewfest - Coren Direbrew
                dungeonId = 287;
                break;

            case 36296: // Love is in the Air - Apothecary Hummel
                dungeonId = 288;
                break;
        }
    }
