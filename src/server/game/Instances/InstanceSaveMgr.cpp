...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load)
{
    ...
    uint8 temp_diff = difficulty;
    if (mapId == 631)
        if (temp_diff > RAID_DIFFICULTY_25MAN_NORMAL)
            temp_diff -= 2;

    ExtendedState extendedState = EXTENDED_STATE_NONE;
    if (load)
    {
        // Load extended state from db
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_INSTANCE_EXTENDED_BY_INSTANCE);
        stmt->setUInt32(0, instanceId);
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            extendedState = ExtendedState(result->Fetch()->GetUInt8());
    }

    InstanceSave* save = new InstanceSave(mapId, instanceId, Difficulty(temp_diff), resetTime, canReset, extendedState);
    if (!load)
        save->SaveToDB();
...
InstanceSave::InstanceSave(uint16 MapId, uint32 InstanceId, Difficulty difficulty, time_t resetTime, bool canReset, ExtendedState extendedState)
    : m_resetTime(resetTime), m_instanceid(InstanceId), m_mapid(MapId), m_difficulty(difficulty), m_canReset(canReset), m_extendedState(extendedState)
{
}
...
time_t InstanceSave::GetResetTime(bool extended) const
{
    uint32 addTime = 0;

    if (extended && m_extendedState != EXTENDED_STATE_DONE)
    {
        MapDifficulty const* mapDiff = GetMapDifficultyData(m_mapid, m_difficulty);
        if (!mapDiff || !mapDiff->resetTime)
        {
            sLog->outError("InstanceSaveManager::GetResetTime: not valid difficulty or no reset delay for map %d", m_mapid);
            return m_resetTime;
        }

        addTime = uint32(((mapDiff->resetTime * sWorld->getRate(RATE_INSTANCE_RESET_TIME)) / DAY) * DAY);
        if (addTime < DAY)
            addTime = DAY;
    }

    return m_resetTime + addTime;
}

bool InstanceSave::CanExtend() const
{
    return m_extendedState == EXTENDED_STATE_NONE;
}

void InstanceSave::SetNeedExtendedState()
{
    if (CanExtend())
    {
        m_extendedState = EXTENDED_STATE_NEEDED;

        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_INSTANCE_EXTENDED);
        stmt->setUInt8(0, uint8(EXTENDED_STATE_NEEDED));
        stmt->setUInt32(1, uint32(m_instanceid));
        CharacterDatabase.Execute(stmt);
    }
}

void InstanceSave::SetExtendedDone()
{
    if (m_extendedState == EXTENDED_STATE_NEEDED)
        m_extendedState = EXTENDED_STATE_DONE;
}
...
void InstanceSaveManager::LoadInstances()
{
    ...
    CharacterDatabase.DirectExecute("DELETE i FROM instance i LEFT JOIN instance_reset ir ON mapid = map AND i.difficulty = ir.difficulty "
                                    "WHERE i.extended < 2 AND (i.resettime > 0 AND i.resettime < UNIX_TIMESTAMP()) "
                                    "OR (ir.resettime IS NOT NULL AND ir.resettime < UNIX_TIMESTAMP())");
...
bool InstanceSaveManager::_ResetSave(InstanceSaveHashMap::iterator &itr)
{
    // unbind all players bound to the instance
    // do not allow UnbindInstance to automatically unload the InstanceSaves
    lock_instLists = true;
    bool extendedPlayerFound = false;

    InstanceSave::PlayerListType &pList = itr->second->m_playerList;
    for (InstanceSave::PlayerListType::const_iterator playerItr = pList.begin(); playerItr != pList.end(); ++playerItr)
    {
        if (Player* player = (*playerItr))
        {
            InstancePlayerBind* pBind = player->GetBoundInstance(itr->second->GetMapId(), itr->second->GetDifficulty());
            if (itr->second->GetExtendedState() < EXTENDED_STATE_NEEDED || !pBind || pBind->GetExtended() < EXTENDED_STATE_NEEDED)
                player->UnbindInstance(itr->second->GetMapId(), itr->second->GetDifficulty(), true);
            else
                extendedPlayerFound = true;
        }
    }
    ...
    if (itr->second->GetExtendedState() < EXTENDED_STATE_NEEDED || !extendedPlayerFound)
    {
        delete itr->second;
        m_instanceSaveById.erase(itr++);
    }
    else
        itr->second->SetExtendedDone();

    lock_instLists = false;
    return extendedPlayerFound;
}
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
    ...
    // Used to define full or not extended delete
    bool extendedDelete = false;
    std::set<uint32> extendedInstances;
    ...
    if (!warn)
    {
        ...
        // check is only for online players, we need db reply
        // First, check instance
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_EXI_INSTANCE_BY_MAP_DIFF_EXTENDED);
        stmt->setUInt16(0, uint16(mapid));
        stmt->setUInt8(1, uint8(difficulty));
        stmt->setUInt8(2, uint8(EXTENDED_STATE_NEEDED));

        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
        {
            // Second, check players
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_INSTANCE_BY_MAP_DIFF_EXTENDED);
            stmt->setUInt16(0, uint16(mapid));
            stmt->setUInt8(1, uint8(difficulty));
            stmt->setUInt8(2, uint8(EXTENDED_STATE_NEEDED));
            ...
            if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            {
                ...
                do
                {
                    extendedInstances.insert(result->Fetch()->GetUInt32());
                } while (result->NextRow());
            }
        }
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_MAP_DIFF_EXTENDED);
        stmt->setUInt16(0, uint16(mapid));
        stmt->setUInt8(1, uint8(difficulty));
        stmt->setUInt8(2, uint8((extendedDelete ? EXTENDED_STATE_DONE : EXTENDED_STATE_NEEDED)));
        trans->Append(stmt);
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_INSTANCE_BY_MAP_DIFF_EXTENDED);
        stmt->setUInt16(0, uint16(mapid));
        stmt->setUInt8(1, uint8(difficulty));
        stmt->setUInt8(2, uint8((extendedDelete ? EXTENDED_STATE_DONE : EXTENDED_STATE_NEEDED)));
        trans->Append(stmt);
        ...
        // Update it in the DB
        SQLTransaction updTrans = CharacterDatabase.BeginTransaction();
        ...
        updTrans->Append(stmt);

        // Finally, decrease state of extended save
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_INSTANCE_DECREASE_EXTENDED_BY_MAP_DIFF);
        stmt->setUInt16(0, uint16(mapid));
        stmt->setUInt8(1, uint8(difficulty));
        updTrans->Append(stmt);

        stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHAR_INSTANCE_DECREASE_EXTENDED_BY_MAP_DIFF);
        stmt->setUInt16(0, uint16(mapid));
        stmt->setUInt8(1, uint8(difficulty));
        updTrans->Append(stmt);

        CharacterDatabase.CommitTransaction(updTrans);
    }
    ...
    for (mitr = instMaps.begin(); mitr != instMaps.end(); ++mitr)
    {
        ...
        if (!extendedInstances.empty() && extendedInstances.find(map2->GetInstanceId()) != extendedInstances.end())
            continue;
