...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    // Check if item in guild bank is holiday item
    if (sWorld->getIntConfig(CONFIG_CORE_TYPE) == NODE_TYPE_MASTER)
    {
        if (const ItemTemplate* pItemProto = pItem->GetTemplate())
        {
            // If item is only available during holiday and holiday is not active, delete it
            if (pItemProto->HolidayId)
            {
                sLog->outError("Item (GUID %u, id: %u) is holiday item, deleting from guild bank and storage!", itemGuid, itemEntry);
...
void Guild::Member::SetStats(Player* player)
{
    ...
    if (mapid == 572 || mapid == 562 || mapid == 559)
    {
        const WorldLocation loc = player->GetBattlegroundEntryPoint();
        m_zoneId = sMapMgr->GetZoneId(loc.m_mapId, loc.m_positionX, loc.m_positionY, loc.m_positionZ);
    }
    else
...
void Guild::PlayerMoveItemData::RemoveItem(SQLTransaction& trans, MoveItemData* /*pOther*/, uint32 splitedAmount)
{
    ...
    else
    {
        ...
        m_pItem->DeleteFromInventoryDB(trans, m_pPlayer->GetGUIDLow());
        m_pItem = NULL;
    }
}
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE) && !AccountMgr::IsVIPorPlayer(m_pPlayer->GetSession()->GetSecurity()))       // TODO: move to scripts
...
void Guild::HandleInviteMember(WorldSession* session, const std::string& name)
{
    ...
    if (pInvitee->GetSocial()->HasIgnore(player->GetGUIDLow()))
        return;
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) && pInvitee->GetTeam(true) != player->GetTeam(true))
    {
...
void Guild::HandleAcceptMember(WorldSession* session)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
        player->GetTeam(true) != sObjectMgr->GetPlayerTeamByGUID(GetLeaderGUID()))
        return;
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    if (!AccountMgr::IsVIPorPlayer(player->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
...
void Guild::MassInviteToEvent(WorldSession* session, uint32 minLevel, uint32 maxLevel, uint32 minRank)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Sent SMSG_CALENDAR_FILTER_GUILD");
}
...
bool Guild::AddMember(Player *player, uint8 rankId)
{
    if (!player)
        return false;
