 ...
 : m_announce(true), m_ownership(true), m_name(name), m_password(""), m_flags(0), m_channelId(channel_id), m_ownerGUID(0), m_Team(Team)
{
    ...
    else                                                    // it's custom channel
    {
        ...
        if (sLogon->getBoolConfig(CONFIG_PRESERVE_CUSTOM_CHANNELS))
        {
...
{
    if (sLogon->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) > 0)
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_OLD_CHANNELS);
        stmt->setUInt32(0, sLogon->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) * DAY);
        CharacterDatabase.Execute(stmt);
...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    Player* player = sObjectMgr->FindPlayer(p);

    if (player)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) &&
            sLogon->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL)) //&& AccountMgr::IsVIPorPlayer(player->GetSession()->GetSecurity()) && player->GetGroup())
        {
            //MakeNotInLfg(&data);
            //SendToOne(&data, p);
            return;
    ...
    if (m_announce && (!player || !AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) || !sLogon->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL)))
    {
        MakeJoined(&data, p);
        SendToAll(&data);
    }
...
void Channel::KickOrBan(uint64 good, const char *badname, bool ban)
{
    ...
    Player* gplr = sObjectMgr->FindPlayer(good);
    if (gplr)
    ...
    {
        Player* bad = sObjectMgr->FindPlayerByName(badname);
        ...
        else
        {
            ...
            WorldPacket data;
            bool notify = !(AccountMgr::IsGMAccount(sec) && sLogon->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL));
...
void Channel::SetMode(uint64 p, const char *p2n, bool mod, bool set)
{
    ...
    {
        Player* newp = sObjectMgr->FindPlayerByName(p2n);
        if (!newp)
        {
            WorldPacket data;
        ...
        if ((!AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) || !AccountMgr::IsGMAccount(newp->GetSession()->GetSecurity())) &&
            player->GetTeam() != newp->GetTeam() && !sLogon->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
        {
            WorldPacket data;
            MakePlayerNotFound(&data, p2n);
            SendToOne(&data, p);
...
void Channel::SetOwner(uint64 p, const char *newname)
{
    ...
    Player* newp = sObjectMgr->FindPlayerByName(newname);
    ...
    if (newp->GetTeam() != player->GetTeam() && !sLogon->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
    {
        WorldPacket data;
        MakePlayerNotFound(&data, newname);
        SendToOne(&data, p);
...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        uint32 gmLevelInWhoList = sLogon->getIntConfig(CONFIG_GM_LEVEL_IN_WHO_LIST);

        uint32 count  = 0;
        for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            Player* member = sObjectMgr->FindPlayer(i->first);

            // PLAYER can't see MODERATOR, GAME MASTER, ADMINISTRATOR characters
            // MODERATOR, GAME MASTER, ADMINISTRATOR can see all
            if (member && (!AccountMgr::IsVIPorPlayer(player->GetSession()->GetSecurity()) || member->GetSession()->GetSecurity() <= AccountTypes(gmLevelInWhoList)) &&
...
void Channel::Say(uint64 p, const char *what, uint32 lang)
{
    if (!what)
        return;
    if (sLogon->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
        lang = LANG_UNIVERSAL;
    ...
    else
    {
        ...
        sLog->outChat("[CHANNEL] Player %s tells channel %s: %s",
                player ? player->GetName() : "<SYSTEM>", GetName().c_str(), what);
...
void Channel::SendToAll(WorldPacket* data, uint64 p)
{
    for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
    {
        Player* player = sObjectMgr->FindPlayer(i->first);
        if (player)
        {
...
{
    Player* player = sObjectMgr->FindPlayer(who);
    if (player)
        player->GetSession()->SendPacket(data);
}
