...
void ClientSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    if (strContains(channelname, " - ") || strContains(channelname, "SucheNachGruppe"))
    {
        SendPacketToNode(&recvPacket);
        return;
    }
...
void ClientSession::HandleChannelList(WorldPacket& recvPacket)
{
    ...
    //If we are on localchan, send the whole Package to the node
    if (strContains(channelname, " - ") || strContains(channelname, "SucheNachGruppe"))
    {
        SendPacketToNode(&recvPacket);
        return;
