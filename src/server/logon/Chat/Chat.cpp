...
ChatCommand* ChatHandler::getCommandTable()
{
    ...
    static ChatCommand autoBroadcastCommadnTable[] =
    {
        { "reload",         SEC_GAMEMASTER,     true,   OldHandler<&ChatHandler::HandleReloadAutobroadcastCommand>,     "", NULL },
        { NULL,             0,                  false, NULL,                                                            "", NULL }
    };
    ...
    static ChatCommand commandTable[] =
    {
        ...
        { "autobroadcast",  SEC_GAMEMASTER,     true,  NULL,                                                        "", autoBroadcastCommadnTable },
...
int ChatHandler::ParseCommands(const char* text)
{
    ...
    if (m_session && AccountMgr::IsVIPorPlayer(m_session->GetSecurity())) // && !sWorld->getBoolConfig(CONFIG_ALLOW_PLAYER_COMMANDS))
       return 0;
...
int ChatHandler::ParseDotCommands(const char* text)
{
    ASSERT(text);
    ASSERT(*text);
...
bool ChatHandler::HandleReloadAutobroadcastCommand(const char*)
{
    ...
    sAutoBroadcast.load();
    sClientSessionMgr->SendGameMasterMessage("DB table `autobroadcast` reloaded.");
    return true;
}
