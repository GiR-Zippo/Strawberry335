...
ControlSocket::ControlSocket() : Socket(sizeof (Flexi::ClientPktHeader)),
...
int ControlSocket::_sendPacket(const WorldPacket &pct)
{
...
int ControlSocket::_handleInputHeader()
{
    ...
    if (header.size < 2)
    {
        sLog->outError("ControlSocket::handle_input_header(): Node sent malformed packet (size: %d , cmd: %d)",
...
int ControlSocket::_processIncoming(WorldPacket *newPacket)
{
    ...
    const ACE_UINT32 opcode = newPacket->GetOpcode();

    /*sLog->outDebug (LOG_FILTER_NETWORKIO, "CONTROL_IN: SOCKET: %u LENGTH: %u OPCODE: %s (0x%.4X)",
                   (uint32) get_handle (),
                   newPacket->size (),
