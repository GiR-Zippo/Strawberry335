...
void GmTicket::SendResponse(ClientSession* session) const
{
...
void GmTicket::SetUnassigned()
{
    ...
    sGMQualityManager->OnTicketUnAssign(this);
}
...
void TicketMgr::Initialize() { SetStatus(sLogon->getBoolConfig(CONFIG_GMTICKET_ALLOW_TICKETS)); }
...
void TicketMgr::CloseTicket(uint32 ticketId, int64 source)
{
    if (GmTicket* ticket = GetTicket(ticketId))
    {
        ...
        sGMQualityManager->OnTicketClose(ticket);
    }
}
...
void TicketMgr::RemoveTicket(uint32 ticketId)
{
    if (GmTicket* ticket = GetTicket(ticketId))
    {
        ...
        sGMQualityManager->OnTicketDelete(ticket);
        delete ticket;
    }
}
...
void TicketMgr::SendTicket(ClientSession* session, GmTicket* ticket) const
{
