...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    bool allowTwoSideWhoList = sLogon->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_WHO_LIST);
    AccountTypes gmLevelInWhoList = AccountTypes(sLogon->getIntConfig(CONFIG_GM_LEVEL_IN_WHO_LIST));
...
void SocialMgr::BroadcastToFriendListers(Player* player, WorldPacket* packet)
{
    ...
    for (SocialMap::const_iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        {
            Player* pFriend = sObjectMgr->FindPlayer(MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER));
