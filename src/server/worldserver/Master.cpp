...
int Master::Run()
{
    ...
    LogonDatabase.DirectPExecute("UPDATE nodelist SET Online = '0' WHERE NodeID = '%d'", realmID);
    ...
    uint16 psport = ConfigMgr::GetIntDefault("PoolServerPort", 8200);
    std::string bind_ip = ConfigMgr::GetStringDefault("BindIP", "0.0.0.0");

    if ((sWorldSocketMgr->StartNetwork(wsport, bind_ip.c_str ()) == -1) ||
        (sPoolSocketMgr->StartNetwork (psport, bind_ip.c_str ()) == -1))
    {
    ...
    LogonDatabase.DirectPExecute("UPDATE nodelist SET Online = '1' WHERE NodeID = '%d'", realmID);
    sLog->outString("%s%s%s (worldserver-daemon) ready...", _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B);
...
bool Master::_StartDB()
{
    ...
    WorldDatabase.PExecute("UPDATE version SET core_version = '%s%s%s', core_revision = '%s'", _FULLVERSION_A, git::getProduktVersionStr(), _FULLVERSION_B, git::getHash());        // One-time query
...
{
    if (ConfigMgr::GetIntDefault("CoreType", 0) > NODE_TYPE_MASTER)
        return;
