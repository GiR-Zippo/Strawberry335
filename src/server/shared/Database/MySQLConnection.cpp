...
ErrorHandlingType MySQLConnection::Execute(const char* sql)
{
    if (!m_Mysql)
        return SQL_ERROR_ABORT;

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            switch (_HandleMySQLErrno(lErrno))
            {
                case SQL_ERROR_RETRY:
                    return Execute(sql);       // Try again
                case SQL_ERROR_RESCHEDULE:
                    return SQL_ERROR_RESCHEDULE;
            }
...
ErrorHandlingType MySQLConnection::Execute(PreparedStatement* stmt)
{
    if (!m_Mysql)
        return SQL_ERROR_ABORT;
...
ErrorHandlingType MySQLConnection::_Query(PreparedStatement* stmt, MYSQL_RES **pResult, uint64* pRowCount, uint32* pFieldCount)
{
    if (!m_Mysql)
        return SQL_ERROR_ABORT;
    ...
    uint32 index = stmt->m_index;
    {
        ...
        if (mysql_stmt_bind_param(msql_STMT, msql_BIND))
        {
            ...
            {
                case SQL_ERROR_RETRY:
                    _Query(stmt, pResult, pRowCount, pFieldCount);      // Try again
                case SQL_ERROR_RESCHEDULE:
                    m_mStmt->ClearParameters();
                    return SQL_ERROR_RESCHEDULE;
            }
...
ResultSet* MySQLConnection::Query(const char* sql)
{
    ...
    if (SQL_ERROR_OK != _Query(sql, &result, &fields, &rowCount, &fieldCount))
        return NULL;

    return new ResultSet(result, fields, rowCount, fieldCount);
}

ErrorHandlingType MySQLConnection::_Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64* pRowCount, uint32* pFieldCount)
{
    if (!m_Mysql)
        return SQL_ERROR_ABORT;
...
ErrorHandlingType MySQLConnection::ExecuteTransaction(SQLTransaction& transaction)
{
...
PreparedResultSet* MySQLConnection::Query(PreparedStatement* stmt)
{
    ...
    if (SQL_ERROR_OK != _Query(stmt, &result, &rowCount, &fieldCount))
        return NULL;
...
ErrorHandlingType MySQLConnection::_HandleMySQLErrno(uint32 errNo)
{
    switch (errNo)
    {
        ...
        case ER_LOCK_DEADLOCK:
            return SQL_ERROR_RESCHEDULE;    // Implemented in TransactionTask::Execute and DatabaseWorkerPool<T>::DirectCommitTransaction
