...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_REALMLIST, "SELECT id, name, address, port, icon, color, timezone, allowedSecurityLevel, population, gamebuild FROM realmlist WHERE color <> 3 ORDER BY name", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_INS_LOG, "INSERT INTO logs (time, realm, type, string) VALUES (UNIX_TIMESTAMP(), ? , ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_INS_STATS, "INSERT INTO stats_replicator VALUES (?, 0, 0, 0, 0, 0)", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_UPD_STATS, "UPDATE stats_replicator SET uptime = ?, maxplayers = ?, online = ?, ally = ?, horde = ? WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_SEL_STATS, "SELECT maxplayers FROM stats_replicator WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_ID_PRIOR_LAST_LOGIN, "SELECT id FROM account WHERE (DATE(last_login) < ?) AND (DATE(joindate) < ?)", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_ID_PRIOR_LAST_LOGIN_NOT_BANNED, "SELECT account.id FROM account LEFT JOIN account_banned ON account_banned.id = account.id WHERE account_banned.id IS NULL AND (DATE(last_login) < ?) AND (DATE(joindate) < ?)", CONNECTION_SYNCH);
}
