...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_NAME_DATA, "SELECT race, class, gender FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INSTANCE, "SELECT id, permanent, map, difficulty, resettime, character_instance.extended FROM character_instance LEFT JOIN instance ON instance = id WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INVENTORY, "SELECT ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags,"
    "ii.enchantments, ii.randomPropertyId, ii.durability, ii.playedTime, ii.text, ci.bag, ci.slot, ci.item, ii.itemEntry, iri.paidMoney, iri.paidExtendedCost,"
    "istd.allowedPlayers, il.gold, il.item_entries FROM character_inventory AS ci JOIN item_instance AS ii ON ci.item = ii.guid "
    "LEFT JOIN item_refund_instance AS iri ON ci.item = iri.item_guid AND ci.guid = iri.player_guid LEFT JOIN item_soulbound_trade_data AS istd ON ci.item = istd.itemGuid "
    "LEFT JOIN item_loot AS il ON ci.item = il.guid WHERE ci.guid = ? ORDER BY ci.bag, ci.slot", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_EQUIPMENTSETS, "SELECT setguid, setindex, name, iconname, item0, item1, item2, item3, item4, item5, item6, item7, item8, "
    ...
    PrepareStatement(CHAR_SEL_MAILITEMS_BY_RECEIVER, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, item_guid, itemEntry, owner_guid, mail_id FROM mail_items mi JOIN item_instance ii ON mi.item_guid = ii.guid WHERE receiver = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_AUCTIONS, "SELECT id, auctioneerguid, itemguid, itemEntry, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = ah.itemguid", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_DEL_ALL_ITEM_INSTANCE, "DELETE FROM item_instance WHERE owner_guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_BANK_RIGHT, "INSERT INTO guild_bank_right (guildid, TabId, rid, gbright, SlotPerDay) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    // Equipmentsets
    PrepareStatement(CHAR_UPD_EQUIP_SET, "UPDATE character_equipmentsets SET name=?, iconname=?, item0=?, item1=?, item2=?, item3=?, item4=?, item5=?, item6=?, item7=?, item8=?, item9=?, item10=?, item11=?, item12=?, item13=?, item14=?, item15=?, item16=?, item17=?, item18=? WHERE guid=? AND setguid=? AND setindex=?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_EQUIP_SET, "INSERT INTO character_equipmentsets (guid, setguid, setindex, name, iconname, item0, item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11, item12, item13, item14, item15, item16, item17, item18) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_INSTANCE_EXTENDED, "UPDATE instance SET extended = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_ARENA_TEAM_STATS, "UPDATE arena_team SET rating = ?, weekGames = ?, weekWins = ?, seasonGames = ?, seasonWins = ?, rank = ?, inactivity = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_EXPIRED_AUCTIONS, "SELECT id, auctioneerguid, itemguid, itemEntry, itemowner, buyoutprice, time, buyguid, lastbid, startbid, deposit FROM auctionhouse ah INNER JOIN item_instance ii ON ii.guid = ah.itemguid WHERE ah.time <= ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_LFG_JOINC, "SELECT count FROM character_joincount WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_LFG_JOINC, "INSERT INTO character_joincount (count,guid) VALUES(?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_LFG_JOINC, "UPDATE character_joincount SET count = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_LFG_JOINC, "DELETE FROM character_joincount WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_INSTANCE_EXTENDED, "UPDATE character_instance SET extended = ? WHERE guid = ? AND instance = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_CLASS_LVL_AT_LOGIN, "SELECT class, level, at_login FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_MAIL, "SELECT id, messageType, sender, receiver, subject, body, has_items, expire_time, deliver_time, money, cod, checked, stationery, mailTemplateId FROM mail WHERE receiver = ? ORDER BY id DESC", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_INSTANCE_BY_MAP_DIFF_EXTENDED, "DELETE FROM character_instance USING character_instance LEFT JOIN instance ON character_instance.instance = id WHERE map = ? AND difficulty = ? AND character_instance.extended <= ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_EXI_INSTANCE_BY_MAP_DIFF_EXTENDED, "SELECT EXISTS(SELECT 1 FROM instance WHERE map = ? AND difficulty = ? AND extended = ? LIMIT 1)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_INSTANCE_EXTENDED_BY_INSTANCE, "SELECT extended FROM instance WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_INSTANCE_BY_EXTENDED, "SELECT id FROM instance WHERE extended = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_INSTANCE_BY_MAP_DIFF_EXTENDED, "SELECT instance FROM character_instance LEFT JOIN instance ON character_instance.instance = id WHERE map = ? AND difficulty = ? AND character_instance.extended = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_UPD_INSTANCE_DECREASE_EXTENDED_BY_MAP_DIFF, "UPDATE instance SET extended = 1 WHERE extended = 2 AND map = ? AND difficulty = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_INSTANCE_DECREASE_EXTENDED_BY_MAP_DIFF, "UPDATE character_instance LEFT JOIN instance ON character_instance.instance = id SET character_instance.extended = 1 WHERE map = ? AND difficulty = ? AND character_instance.extended = 2", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_INSTANCE_BY_MAP_DIFF_EXTENDED, "DELETE FROM instance WHERE map = ? AND difficulty = ? AND extended <= ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_PET_AURA, "INSERT INTO pet_aura (guid, caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxduration, remaintime, remaincharges) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_INVENTORY_BY_ITEM, "DELETE FROM character_inventory WHERE item = ? and guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_QUESTSTATUS_REWARDED, "INSERT IGNORE INTO character_queststatus_rewarded (guid, quest) VALUES (?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_STATS, "INSERT INTO character_stats (guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, maxpower6, maxpower7, strength, agility, stamina, intellect, spirit, armor, resHoly, resFire, resNature, resFrost, resShadow, resArcane, blockPct, dodgePct, parryPct, critPct, rangedCritPct, spellCritPct, attackPower, rangedAttackPower, spellPower, resilience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_RATE_OVERRIDES, "SELECT rate, value FROM character_rate_override WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_RATE_OVERRIDES, "DELETE FROM character_rate_override WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_RATE_OVERRIDES, "INSERT INTO character_rate_override (guid, rate, value) VALUES (?, ?, ?)", CONNECTION_SYNCH);

    PrepareStatement(CHAR_SEL_CHAR_TRANSMOG_UNLOCKED, "SELECT transmog_id FROM character_transmog_unlocked WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_TRANSMOG_UNLOCKED, "REPLACE INTO character_transmog_unlocked (guid, transmog_id) VALUES (?, ?)", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_CHAR_TRANSMOG_ACTIVE, "SELECT item_guid, transmog_id FROM character_transmog_active WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_TRANSMOG_ACTIVE, "DELETE FROM character_transmog_active WHERE guid = ? AND item_guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_TRANSMOG_ACTIVE, "REPLACE INTO character_transmog_active (guid, item_guid, transmog_id) VALUES (?, ?, ?)", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_CHAR_TRANSMOG_HISTORY, "SELECT transmog_id FROM character_transmog_history WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_TRANSMOG_HISTORY, "REPLACE INTO character_transmog_history (guid, transmog_id) VALUES (?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_ITEM_LOOT, "SELECT gold, item_entries FROM item_loot WHERE guid = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_REP_ITEM_LOOT, "REPLACE INTO item_loot (guid, gold, item_entries) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ITEM_LOOT, "DELETE FROM item_loot WHERE guid = ?", CONNECTION_ASYNC);
}
