...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_INS_NPC_VENODR, "INSERT INTO npc_vendor (entry, item, maxcount, incrtime, extendedcost) VALUES(?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_SEL_TRANSGEAR_SPECS, "SELECT spec FROM player_trans_gear WHERE class = ? AND pvp = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_TRANSGEAR_ITEMS, "SELECT items FROM player_trans_gear WHERE class = ? AND spec = ? AND pvp = ? AND set_id = ?", CONNECTION_SYNCH);
}
