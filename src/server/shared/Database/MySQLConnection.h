...
class MySQLConnection
{
    ...
    public:
        ErrorHandlingType Execute(const char* sql);
        ErrorHandlingType Execute(PreparedStatement* stmt);
        ResultSet* Query(const char* sql);
        PreparedResultSet* Query(PreparedStatement* stmt);
        ErrorHandlingType _Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64* pRowCount, uint32* pFieldCount);
        ErrorHandlingType _Query(PreparedStatement* stmt, MYSQL_RES **pResult, uint64* pRowCount, uint32* pFieldCount);

        void BeginTransaction();
        void RollbackTransaction();
        void CommitTransaction();
        ErrorHandlingType ExecuteTransaction(SQLTransaction& transaction);
    ...
    private:
        ErrorHandlingType _HandleMySQLErrno(uint32 errNo);
