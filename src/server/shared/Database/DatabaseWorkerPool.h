    ...
    public:
        ...
        */
        ...
        void DirectCommitTransaction(SQLTransaction& transaction)
        {
            ...
            ErrorHandlingType result = con->ExecuteTransaction(transaction);
            ...
            switch (result)
            {
                case SQL_ERROR_ABORT:
                {
                    ...
                    {
                        sLog->outSQLDriver("[Error] DirectCommitTransaction: Transaction aborted. %u queries not executed. :\n %s",
                            transaction->GetSize(), transaction->QueriesToText().c_str());
                    }
                    break;
                }
                case SQL_ERROR_RESCHEDULE:
                {
                    if (transaction->GetSize())
                    {
                        sLog->outSQLDriver("[Warning] DirectCommitTransaction: Transaction rescheduled. %u queries not executed. :\n %s",
                            transaction->GetSize(), transaction->QueriesToText().c_str());
                    }
